...
| Current task | `AM-025` blocked from acceptance but active for remediation; tactical camera state is isolated, Editor-only switches have reset ownership, and match-start, building-AI, plus diagnostics now have replacement-World proof. The `575` historical rows now contain `427` non-debt and `148` genuine-debt rows grouped into `72` unique file/rule items; five external helper paths remain |
