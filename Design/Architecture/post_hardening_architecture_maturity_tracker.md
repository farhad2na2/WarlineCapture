...
| Current task | `AM-025` blocked from acceptance but active for remediation; tactical camera state is isolated, Editor-only performance switches have verified reset ownership, and match-start plus building-AI caches now have two-World recovery proof. The `575` historical rows now contain `426` non-debt and `149` genuine-debt rows grouped into `73` unique file/rule items; five external helper paths remain |
