...
| Field | Status |
|---|---|
| Checklist complete | 72 / 114 |
| Checklist percent complete | 63.2% |
| Current phase | Phase 8 - Fuel/Oil ECS Drift Correction |
| Quick wins complete | 6 / 6 |
| Current target | Production/conversion, hauler load/revert/unload, faction resource summaries, resource sell/drain requests, and selected-building resource display now prefer the live `BuildingResourceStorageComponent` on each runtime building combat entity, commit mutation to that ECS component where applicable, and mirror mutation results back to the runtime-building object for compatibility. Runtime-building fallback remains for buildings without an ECS storage component. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, focused Unity 6.5.2 ECS mirror validation `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-ecs-validation.log`, focused Unity 6.5.2 faction resource validation `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-faction-validation.log`, focused Unity 6.5.2 hauler mirror validation `/private/tmp/warline-arch-followup-fuel-oil-hauler-mirror-sync-validation.log`, focused Unity 6.5.2 resource transfer validation `/private/tmp/warline-arch-followup-resource-transfer-helper-validation.log`, focused Unity 6.5.2 hauler ECS transfer validation `/private/tmp/warline-arch-followup-hauler-transfer-ecs-validation.log`, focused Unity 6.5.2 live hauler ECS transfer validation `/private/tmp/warline-arch-followup-hauler-live-ecs-transfer-validation.log`, focused Unity 6.5.2 live ECS-storage-preferred hauler validation `/private/tmp/warline-arch-followup-hauler-live-storage-prefer-ecs-validation.log`, focused Unity 6.5.2 live ECS-storage-preferred production validation `/private/tmp/warline-arch-followup-production-live-storage-prefer-ecs-validation.log`, focused Unity 6.5.2 ECS-backed faction summary/drain validation `/private/tmp/warline-arch-followup-faction-resource-ecs-read-drain-validation.log`, and focused Unity 6.5.2 ECS-backed selected resource display validation `/private/tmp/warline-arch-followup-selected-resource-display-ecs-validation.log` passed with 0 compiler errors after the Fuel/Oil mirror-sync and transfer-surface slices. |
...
| Fuel/Oil drift status | Drift reduced further: production/conversion math is no longer embedded in `FactionResourceCompositionSystemHelper`, runtime-created building combat entities carry their runtime building ids, resource buildings now have a per-building ECS storage mirror, a disabled Burst-capable ECS tick/apply surface is validated, managed production applies through that ECS surface, production plus hauler updates refresh the live ECS storage mirror for resource-capable buildings, hauler load/unload storage mutation routes through `BuildingResourceHaulerTransferEcsSystem`, both live production/conversion and live hauler mutation now prefer the live ECS storage component before mirroring back to runtime-building state, faction summary/sell-drain paths now prefer live ECS storage too, and selected-building resource storage display now reads live ECS storage when available. Header/resource summary paths remain read-only display consumers. |
| Validation status | Phase 0 inventory completed; Phase 1 config edited; Phase 2 release OnGUI strip implemented; Phase 3 release diagnostic system gate implemented; Phase 4 GC quick wins completed; Phase 5 Burst inventory/classification report added and guardrail implemented; Phase 6 instantiate classification report added and first placement-visual pooling slice implemented; Phase 8 Fuel/Oil behavior safety-net tests added, production math extracted into a pure system-helper surface, runtime-created building combat entities receive their runtime building ids, per-building ECS resource storage mirrors are populated for resource buildings, the disabled ECS resource-production tick/apply surface has focused validation, managed production applies through the ECS surface without double-ticking, production/hauler ticks sync managed results back to `BuildingResourceStorageComponent`, hauler transfer mutation uses the component-based transfer helper, the disabled ECS hauler transfer surface has focused validation, live hauler load/revert/unload enters through that ECS transfer surface, live hauler mutation now prefers the combat entity storage component, live production/conversion now prefers the combat entity storage component too, live faction summaries/resource sell-drains now prefer the combat entity storage component, and selected-building resource display now prefers the combat entity storage component. Latest validations passed: `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-ecs-validation.log`, `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-faction-validation.log`, `/private/tmp/warline-arch-followup-fuel-oil-hauler-mirror-sync-validation.log`, `/private/tmp/warline-arch-followup-resource-transfer-helper-validation.log`, `/private/tmp/warline-arch-followup-hauler-transfer-ecs-validation.log`, `/private/tmp/warline-arch-followup-hauler-live-ecs-transfer-validation.log`, `/private/tmp/warline-arch-followup-hauler-live-storage-prefer-ecs-validation.log`, `/private/tmp/warline-arch-followup-production-live-storage-prefer-ecs-validation.log`, `/private/tmp/warline-arch-followup-faction-resource-ecs-read-drain-validation.log`, and `/private/tmp/warline-arch-followup-selected-resource-display-ecs-validation.log`. |
| Still wrong / next iteration | No visual defects are known from code/compile validation, but manual visual smoke checks for placement-preview pooling and command behavior are still pending. Fuel/Oil still keeps runtime-building storage mirrors for compatibility and selected unit/hauler cargo detail surfaces should be audited for ECS preference; next Fuel/Oil slice should audit selected unit/hauler cargo display helpers, or pause Fuel/Oil drift work and return to Android baseline capture. Android baseline is still not captured, and HDR/soft-shadow tier ownership remains unresolved. |
...
- Known unresolved: visual/playmode smoke validation is pending, HDR/soft-shadow tier ownership is still unresolved, Android baseline is not captured, Fuel/Oil still keeps runtime-building storage mirrors for compatibility, selected unit/hauler cargo detail surfaces should be audited for ECS preference, and Phase 5 still has no truthful blind Burst candidates.
- Next iteration: audit selected unit/hauler cargo display helpers for ECS preference, or pause Fuel/Oil drift work and return to Android baseline capture.
