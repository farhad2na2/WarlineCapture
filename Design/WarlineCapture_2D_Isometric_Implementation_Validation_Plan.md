...
This plan defines the current implementation path for the 2D isometric battlefield direction.

The active path is large authored terrain macro tiles plus deterministic gameplay metadata. The goal is to get high-quality map visuals without breaking the existing RTS simulation, pathfinding, building placement, combat, HUD, or mission systems.
