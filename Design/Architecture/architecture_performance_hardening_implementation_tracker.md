...
| Current task | Validate the scoped BrightCommand Android ASTC 4x4 pilot against the clean APH-804 APK `<=463,359,198` byte and APH-803 peak allocated-memory `<=967.5 MB` limits without visual downgrade, then collect release and visual evidence |
...
| Last verified commit | `bcee23f3fceb76a33ad0863aaa4dbd22e3d779ad` on `origin/main`; architecture implementation commit `98cb2bb6d4321132574ebc5247aa3f2c93359eac` |
| Last update | 2026-07-15 - clean APH-803 device evidence passes all approved limits except allocated memory, and the clean APH-804 release APK is `48,722,883` bytes over budget; package/memory reduction and APH-809 visual evidence remain open |
...
- [~] `APH-501` Add tracked budgets for APK/AAB size, installed size, peak allocated memory, texture memory, mesh memory, audio memory, and graphics-driver memory.
  - Active result: schema 4 pins clean ARM64 IL2CPP release budgets at APK `<= 463,359,198` bytes and AAB `<= 426,399,778` bytes, including exact artifact commit, SHA-256, immutable evidence commit, and report path. The validator/test now follow schema 4 and the accepted 20 ms Editor p95 ratchet. The clean `bcee23f3fceb76a33ad0863aaa4dbd22e3d779ad` APK is `512,082,081` bytes with SHA-256 `4f3b8e5ce754f89fa3cb485d0cd4b5d4facfcd3b0d6ea3620b5270aececd3dc7`, so it is rejected by `48,722,883` bytes before device access. A bounded pilot now applies explicit Android ASTC 4x4 high-quality compression to exactly 60 `MainMenuBrightCommand/Sprites` textures while preserving sprite geometry, mipmaps, readability, alpha, and every non-Android setting; exact release size, device memory, and visual acceptance remain pending. Installed size and absolute peak/texture/mesh/audio/graphics-driver memory limits remain fail-closed `null` until an accepted release artifact reaches device collection. Evidence: `Design/AgentReports/2026-07-13_aph-501_product_budget_evidence_handoff.md`, the canonical APK BuildReport, and `/private/tmp/warline-aph501-main-menu-policy-tests-r2.xml` (`12/12`).
...
- [~] `APH-510` Rebuild Android, compare BuildReport/memory/frame metrics, and record per-category deltas.
  ...
  - Remaining: rebuild a clean same-artifact APK from the scoped BrightCommand ASTC pilot, verify the exact package delta and visual output, then produce AAB, device-performance, category-residency, and I/O evidence and accept the comparison.
