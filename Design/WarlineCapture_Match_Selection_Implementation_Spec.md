...
Parent match-screen source: `WarlineCapture_Match_HUD_And_Gameplay_Implementation_Spec.md` owns the full `SCN-08` match HUD contract for all buttons, panels, warnings, overlays, routes, and gameplay feedback. This file owns selection only.

Related sources:

- `WarlineCapture_UIUX_Gameplay_Element_Alignment.md` defines the high-level UI element roles.
- `WarlineCapture_Gameplay_UI_Integration_Handoff_Spec.md` defines `BattleHudGameplayBridge`.
- `WarlineCapture_M01_FirstContact_Production_Contract.md` defines the first tutorial mission exception rules.
