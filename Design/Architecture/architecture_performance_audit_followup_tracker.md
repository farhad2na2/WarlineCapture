...
| Field | Status |
|---|---|
| Checklist complete | 60 / 105 |
| Checklist percent complete | 57.1% |
| Current phase | Phase 8 - Fuel/Oil ECS Drift Correction |
| Quick wins complete | 6 / 6 |
| Current target | Runtime-building combat entity id bridge complete: runtime-created building combat entities now receive the allocated runtime building id in `RuntimeBuildingCombatInfo.RuntimeBuildingId`. This is a prerequisite for moving Fuel/Oil storage ownership into ECS without adding a parallel managed/gameplay owner. Next step is moving production storage state and ticking into ECS while reusing `BuildingResourceProductionSystemHelper`. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, and Unity 6.5.2 batchmode compile log `/private/tmp/warline-arch-followup-runtime-building-id-compile.log` passed with 0 compiler errors after the runtime-building combat id bridge. Unity MCP is usable for inspection but currently adds tracing/logging traffic to performance captures, so batchmode/editor-log validation remains the compiler path. |
...
- Next iteration: move Fuel/Oil production storage state and ticking into ECS while reusing `BuildingResourceProductionSystemHelper`, preserving the newly locked behavior tests, and using populated runtime building combat ids as the bridge to existing runtime-building identity.
