...
**Workflow transition:** The Architecture and Performance Hardening coordinator assignment was already active when the pull request workflow was introduced. Its assigned implementation and this user-approved early-development closeout remain one grandfathered direct-`main` program. Decision `D-172` ends that assignment with `107 / 107` explicit dispositions. The deferred pre-release backlog is future scope and is not implicitly grandfathered into this program.
...
| Architecture rating | `8.8 / 10` after hardening; baseline was `6.5 / 10` |
| Performance engineering rating | `8.2 / 10` after hardening; baseline was `6.0 / 10`; release certification is deferred |
| Current program status | Complete for the early-development hardening phase: `92` accepted tasks plus `15` explicitly deferred pre-release certifications give `107 / 107` dispositions |
...
- `[>]` disposition complete by explicit scope transfer to the pre-release certification backlog; this is not a pass or waiver
...
### Early-Development Rebaseline

On 2026-07-16 the user approved ending repeated release-cycle endurance work during early game development. Ten-minute thermal sessions, repeated cold/warm launch qualification, same-artifact release certification, and the complete visual certification matrix are therefore transferred without acceptance to `pre_release_performance_certification_backlog.md`.

This rebaseline does not weaken an existing gate. Deferred rows use `[>]`, remain unpassed, and must be completed before beta/release certification or before any production-readiness claim relies on them. The current program closes on code quality, architecture ownership, deterministic compiler/tests, short regression performance, GC budgets, build/package contracts, and recorded residual risks. Architecture and code-quality gates remain mandatory and green.
...
| Product/device/visual certification | Deferred: clean same-artifact Android development/release thermal evidence and the complete visual matrix are pre-release work | execute `pre_release_performance_certification_backlog.md` when its activation criteria are met |

There are no current architecture, ECS/Burst, Editor Match performance, GC, critical PlayMode, or static-map structural violations in the local integration candidate. Device endurance, full visual certification, streaming expansion, and absolute residency limits remain explicitly uncertified and are not production-readiness claims.
...
The early-development hardening program is complete when all of the following are true. Release-only outcomes transferred to `pre_release_performance_certification_backlog.md` are intentionally excluded from this phase and remain mandatory before release certification.
...
7. World texture streaming has a fail-closed policy and cannot expand without measured quality validation.
8. Match startup does not build duplicate static map meshes at runtime when equivalent editor-baked chunks are available.
9. The current short Android evidence remains recorded, while sustained 30/60 FPS release certification is deferred without a pass claim.
10. Memory, texture, mesh, audio, and graphics-driver evidence gaps are fail-closed and transferred to pre-release certification.
11. Android APK/AAB package ceilings are tracked; installed and runtime-residency limits remain measurement-required in the pre-release backlog.
...
| Field | Status |
|---|---|
| Checklist disposition | `107 / 107` (`100%`) |
| Accepted complete | `92 / 107` (`86.0%`) |
| Deferred to pre-release certification | `15 / 107` (`14.0%`): `APH-311`, `APH-501`, `APH-502`, `APH-504`, `APH-505`, `APH-506`, `APH-508`, `APH-509`, `APH-510`, `APH-601`, `APH-609`, `APH-803`, `APH-804`, `APH-809`, `APH-902` |
| Checklist in progress | `0 / 107` |
| Current phase | Early-development hardening closeout complete |
| Current task | None; use `pre_release_performance_certification_backlog.md` only after its activation criteria are met |
...
| Deferred visual findings | `2`: 23:00 Match readability is too dark, and the current near static-map capture contains black/missing surface regions; neither is accepted or hidden |
...
| Last update | 2026-07-16 - user-approved early-development rebaseline closes architecture/code-quality hardening and transfers 15 release-cycle certifications to the pre-release backlog without marking them passed |

For every `[>]` row below, existing `Active result`, `Remaining`, and diagnostic text is retained as implementation history and future input. It no longer denotes current work in this completed tracker; the one-to-one obligation and activation order are authoritative in `pre_release_performance_certification_backlog.md`.
...
- [~] `APH-714` Register and validate the Watch ARIA managed virtual-touch boundary. User-requested implementation; one helper, 171 lines/6898 bytes, no gameplay policy, ECS decisions remain separate. Evidence: `Design/AgentReports/AriaWatchPlay/implementation-status.md`; mission and platform coverage still open.

- [x] `APH-713` Register the isolated runtime operation-map R&D helper boundaries under exact source-growth ceilings.
  - Approval: the project owner explicitly authorized this runtime-generation R&D to proceed without treating the prototype rules as blockers, while requiring the implementation to remain aligned with the architecture and performance contracts.
  - Result: nine new R&D helper paths and fourteen reused RuntimeCity helper-growth paths are registered through 25 exact path/scope/line/byte decisions, including separate over-500 review ceilings for the two composition owners. `RuntimeCityRAndDMapView` remains passive, `RuntimeCityRAndDMapSystem : SystemBase` remains the only frame owner, and the recovery helper adds no update loop, LINQ, recurring collection, or diagnostics formatting path. The source-growth gate now passes `15/15` without changing frozen baselines or authorizing additional growth.
...
- [>] `APH-804` Add Android release-build acceptance for the documented 30 FPS tier; keep 60 FPS as a separate high-end target.
  ...
  - Current candidate: clean commit `2327b2bbf5bf1bb03a1f6fa349b11eb0b90d357e` produces a provenance-bound ARM64 IL2CPP release APK and complete 6,434-asset BuildReport. The `449,902,496`-byte artifact passes the accepted `463,359,198`-byte ceiling with `13,456,702` bytes of headroom and retains local offline First Launch plus unchanged operation-map content. Device preflight now queries package state exactly: an absent package is installed, while one exact existing package is reused only after the same launcher, ABI, non-debuggable `run-as`, device-side base-APK hash, and installed-artifact verification; ambiguous rows or stale hashes fail closed. This avoids repeatedly transferring the 450 MB package after a validated wireless transport reconnect, and five cold starts still clear package data before launch. Installation succeeds on the Xiaomi reference device. Its shell can measure the installed `base.apk` and extracted `lib` directory but cannot traverse the protected `oat` directory, so the collector records their permission-safe sum (`621,388,272` bytes on this install) instead of failing or silently discarding stderr; command failure details now preserve bounded stdout and stderr. Unplugged collection accepts either the pinned hardware serial transport or a syntactically valid IPv4 ADB endpoint, but live `ro.serialno` must still equal the pinned Xiaomi serial before device evidence is accepted. Release/development collector regressions pass `29/29`. The installed-size value is an accessible-artifact measurement and does not authorize an absolute full-footprint budget while protected generated code remains unavailable. APH-804 remains active until the serialized collector completes five cold/five warm starts, 60-second warmup, 600-second sustained release capture, memory/installed-size measurements, process/crash integrity, and zero-thermal acceptance on this exact artifact.
  - Latest device result: isolated wireless ADB on private server port `5038` completed the formal startup sequence and a `600.003 s` sustained run with `27,183` samples, `22.07 ms` average, `32.79 ms` p95, `114.56 ms` p99, `914.35 MB` peak Unity allocation, `2,140.47 MB` peak PSS, and three percentage points of battery drain. This result is diagnostic only: the recorder emitted `complete=0` because the release artifact was built with `PlayerSettings.enableFrameTimingStats: 0`, producing zero CPU/GPU timing samples. The project, Android build configuration, canonical BuildReport, CI wrapper, and device preinstall validator now require Frame Timing Stats; a replacement clean package-compliant APK is required before acceptance can resume. The collector also temporarily extends the five-minute display timeout, wakes/dismisses the keyguard, and restores the exact original timeout after collection so unattended evidence cannot enter Doze mid-run.
  - Replacement artifact: clean commit `2b19449570ccf1b78c43ea0154e83785ecb3031e` produces a `380,801,943`-byte APK with SHA-256 `31bb8749ec04f6be69d65a323a3e60956927aa7ebaa13a9af08c5c5ab4b4e74d`. The canonical report records `dirty=false`, `frameTimingStatsEnabled=true`, ARM64 IL2CPP release identity, 6,440 included assets, and exact artifact hash/size agreement. The artifact is `82,557,255` bytes below the accepted package ceiling and is ready for unplugged device collection.
  - Final early-development diagnostic: the hash-verified replacement artifact completed a `600.002 s` recorder session with 27,856 CPU and 27,785 GPU timing samples, approximately `21.54 ms` average, `32.79 ms` p95, `98.24 ms` p99, and `18,978.95 ms` process-to-Match-ready. It remains rejected because required release profiler counters were unavailable and the recorder emitted `complete=false`. Per the 2026-07-16 rebaseline, no further endurance rerun is required until pre-release certification activates.
...
- [>] `APH-902` Produce final same-device Android development and release reports, including 10-minute thermal sessions.
  - Disposition: transferred without acceptance to `pre_release_performance_certification_backlog.md`; current diagnostic evidence is retained, but no additional endurance run is required during early development.
- [x] `APH-903` Compare final architecture/performance metrics against this baseline and write `Design/AgentReports/architecture_performance_hardening_final_report.md`.
  - Result: the final report compares architecture, Editor frame/GC, short Android, package, and release-diagnostic evidence against the audit baseline. It separates accepted evidence from deferred certification and records seven residual-risk groups without converting a failed diagnostic into a pass.
- [x] `APH-904` Re-rate architecture and performance, list residual risks, update the root architecture index, and mark this tracker complete only when no required task remains.
  - Result: architecture is re-rated `8.8 / 10` and performance engineering `8.2 / 10` for early development. All `107` rows now have an explicit disposition: `92` accepted and `15` transferred without acceptance to the linked pre-release backlog. The architecture index identifies this tracker as complete for early development and keeps release readiness false.
...
### 2026-07-16 - APH-804 - Release timing capability preflight

- Status: stable timing-capable replacement artifact ready; device acceptance rerun remains active
- Device evidence: the exact `2327b2bbf` APK completed five cold/five warm starts and a `600.003 s` sustained unplugged run over isolated wireless ADB, but the recorder correctly emitted `complete=0` after receiving zero CPU/GPU frame timings
- Root cause: `ProjectSettings.enableFrameTimingStats` was disabled in the clean release artifact; Unity development players enable FrameTimingManager automatically, but release players require the Player setting
- Build contract: Android configuration now enforces Frame Timing Stats; the canonical BuildReport serializes `frameTimingStatsEnabled=true`; CI and device preinstall reject a report or artifact without it before deployment
- Replacement package: clean commit `2b19449570ccf1b78c43ea0154e83785ecb3031e`, `380,801,943` bytes, SHA-256 `31bb8749ec04f6be69d65a323a3e60956927aa7ebaa13a9af08c5c5ab4b4e74d`, `82,557,255` bytes below the APK ceiling, and exact report/hash/size agreement
- Device lifecycle: the collector records, temporarily extends, and restores the exact screen timeout while waking/dismissing keyguard, preventing the observed five-minute Doze transition from invalidating unattended foreground continuity
- Validation: combined Python collector/gate/CI coverage passes `57/57`; Python bytecode compilation and `git diff --check` pass; focused Android BuildReport Unity validation passes `12/12`; `Game.Editor` and `Game.Tests.Editor` compile with zero errors
- Evidence: `/private/tmp/warline-main-aph804-2327b2bbf-r15/aph804_android_release_recorder_failed.json`, `/private/tmp/warline-main-aph804-2327b2bbf-r15/aph804_android_release_device.log`, and `/private/tmp/warline-aph804-frame-timing-build-report.log`
- Next action: install this exact replacement APK and repeat unplugged APH-804 on isolated ADB port `5038`

### 2026-07-16 - Program Closeout - Early-Development Rebaseline

- Status: `107 / 107` explicit dispositions; `92` accepted and `15` transferred without acceptance to pre-release certification
- User decision: stop repeated release-cycle endurance, cold/warm, thermal, and full visual certification while the game remains in early development; prioritize code quality and architecture
- Final diagnostic: the timing-capable replacement APK completed 600 seconds with CPU/GPU timings but emitted `complete=false` because required release profiler counters were unavailable; it remains rejected and is not a closeout pass
- Architecture closeout: integrated architecture, ECS/Burst, naming, source growth, assembly, lifecycle, critical PlayMode, Editor frame, and GC gates remain green
- Ratings: architecture `8.8 / 10`; performance engineering `8.2 / 10`; Android release performance remains uncertified
- Deferred authority: `Design/Architecture/pre_release_performance_certification_backlog.md`
- Final report: `Design/AgentReports/architecture_performance_hardening_final_report.md`
- Unrelated ownership: no operation-map, FirstLaunch, audio, scene, prefab, or runtime source was modified by this closeout
...
| 2026-08-14 | `D-114` | Retire the UI shell component growth exception | M01 result contracts moved to the bounded `UiMissionResultModels.cs` owner, leaving `UiShellComponents.cs` below its reviewed baseline; the stale strict exception is removed and future growth remains fail-closed | Focused production source-growth gate |
...
| 2026-07-16 | `D-166` | Bind unplugged wireless ADB to pinned hardware identity | The collector accepts a valid IPv4 transport endpoint only when live `ro.serialno` still matches the release profile's Xiaomi hardware serial, preserving exact device identity while USB power is physically absent | `android_release_device_collection.py`; collector regressions `29/29`; wireless preflight `USB powered: false` |
| 2026-07-16 | `D-167` | Reuse only a hash-verified exact release installation | A validated existing package avoids another long wireless transfer after transport reconnect, but reuse is accepted only after launcher, ABI, non-debuggable, device hash, and installed-artifact checks; absent packages are still installed and cold samples still clear application data | `android_release_device_collection.py`; collector regressions `29/29`; exact installed APK SHA-256 proof |
| 2026-07-16 | `D-168` | Preserve an awake display during unattended device evidence | The reference phone's five-minute timeout entered Doze and removed its resumed activity during a valid release launch; the collector now temporarily extends the timeout, wakes and dismisses keyguard, and restores the exact original value in a finally boundary | APH-804 r12 foreground failure; collector regressions `31/31` |
| 2026-07-16 | `D-169` | Require Frame Timing Stats in every APH-804 release artifact | The first complete sustained release run produced zero CPU/GPU timings because the clean Player had Frame Timing Stats disabled; project/build/report/CI/device contracts now fail before deployment unless the release capability is enabled and recorded | APH-804 r15 recorder JSON; Unity FrameTimingManager release requirement; focused build-report validation `12/12` |
| 2026-07-16 | `D-170` | Ignore only Unity-generated Android build caches | Addressables profile/link outputs and scene-dependency cache rows appear before build provenance capture but are regenerated build intermediates; narrow ignore rules keep clean provenance meaningful without removing or modifying operation-map source or tracked cache rows | `.gitignore`; clean `2b1944957` release report |
| 2026-07-16 | `D-171` | Advance APH-804 to the timing-capable clean artifact | The replacement APK is clean, substantially below the accepted package ceiling, and proves Frame Timing Stats plus exact report/hash/size identity before ADB | `architecture_performance_android_apk_build_report.json`; clean release build log |
| 2026-07-16 | `D-172` | Close hardening at the early-development architecture gate and defer release certification | Repeated endurance, cold/warm, thermal, and full visual qualification is premature while core game content is changing; preserving each obligation in a fail-closed pre-release backlog maintains rigor without spending current development time on rapidly stale evidence | explicit user approval; `pre_release_performance_certification_backlog.md`; `architecture_performance_hardening_final_report.md` |
| 2026-07-16 | `D-173` | Register algorithmic aftermath presentation | Deterministic grouped aftermath realization is a bounded generation-time collaborator with no update loop or retained global state | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityAlgorithmicAftermathPresentationSystemHelper.cs;scope=system-helper;maxLines=425;maxBytes=16141;task=APH-713)` |
| 2026-07-16 | `D-174` | Register algorithmic district presentation | Twelve bounded district surfaces are created only during generation and remain owned by the isolated runtime map root | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityAlgorithmicDistrictPresentationSystemHelper.cs;scope=system-helper;maxLines=142;maxBytes=5393;task=APH-713)` |
| 2026-07-16 | `D-175` | Register runtime-city progress state | Immutable stage/work snapshots expose existing generation without adding RNG calls, scheduling ownership, or recurring allocation | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityGenerationProgressSystemHelper.cs;scope=system-helper;maxLines=193;maxBytes=6447;task=APH-713)` |
| 2026-07-16 | `D-176` | Register isolated map composition ownership | The composition helper remains subordinate to one managed ECS frame owner and contains generation lifecycle, presentation coordination, cancellation, and cleanup policy without becoming a MonoBehaviour | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityRAndDMapCompositionSystemHelper.cs;scope=system-helper;maxLines=938;maxBytes=39593;task=APH-713)` |
| 2026-07-16 | `D-177` | Review isolated map composition size | The 938-line R&D composition is accepted at an exact ceiling while follow-up extraction remains required only where it creates a cohesive policy boundary; this authorizes no further growth | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityRAndDMapCompositionSystemHelper.cs;scope=production-over-500-review;maxLines=938;maxBytes=39593;task=APH-713)` |
| 2026-07-16 | `D-178` | Register runtime road visual presentation | Road strokes reuse shared planning callbacks and realize bounded prototype geometry only under the disposable R&D root | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityRoadVisualPrototypeSystemHelper.cs;scope=system-helper;maxLines=280;maxBytes=10568;task=APH-713)` |
| 2026-07-16 | `D-179` | Register runtime map recovery policy | One field-only helper schedules at most one next-frame deterministic fallback and preserves terminal evidence without LINQ, temporary collections, or its own tick owner | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeOperationMapGenerationRecoverySystemHelper.cs;scope=system-helper;maxLines=79;maxBytes=2716;task=APH-713)` |
| 2026-07-16 | `D-180` | Register runtime surface geometry policy | Foundation and district surface mesh data is created in bounded generation batches and has no steady-state update path | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeOperationMapSurfaceGeometrySystemHelper.cs;scope=system-helper;maxLines=79;maxBytes=2658;task=APH-713)` |
| 2026-07-16 | `D-181` | Register runtime visual quality policy | Ground coverage and obstruction suppression run only while realizing generated content and publish bounded counters for validation | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeOperationMapVisualQualitySystemHelper.cs;scope=system-helper;maxLines=185;maxBytes=7705;task=APH-713)` |
| 2026-07-16 | `D-182` | Register accepted-recipe presentation | Incremental recipe realization is bounded by item and time budgets, owns no frame loop, and disposes all output through the isolated generated root | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeOperationMapVisualRecipePresentationSystemHelper.cs;scope=system-helper;maxLines=385;maxBytes=17442;task=APH-713)` |
| 2026-07-16 | `D-183` | Review shared RuntimeCity visual-prototype extension | The reused city composition gained only explicit plan-only and visual-only adapter wiring for the isolated R&D path; production defaults, scheduling ownership, and RNG order remain unchanged | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityCompositionSystemHelper.cs;scope=production-over-500-review;maxLines=858;maxBytes=47077;task=APH-713)` |
| 2026-07-16 | `D-184` | Bound shared RuntimeCity helper growth | The exact helper-growth ceiling covers the existing visual-prototype bridge and observability surface without authorizing additional responsibilities or recurring allocation | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityCompositionSystemHelper.cs;scope=system-helper-growth;maxLines=858;maxBytes=47077;task=APH-713)` |
| 2026-07-16 | `D-185` | Bound visual-only building placement growth | Visual-only placement retains the shared footprint and rejection path while production realization remains unchanged. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityBuildingPlacementPrefabSystemHelper.cs;scope=system-helper-growth;maxLines=256;maxBytes=10862;task=APH-713)` |
| 2026-07-16 | `D-186` | Bound district-aware plot growth | Deterministic district intent extends existing candidate policy without adding a planner or frame owner. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityBuildingPlotUtilitySystemHelper.cs;scope=system-helper-growth;maxLines=282;maxBytes=10679;task=APH-713)` |
| 2026-07-16 | `D-187` | Bound staged bulk-spawn observability growth | Existing generation batches publish work progress while preserving their yield cadence and placement order. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityBulkBuildingSpawnRoutinePrefabSystemHelper.cs;scope=system-helper-growth;maxLines=170;maxBytes=8947;task=APH-713)` |
| 2026-07-16 | `D-188` | Bound plan-only bulk plot growth | The visual prototype records accepted plots through the existing planner boundary without creating gameplay entities. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityBulkPlotPlanUtilitySystemHelper.cs;scope=system-helper-growth;maxLines=227;maxBytes=9324;task=APH-713)` |
| 2026-07-16 | `D-189` | Bound visual decoration realization growth | Explicit visual-only decoration realization remains behind the shared spawn bridge and changes no production default. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityDecorationBuildingSpawnPrefabSystemHelper.cs;scope=system-helper-growth;maxLines=133;maxBytes=5234;task=APH-713)` |
| 2026-07-16 | `D-190` | Bound visual free-scatter policy growth | Allocation-free visual envelopes and authored-anchor reserves apply only to the isolated prototype mode. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityFreeScatterDecorationPrefabSystemHelper.cs;scope=system-helper-growth;maxLines=186;maxBytes=6929;task=APH-713)` |
| 2026-07-16 | `D-191` | Bound generation progress growth | Immutable stage and work reporting observes existing lifecycle transitions without changing RNG order or scheduling. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityGenerationCompositionSystemHelper.cs;scope=system-helper-growth;maxLines=412;maxBytes=21933;task=APH-713)` |
| 2026-07-16 | `D-192` | Bound visual repetition policy growth | Deterministic consecutive-selection limits are explicit visual-prototype policy and production selection remains the default. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityPrefabSelectionPrefabSystemHelper.cs;scope=system-helper-growth;maxLines=238;maxBytes=8432;task=APH-713)` |
| 2026-07-16 | `D-193` | Bound runtime-city read-model growth | The read model exposes one immutable progress snapshot and adds no allocation or update owner. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityReadModelCompositionSystemHelper.cs;scope=system-helper-growth;maxLines=22;maxBytes=797;task=APH-713)` |
| 2026-07-16 | `D-194` | Bound visual terminal-road policy growth | Configured terminal compaction applies only to visual prototype runs while shared road planning and production defaults remain intact. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityRoadLayoutUtilitySystemHelper.cs;scope=system-helper-growth;maxLines=390;maxBytes=17084;task=APH-713)` |
| 2026-07-16 | `D-195` | Bound visual rural-envelope growth | Allocation-free rural bounds constrain prototype edge outliers without changing the production radius or RNG sequence. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityRuralBuildingSpawnPrefabSystemHelper.cs;scope=system-helper-growth;maxLines=158;maxBytes=6229;task=APH-713)` |
| 2026-07-16 | `D-196` | Bound plan-only and visual spawn bridge growth | Explicit gameplay, visual-only, and plan-only modes preserve one shared placement contract and stable run-local cleanup. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCitySpawnBridgePrefabSystemHelper.cs;scope=system-helper-growth;maxLines=261;maxBytes=9456;task=APH-713)` |
| 2026-07-16 | `D-197` | Bound grounded visual presentation growth | Generated visual prefabs are grounded at their renderer bounds during one-time realization with no steady-state work. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityVisualPresentationSystemHelper.cs;scope=system-helper-growth;maxLines=166;maxBytes=6374;task=APH-713)` |
| 2026-07-16 | `D-198` | Supersede accepted-recipe presentation ceiling for exact indexed replay | Bounded sibling-index resolution replaces ambiguous duplicate-name paths during generation only; reusable collider and renderer buffers remove per-slice component-array garbage, while the helper retains no update owner, LINQ, reflection, or steady-state work and remains disposed through the isolated generated root. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeOperationMapVisualRecipePresentationSystemHelper.cs;scope=system-helper;maxLines=454;maxBytes=20437;task=APH-713)` |
| 2026-07-16 | `D-199` | Require explicit cleanup authority for visual suppression | Exact replay passes no cleanup footprint, so the existing generation-time quality helper must preserve all authored terrain; obstruction suppression remains available only when reviewed cleanup data opts in and adds no scheduling or steady-state work. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeOperationMapVisualQualitySystemHelper.cs;scope=system-helper;maxLines=185;maxBytes=7730;task=APH-713)` |
| 2026-07-16 | `D-200` | Supersede isolated map composition helper ceiling | A six-line recipe-only early return prevents the legacy algorithmic realization from adding duplicate physical visuals when an exact replay recipe is active; the helper remains subordinate to `RuntimeCityRAndDMapSystem`, owns no independent update, and receives no additional growth allowance. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityRAndDMapCompositionSystemHelper.cs;scope=system-helper;maxLines=944;maxBytes=39735;task=APH-713)` |
| 2026-07-16 | `D-201` | Re-review isolated map composition size after exact replay | The exact-replay guard is accepted at the measured 944-line ceiling because it removes duplicate realization rather than adding a new responsibility; further growth still requires a separate cohesive-boundary review. | `source-growth-exception(path=Assets/Game/Scripts/Environment/RuntimeCityRAndDMapCompositionSystemHelper.cs;scope=production-over-500-review;maxLines=944;maxBytes=39735;task=APH-713)` |
| 2026-07-16 | `D-202` | Supersede the FirstLaunch panel presentation ceiling after preload coverage | Narrative-panel preload coverage extends the established presentation boundary without adding an update owner; the exact accepted source remains below the production review threshold and any further growth requires another appended decision. | Accepted FirstLaunch commit `7be5f22b365f282abdc247456ff2a5de533db279`; `source-growth-exception(path=Assets/Game/Scripts/Composition/Narrative/FirstLaunchNarrativePanelPresentationSystemHelper.cs;scope=system-helper;maxLines=140;maxBytes=5664;task=APH-712)` |
| 2026-07-16 | `D-203` | Supersede the FirstLaunch shell composition ceiling after preload coverage | The existing shell composition boundary receives the accepted preload handoff without adding a new ECS system, managed loop, or authority; any further growth requires another appended decision. | Accepted FirstLaunch commit `7be5f22b365f282abdc247456ff2a5de533db279`; `source-growth-exception(path=Assets/Game/Scripts/Composition/Narrative/FirstLaunchNarrativeShellCompositionSystemHelper.cs;scope=system-helper;maxLines=87;maxBytes=3386;task=APH-712)` |
| 2026-07-16 | `D-204` | Supersede the FirstLaunch sequence-presentation ceiling after preload coverage | The accepted preload integration remains in the established managed presentation adapter, stays below the 500-line review threshold, and authorizes no additional responsibility or future growth. | Accepted FirstLaunch commit `7be5f22b365f282abdc247456ff2a5de533db279`; `source-growth-exception(path=Assets/Game/Scripts/Composition/Narrative/FirstLaunchNarrativeSequencePresentationSystemHelper.cs;scope=system-helper;maxLines=487;maxBytes=21062;task=APH-712)` |
| 2026-07-19 | `D-205` | Register FirstLaunch locale text composition | One immutable locale overlay composes authored text with the established fallback resolver; it owns no update loop and its composition suffix reflects the managed boundary. | `source-growth-exception(path=Assets/Game/Scripts/Composition/Narrative/FirstLaunchNarrativeLocaleTextCompositionSystemHelper.cs;scope=system-helper;maxLines=61;maxBytes=2066;task=APH-712)` |
| 2026-07-19 | `D-206` | Supersede FirstLaunch portrait and voice selection | Localized voice indexing moved out of sequence orchestration into the existing portrait/voice presentation owner, keeping one bounded selection responsibility and shrinking the sequence owner below 500 lines. | `source-growth-exception(path=Assets/Game/Scripts/Composition/Narrative/FirstLaunchNarrativePortraitVoiceSelectionPresentationSystemHelper.cs;scope=system-helper;maxLines=99;maxBytes=3526;task=APH-712)` |
| 2026-07-19 | `D-207` | Supersede FirstLaunch profile composition | Language projection and persistence extend the existing profile load/save/default boundary without introducing another state or update owner. | `source-growth-exception(path=Assets/Game/Scripts/Composition/Narrative/FirstLaunchNarrativeProfileCompositionSystemHelper.cs;scope=system-helper;maxLines=163;maxBytes=6425;task=APH-712)` |
| 2026-07-19 | `D-208` | Supersede FirstLaunch sequence presentation after locale extraction | The sequence boundary retains only language application and localized control presentation after localized voice storage and selection moved to its dedicated presentation owner; it remains below 500 lines with an exact ceiling. | `source-growth-exception(path=Assets/Game/Scripts/Composition/Narrative/FirstLaunchNarrativeSequencePresentationSystemHelper.cs;scope=system-helper;maxLines=491;maxBytes=21331;task=APH-712)` |
| 2026-07-19 | `D-209` | Supersede Match gameplay startup composition | Three startup-state-machine lines preserve cohesive sequencing and recovery wiring; the exact ceiling adds no policy or update owner and avoids line-count-only fragmentation. | `source-growth-exception(path=Assets/Game/Scripts/Composition/MatchGameplayStartupCompositionSystemHelper.cs;scope=system-helper;maxLines=393;maxBytes=18334;task=APH-710)` |
| 2026-07-19 | `D-210` | Register Match scene reference composition | One World-scoped ECS reference boundary owns Match scene-view registration, lookup, replacement protection, and cleanup without a recurring loop. | `source-growth-exception(path=Assets/Game/Scripts/Composition/MatchSceneReferenceCompositionSystemHelper.cs;scope=system-helper;maxLines=80;maxBytes=2778;task=APH-710)` |
| 2026-07-19 | `D-211` | Register performance diagnostics reference composition | One World-scoped managed-component boundary owns diagnostics registration, lookup, replacement protection, and cleanup without a recurring loop. | `source-growth-exception(path=Assets/Game/Scripts/Composition/PerformanceDiagnosticsReferenceCompositionSystemHelper.cs;scope=system-helper;maxLines=84;maxBytes=3102;task=APH-710)` |
| 2026-08-26 | `D-212` | Register Campaign mission debrief composition | The existing Menu player loop delegates exact first-clear M02 sequence lookup, shared narrative presentation, World rebinding cleanup, and post-debrief Campaign routing to one bounded managed edge with no new update owner or gameplay authority. | `source-growth-exception(path=Assets/Game/Scripts/Composition/Narrative/CampaignMissionDebriefCompositionSystemHelper.cs;scope=system-helper;maxLines=248;maxBytes=9684;task=APH-712)` |
| 2026-08-26 | `D-213` | Supersede Campaign mission narrative composition for the playable M02 slice | The same Menu-owned helper now coordinates exact M02 brief, warning comms, and debrief stages through the shared presenter; attempt-scoped simulation pause preserves authoritative mission and wave timing, brief/comms remain in Match, debrief alone routes to Campaign, and no new update owner or narrative framework is added. | `source-growth-exception(path=Assets/Game/Scripts/Composition/Narrative/CampaignMissionDebriefCompositionSystemHelper.cs;scope=system-helper;maxLines=424;maxBytes=17005;task=APH-712)` |
| 2026-08-26 | `D-214` | Isolate the Campaign mission briefing read model from the dense UI root archetype | The real M02 HUD proved the 5 KB fixed-string briefing payload can exceed the chunk limit when attached to the existing dense UI root. One dedicated singleton now carries that projection; the existing Campaign projection and shell gateway remain its sole writer/reader, with no new loop, presenter, route, or managed runtime owner. | M02EB-029 Metal capture pass; Campaign UI 9/9; M01 Campaign 10/10; M01 briefing 11/11; source growth 17/17 |
| 2026-08-30 | `D-215` | Supersede the operation-map runtime bootstrap ceiling for published-generation reuse | The existing one-shot metadata/blob publication owner now exposes the exact generation of a reused active map so Match validates readiness against the published ECS identity instead of a hardcoded first generation. It adds no scene-loading authority, recurring update, gameplay policy, or allocation after publication. | `source-growth-exception(path=Assets/Game/Scripts/Composition/OperationMapRuntimeBootstrapSceneSystemHelper.cs;scope=system-helper;maxLines=296;maxBytes=10734;task=APH-710)` |

| 2026-09-19 | `D-216` | Register the Watch ARIA virtual-touch boundary | Before: no continuous touch actuator. After: one disposable InputSystem device owner, cancellation and contact sampling; no Button invocation, world targeting, gameplay orders or MonoBehaviour update. Exact ceiling covers the user-requested touch-only feature; no unrelated baseline is raised. | `source-growth-exception(path=Assets/Game/Scripts/UI/Screens/AriaTouchInputUiSystemHelper.cs;scope=system-helper;maxLines=171;maxBytes=6898;task=APH-714)` |
| 2026-09-19 | `D-217` | Cancel pointer ownership without committing a placement | User-requested instant handback requires clearing transient pointer state at the established UI input consumer. Adds no orders or new update owner; exact current ceiling only. Pre-existing content is preserved, not refactored or accepted as a broad architecture pass. | `source-growth-exception(path=Assets/Game/Scripts/Systems/BuildingPlacementInputRuntimeTickUiSystemHelper.cs;scope=system-helper-growth;maxLines=261;maxBytes=12327;task=APH-714)` |
| 2026-09-19 | `D-218` | Cancel virtual touch without releasing a selection or queued move | User-requested instant handback requires clearing transient pointer state at the established UI input consumer. Adds no orders or new update owner; exact current ceiling only. Pre-existing content is preserved, not refactored or accepted as a broad architecture pass. | `source-growth-exception(path=Assets/Game/Scripts/Systems/RtsSelectionRuntimeInputCompositionSystemHelper.cs;scope=system-helper-growth;maxLines=827;maxBytes=43798;task=APH-714)` |
| 2026-09-19 | `D-219` | Review bounded cancellation in the existing selection input owner | User-requested instant handback requires clearing transient pointer state at the established UI input consumer. Adds no orders or new update owner; exact current ceiling only. Pre-existing content is preserved, not refactored or accepted as a broad architecture pass. | `source-growth-exception(path=Assets/Game/Scripts/Systems/RtsSelectionRuntimeInputCompositionSystemHelper.cs;scope=production-over-500-review;maxLines=827;maxBytes=43798;task=APH-714)` |
