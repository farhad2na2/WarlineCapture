...
| Field | Status |
|---|---|
| Checklist complete | 91 / 128 |
| Checklist percent complete | 71.1% |
| Current phase | Phase 8 - Managed Helper Hot-path Migration |
| Quick wins complete | 6 / 6 |
| Current target | `RtsCameraRequestSystem.TryGetGridConfig` was fixed by caching the camera queue, grid config, and tactical-follow pose queries on the existing camera request system. The follow-up battle capture no longer reports `TryGetGridConfig` in the searched/top rows, and player-relevant GC dropped from 6,805 samples / 435,981 bytes to 4,539 samples / 229,108 bytes. The next exact action is to investigate the remaining `SelectionGameplayStartupSystemHelper.UpdateSelectionRuntimePhases` / selection-panel attribution around `UpdateMatchHudSelectionPanel` while respecting that the direct selection panel probe still reports `0` bytes. |
| Compiler status | Latest RtsCamera query-cache slice passed `git diff --check`, `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, and `dotnet build Game.Tests.Editor.csproj --no-restore -v:q -clp:ErrorsOnly` with 0 errors. Unity 6.5.2 focused camera validation and battle GC capture both passed. |
| Android baseline status | Not started |
| GC baseline status | Latest completed 2026-07-05 08:02:46 UTC battle-state Match GC capture after RtsCamera query caching: 300 requested frames after 180 warmup frames, 301 scanned profiler frames, 4,539 raw/player-relevant GC.Alloc samples / 229,108 raw/player-relevant bytes, and 0 editor/tooling/diagnostic samples. Runtime allocation probe assertion passed. `UIShellEcsPresentationSystem.Update`, `MenuBootstrapView.Update`, top-level gameplay runtime phases, selection subphases, `BuildingPlacementVisualPresentationSystemHelper.CreateBuildingVisualInstance`, `BuildingProductionRuntimeTick.UpdateActiveProductionTransports`, and `TransportBoardingCommandSystem` all reported `0` direct bytes. `RtsCameraRequestSystem.TryGetGridConfig` no longer appears in searched/top rows. |
...
- Next iteration: investigate the remaining `SelectionGameplayStartupSystemHelper.UpdateSelectionRuntimePhases` / `UpdateMatchHudSelectionPanel` hierarchy attribution with narrower evidence, because the direct selection panel allocation probe still reports `0` bytes.
