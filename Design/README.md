...
- First-launch narrative production: `NarrativeVision/FirstLaunch/IMPLEMENTATION_TRACKER.md` owns the gated reference, style, continuity, storyboard, animatic, layered-art, reusable-player, reviewer-mode, Skip-to-game, handoff, and acceptance plan for the first screens. Phase 10R is currently active and blocks M01 until the second live review accepts UI scale, introductions, and layered runtime sound.
- First-launch runtime presentation: `NarrativeVision/FirstLaunch/RUNTIME_DIALOGUE_IMPLEMENTATION_SPEC.md` owns the live dialogue, control sizing, 9-slice quality, location/character introduction, voice/music/ambience/event-audio, accessibility, and revalidation contract.
- First-launch Gate 6 review: `NarrativeVision/FirstLaunch/ArtReview/FinalArt/GATE6_REVIEW_PACKAGE.md` owns the current 22-panel review package and links its safe-area, comparison, motion, provenance, and disposition evidence.
...
- Architecture/performance hardening: `Architecture/architecture_performance_hardening_implementation_tracker.md` is complete for early development with `107 / 107` explicit dispositions. `Architecture/post_hardening_architecture_maturity_tracker.md` is ready to start its 68-task Core Architecture Lane now. `Architecture/pre_release_performance_certification_backlog.md` and the maturity tracker's 18-task Release Certification Lane remain inactive until beta/release activation criteria are met.
...
- Main menu visual direction: `UIUX_MainMenu_Visual_Contract.md` now points at the command-base style with Campaign, Operations, Skirmish, Store, Commander, Settings, the persistent Credits and Command counters, and Deploy Operation.
...
48. `Architecture/post_hardening_architecture_maturity_tracker.md`
49. `Architecture/pre_release_performance_certification_backlog.md`
50. `Architecture/performance_regression_contract.md`
51. `Designer_Role_And_Documentation_Workflow.md`
52. `Agent_Coordination_Workflow.md`
...
- `NarrativeVision/FirstLaunch/IMPLEMENTATION_TRACKER.md` - detailed progress tracker for the reviewable first-launch motion-comic slice, reusable art, Unity sequence player, reviewer controls, Skip-to-game behavior, active Phase 10R presentation revision, 3D handoff, and validation gates.
- `NarrativeVision/FirstLaunch/RUNTIME_DIALOGUE_IMPLEMENTATION_SPEC.md` - current implementation authority for live comic dialogue, phone-readable controls, introductions, layered runtime audio, accessibility, and Gate 9R evidence.
- `NarrativeVision/FirstLaunch/ArtReview/FinalArt/GATE6_REVIEW_PACKAGE.md` - current complete Gate 6 art review entry point for all 22 panel revisions and supporting evidence.
...
- [Operations roadmap](Roadmap/Operations/PLAN.md) - 60-mission Operations design, original shared-runtime architecture and acceptance requirements; P0–P4 currently provide isolated models and O001–O003 capture prototypes.
- [O001 player-ready implementation](Roadmap/Operations/O001_PLAYER_READY_IMPLEMENTATION.md) - immediate P4R completion plan: real launch/input/gameplay, Old Quarter tactical experience, durable saves, EN/FA/ARIA, and separate manual-playable and release gates. Explains the gap between the original plan and delivered prototype scope.
...
- `Architecture/post_hardening_architecture_maturity_tracker.md` - ready 68-task Core Architecture Lane for ownership, lifecycle, UI allocation, pools, determinism, diagnostics, and CI maturity; its 18 release tasks stay deferred.
