...
- Menu/camp UI money reads, camp catalog reads, camp request validation/commands, friendly pending-production UI read models, selected-building display/health/preview reads, and selected-building modal confirm/destroy commands belong behind `BuildingUiCommandSystem`; `MenuView` must not call `BuildingPlacementSystem` directly for those UI paths.
