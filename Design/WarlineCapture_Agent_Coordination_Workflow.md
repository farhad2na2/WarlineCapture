...
Rule: update the source document first, then regenerate generated outputs. Do not manually edit `WarlineCapture_Project_State_Dashboard.md`.
...
- Update `WarlineCapture_Project_State_Source.json` only when project state changed.
- Regenerate `WarlineCapture_Project_State_Dashboard.md` after state-source updates.
- Update `WarlineCapture_Art_Asset_Requirements_Register.csv` only when asset status, approval, or completion changed.
