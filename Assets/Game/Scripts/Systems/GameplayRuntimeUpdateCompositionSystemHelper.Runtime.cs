...
{
    public sealed partial class GameplayRuntimeUpdateCompositionSystemHelper
    {
            ...
            ref bool gameplayStartPending)
        {
            ...
            if (simulationActive)
            {
                ...
                if (!GameplayRuntimeUpdateDebugFlags.DisableBuildingPlacementRuntime &&
                    !ShouldSkipBuildingSimulation(runtimeWorld))
                {
                    stepStart = performanceDiagnosticsSystem.BeginStep();
#if UNITY_EDITOR
                    using (new GameplayRuntimeAllocationProbeScope(GameplayRuntimeAllocationProbePhase.BuildingPlacement))
        ...
        internal static bool ShouldSkipBuildingSimulation(World runtimeWorld)
        {
            ...
            CampaignMissionRuntimeComponent mission = query.GetSingleton<CampaignMissionRuntimeComponent>();
            return mission.MissionId.Equals(FirstContactMissionId) &&
                   mission.Phase != MissionPhaseKind.None;
        }
