...
| Current task | `AM-025` blocked from acceptance but active for remediation; helicopter diagnostics now belong to their ECS World, while tactical-camera and building-command caches have verified lifecycle ownership. The `575` historical rows now contain `410` non-debt and `165` genuine-debt rows grouped into `77` unique file/rule items; five external helper paths remain |
