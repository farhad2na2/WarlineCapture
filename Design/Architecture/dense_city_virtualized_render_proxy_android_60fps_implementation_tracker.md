...
| Phase 6: Building and attachment state | In progress through VRP-065. Deterministic contiguous state-owner indices remain owned by canonical gameplay buildings, resident destruction behavior remains unchanged, and the first complete generated House family is candidate-virtualized. Its 3,638 canonical source owners each own exactly one intact and one destroyed recipe (7,276 state-linked placements), while 9,721 render-only Vegetation/Prop placements retain self-owned identities. The source-owner/logical-placement split produces 16,997 placements, 7,291 prototypes, 10,778 parts, 1,651 cells, and 929 fixed slots from 22,058 eligible renderer/material rows; 59,951 independently counted source render rows remain resident and 7,572 non-House building-family rows remain explicitly deferred. Focused stateful compound coverage forces two distant owners through one three-slot bucket and proves exact visible/off-camera/recycle state behavior. A new fail-closed architecture gate scans selection, targeting, minimap, navigation, and blocker consumers, requires their canonical gameplay-data anchors, and rejects any dependency on proxy-assignment/render-state components; the existing functional suites pass against canonical entity/component state. `OperationMapRenderStateSyncSystem` continues to initialize exact canonical state once per map generation, apply bounded changes in Burst, and mark only affected placements/cells. Production remains `StaticSceneChunks + ResidentEntities` with `productionCutover=0`; broader-family expansion is VRP-066 and Android destruction/recycling acceptance remains VRP-095. |
...
Checklist progress: `52 / 80` complete.
