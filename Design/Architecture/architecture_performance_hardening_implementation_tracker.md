...
| Last verified commit | `434bc7c44` on `main`; the compact APH-704 candidate remains disabled after exact `9/9` equivalence, `74.88%` benchmark improvement, zero warmed allocation, and affected architecture/build gates. |
| Last update | 2026-07-14 - Building path-interaction context wiring moved into a 99-line stateless boundary, reducing its existing owner from 557 to 499 lines. Post-rebase AI Resource Exchange startup projection moved into a 239-line startup-only helper, restoring the shared facade from 463 to 294 lines; the 12 cohesive AI recovery component-initialization lines are frozen at the exact 554-line AI startup boundary. Focused behavior, exact source-policy, both project builds, and Menu-to-Match smoke pass. The global inventory remains 27 unrelated oversized owners. |
