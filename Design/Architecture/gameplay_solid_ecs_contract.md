...
- RoadBuildRuntimeStateSystem follow-up refactor is tracked in `Design/Architecture/road_build_runtime_state_system_refactor_roadmap.md`. The final target is deletion of `RoadBuildRuntimeStateSystem.cs`; until then it is temporary compatibility debt and must not grow beyond its baseline or gain new public/static runtime commands.
- Do not replace the temporary holder with `RoadBuildManager`, `RoadBuildFacade`, `RoadBuildController`, `RoadRuntimeStateSystem`, or any other broad managed shell. New road behavior must land in the narrow road systems named by the roadmap.
