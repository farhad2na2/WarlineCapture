...
{
    public static class CH02M01GridlockNarrativeBuilder
    {
        public const string Path = "Assets/Game/Configs/Narrative/Chapter02/CH02M01_Gridlock_Narrative.asset";
        [MenuItem("Game/Campaign/Gridlock/Build Voiced Narrative")]
        ...
        {
            CH02M01GridlockMediaImporter.ConfigureArt();
            if(voices) CH02M01GridlockMediaImporter.ConfigureVoices();
            System.IO.Directory.CreateDirectory("Assets/Game/Configs/Narrative/Chapter02"); AssetDatabase.Refresh();
            GridlockNarrativeLine[][] lines = {CH02M01GridlockNarrativeCopy.Opening,CH02M01GridlockNarrativeCopy.Brief,CH02M01GridlockNarrativeCopy.Comms,CH02M01GridlockNarrativeCopy.Debrief};
            string[] stages = {"opening","brief","comms","debrief"};
            ...
            InstallFadi(); AddPersian(); AssetDatabase.SaveAssets();
            ...
            var merged = (bootstrap.CampaignMissionNarrativeConfigs ?? System.Array.Empty<NarrativeSequenceConfig>())
                .Where(s => s != null && !s.SequenceId.StartsWith("seq.ch02.m01.",StringComparison.Ordinal) && s.SequenceId != "seq.ch02.open.broken_grid")
            ...
            Debug.Log("[CH02M01GridlockNarrativeBuilder] result=Passed sequences=4 panels=12 artSources=12 crops=24 locales=en,fa-IR voices="+voices);
        }

        private static void InstallFadi()
        {
            var catalog = AssetDatabase.LoadAssetAtPath<NarrativeSpeakerCatalog>("Assets/Game/Configs/Narrative/FirstLaunch/FirstLaunchSpeakers.asset");
            var data = new SerializedObject(catalog);
            var entries = data.FindProperty("speakers");
            SerializedProperty entry = null;
            for (int i=0; i<entries.arraySize; i++)
                if (entries.GetArrayElementAtIndex(i).FindPropertyRelative("speakerId").intValue == (int)NarrativeSpeakerId.Fadi)
                    entry = entries.GetArrayElementAtIndex(i);
            entry ??= entries.GetArrayElementAtIndex(entries.arraySize++);
            entry.FindPropertyRelative("speakerId").intValue = (int)NarrativeSpeakerId.Fadi;
            foreach (var pair in new[]{("name","Fadi"),("role","Road crew leader"),("accessibleLabel","Fadi, road crew leader")})
            {
                entry.FindPropertyRelative(pair.Item1+"Key").stringValue = "narrative.speaker.fadi."+pair.Item1;
            ...
            // Dialogue portrait derived from the canonical civilian model; provenance is retained.
            var portraitPath = "Assets/Game/Art/UI/Portraits/Generated/Portrait_Fadi_Gridlock.png";
            var importer=AssetImporter.GetAtPath(portraitPath) as TextureImporter;
            if(importer==null)throw new InvalidOperationException("Fadi dialogue portrait is missing.");
            importer.textureType=TextureImporterType.Sprite;importer.spriteImportMode=SpriteImportMode.Single;
            importer.mipmapEnabled=false;importer.isReadable=false;importer.maxTextureSize=1024;
            ...
            if (portrait == null) throw new InvalidOperationException("Canonical Fadi portrait is missing.");
            entry.FindPropertyRelative("identitySprite").objectReferenceValue = portrait;
            entry.FindPropertyRelative("accentColor").colorValue = new Color(.76f,.65f,.43f);
            data.ApplyModifiedPropertiesWithoutUndo();
            EditorUtility.SetDirty(catalog);
        }

        private static void Configure(string stage,GridlockNarrativeLine[] lines,NarrativeSequenceConfig basis)
        {
            string sequenceId = stage=="opening" ? "seq.ch02.open.broken_grid" : "seq.ch02.m01."+stage;
            ...
            if(sequence == null)
            {
                sequence = ScriptableObject.CreateInstance<NarrativeSequenceConfig>(); sequence.name = "CH02M01_Gridlock_"+stage;
            ...
            sequence.name="CH02M01_Gridlock_"+stage;
            SerializedObject data = new(sequence);
            data.FindProperty("sequenceId").stringValue = sequenceId;
            data.FindProperty("entryStateId").stringValue = "Gridlock-"+stage+"-0";
            data.FindProperty("defaultSkipDestinationId").stringValue = "Gridlock-"+stage+"-complete";
            ...
            for(int i=0;i<states.arraySize;i++)
            {
                ...
                string completionId = "Gridlock-"+stage+"-complete";
                S(state,"stateId",complete ? completionId : "Gridlock-"+stage+"-"+i);
                ...
                S(state,"continueStateId",complete ? string.Empty : i+1==lines.Length ? completionId : "Gridlock-"+stage+"-"+(i+1));
                S(state,"skipStateId",complete ? string.Empty : completionId);
                S(state,"completionPayloadId",complete ? "request.gridlock."+(stage=="brief" ? "interactive_brief" : stage)+".complete" : string.Empty);
                ...
                SerializedProperty line = authored.GetArrayElementAtIndex(0); GridlockNarrativeLine copy = lines[i];
                ...
                line.FindPropertyRelative("voiceClip").objectReferenceValue = CH02M01GridlockMediaImporter.Voice(copy.Id,false);
        ...
        private static void SetPanelReference(NarrativeStateRecord state,string field,string panel,bool wide)
        {
            ...
            if(panel!=null)
            {
                ...
                var group=settings.FindGroup("Gridlock Narrative") ?? settings.CreateGroup("Gridlock Narrative",false,false,false,null,
                    typeof(BundledAssetGroupSchema),typeof(ContentUpdateGroupSchema));
                var schema=group.GetSchema<BundledAssetGroupSchema>();
                schema.BundleMode=BundledAssetGroupSchema.BundlePackingMode.PackSeparately;
                var sprite=CH02M01GridlockMediaImporter.Panel(panel,wide);
                string guid=AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(sprite));
                AddressableAssetEntry entry=settings.CreateOrMoveEntry(guid,group,false,false);
                entry.SetAddress("narrative.gridlock."+panel,false);
        ...
        private static void AddPersian()
        {
            ...
            for(int i=entries.arraySize-1;i>=0;i--)
                if(entries.GetArrayElementAtIndex(i).FindPropertyRelative("key").stringValue.StartsWith("narrative.gridlock.",StringComparison.Ordinal)) entries.DeleteArrayElementAtIndex(i);
            SerializedProperty voices=data.FindProperty("voices");
            for(int i=voices.arraySize-1;i>=0;i--)
                if(voices.GetArrayElementAtIndex(i).FindPropertyRelative("lineId").stringValue.StartsWith("gridlock-",StringComparison.Ordinal)) voices.DeleteArrayElementAtIndex(i);
            foreach(GridlockNarrativeLine line in CH02M01GridlockNarrativeCopy.Opening.Concat(CH02M01GridlockNarrativeCopy.Brief).Concat(CH02M01GridlockNarrativeCopy.Comms).Concat(CH02M01GridlockNarrativeCopy.Debrief))
            {
                ...
                voice.FindPropertyRelative("voiceClip").objectReferenceValue=CH02M01GridlockMediaImporter.Voice(line.Id,true);
        ...
        private static float Duration(in GridlockNarrativeLine line)=>Mathf.Max(8,
            Mathf.Max(CH02M01GridlockMediaImporter.Voice(line.Id,false)?.length ?? 0,CH02M01GridlockMediaImporter.Voice(line.Id,true)?.length ?? 0)+1,
