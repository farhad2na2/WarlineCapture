...
| Field | Status |
|---|---|
| Checklist complete | 62 / 107 |
| Checklist percent complete | 57.9% |
| Current phase | Phase 8 - Fuel/Oil ECS Drift Correction |
| Quick wins complete | 6 / 6 |
| Current target | Disabled Burst-capable `BuildingResourceProductionEcsSystem` tick/apply surface added for explicit handoff validation. It reuses `BuildingResourceProductionSystemHelper` and mutates only `BuildingResourceStorageComponent` passed by ref; it is `[DisableAutoCreation]` and disabled on create, so it does not double-tick or replace the existing managed runtime-building production owner yet. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, and escalated Unity 6.5.2 focused validation log `/private/tmp/warline-arch-followup-resource-production-ecs-focused-escalated.log` passed with 0 compiler errors after the disabled ECS resource-production tick surface. Sandboxed Unity batchmode still hits the known licensing loop, so the documented out-of-sandbox workaround was used for Unity validation. |
...
- Next iteration: wire a single explicit Fuel/Oil managed-runtime-to-ECS apply handoff or ownership transfer using `BuildingResourceProductionEcsSystem.ApplyTick`, preserving the behavior tests and using populated runtime building combat ids as the bridge to existing runtime-building identity without double-ticking.
