...
| Current task | `AM-025` remains blocked from acceptance but active for remediation. The UI shell gateway now has focused proof that a new game session replaces stale application state and rebinds to the replacement ECS World. The `575` historical rows contain `511` non-debt and `64` genuine-debt rows grouped into `29` unique file/rule items. Separately owned FirstLaunch and operation-map helper paths still block the canonical source-growth gate. |
...
| Latest evidence | Schema-2 `am025_phase2_ownership_delta.json/.md` records one decision for each of `575` historical intake rows: `503` resolved, `8` protected-deferred, and `64` genuine debt grouped into `29` unique file/rule items. Focused Unity validation proves the UI gateway replaces stale session state and follows a replacement ECS World. `am025_phase2_ownership_delta_intake.json` preserves the immutable v1 intake, and policy schema 2 lists all five exact source-growth blockers |
