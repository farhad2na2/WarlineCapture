...
Planning/balance specification, 2026-09-20. These values are concrete initial tuning inputs, not shipped values or measured balance. Keep the two existing small presets unchanged until the expanded configurations pass their gates. This document and [BATTLE_CATALOG.md](BATTLE_CATALOG.md) refine the original [plan](PLAN.md); no Operations progression is specified here.
...
**Producer mapping dependency:** the current Barracks, Helipad and Airport exist; a ground vehicle staging producer and validated oil/refinery/materials throughput must be mapped to suitable existing buildings during E0/E1. These are proposed functional roles, not assertions that a factory or each conversion already works. If there is no suitable asset, document the required minimal new producer implementation. No card may deploy a unit without its real producer/delivery route.
...
Initial Standard throughput target per functioning supply backbone: **100 M/min fabrication and 120 Oil/min extraction**, delivered through real logistics. Refinery priority is visible and configurable; proposed conversion recipes consume 100 Oil for 300 M or 150 Fuel in 45 seconds. These are proposed expansion recipes, not a statement of the current exchange values. One module processes one recipe at a time, no unlimited concurrent conversions. Baseline fabrication creates Materials only through a functioning configured facility; it is not an invisible wallet pulse. No passive fuel regeneration without oil/refinery delivery.

War throughput target 160 M/min + 180 Oil/min; Large War 220 + 240. Implement as explicit facility capacities/modules shared by both factions, not global difficulty multipliers. Expansion facilities use their documented rates. Verify stock/capacity, truck throughput and queue utilization together. The target is first counter within roughly 45–90 sec, first bought vehicle 1–3 min, first offensive helicopter 4–7 min, advanced air 8–12 min in Field starts, and a viable War army by 12–18 min. Established starts compress this. Failed timing calculations reopen costs/rates; do not publish an impossible economy table.
