...
- `2026-06-22` - `P7-0261` `BuildingPlacementVisualSystem`: folded disabled building placement visual `SystemBase` wrapper into a plain direct-owned helper. `BuildingGameplayCompositionSourceSystem` now instantiates the helper directly; visual wrapper creation, prefab instantiation, combined-mesh renderer filtering, placement positioning, and gate alignment behavior stayed unchanged.
- `2026-06-22` - `P7-0263` `BuildingRuntimeVisualSystem`: folded disabled runtime building visual `SystemBase` wrapper into a plain direct-owned helper. `BuildingGameplayCompositionSourceSystem` now instantiates the helper directly; runtime visual initialization, Door_Z/barrier setup, animated parts, faction renderer caching/application, resource visual animation updates, and destroyed-building marker visibility refresh stayed unchanged.
- `2026-06-22` - `P7-0264` `BuildingSelectionMarkerSystem`: folded disabled building selection marker `SystemBase` wrapper into a plain direct-owned helper. `BuildingGameplayCompositionSourceSystem` now instantiates the helper directly; marker prefab instantiation, active-building selection resolution, marker scale/positioning, premium boundary view setup, object outline setup, hide, and dispose behavior stayed unchanged.
