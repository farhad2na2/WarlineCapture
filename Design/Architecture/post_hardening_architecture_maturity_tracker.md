...
| Current task | `AM-025` remains blocked from acceptance but active for remediation. Menu startup settings now track their ECS World inside the active menu composition and release it during shutdown. The `575` historical rows contain `512` non-debt and `63` genuine-debt rows grouped into `28` unique file/rule items. Separately owned FirstLaunch and operation-map helper paths still block the canonical source-growth gate. |
...
| Latest evidence | Schema-2 `am025_phase2_ownership_delta.json/.md` records one decision for each of `575` historical intake rows: `504` resolved, `8` protected-deferred, and `63` genuine debt grouped into `28` unique file/rule items. Focused Unity validation proves menu settings ownership is isolated per menu instance and released during shutdown. `am025_phase2_ownership_delta_intake.json` preserves the immutable v1 intake, and policy schema 2 lists all five exact source-growth blockers |
