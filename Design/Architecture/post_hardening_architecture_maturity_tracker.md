...
| Current task | `AM-025` blocked from acceptance but active for remediation; tactical camera state is isolated, Editor-only switches have reset ownership, and match-start, building-AI, diagnostics, logistics telemetry, unit lookup, selected-unit lifecycle, follow-camera mode, pathfinding pending state, central match state, scene-transition queue, diagnostics state, startup readiness counts, player-input state, visible-unit selection, building queries, plus the building runtime boundary now have replacement-World proof. The `575` historical rows now contain `440` non-debt and `135` genuine-debt rows grouped into `59` unique file/rule items; five external helper paths remain |
