...
Date: 2026-07-28
Status: Candidate implementation in progress through fail-closed map-generation initialization and exact bounded persistent native render-virtualization state; persisted candidate and production cutover remain disabled
...
| Phase 4: Jobified runtime assignment | In progress; the ordered camera boundary publishes one immutable unmanaged snapshot per update with a nonzero version and deterministic 128-bit signature. The baked database entity now owns exactly one default-safe virtualization state, metrics component, and bounded state-change buffer. `OperationMapRenderVirtualizationInitializationSystem` validates one active map/database/state owner, exact map/schema/content/residency identity, contiguous bucket capacities, and unique in-range slot identities before allocating exact persistent slot-to-logical-row, logical-row-to-slot, and placement-prefix arrays initialized unbound. Same-generation updates are no-ops; generation changes complete dependencies, dispose the old arrays, rebuild exact capacities, clear boundary state, and retain readiness publication in `Game.Composition`. The pure guard-envelope decision rebuilds only for initial view, map-generation change, explicit force/camera discontinuity, required-cell escape, or pending visual-state change; invalid envelopes reject instead of silently scheduling. Burst cell selection, assignment, apply, metrics publication, and full unload/job-lifetime proof remain open. |
...
Checklist progress: `34 / 80` complete.
