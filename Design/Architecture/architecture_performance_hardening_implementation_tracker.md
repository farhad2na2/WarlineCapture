...
| Last verified commit | `f74b931c7` on `main`; the schema-4 product-budget contract passes `5/5` and Editor-test compilation with zero errors without changing any accepted budget. |
| Last update | 2026-07-13 - APH-704 design/benchmark is complete locally with a compact fixed-head linked-cell index. The disabled candidate preserves exact results at `9/9`; the post-rebase run measures direct/indexed complete acquisition at `2.723/0.684 ms`, improves `74.88%`, allocates zero managed bytes, and uses `18,784` bytes. ECS/Burst, naming/non-ECS, assembly boundaries, and affected builds pass. Runtime consumer adoption remains disabled and must be a separate scheduling/equivalence slice. APH-803/804 package/device acceptance and APH-501 measured release limits remain active. The global source-growth run stops first on four pre-existing resource/tactical helper ledger entries; audio/building-defense findings remain separately owned. |
