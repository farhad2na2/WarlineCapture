...
| Parallel preparatory work | `AM-026` remains dependency-blocked and receives no completion credit; `ui_projection_allocation_inventory.md` inventories 29 surfaces and reconciles every declared route/popup contract; drafts `AM-WP-001` through `AM-WP-008` bound Resource Exchange, Build Drawer, Selection/Passenger, Placement, Loading, Shell Layout, Match Status, and Squad Tray/Command Wheel ownership, lifecycle, semantic identities, and real-path performance/allocation gates; `AM-WP-007` is additionally blocked on explicit mission/objective owner handoff |
