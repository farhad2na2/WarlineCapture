...
namespace Game.UI.Runtime
{
    public sealed partial class MissionResultPopupView
    {
        ...
        private void ApplyExtractionOutcome(in UiMissionResultPopupModel model)
        {
            ...
            titleText?.GetComponent<V3LocalizedTextBinding>()?.SetFontBounds(24,48);
            missionNameText?.GetComponent<V3LocalizedTextBinding>()?.SetFontBounds(20,36);
            ...
            if(defenseLabels!=null)foreach(var label in defenseLabels)
            {
                ...
                label.Text?.GetComponent<V3LocalizedTextBinding>()?.SetFontBounds(18,27);SetDefenseText(label.Text,GameText.Get(key));
            }
            ...
            SetDefenseText(starCountText,GameText.Format("mission.m03.result.stars","",model.Stars));SetDefenseText(civilianLostText,model.Extraction.Losses.ToString());
            SetDefenseText(objectivePatrolStatusText,model.Extraction.Delivered+" / 4");
            SetDefenseText(objectiveSquadStatusText,GameText.Get(model.Extraction.CarrierLost||model.Extraction.AircraftLost?"mission.m03.result.losses":"mission.m03.result.safe"));
            SetDefenseText(objectiveCivilianStatusText,GameText.Get(model.Outcome==UiMissionResultOutcome.Victory?"mission.m03.result.safe":"mission.m03.result.incomplete"));
