...
public sealed class SkirmishExpandedDefinitionTests
{
    ...
    [Test]
    public void S004AssetsReuseDesertBaseLayoutAndStayPlayable()
    {
        var definition = AssetDatabase.LoadAssetAtPath<SkirmishScenarioDefinitionConfig>(
    ...
    [Test]
    public void S005RegularStandardCompilesFirstVisitSeeds()
    {
        uint previous = 0;
        for (int i = 0; i < SkirmishS005FirstVisit.RegularStandardSeeds.Length; i++)
        {
            int seed = SkirmishS005FirstVisit.RegularStandardSeeds[i];
            CompileS005(SkirmishSizeId.Standard, seed, out SkirmishResolvedSetup setup, out SkirmishLaunchPayload payload);
            Assert.AreEqual("S005", setup.CatalogId);
            Assert.AreEqual("skirmish.s005", setup.DefinitionId);
            Assert.AreEqual("scenario.skirmish.s005", setup.ScenarioSetupId);
            Assert.AreEqual("opmap.skirmish.desert_base_01", setup.OperationMapId);
            Assert.AreEqual("layout.skirmish.db.ba", setup.LayoutId);
            Assert.AreEqual(SkirmishArmyProfileId.CombinedArms, setup.ArmyProfileId);
            Assert.AreEqual(SkirmishStartPackageId.FieldBase, setup.StartPackageId);
            Assert.AreEqual(SkirmishObjectiveKind.BaseAssault, setup.ObjectiveKind);
            Assert.AreEqual(1, (int)setup.Readiness);
            Assert.AreEqual(8, Count(setup, 1, SkirmishRoleKind.Rifle));
            ...
            Assert.AreEqual(1, Count(setup, 1, SkirmishRoleKind.ApcArmored));
            Assert.AreEqual(0, Count(setup, 1, SkirmishRoleKind.Tank));
            Assert.AreEqual(0, Count(setup, 1, SkirmishRoleKind.AntiAir));
            Assert.AreEqual(0, Count(setup, 1, SkirmishRoleKind.AttackHeli));
            Assert.AreEqual(12, setup.PlayerInfantry);
            Assert.AreEqual(2, setup.PlayerGround);
            Assert.AreEqual(0, setup.PlayerAir);
            ...
            Assert.AreEqual(1080, setup.DeadlineSeconds);
            Assert.AreEqual(7, setup.PlayerStartingStructures);
            Assert.AreEqual(1, setup.InfantryQueuesEach);
            Assert.AreEqual(1, setup.VehicleQueuesEach);
            Assert.AreEqual(1, setup.LogisticsQueuesEach);
            Assert.AreEqual(seed, payload.Seed);
            Assert.IsTrue(setup.MeasuredLayoutBound);
            Assert.AreEqual(-228f, setup.PlayerBaseWorldX, 0.05f);
            Assert.AreEqual(-192f, setup.PlayerStagingWorldX, 0.05f);
            Assert.IsFalse(HasStructure(setup, SkirmishStructureIds.Helipad, false));
            Assert.IsFalse(HasStructure(setup, SkirmishStructureIds.Airport, false));
            Assert.AreEqual(4, setup.Structures.Length);
            Assert.IsTrue(SkirmishRoleOverlayCatalog.TryGet(setup.RoleOverlays, SkirmishRoleKind.Tank, out _));
            Assert.IsTrue(SkirmishRoleOverlayCatalog.TryGet(setup.RoleOverlays, SkirmishRoleKind.AttackHeli, out _));
            Assert.IsTrue(SkirmishRoleOverlayCatalog.TryGet(setup.RoleOverlays, SkirmishRoleKind.Siege, out _));
            Assert.AreNotEqual(previous, setup.SetupHash);
            previous = setup.SetupHash;
        }

        CompileS005(SkirmishSizeId.Standard, SkirmishS005FirstVisit.SeedA, out SkirmishResolvedSetup again, out _);
        CompileS005(SkirmishSizeId.Standard, SkirmishS005FirstVisit.SeedA, out SkirmishResolvedSetup twin, out _);
        Assert.AreEqual(again.SetupHash, twin.SetupHash);
        CompileS003(SkirmishSizeId.Standard, SkirmishS003FirstVisit.SeedA, out SkirmishResolvedSetup airField, out _);
        Assert.IsFalse(SkirmishRoleOverlayCatalog.TryGet(airField.RoleOverlays, SkirmishRoleKind.Tank, out _));
        Assert.IsFalse(HasStructure(airField, SkirmishStructureIds.Helipad, false));
    }

    [Test]
    public void S005FieldSizesMatchMatrixWithoutStartingAir()
    {
        CompileS005(SkirmishSizeId.War, 393246, out SkirmishResolvedSetup war, out _);
        CompileS005(SkirmishSizeId.LargeWar, 458884, out SkirmishResolvedSetup large, out _);
        Assert.AreEqual(1500, war.DeadlineSeconds);
        Assert.AreEqual(675, war.MaterialsEach);
        Assert.AreEqual(12, Count(war, 1, SkirmishRoleKind.Rifle));
        Assert.AreEqual(4, Count(war, 1, SkirmishRoleKind.Gunner));
        Assert.AreEqual(2, Count(war, 1, SkirmishRoleKind.ApcArmored));
        Assert.AreEqual(0, Count(war, 1, SkirmishRoleKind.Tank));
        Assert.AreEqual(0, war.PlayerAir);
        Assert.AreEqual(23, war.PlayerCombat);
        Assert.AreEqual(7, war.PlayerStartingStructures);
        Assert.AreEqual(2, war.InfantryQueuesEach);
        Assert.AreEqual(1800, large.DeadlineSeconds);
        Assert.AreEqual(900, large.MaterialsEach);
        Assert.AreEqual(16, Count(large, 1, SkirmishRoleKind.Rifle));
        Assert.AreEqual(2, Count(large, 1, SkirmishRoleKind.Car));
        Assert.AreEqual(0, large.PlayerAir);
        Assert.AreEqual(28, large.PlayerCombat);
        Assert.IsFalse(war.MeasuredLayoutBound);
        Assert.IsFalse(large.MeasuredLayoutBound);
        Assert.IsFalse(HasStructure(war, SkirmishStructureIds.Helipad, false));
        Assert.IsFalse(HasStructure(large, SkirmishStructureIds.Airport, false));
    }

    [Test]
    public void S005CombinedArmsKeepsArmorAndGatesFieldAir()
    {
        SkirmishExpansionAuthoredSet authored = SkirmishExpansionCatalogFactory.CreateInMemory();
        Assert.IsTrue(authored.ArmyCombined.AllowsOffensiveAir);
        Assert.IsTrue(authored.ArmyCombined.AllowsAdvancedAir);
        Assert.IsTrue(authored.ArmyCombined.AllowsAdvancedGround);
        Assert.IsFalse(authored.ArmyGround.AllowsOffensiveAir);
        Assert.IsFalse(authored.ArmyAir.AllowsAdvancedGround);
        Assert.IsTrue(authored.ArmyCombined.Allows(SkirmishRoleIds.Tank));
        Assert.IsTrue(authored.ArmyCombined.Allows(SkirmishRoleIds.ApcHeavy));
        Assert.IsTrue(authored.ArmyCombined.Allows(SkirmishRoleIds.Siege));
        Assert.IsTrue(authored.ArmyCombined.Allows(SkirmishRoleIds.AntiAir));
        Assert.IsTrue(authored.ArmyCombined.Allows(SkirmishRoleIds.AttackHeli));
        Assert.AreEqual(SkirmishStartPackageId.FieldBase, authored.DefinitionS005.StartPackageConfig.Kind);
        Assert.AreEqual(SkirmishReadinessStage.Field, authored.DefinitionS005.StartPackageConfig.Readiness);
        SkirmishRoleOverlay[] overlays = SkirmishRoleOverlayCatalog.CreateCombinedArmsSlice();

        SkirmishProductionDecision aa = EvaluateCombined(
            authored, overlays, SkirmishRoleIds.AntiAir, SkirmishRoleKind.AntiAir, SkirmishReadinessStage.Field, false, false);
        Assert.IsTrue(aa.Accepted);
        Assert.AreEqual(220, aa.MaterialsCost);
        Assert.AreEqual(SkirmishProducerKind.GroundStaging, aa.Producer);

        SkirmishProductionDecision earlyTank = EvaluateCombined(
            authored, overlays, SkirmishRoleIds.Tank, SkirmishRoleKind.Tank, SkirmishReadinessStage.Field, false, false);
        Assert.IsFalse(earlyTank.Accepted);
        Assert.AreEqual(SkirmishReasonCode.MissingReadiness, earlyTank.Reason);

        SkirmishProductionDecision tank = EvaluateCombined(
            authored, overlays, SkirmishRoleIds.Tank, SkirmishRoleKind.Tank, SkirmishReadinessStage.Established, false, false);
        Assert.IsTrue(tank.Accepted);
        Assert.AreEqual(360, tank.MaterialsCost);
        Assert.AreEqual(SkirmishProducerKind.GroundStaging, tank.Producer);

        SkirmishProductionDecision earlyHeavy = EvaluateCombined(
            authored, overlays, SkirmishRoleIds.ApcHeavy, SkirmishRoleKind.ApcHeavy, SkirmishReadinessStage.Field, false, false);
        Assert.AreEqual(SkirmishReasonCode.MissingReadiness, earlyHeavy.Reason);
        SkirmishProductionDecision heavy = EvaluateCombined(
            authored, overlays, SkirmishRoleIds.ApcHeavy, SkirmishRoleKind.ApcHeavy, SkirmishReadinessStage.Established, false, false);
        Assert.IsTrue(heavy.Accepted);
        Assert.AreEqual(280, heavy.MaterialsCost);

        SkirmishProductionDecision earlySiege = EvaluateCombined(
            authored, overlays, SkirmishRoleIds.Siege, SkirmishRoleKind.Siege, SkirmishReadinessStage.Established, false, false);
        Assert.AreEqual(SkirmishReasonCode.MissingReadiness, earlySiege.Reason);
        SkirmishProductionDecision siege = EvaluateCombined(
            authored, overlays, SkirmishRoleIds.Siege, SkirmishRoleKind.Siege, SkirmishReadinessStage.FullArsenal, false, false);
        Assert.IsTrue(siege.Accepted);
        Assert.AreEqual(420, siege.MaterialsCost);
        Assert.AreEqual(SkirmishProducerKind.GroundStaging, siege.Producer);

        SkirmishProductionDecision earlyAir = EvaluateCombined(
            authored, overlays, SkirmishRoleIds.AttackHeli, SkirmishRoleKind.AttackHeli, SkirmishReadinessStage.Field, false, false);
        Assert.AreEqual(SkirmishReasonCode.MissingReadiness, earlyAir.Reason);
        SkirmishProductionDecision missingPad = EvaluateCombined(
            authored, overlays, SkirmishRoleIds.AttackHeli, SkirmishRoleKind.AttackHeli, SkirmishReadinessStage.Established, false, false);
        Assert.AreEqual(SkirmishReasonCode.MissingProducer, missingPad.Reason);
        Assert.AreEqual("producer.helipad", missingPad.Field);
        SkirmishProductionDecision heli = EvaluateCombined(
            authored, overlays, SkirmishRoleIds.AttackHeli, SkirmishRoleKind.AttackHeli, SkirmishReadinessStage.Established, true, false);
        Assert.IsTrue(heli.Accepted);
        Assert.AreEqual(420, heli.MaterialsCost);

        SkirmishProductionDecision jet = EvaluateCombined(
            authored, overlays, SkirmishRoleIds.Fighter, SkirmishRoleKind.Fighter, SkirmishReadinessStage.FullArsenal, true, false);
        Assert.AreEqual(SkirmishReasonCode.MissingProducer, jet.Reason);
        Assert.AreEqual("producer.airport", jet.Field);
        SkirmishProductionDecision advanced = EvaluateCombined(
            authored, overlays, SkirmishRoleIds.Fighter, SkirmishRoleKind.Fighter, SkirmishReadinessStage.FullArsenal, true, true);
        Assert.IsTrue(advanced.Accepted);
        Assert.AreEqual(480, advanced.MaterialsCost);

        SkirmishProductionDecision airTank = SkirmishProductionEligibility.Evaluate(
            new SkirmishProductionRequest
            {
                RoleId = SkirmishRoleIds.Tank,
                RoleKind = SkirmishRoleKind.Tank,
            ...
            SkirmishReadinessStage.Established,
            overlays);
        Assert.AreEqual(SkirmishReasonCode.UnsupportedRole, airTank.Reason);
    ...
    [Test]
    public void S005AssetsReuseDesertBaseLayoutAndStayInProgress()
    {
        var definition = AssetDatabase.LoadAssetAtPath<SkirmishScenarioDefinitionConfig>(
            "Assets/Game/Configs/SkirmishExpansion/Scenarios/S005/SkirmishScenario_S005.asset");
        var scenario = AssetDatabase.LoadAssetAtPath<ScenarioSetupConfig>(
            "Assets/Game/Configs/SkirmishExpansion/Scenarios/S005/ScenarioSetup_S005.asset");
        var layout = AssetDatabase.LoadAssetAtPath<SkirmishMapLayoutConfig>(
            "Assets/Game/Configs/SkirmishExpansion/Scenarios/S005/SkirmishLayout_S005.asset");
        var shared = AssetDatabase.LoadAssetAtPath<SkirmishMapLayoutConfig>(
            "Assets/Game/Configs/SkirmishExpansion/Shared/SkirmishLayout_DB_BA.asset");
        var army = AssetDatabase.LoadAssetAtPath<SkirmishArmyProfileConfig>(
            "Assets/Game/Configs/SkirmishExpansion/Shared/SkirmishArmy_CombinedArms.asset");
        var publication = AssetDatabase.LoadAssetAtPath<SkirmishPublicationConfig>(
            "Assets/Game/Configs/SkirmishExpansion/Shared/SkirmishPublicationManifest.asset");
        Assert.IsNotNull(definition);
        Assert.IsNotNull(army);
        Assert.AreEqual("S005", definition.CatalogId);
        Assert.AreEqual("skirmish.s005", definition.DefinitionId);
        Assert.AreEqual(SkirmishArmyProfileId.CombinedArms, definition.ArmyProfileConfig.Kind);
        Assert.AreEqual(SkirmishArmyProfileId.CombinedArms, army.Kind);
        Assert.AreEqual(SkirmishStartPackageId.FieldBase, definition.StartPackageConfig.Kind);
        Assert.AreEqual(SkirmishSizeId.Standard, definition.FirstVisitSize);
        Assert.AreEqual(SkirmishSizeId.War, definition.RecommendedSize);
        Assert.AreEqual("layout.skirmish.db.ba", definition.MapLayoutId);
        Assert.AreEqual("skirmish.s005.title", definition.TitleKey);
        Assert.AreEqual("skirmish.s005.brief", definition.BriefingKey);
        Assert.AreEqual("skirmish.s005.objective", definition.ObjectiveKey);
        Assert.AreEqual("skirmish.s005.result.victory", definition.ResultVictoryKey);
        Assert.AreEqual("skirmish.s005.result.defeat", definition.ResultDefeatKey);
        Assert.AreEqual(shared.LayoutId, layout.LayoutId);
        Assert.AreEqual(shared.WorldWidthMetres, layout.WorldWidthMetres);
        Assert.AreEqual(shared.WorldDepthMetres, layout.WorldDepthMetres);
        Assert.IsTrue(layout.TryGetAnchor("staging.player", out SkirmishLayoutAnchorConfig staging));
        Assert.AreEqual(-192f, staging.WorldX, 0.05f);
        Assert.IsTrue(layout.TryGetAnchor("base.player", out SkirmishLayoutAnchorConfig barracks));
        Assert.AreEqual(-228f, barracks.WorldX, 0.05f);
        Assert.AreEqual("scenario.skirmish.s005", scenario.ScenarioId);
        Assert.AreEqual(SkirmishS005FirstVisit.SeedA, scenario.DeterministicSeed);
        Assert.IsTrue(publication.TryGet("S002", out SkirmishPublicationRowConfig s002));
        Assert.AreEqual(SkirmishPublicationStatus.Playable, s002.Status);
        Assert.IsTrue(publication.TryGet("S003", out SkirmishPublicationRowConfig s003));
        Assert.AreEqual(SkirmishPublicationStatus.Playable, s003.Status);
    ...
    [Test]
    public void S005CatalogWalkCompilesCombinedRowsOnFullManifest()
    {
        LoadMatrix(out List<SkirmishSetupMatrixRow> matrix);
        SkirmishExpansionAuthoredSet authored = SkirmishExpansionCatalogFactory.CreateInMemory();
        var ground = new SkirmishContentManifest { RequiredFeatureIds = authored.DefinitionS002.RequiredFeatureIds };
        Assert.IsTrue(SkirmishSetupCompiler.TryCompileCatalog(
            authored, matrix, ground, out List<SkirmishResolvedSetup> groundCompiled, out List<SkirmishCompileReason> groundReasons));
        int s005OnGround = 0;
        for (int i = 0; i < groundCompiled.Count; i++)
        {
            if (groundCompiled[i].CatalogId == "S005")
                s005OnGround++;
        }

        Assert.AreEqual(0, s005OnGround);
        bool missingAir = false;
        for (int i = 0; i < groundReasons.Count; i++)
        {
            if (groundReasons[i].Code == SkirmishReasonCode.UnsupportedCapability &&
                (groundReasons[i].Detail == "offensive_air" || groundReasons[i].Detail == "advanced_air"))
                missingAir = true;
        }

        Assert.IsTrue(missingAir);

        var manifest = new SkirmishContentManifest { RequiredFeatureIds = SkirmishS005FirstVisit.RequiredFeatureIds };
        Assert.IsTrue(SkirmishSetupCompiler.TryCompileCatalog(
            authored, matrix, manifest, out List<SkirmishResolvedSetup> compiled, out _));
        int s002 = 0;
        int s003 = 0;
        int s004 = 0;
        int s005 = 0;
        for (int i = 0; i < compiled.Count; i++)
        {
            ...
            if (compiled[i].CatalogId == "S004")
                s004++;
            if (compiled[i].CatalogId == "S005")
                s005++;
        }

        Assert.AreEqual(3, s002);
        Assert.AreEqual(3, s003);
        Assert.AreEqual(3, s004);
        Assert.AreEqual(3, s005);
    }
    ...
    public static void RunFocusedValidation()
    {
        try
        {
            ...
            suite.S004AssetsReuseDesertBaseLayoutAndStayPlayable();
            suite.S004CatalogWalkCompilesAirRowsAndLeavesS002OnItsOwnManifest();
            suite.S005RegularStandardCompilesFirstVisitSeeds();
            suite.S005FieldSizesMatchMatrixWithoutStartingAir();
            suite.S005CombinedArmsKeepsArmorAndGatesFieldAir();
            suite.S005AssetsReuseDesertBaseLayoutAndStayInProgress();
            suite.S005CatalogWalkCompilesCombinedRowsOnFullManifest();
    ...
    private static void CompileS005(
        SkirmishSizeId size,
        int seed,
        out SkirmishResolvedSetup setup,
        out SkirmishLaunchPayload payload)
    ...
    private static SkirmishProductionDecision EvaluateCombined(
