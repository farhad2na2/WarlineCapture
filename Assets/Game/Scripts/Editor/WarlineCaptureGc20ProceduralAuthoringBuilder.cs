...
public static class WarlineCaptureGc20ProceduralAuthoringBuilder
{
    private const string ScenePath = "Assets/Game/Scenes/Generated/GC20_ProceduralAuthoring_2048.unity";
    private const string CaptureRoot = "Design/AgentReports/Captures/GeneratedScenes/GC20_ProceduralAuthoring_2048";
    private const string ReportPath = "Design/AgentReports/2026-05-21_gameplay_gc20-procedural-authoring.md";
    private const string DataPath = "Design/AgentReports/Data/GeneratedScenes/GC20_ProceduralAuthoring_2048/gc20_procedural_authoring_contract.json";
    ...
    [MenuItem("WarlineCapture/Design/Build GC20 Procedural Authoring 2048")]
    public static void BuildGc20ProceduralAuthoring2048()
    {
        MacroRoads.Clear();
        ...
        GameObject root = new("GC20_ProceduralAuthoring_2048_Root");
        ...
        Debug.Log($"WARLINECAPTURE_GC20_PROCEDURAL_AUTHORING_BUILT scene={ScenePath} report={ReportPath}");
    ...
    private static void BuildEnvironment(GameObject root)
    {
        ...
        Volume volume = Child(root, "GC20_RTS_PresentationVolume").AddComponent<Volume>();
    ...
    {
        Material baseMat = CreateMaterial("GC20_SandBase", new Color(0.61f, 0.50f, 0.31f, 1f));
        Material roadMat = CreateMaterial("GC20_AuthoredCompactedRoad", new Color(0.64f, 0.53f, 0.35f, 1f));
        Material shoulderMat = CreateMaterial("GC20_AuthoredRoadShoulder", new Color(0.53f, 0.43f, 0.27f, 1f));
        Material trackMat = CreateMaterial("GC20_DirtTireTrack", new Color(0.42f, 0.32f, 0.18f, 1f));
        Material runwayMat = CreateMaterial("GC20_RunwayWeatheredAsphalt", new Color(0.36f, 0.35f, 0.31f, 1f));
        Material plazaMat = CreateMaterial("GC20_LocalPackedPlaza", new Color(0.70f, 0.58f, 0.37f, 1f));
        Material vehicleMat = CreateMaterial("GC20_VehicleYardDust", new Color(0.55f, 0.46f, 0.33f, 1f));
    ...
    private static void BuildDebugOverlay(GameObject root)
    {
        ...
        Material green = CreateMaterial("GC20_DebugSoldierWalkable", new Color(0f, 1f, 0.1f, 0.6f));
        Material cyan = CreateMaterial("GC20_DebugVehicleWalkable", new Color(0f, 0.8f, 1f, 0.6f));
        Material red = CreateMaterial("GC20_DebugBlocker", new Color(1f, 0f, 0f, 0.7f));
        foreach (Zone zone in SoldierLocalZones)
    ...
    private static void BuildGeneratorAuthoringOverlay(GameObject root)
    {
        ...
        Material city = CreateMaterial("GC20_AuthoringDistrict_City", new Color(0.20f, 0.42f, 0.80f, 0.85f));
        Material baseMat = CreateMaterial("GC20_AuthoringDistrict_Base", new Color(0.30f, 0.62f, 0.28f, 0.85f));
        Material logistics = CreateMaterial("GC20_AuthoringDistrict_Logistics", new Color(0.84f, 0.58f, 0.18f, 0.85f));
        Material airfield = CreateMaterial("GC20_AuthoringDistrict_Airfield", new Color(0.68f, 0.68f, 0.74f, 0.85f));
    ...
    private static void Validate()
    {
        ...
        if (detailBlockerPlacements < 60)
            ValidationLog.Add($"ERROR: GC20 placed only {detailBlockerPlacements} legal detail props; densification target is at least 60.");

        if (Districts.Count < 7)
            ValidationLog.Add($"ERROR: GC20 exported only {Districts.Count} districts; procedural authoring target is at least 7 named districts.");
        ...
        if (ValidationLog.Count == 0)
            ValidationLog.Add($"PASS: GC20 exported a procedural authoring contract with {Districts.Count} named districts, preserved the GC17/GC18 walkability masks, and kept all legal visuals on blocker zones. Detail props: {detailBlockerPlacements}; primary blocker visuals: {mainBlockerPlacements}; total placements: {Placements.Count}; blocker zones: {BlockerZones.Count}; soldier zones: {SoldierLocalZones.Count}; vehicle zones: {VehicleZones.Count}; macro roads: {MacroRoads.Count}.");
    }
    ...
    {
        BuildRtsCamera(root, "Camera_GC20_RtsOverview", new Vector3(-620f, 520f, -820f), BlueprintPoint(388f, 430f), 38f);
        BuildRtsCamera(root, "Camera_GC20_RtsCityReadable", new Vector3(-930f, 330f, -430f), BlueprintPoint(172f, 366f), 34f);
        BuildRtsCamera(root, "Camera_GC20_RtsAirfieldCommand", new Vector3(260f, 430f, -620f), BlueprintPoint(660f, 320f), 36f);
        BuildRtsCamera(root, "Camera_GC20_RtsDenseCityClose", new Vector3(-700f, 210f, -250f), BlueprintPoint(176f, 392f), 30f);

        Camera top = CameraObject(root, "Camera_GC20_TopDownContractVisualProof");
    ...
    private static void CaptureScene()
    {
        ...
        {
            if (camera.name == "Camera_GC20_RtsOverview")
                Render(camera, ProjectPath(CaptureRoot + "/gc20_rts_overview_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC20_RtsCityReadable")
                Render(camera, ProjectPath(CaptureRoot + "/gc20_rts_city_readable_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC20_RtsAirfieldCommand")
                Render(camera, ProjectPath(CaptureRoot + "/gc20_rts_airfield_command_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC20_RtsDenseCityClose")
                Render(camera, ProjectPath(CaptureRoot + "/gc20_rts_dense_city_close_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC20_TopDownContractVisualProof")
            {
                RenderContractProof(camera, ProjectPath(CaptureRoot + "/gc20_topdown_contract_visual_proof_2048x2048.png"));
                RenderGeneratorBlueprint(camera, ProjectPath(CaptureRoot + "/gc20_topdown_generator_blueprint_2048x2048.png"));
            }
        }
    }
    ...
    private static void WriteContractJson()
    {
        ...
        {
            generatedBy = nameof(WarlineCaptureGc20ProceduralAuthoringBuilder),
            scene = ScenePath,
    ...
    private static void WriteReport()
    {
        ...
        report.AppendLine("# GC20 Procedural Authoring Scene");
        report.AppendLine();
        report.AppendLine("Lane: Gameplay");
        report.AppendLine("Task: Continue from GC19 by adding a Unity-native procedural authoring contract: named districts, generator settings, walkable roads/yards, blocker-only visual placement, and proof captures suitable for PM review before replacing modules.");
        report.AppendLine();
        report.AppendLine("Files changed:");
        report.AppendLine("- `Assets/Game/Scripts/Editor/WarlineCaptureGc20ProceduralAuthoringBuilder.cs`");
        report.AppendLine("- `Assets/Game/Scenes/Generated/GC20_ProceduralAuthoring_2048.unity`");
        report.AppendLine("- `Design/AgentReports/Data/GeneratedScenes/GC20_ProceduralAuthoring_2048/gc20_procedural_authoring_contract.json`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC20_ProceduralAuthoring_2048/gc20_rts_overview_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC20_ProceduralAuthoring_2048/gc20_rts_city_readable_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC20_ProceduralAuthoring_2048/gc20_rts_airfield_command_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC20_ProceduralAuthoring_2048/gc20_rts_dense_city_close_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC20_ProceduralAuthoring_2048/gc20_topdown_contract_visual_proof_2048x2048.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC20_ProceduralAuthoring_2048/gc20_topdown_generator_blueprint_2048x2048.png`");
        report.AppendLine();
        report.AppendLine("Contracts touched: GC17/GC18 walkability visual contract remains the source of truth; GC20 adds procedural district metadata and keeps visuals constrained to blocker zones.");
        report.AppendLine("User-visible behavior: no shipped runtime behavior changed; generated scene now includes a city-generator-style contract that explains what each district is supposed to become.");
        report.AppendLine("Validation run: Unity batchmode `WarlineCaptureGc20ProceduralAuthoringBuilder.BuildGc20ProceduralAuthoring2048`.");
        report.AppendLine("Validation result: " + (ValidationLog.Any(line => line.StartsWith("ERROR:", StringComparison.Ordinal)) ? "failed; see validation log below." : "passed procedural authoring validation."));
        report.AppendLine("Known gaps: This is still a generated authoring pass, not final art quality. Full Demo modules are still not promoted until their internal blocker/walkable masks are explicit.");
        report.AppendLine("Cross-lane impacts: Design can review district roles/densities before Gameplay converts selected Demo-quality clusters into reusable masked modules.");
        report.AppendLine("Next recommended task: GC21 should convert selected Demo-authored clusters into reusable district modules with explicit blocker masks, then replace the GC20 placeholder blocker visuals.");
        report.AppendLine();
