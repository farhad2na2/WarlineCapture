...
| Current task | `AM-025` blocked from acceptance but active for remediation; tactical camera state is isolated, Editor-only switches have reset ownership, and match-start, building-AI, diagnostics, logistics telemetry, unit lookup, selected-unit lifecycle, follow-camera mode, pathfinding pending state, central match state, plus scene-transition queue now have replacement-World proof. The `575` historical rows now contain `434` non-debt and `141` genuine-debt rows grouped into `65` unique file/rule items; five external helper paths remain |
