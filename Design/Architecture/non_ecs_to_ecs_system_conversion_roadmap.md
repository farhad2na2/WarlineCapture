...
- Authoritative non-ECS runtime `*System` inventory: `350` after excluding `109` Unity ECS systems, `1` MonoBehaviour system, and `8` editor-only systems.
- Generated inventory artifact: `Design/Architecture/non_ecs_to_ecs_system_inventory.md`.
- Proposed inventory dispositions: `6` ConvertToISystem, `255` ConvertToSystemBase, `68` FoldIntoOwner, `21` PassiveBoundary, and `0` ReviewRequired.
- Converted to `ISystem`: `17`.
- Converted to `SystemBase`: `3`.
- Folded into ECS owners/jobs: `37`.
- Kept as passive view/config/authoring/editor boundary: `5`.
- Remaining plain runtime gameplay `*System` classes: `350`.
- Counting rule: only checklist lines beginning with `- [ ]`, `- [x]`, or `- [~]` count toward checklist progress.
- Last implementation update: 2026-06-15 converted map-surface startup projection into a managed ECS initialization boundary.
