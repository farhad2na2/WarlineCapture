...
Date: 2026-07-28
Status: Candidate implementation in progress through bounded Burst selection, deterministic slot assignment, and parallel complete-state slot apply kernels; runtime scheduling, persisted candidate, and production cutover remain disabled
...
| Phase 4: Jobified runtime assignment | In progress; the ordered camera boundary, exact map-generation native initialization, pure guard-envelope decision, bounded selection, deterministic assignment, and parallel apply kernels are accepted. Burst jobs scan only the precomputed cell table and selected ranges, deduplicate/filter/expand logical rows, retain/release/assign fixed slots, and emit one final command per dirty slot. The parallel chunk apply writes only dirty immutable slot identities, composes exact placement/part matrices, restores local bounds, mesh/material/submesh, color, binding, and generation before enabling `MaterialMeshInfo`, and fully resets released state before disabling it. Invalid commands leave their slot untouched and publish a bounded per-slot failure. Runtime scheduling/dependency wiring, metrics publication, and full unload/job-lifetime proof remain open. |
...
Checklist progress: `37 / 80` complete.
