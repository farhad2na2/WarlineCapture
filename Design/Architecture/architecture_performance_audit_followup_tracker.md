...
| Current phase | Phase 4/6 - GC Re-baseline After Instantiate Pooling |
| Quick wins complete | 6 / 6 |
| Current target | Attack-target command processing now follows the existing board/scan cached-query pattern and no longer appears in searched/top GC rows. Next measured recurring target is the broad `SelectionGameplayStartupSystemHelper.UpdateSelectionRuntimePhases` capture, split by hierarchy into `GameplayRuntimeUpdate.BuildingPlacement`, `GameplayRuntimeUpdate.MainMenu`, `GameplayRuntimeUpdate.Selection.Panel`, and `GameplayRuntimeUpdate.Selection.FocusedReadModel` rows. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, and Unity 6.5.2 batchmode compile log `/private/tmp/warline-arch-followup-attack-target-cache-compile.log` passed with 0 compiler errors after caching attack-target command queries. Unity MCP is still treated as unreliable for this tracker, so editor-log and batchmode validation remain the Unity path. |
| Android baseline status | Not started |
| GC baseline status | Latest 2026-07-03 17:54:19 UTC steady-state Match GC capture after attack-target cached-query cleanup: 300 requested frames after 180 warmup frames, 301 scanned profiler frames, 21,163 raw GC.Alloc samples / 1,682,750 raw bytes, 20,796 player-relevant samples / 1,642,133 bytes after excluding editor/tooling rows, 367 editor/tooling samples / 40,617 bytes, and runtime allocation probe assertion passed for `UIShellEcsPresentationSystem.Update` plus `MenuBootstrapView.Update`. `RtsSelectionAttackTargetModeCommandSystem.ProcessPendingRequests` no longer appears in searched/top rows; current recurring rows are now grouped under `SelectionGameplayStartupSystemHelper.UpdateSelectionRuntimePhases` and must be split by the profiler hierarchy before the next cleanup. |
...
| Validation status | Phase 0 inventory completed; Phase 1 config edited; Phase 2 release OnGUI strip implemented; Phase 3 release diagnostic system gate implemented; Phase 4 GC quick wins completed; Phase 5 Burst inventory/classification report added and guardrail implemented; Phase 6 instantiate classification report added and first placement-visual pooling slice implemented. Latest validations passed: `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, Unity 6.5.2 compile log `/private/tmp/warline-arch-followup-attack-target-cache-compile.log` with `Exiting batchmode successfully now!`, Match GC capture log `/private/tmp/warline-arch-followup-gc-attack-target-cache.log` with `[MatchGcAllocationCallstackCapture] result=Passed frames=300`, and earlier focused architecture validation `/private/tmp/warline-arch-followup-toarray-focused.log` with `[EcsBurstHotPathArchitectureValidation] result=Passed tests=10`. |
| Still wrong / next iteration | No visual defects are known from code/compile validation, but manual visual smoke checks for placement-preview pooling and attack-target command behavior are still pending. Player-relevant allocations remain high at `20,796` samples / `1,642,133` bytes; next slice should classify the broad `SelectionGameplayStartupSystemHelper.UpdateSelectionRuntimePhases` rows by hierarchy and start with the highest recurring owner, currently `GameplayRuntimeUpdate.BuildingPlacement > BuildingPlacementRuntimeTick.UpdateBuildingRuntimeState` at `85,595` bytes / `1,130` samples across `299` frames. |
...
- Top recurring allocation rows from the latest report:
  ...
  - Rank 3: `76,544` bytes / `2,093` samples / `299` frames, parent hierarchy `GameplayRuntimeUpdate.MainMenu`.
  - Rank 4: `76,544` bytes / `598` samples / `299` frames, parent hierarchy `GameplayRuntimeUpdate.Selection > GameplayRuntimeUpdate.Selection.Panel`.
  - Rank 5: `69,278` bytes / `985` samples / `1` frame, parent hierarchy `GameplayRuntimeUpdate.BuildingPlacement > BuildingPlacementRuntimeTick.UpdateActiveProductionTransports > Burst.Compiler.IL`.
  - Rank 6: `65,472` bytes / `939` samples / `1` frame, parent hierarchy `Default World Game.Runtime.UnitPathfindingSystem > Burst.Compiler.IL`.
  - Rank 7: `65,472` bytes / `939` samples / `1` frame, parent hierarchy `Default World Game.Runtime.UnitLookAtTargetSystem > Burst.Compiler.IL`.
  - Rank 8: `64,482` bytes / `962` samples / `299` frames, parent hierarchy `GameplayRuntimeUpdate.BuildingPlacement > BuildingPlacementRuntimeTick.UpdateActiveProductionTransports`.
...
- Known unresolved: Unity MCP remains unreliable for this tracker, visual/playmode smoke validation is pending, HDR/soft-shadow tier ownership is still unresolved, Android baseline is not captured, Fuel/Oil authoritative gameplay still lives in managed helper/runtime-building paths, Phase 5 still has no truthful blind Burst candidates, and player-relevant GC remains high in broad selection/startup update rows.
- Next iteration: classify the `SelectionGameplayStartupSystemHelper.UpdateSelectionRuntimePhases` allocation rows by profiler hierarchy and start with the highest recurring actionable owner, currently `GameplayRuntimeUpdate.BuildingPlacement > BuildingPlacementRuntimeTick.UpdateBuildingRuntimeState`.
