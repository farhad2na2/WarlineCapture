...
- `WarlineCapture_Project_State_Dashboard.md` and its JSON source still show the earlier 32% overall baseline, Visual Direction Lock at 41%, World Gameplay Iso Assets at 14%, and no note that the M01 sprite-renderer/capture evidence milestone was accepted.
...
Recommended fix:
- After the QA/HCI smoke result lands, update `Design/WarlineCapture_Project_State_Source.json` and regenerate `Design/WarlineCapture_Project_State_Dashboard.md`.
