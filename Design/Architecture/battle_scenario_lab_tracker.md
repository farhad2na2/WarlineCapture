...
- Checklist progress: `88 / 88 complete (100.0%)`.
- In progress: `0`.
- Remaining open: `0`.
- Current target: `complete; Scenario Lab V1 AD-001 through AD-010 plus GM-001 and DR-001 implemented, tuned, validated, and AD-001 manual scene upgraded from passive metrics to visual playback`.
...
- Isolated scene status: `scene shell created at Assets/Game/Scenes/ScenarioLab/BattleScenarioLab.unity with neutral ground, camera, root reference holder, AD-001 launcher/radar/target/missile/interceptor visuals, passive metrics overlay, Play bootstrap, restart button event, AD-001 manual variant selector, and presentation-only visual playback showing ground launch cue, incoming missile, air-defense launch cue, interceptor chase, impact burst, trails, and camera cuts`.
...
- Validation status: `git diff --check passed after Phase 6 tuning and selector work; static old-value scan found no remaining old radar-support magic values in the relevant runtime/config paths; Unity EditMode BattleScenarioLabModelTests passed 5/5; Unity EditMode BattleScenarioAd001RunnerTests passed 2/2 including asset-backed run after overlay/bootstrap compile; Unity EditMode BattleScenarioAd002RunnerTests passed 2/2; Unity EditMode BattleScenarioAd003RunnerTests passed 2/2; Unity EditMode BattleScenarioEcsSpawnHelpersTests passed 4/4 before the GM-001 helper additions; latest broad BattleScenario EditMode filter passed 15/15 before AD-004; batch executeMethod BattleScenarioLabSceneBuilder.CreateManualSceneShell regenerated the selector-enabled manual scene; batch executeMethod BattleScenarioLabValidationRunner.ValidateManualSceneSmoke passed with selector/dropdown and restart listener checks; latest post-tuning batch executeMethod BattleScenarioLabSuiteRunner.RunScenarioSuite passed with AD-001 through AD-010 plus GM-001 and DR-001 non-skipped; latest shadow-project batch executeMethod BattleScenarioLabSceneBuilder.CreateManualSceneShell regenerated the visual playback scene; latest shadow-project batch executeMethod BattleScenarioLabValidationRunner.ValidateManualSceneSmoke passed with playback reference/object checks. Latest suite index: /private/tmp/warline-scenario-lab-suite-index.json; latest suite log: /private/tmp/warline-scenario-lab-suite-phase6-tuning-rerun.log; latest visual scene smoke log: /private/tmp/warline-scenario-lab-visual-playback-shadow-smoke.log.`
...
- [x] Add a presentation-only AD-001 visual playback component with no gameplay `Update()` loop.
...
- The manual scene visual is a deterministic presentation replay driven by the same AD-001 variant metrics.
- Automated pass/fail still comes from the fixed-step ECS runner.
- The first visual pass uses scenario-lab proxy props so the lab remains isolated and reusable; replacing proxy props with production model prefabs can be a later visual-quality pass.
