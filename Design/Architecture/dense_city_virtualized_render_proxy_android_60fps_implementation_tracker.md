...
| Phase 2: Deterministic database builder | In progress; generated bake-config schema, exact output ownership, deterministic source-row inventory, shared compound prototype recipes, exact render-only logical placements, accepted-origin-aligned 32 m spatial index, provisional per-policy capacity/entity/active-slot budgets, the transactionally persisted candidate config/report, and unchanged-input serialized-byte determinism are accepted; camera/projection acceptance, building state linkage, and full logical parity remain open |
...
Checklist progress: `19 / 80` complete.
