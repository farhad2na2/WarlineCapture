...
- [~] `APH-704` Design and benchmark a shared read-only unit spatial index for building defense, AI targeting, threat detection, and selection candidates. Adopt it only when it beats the Phase 2 direct-query baseline and preserves results.
  - Active result: the shadow-design plan preserves global ECS query order and domain-owned scoring, benchmarks 16/32/64-cell buckets, and forbids new completion fences, managed World lookup, or measured-path allocation. Adoption requires zero mismatches, at least 10% combined median/p95 improvement, no consumer regression above 5%, and no Match p95 regression outside accepted variance.
  - Evidence: `Design/AgentReports/2026-07-11_aph-704_shared_unit_spatial_index_plan.md`. Remaining work is the equivalence fixture and alternating direct-versus-indexed benchmark; production consumers remain unchanged.
...
- [~] `APH-803` Add an Android development-build gate for p95/p99 frame, peak memory, startup time, and thermal status on the reference device profile.
  - Active result: existing build, auto-start, diagnostics, profiler-export, and product-budget foundations are mapped to a single structured-recorder design. The gate will use exact post-warmup samples, five cold and five warm starts, one sustained run, device/APK/revision provenance, thermal/memory polling, screenshot/crash evidence, and fail closed while p99 or startup limits remain unset.
  - Evidence: `Design/AgentReports/2026-07-11_aph-803_android_development_gate_plan.md`. Remaining work is the flag-gated recorder, device orchestrator/schema/tests, accepted p99/startup limits, and live reference-device proof.
