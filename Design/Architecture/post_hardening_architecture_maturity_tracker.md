...
| Current task | `AM-025` blocked from acceptance but active for remediation; tactical camera state is isolated, Editor-only switches have reset ownership, and match-start, building-AI, diagnostics, logistics telemetry, unit lookup, selected-unit lifecycle, follow-camera mode, pathfinding pending state, plus central match state now have replacement-World proof. The `575` historical rows now contain `433` non-debt and `142` genuine-debt rows grouped into `66` unique file/rule items; five external helper paths remain |
