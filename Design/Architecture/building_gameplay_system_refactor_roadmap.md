...
Step 25 visual helper transition size: 1583 lines. Placement visual instance creation, placement visual positioning, footprint-center delegates, runtime visual initialization, runtime marker visibility refresh, and owner-faction visual tint now route directly through `BuildingPlacementVisualSystem`, `BuildingPlacementGridSystem`, `BuildingRuntimeVisualSystem`, and `BuildingRuntimeOwnershipSystem`; `BuildingGameplaySystem` no longer keeps visual helper wrapper methods.
...
25. Complete: Move visual instance and positioning helpers
   ...
   - Placement visual instance creation and positioning now route directly to `BuildingPlacementVisualSystem` from context construction.
