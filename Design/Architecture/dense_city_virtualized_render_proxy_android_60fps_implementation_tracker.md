...
Date: 2026-07-28
Status: Candidate implementation in progress through the ordered, versioned, Burst-usable runtime camera snapshot boundary; persisted candidate and production cutover remain disabled
...
| Phase 4: Jobified runtime assignment | In progress; `RuntimeCameraReferenceSystem` is now an order-first `SimulationSystemGroup` managed boundary that publishes exactly once per system update. Consumers only read the immutable unmanaged snapshot; they no longer republish through managed `Camera` access. Every valid or invalid boundary carries a monotonic nonzero publication version, and valid snapshots carry a deterministic 128-bit signature over exact position, rotation, view, and projection bits. Runtime initialization, guard-envelope selection, assignment, apply, metrics, and lifecycle ownership remain open. |
...
Checklist progress: `32 / 80` complete.
