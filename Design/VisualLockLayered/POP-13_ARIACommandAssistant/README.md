...
Status: target-lock reference saved, audited implementation-ready functional tracker locked, runtime code-built popup pass in progress. A separated production layer pack is deferred and is not required for functional acceptance.
...
1. Keep the current code-built ARIA popup aligned with this target for the immediate readability fix.
2. Follow `IMPLEMENTATION_TRACKER.md` before adding mockup-only details, so every panel row and metric is backed by real ECS data.
3. Complete functional/visual validation with code-built Unity UI and existing approved sprites. Treat any later separated green-key layer pack as optional production-art replacement, not a blocker for the functional POP-13 pass.
