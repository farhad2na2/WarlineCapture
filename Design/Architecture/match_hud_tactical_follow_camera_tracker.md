...
- 2026-06-30 sixth follow-up: fixed CameraButton selected visual persistence. `MatchHudSelectionPanelView.SetCameraActionSelected(true)` now asserts the selected sprite on both the base `Image.sprite` and the rendered `Image.overrideSprite`, so Unity SpriteSwap transitions cannot clear the selected look after entering tactical follow mode. Exiting follow mode clears only the selected override it owns, leaving normal hover/press transitions intact. Added prefab regression coverage for selected override persistence and reassertion after UI transitions clear the override. Validation: `git diff --check` passed; `dotnet build Assembly-CSharp.csproj --no-restore` passed with `0 Warning(s), 0 Error(s)`; Unity batch focused validation was blocked because the main project was already open; `dotnet build Game.Tests.Editor.csproj --no-restore` still fails before project tests in generated `UnityEditor.UI.csproj` on read-only `DefaultControls.factory`.
- 2026-06-30 seventh follow-up: added the RightContent/ZommButtons `ZoomInButton` and `ZoomOutButton` three-step camera zoom control. The buttons start at default zoom with both enabled, step default -> min zoom and default -> max zoom through the existing `IMatchHudCameraControl` / `SelectionUiCameraSystemHelper` / `RtsCameraRequestSystem` path, disable visually at the min/max ends, and step back through default when the opposite button is clicked. `MainMenuPlayUI` binds the rail buttons and marks their clicks as UI clicks; `MatchHudRightQuickRailView` resolves the new buttons by child name so the current prefab does not need manual serialized rework, while preserving their SpriteSwap hover/pressed/selected/disabled sprites. Added focused tests for the camera zoom level state and rail button SpriteSwap/interactable state. Validation: `git diff --check` passed; `dotnet build Assembly-CSharp.csproj --no-restore` passed with `0 Warning(s), 0 Error(s)`; `dotnet build Game.UI.Contracts.Player.csproj --no-restore` passed with `0 Warning(s), 0 Error(s)`. Main-project Unity validation was blocked because the project is already open; shadow validation reached compile but failed on unrelated stale shadow-project selection/helper drift before the zoom tests ran. Direct asmdef/player/editor test `dotnet` builds remain blocked by existing generated Unity package/project errors outside this slice.
- 2026-06-30 eighth follow-up: fixed zoom-button snapping and max-zoom rebound. Match HUD zoom buttons now start a per-frame smooth perspective zoom transition instead of using instant camera mode application. Starting a button zoom clears any active normal RTS zoom transition and resets camera transition velocity first, so max zoom-out is not immediately pulled back toward the normal/default zoom. The transition preserves the current ground-center focus while height/pitch/yaw/FOV settle. Added regression coverage that zoom buttons do not snap directly to min/max height and clear the normal camera transition. Validation: `git diff --check` passed; `dotnet build Assembly-CSharp.csproj --no-restore` passed with `0 Warning(s), 0 Error(s)`; `dotnet build Game.UI.Contracts.Player.csproj --no-restore` passed with `0 Warning(s), 0 Error(s)`.
- 2026-06-30 ninth follow-up: fixed max zoom-out fighting the RTS camera clamp. `RtsCameraSystem` now exposes a clamp-safe perspective height resolver that measures the requested camera footprint against the active ground boundary before the smooth zoom starts, and `SelectionUiCameraSystemHelper` uses it for the Match HUD max zoom-out target with spare boundary room for drag pan. This keeps the normal camera clamp active while avoiding a max-zoom pose that consumes the whole playable boundary and makes dragging feel blocked. Added regression coverage that an unsafe max zoom-out height is lowered before clamp conflict and still allows pan. Validation: `git diff --check` passed; `dotnet build Assembly-CSharp.csproj --no-restore` passed with `0 Warning(s), 0 Error(s)`; `dotnet build Game.UI.Contracts.Player.csproj --no-restore` passed with `0 Warning(s), 0 Error(s)`. Focused Unity validation was attempted at `/private/tmp/warline-rts-camera-zoom-clamp-validation.log` but Unity aborted because `/Users/farhad/Projects/WarlineCapture` is already open in another editor instance.
- 2026-06-30 tenth follow-up: fixed the remaining max zoom-out pan lock. The runtime RTS camera no longer uses the broad `IsCameraAtMatchIntroZoomOut` heuristic to trigger the match-intro settle transition, because manual HUD max zoom-out is also intentionally above normal height. `RtsCameraSystem` now stores `MatchIntroZoomSettlePending`, mirrored through `RtsCameraStateComponent` and `RtsCameraRequestSystem`, and `RtsSelectionRuntimeCameraSystemHelper` only starts the intro settle transition from that explicit flag. Added a regression that simulates play already started, camera manually at max zoom-out, runtime tick executed, then drag pan must still move the camera. Validation: `git diff --check` passed; `dotnet build Assembly-CSharp.csproj --no-restore` passed with `0 Warning(s), 0 Error(s)`; `dotnet build Game.UI.Contracts.Player.csproj --no-restore` passed with `0 Warning(s), 0 Error(s)`; Unity focused validation `/private/tmp/warline-rts-camera-manual-zoom-pan-validation.log` passed with `[RtsCameraFocusedValidation] result=Passed tests=29`.
