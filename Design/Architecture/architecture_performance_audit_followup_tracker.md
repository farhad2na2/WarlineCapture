...
| Field | Status |
|---|---|
| Checklist complete | 86 / 123 |
| Checklist percent complete | 69.9% |
| Current phase | Phase 8 - Managed Helper Hot-path Migration |
| Quick wins complete | 6 / 6 |
| Current target | Selection, building-placement visual creation, and production transport visual attribution have now been measured with editor-only allocation probes. `BuildingProductionRuntimeTick.UpdateActiveProductionTransports` reported `0` bytes across `124` active update calls, `0` transport acquire calls, and one drop-visual create/acquire call that also reported `0` bytes, so the remaining transport hierarchy rows are treated as profiler/callstack attribution until contradicted by a focused probe. The next exact action is to instrument/classify the recurring `TransportBoardingCommandSystem` row before any Phase 9 decomposition. |
| Compiler status | Latest production transport/drop-visual allocation-probe slice passed `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, and `git diff --check` with 0 errors. Unity 6.5.2 battle capture passed through the documented non-headless batchmode workaround at `/private/tmp/warline-arch-followup-production-transport-dropvisual-probe-windowed-battle.log` with 0 project compiler errors. |
| Android baseline status | Not started |
| GC baseline status | Latest completed 2026-07-04 20:51:13 UTC battle-state Match GC capture after production transport/drop-visual allocation probe: 300 requested frames after 180 warmup frames, 301 scanned profiler frames, 52,596 raw GC.Alloc samples / 4,196,128 raw bytes, 7,572 player-relevant samples / 496,120 bytes after excluding editor/tooling rows, and 45,024 editor/tooling samples / 3,700,008 bytes. Runtime allocation probe assertion passed. `UIShellEcsPresentationSystem.Update`, `MenuBootstrapView.Update`, selection subphases, `BuildingPlacementVisualPresentationSystemHelper.CreateBuildingVisualInstance`, and `BuildingProductionRuntimeTick.UpdateActiveProductionTransports` all reported `0` bytes; the transport probe recorded `124` active update calls, `0` transport acquire calls, and one drop-visual create/acquire call with `0` thread-allocation bytes. |
...
- Next iteration: add a focused allocation/phase probe around `TransportBoardingCommandSystem` and use that result to decide whether Phase 9 decomposition should begin from measured allocation work or stay deferred behind Android/mobile baselining.
