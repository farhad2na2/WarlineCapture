...
| Parallel preparatory work | `AM-026` remains dependency-blocked and receives no completion credit; `ui_projection_allocation_inventory.md` inventories 29 surfaces and reconciles every declared route/popup contract; drafts `AM-WP-001` through `AM-WP-009` bound Resource Exchange, Build Drawer, Selection/Passenger, Placement, Loading, Shell Layout, Match Status, Squad Tray/Command Wheel, and Settings ownership, lifecycle, semantic identities, and real-path performance/allocation gates; `AM-WP-007` is additionally blocked on explicit mission/objective owner handoff; `AM-WP-009` preserves separately owned audio/FirstLaunch consumers as read-only dependencies |
