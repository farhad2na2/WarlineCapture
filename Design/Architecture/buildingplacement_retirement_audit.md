...
- `Assets/Game/Scripts/UI/BuildingPlacementSystem.cs`: 2148 lines.
- Public/internal facade declarations: 126, excluding the class declaration.
...
3. Move active placement lifecycle/session command wrappers out of the facade.
4. Move runtime/manual building and wall spawn wrapper methods out of the facade.
5. Move placement grid/input/preview/commit context wiring out of the facade.
6. Move UI command/query compatibility wrappers out of the facade.
7. Move selection and interaction compatibility wrappers out of the facade.
8. Replace `BuildingGameplayCompositionSystem` construction so it no longer calls `new BuildingPlacementSystem()`.
9. Migrate remaining editor validation tests to a narrow building gameplay harness.
10. Delete `Assets/Game/Scripts/UI/BuildingPlacementSystem.cs` and its `.meta`.
11. Remove facade allowlists and update the architecture contract to require zero facade references.

## Drift Guard

Until deletion:
- `BuildingPlacementSystem.cs` must not grow beyond 2148 lines.
- Public/internal facade declarations must not exceed 126.
