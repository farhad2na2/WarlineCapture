...
| Field | Status |
|---|---|
| Checklist complete | 68 / 110 |
| Checklist percent complete | 61.8% |
| Current phase | Phase 8 - Fuel/Oil ECS Drift Correction |
| Quick wins complete | 6 / 6 |
| Current target | Live hauler load/revert/unload mutation now routes through `BuildingResourceHaulerTransferEcsSystem`, the validated disabled Burst-capable ECS transfer surface over `BuildingResourceStorageComponent` and `UnitResourceHauler`. The existing runtime-building caller still adapts and writes back the result, so there is no parallel live hauler owner. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, focused Unity 6.5.2 ECS mirror validation `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-ecs-validation.log`, focused Unity 6.5.2 faction resource validation `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-faction-validation.log`, focused Unity 6.5.2 hauler mirror validation `/private/tmp/warline-arch-followup-fuel-oil-hauler-mirror-sync-validation.log`, focused Unity 6.5.2 resource transfer validation `/private/tmp/warline-arch-followup-resource-transfer-helper-validation.log`, focused Unity 6.5.2 hauler ECS transfer validation `/private/tmp/warline-arch-followup-hauler-transfer-ecs-validation.log`, and focused Unity 6.5.2 live hauler ECS transfer validation `/private/tmp/warline-arch-followup-hauler-live-ecs-transfer-validation.log` passed with 0 compiler errors after the Fuel/Oil mirror-sync and transfer-surface slices. |
...
- Known unresolved: visual/playmode smoke validation is pending, HDR/soft-shadow tier ownership is still unresolved, Android baseline is not captured, Fuel/Oil live storage is still mirrored rather than fully authoritative on ECS entities, and Phase 5 still has no truthful blind Burst candidates.
- Next iteration: move authoritative resource storage mutation onto the live ECS entity path without a parallel owner, or pause Fuel/Oil drift work and return to Android baseline capture.
