...
Continue the tactical HUD vertical slice around `Screen_MatchOverlay`, `SCN-08 RTS Battle HUD`, `SCN-09 Build Drawer / Production`, and `SCN-10 Unit Command` while staying aligned with the full 3D single-map gameplay direction.

This phase keeps the accepted HUD chrome and Canvas layering work, but treats old target-cropped gameplay content as temporary unless it is replaced by 3D operation-map captures, config-backed unit/building presentation, or explicit placeholder states.
...
- Gameplay-facing content must follow `Design/3D_SingleMap_Gameplay_Direction.md`.
- Squad thumbnails, minimap content, map previews, mission art, battlefield capture backgrounds, unit thumbnails, building thumbnails, and tactical overlay art should come from 3D operation-map captures and the prefab catalog under `Assets/Game/Configs/Prefabs`.
- Do not generate or expand archived 2D isometric, old Synty/desert, or low-poly tactical content for new UI work.
...
0. Apply the 3D operation-map HUD work order.
   - Use `Design/UIUX_Gameplay_Element_Alignment.md` as the current handoff for operation-map gameplay UI.
   - Use `Design/M01_FirstContact_Production_Contract.md` as the first concrete validation target for the UI agent.
   - Use `Design/3D_SingleMap_Gameplay_Direction.md` as the current target audit for stale/missing tactical state targets.
   ...
   - Treat planning, minimap, deployment, and battle states as camera/UI layers on the same 3D operation map. `SCN-08` gameplay validation must happen over an approved nonblank operation-map capture with separate runtime units, markers, and UI layers.
...
5. Preserve gameplay contracts.
   - Every visible HUD, drawer, command, squad, minimap, and resource element must remain covered by `UIUX_Gameplay_Element_Alignment.md`.
...
- Gameplay-facing content either references accepted 3D operation-map captures/config-backed assets or has an explicit temporary/placeholder state.
- M01 First Contact can be captured with selected rifle squad, move/attack markers, invalid command toast, objective jump, minimap viewport, and disabled Build feedback as described in `M01_FirstContact_Production_Contract.md`.
- Selected entity, command mode, world command markers, invalid command feedback, and minimap camera-jump hooks exist as named UI elements in `UIUX_Gameplay_Element_Alignment.md`.
