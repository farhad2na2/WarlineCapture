...
public static class BattleScenarioAd009Runner
{
    public const string ScenarioId = "AD-009_AirMissileLauncher_SupportModeComparison_RadarSatelliteCombined";

    private const int Seed = 92345;
    ...
    public static BattleScenarioVariant[] CreateDefaultVariants()
    {
        return new[]
        {
            new BattleScenarioVariant
            {
                ...
                IncomingThreatStartDistance = 220f,
            ...
            new BattleScenarioVariant
            {
                VariantId = "AD-009-C-Satellite",
                Label = "Satellite",
                SupportMode = BattleScenarioSupportMode.Satellite,
                IncomingThreatKind = BattleScenarioIncomingThreatKind.GroundMissile,
                IncomingThreatSpeedMultiplier = 1f,
                IncomingThreatStartDistance = 220f,
                IncomingThreatAltitude = 10f,
                LauncherCount = 1,
                RadarDistanceFromLauncher = 14f,
                ExpectedOutcome = BattleScenarioExpectedOutcome.MustImproveOrMatchBaseline
            },
            new BattleScenarioVariant
            {
                VariantId = "AD-009-D-Combined",
                Label = "Radar Plus Satellite",
                SupportMode = BattleScenarioSupportMode.Combined,
                IncomingThreatKind = BattleScenarioIncomingThreatKind.GroundMissile,
                IncomingThreatSpeedMultiplier = 1f,
                IncomingThreatStartDistance = 220f,
                IncomingThreatAltitude = 10f,
    ...
    public static BattleScenarioResult RunDefinition(BattleScenarioDefinition definition)
    {
        ...
        if (definition.ScenarioId != ScenarioId)
            throw new ArgumentException($"Unsupported AD-009 scenario id '{definition.ScenarioId}'.", nameof(definition));
        ...
        float maxDurationSeconds)
    {
        ...
        result.Comparisons = new[]
        {
            ...
            BattleScenarioResultComparison.CompareRadarSupport(result.Variants[0], result.Variants[3])
        };
        ...
        float maxDurationSeconds)
    {
        ...
        CreateSupportProviders(em, variant);
        ...
        return BattleScenarioFixedStepRunner.RunVariant(
            ...
            (state, metrics) =>
            {
                ...
                CaptureLauncherMetrics(em, launcher, incomingMissile, metrics, state);
    ...
    {
        if (result.Variants.Length < 4 || result.Comparisons.Length < 3)
            return false;

        BattleScenarioMetrics radar = result.Variants[1];
        BattleScenarioMetrics satellite = result.Variants[2];
        BattleScenarioMetrics combined = result.Variants[3];
        return radar.Intercepted &&
               satellite.Intercepted &&
               combined.Intercepted &&
               radar.RadarProviderUsed &&
               !radar.SatelliteProviderUsed &&
               !satellite.RadarProviderUsed &&
               satellite.SatelliteProviderUsed &&
               combined.RadarProviderUsed &&
               combined.SatelliteProviderUsed &&
               combined.LauncherEffectiveRange >= satellite.LauncherEffectiveRange &&
               combined.LauncherEffectiveTurnRateDegreesPerSecond >= radar.LauncherEffectiveTurnRateDegreesPerSecond &&
               result.Comparisons[1].RadarImprovedDetectionTime &&
               result.Comparisons[2].RadarImprovedLockTime &&
               result.Comparisons[2].RadarImprovedOrMatchedOutcome;
    }
    ...
    private static Entity CreateIncomingGroundMissile(EntityManager em, BattleScenarioVariant variant)
    {
        ...
        return BattleScenarioEcsSpawnHelpers.CreateIncomingGroundMissile(
            ...
            9f / math.max(0.1f, variant.IncomingThreatSpeedMultiplier),
            10f,
            8f,
            120);
    }

    private static void CreateSupportProviders(EntityManager em, BattleScenarioVariant variant)
    {
            ...
            variant.SupportMode == BattleScenarioSupportMode.Combined)
        {
            BattleScenarioEcsSpawnHelpers.CreateAirDefenseSupportProvider(
                em,
                LauncherPosition + new float3(variant.RadarDistanceFromLauncher, 0f, 0f),
                FactionIdentity.PlayerFactionId,
        ...
        if (variant.SupportMode == BattleScenarioSupportMode.Satellite ||
            variant.SupportMode == BattleScenarioSupportMode.Combined)
        {
            BattleScenarioEcsSpawnHelpers.CreateAirDefenseSupportProvider(
                em,
                LauncherPosition + new float3(-variant.RadarDistanceFromLauncher, 0f, 0f),
                FactionIdentity.PlayerFactionId,
                AirDefenseSupportProviderKind.Satellite,
                1,
                120f,
