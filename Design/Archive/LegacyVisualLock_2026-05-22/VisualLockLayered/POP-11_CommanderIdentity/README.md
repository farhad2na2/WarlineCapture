# POP-11_CommanderIdentity Flat Visual Target Reference

This folder keeps a reference copy for the WarlineCapture visual-lock index.

- Reference target: `reference/POP-11_CommanderIdentity_Landscape_Target.png`
- Source background: `Design/VisualLock/SCN-03_CommanderProfile/SCN-03_CommanderProfile_Landscape_Target.png` blurred and dimmed for panel/popup context.
- Source design: `Design/FTUE_And_Command_Assistant_Design.md`
- Purpose: Commander profile icon selection popup over blurred Commander Profile context.

This target is a flat high-quality panel/popup mockup. It does not require a new sliced layer pack for this design pass.
