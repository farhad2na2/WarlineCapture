...
public sealed class CombatDeathPlayModeTests
{
    ...
    public void SoldierAttackDamageThenDeath_DestroysTargetWithoutRespawn()
    {
        ...
        Assert.IsTrue(em.Exists(target), "The target should still exist immediately after lethal damage so death cleanup can run.");
        Assert.AreEqual(0, em.GetComponentData<UnitHealth>(target).Current);
        Assert.IsTrue(em.HasComponent<RecentAttacker>(target), "The lethal hit should record the recent attacker before death cleanup strips active state.");
