...
- Menu/camp UI money reads, camp catalog reads, camp request validation/commands, and friendly pending-production UI read models belong behind `BuildingUiCommandSystem`; `MenuView` must not call `BuildingPlacementSystem` directly for those camp/resource request paths.
