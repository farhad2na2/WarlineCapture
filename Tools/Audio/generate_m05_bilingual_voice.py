#!/usr/bin/env python3
"""Generate M05 voice from the authoritative C# copy catalogs; no runtime TTS."""
...
MANIFEST = ROOT / "Assets/Game/Audio/Narrative/M05BreachAssault/m05_voice_manifest.json"
STRING = r'"((?:[^"\\]|\\.)*)"'
...
def lines():
    source = (ROOT / "Assets/Game/Scripts/Configs/Localization/M05BreachAssaultCopyCatalog.cs").read_text()
    ...
    for identity, speaker, english, persian in narrative:
        yield "narrative", "m05-" + decode(identity), speaker.upper(), decode(english), decode(persian)
    lessons = source[source.index("[] Lessons="):source.index("[] Ui=")]
    tutorial = re.findall(r'\(' + ','.join([STRING] * 8) + r'\)', lessons)
    if len(tutorial) != 8:
        raise RuntimeError(f"Expected 8 canonical tutorial steps, got {len(tutorial)}")
    for i, row in enumerate(tutorial, 1):
        yield "tutorial", f"tutorial-m05-{i:02}", "ARIA", decode(row[2]), decode(row[3])
...
def main():
    ...
    review = json.loads((ROOT / "Design/AgentReports/M05BreachAssault/voice_payload_review.json").read_text())
    if [list(entry) for entry in entries] != review["entries"]:
        raise RuntimeError("Canonical M5 copy differs from the prepared approval payload.")
    if args.dry_run:
        print(f"[M05VoiceCopy] result=Passed narrative=7 tutorial=8 locales=2 characters={sum(len(e[3])+len(e[4]) for e in entries)}")
        return
    ...
    manifest = {"schema": "WarlineCapture.M05BilingualVoice.v1", "missionId": "saga.ch01.m05.breach_assault",
    ...
    for index, (kind, identity, speaker, english, persian) in enumerate(entries):
        for language, locale, text in (("en", "en-US", english), ("fa", "fa-IR", persian)):
            ...
            print(f"[M05Voice] {kind} {identity} {locale} duration={clip['durationSeconds']:.2f}s", flush=True)
    register_events(manifest["clips"])
    print("[M05BilingualVoiceGeneration] result=Passed narrative=14 tutorial=16 locales=2")
...
def register_events(clips):
    ...
    catalog["events"]=[e for e in catalog["events"] if not e["eventId"].lower().startswith(("vo.aria.tutorial.m05.","vo.aria.m05.comms."))]
    for clip in clips:
        if clip["kind"]!="tutorial":
            continue
        suffix="fa" if clip["locale"]=="fa-IR" else "en"
        identity="vo.aria.tutorial.m05."+clip["id"].rsplit("-",1)[1] if clip["kind"]=="tutorial" else "vo.aria.m05.comms.01"
