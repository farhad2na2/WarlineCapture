...
| Current task | Remove the remaining `41,770,935`-byte clean APH-804 APK overage while retaining the measured BrightCommand memory pilot, then validate the `<=967.5 MB` allocated-memory limit and visual matrix on one exact Android artifact |
...
| Last verified commit | `17566e35605b3925c96df5d86f173f382251f12a` on `origin/main`; architecture implementation commit `98cb2bb6d4321132574ebc5247aa3f2c93359eac` |
| Last update | 2026-07-15 - the scoped BrightCommand ASTC pilot reduced the clean APK by `6,951,948` bytes to `505,130,133`, leaving `41,770,935` bytes over budget; allocated-memory and APH-809 visual evidence remain open |
...
- [~] `APH-501` Add tracked budgets for APK/AAB size, installed size, peak allocated memory, texture memory, mesh memory, audio memory, and graphics-driver memory.
  - Active result: schema 4 pins clean ARM64 IL2CPP release budgets at APK `<= 463,359,198` bytes and AAB `<= 426,399,778` bytes, including exact artifact commit, SHA-256, immutable evidence commit, and report path. The validator/test now follow schema 4 and the accepted 20 ms Editor p95 ratchet. The baseline clean `bcee23f3fceb76a33ad0863aaa4dbd22e3d779ad` APK is `512,082,081` bytes. The bounded BrightCommand pilot applies explicit Android ASTC 4x4 high-quality compression to exactly 60 sprites while preserving sprite geometry, mipmaps, readability, alpha, and every non-Android setting. Its exact clean `17566e35605b3925c96df5d86f173f382251f12a` artifact is `505,130,133` bytes with SHA-256 `7cf40efabd5e2c31298762745f4b68ad235a1b3f661065845a9c4d9487f26a0a`: a `6,951,948`-byte improvement, but still rejected by `41,770,935` bytes. Device memory and visual acceptance remain pending. Installed size and absolute peak/texture/mesh/audio/graphics-driver memory limits remain fail-closed `null` until an accepted release artifact reaches device collection. Evidence: `Design/AgentReports/2026-07-13_aph-501_product_budget_evidence_handoff.md`, the canonical APK BuildReport, and `/private/tmp/warline-aph501-main-menu-policy-post-rebase.xml` (`12/12`).
...
- [~] `APH-510` Rebuild Android, compare BuildReport/memory/frame metrics, and record per-category deltas.
  - Active result: `Tools/CI/aph510_android_category_comparison.py` binds baseline/candidate evidence to exact revision, APK/AAB hashes, build profile, and device identity; compares package, installed, allocated/PSS, texture, mesh, audio, graphics-driver, frame, startup, and I/O metrics; and emits deterministic JSON/Markdown while null measurement-required limits remain explicit blockers. Focused contracts pass `9/9`. The first clean pilot build reduces BrightCommand top-100 serialized contribution from `179,595,448` to `157,452,404` bytes and the APK from `512,082,081` to `505,130,133` bytes.
  - Remaining: close the remaining `41,770,935`-byte package gap, verify visual output, then produce AAB, device-performance, category-residency, and I/O evidence and accept the comparison.
...
- [~] `APH-804` Add Android release-build acceptance for the documented 30 FPS tier; keep 60 FPS as a separate high-end target.
  ...
  - Current blocker: clean commit `bcee23f3fceb76a33ad0863aaa4dbd22e3d779ad` produces a provenance-bound ARM64 IL2CPP release APK and complete 6,430-asset BuildReport, but the `512,082,081`-byte artifact exceeds the accepted `463,359,198`-byte ceiling by `48,722,883` bytes. The serialized collector rejects it before any ADB access. Package attribution/reduction is required before unplugged/idle-thermal device collection; top-100 changes span multiple intervening UI, audio, generated-character, and operation-map additions and are not treated as causal proof without reference tracing.
...
- Policy: explicit Android ASTC 4x4, 2048 maximum size, high-quality compression, and quality `100`; existing sprite borders, pivots, PPU, mipmaps, readability, alpha, wrap/filter modes, GUIDs, and every non-Android setting remain unchanged
