...
- Shadow project validation status: `SCN-08 slice 09 files synced to /Users/farhad/Projects/WarlineCapture-CodexUnity1; shadow Unity batch import/compile passed; shadow UI Builder/static crop still pending because the controllable GUI preview did not stay available`.
- Validation status: `git diff --check passed for SCN-08 slice 09; no Unity compile/import errors found in /private/tmp/warline-scn08-slice09-shadow-batch.log`.
...
| SCN-08 Match HUD | In progress | 9 | Reopened after user review: slice 08 removed the top health/progress strips but still used a weak inner selected overlay and asymmetric squad tray padding. Slice 09 replaces selected/hover/focus with a dedicated selected squad-card chrome sprite and balances the five-card tray margins. Existing runtime selection already moves `squad-card-selected` by `SelectedSlot`, so every squad card can receive the same selected visual state. Existing runtime bindings remain intact. | `Design/VisualLockLayered/_UIToolkitVisualMatch/SCN-08_MatchHud/iteration_01/notes.md`; `Assets/Game/Art/UI/Generated/MatchHUD/TargetLockV02/scn08_v02_squad_card_selected_frame.png`; `Design/VisualLockLayered/SCN-08_RTSBattleHUD/reference/SCN-08_RTSBattleHUD_NewMainMenuArtDirection_TargetLock_V02.png` |
