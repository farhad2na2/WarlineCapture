...
public static class WarlineCaptureGc17WalkabilityFirstVisualBuilder
{
    private const string ScenePath = "Assets/Game/Scenes/Generated/GC17_WalkabilityFirstVisual_2048.unity";
    private const string CaptureRoot = "Design/AgentReports/Captures/GeneratedScenes/GC17_WalkabilityFirstVisual_2048";
    private const string ReportPath = "Design/AgentReports/2026-05-21_gameplay_gc17-walkability-first-visual.md";
    private const string DataPath = "Design/AgentReports/Data/GeneratedScenes/GC17_WalkabilityFirstVisual_2048/gc17_walkability_visual_contract.json";
    ...
    [MenuItem("WarlineCapture/Design/Build GC17 Walkability First Visual 2048")]
    public static void BuildGc17WalkabilityFirstVisual2048()
    {
        MacroRoads.Clear();
        ...
        GameObject root = new("GC17_WalkabilityFirstVisual_2048_Root");
        ...
        Debug.Log($"WARLINECAPTURE_GC17_WALKABILITY_FIRST_VISUAL_BUILT scene={ScenePath} report={ReportPath}");
    ...
    private static void BuildEnvironment(GameObject root)
    {
        ...
        Volume volume = Child(root, "GC17_RTS_PresentationVolume").AddComponent<Volume>();
    ...
    {
        Material baseMat = CreateMaterial("GC17_SandBase", new Color(0.66f, 0.55f, 0.36f, 1f));
        Material roadMat = CreateMaterial("GC17_CompactedRoad", new Color(0.48f, 0.39f, 0.26f, 1f));
        Material shoulderMat = CreateMaterial("GC17_RoadShoulder", new Color(0.72f, 0.60f, 0.39f, 1f));
        Material localMat = CreateMaterial("GC17_LocalPackedYard", new Color(0.74f, 0.62f, 0.40f, 1f));
        Material vehicleMat = CreateMaterial("GC17_VehicleYardDust", new Color(0.58f, 0.50f, 0.38f, 1f));

        Surface(root, "FlatGameplayBase_2048", Vector3.zero, new Vector2(MapSize, MapSize), baseMat, -0.04f);

        GameObject local = Child(root, "VisualLocalYards_FromWalkabilityContract");
        foreach (Zone zone in SoldierLocalZones)
            Surface(local, zone.Name, Center(zone.Rect, 0.015f), new Vector2(zone.Rect.width, zone.Rect.height), localMat, 0.015f);
        ...
        GameObject roads = Child(root, "MacroRoads_FromWalkabilityContract");
        foreach (Zone road in MacroRoads)
        {
            Surface(roads, road.Name + "_Shoulder", Center(Expanded(road.Rect, 18f), 0.035f), new Vector2(road.Rect.width + 36f, road.Rect.height + 36f), shoulderMat, 0.035f);
            Surface(roads, road.Name, Center(road.Rect, 0.05f), new Vector2(road.Rect.width, road.Rect.height), roadMat, 0.05f);
        }
    }

    private static void PlaceVisualsFromContract(GameObject root)
    {
        GameObject visuals = Child(root, "Visuals_PlacedOnBlockerZones_NotWalkableZones");
        int index = 0;
        foreach (Zone blocker in BlockerZones)
        {
            ...
            PlacePrefab(visuals, blocker.Name, path, Center(blocker.Rect, 0f), footprint, RotationFor(index), ScaleFor(blocker, path), blocker.Name);
            index++;
        }
    ...
    private static void BuildDebugOverlay(GameObject root)
    {
        ...
        Material green = CreateMaterial("GC17_DebugSoldierWalkable", new Color(0f, 1f, 0.1f, 0.6f));
        Material cyan = CreateMaterial("GC17_DebugVehicleWalkable", new Color(0f, 0.8f, 1f, 0.6f));
        Material red = CreateMaterial("GC17_DebugBlocker", new Color(1f, 0f, 0f, 0.7f));
        foreach (Zone zone in SoldierLocalZones)
    ...
    private static void Validate()
    {
        ...
        if (ValidationLog.Count == 0)
            ValidationLog.Add($"PASS: GC17 placed {Placements.Count} visuals from {BlockerZones.Count} blocker zones while preserving {SoldierLocalZones.Count} soldier zones, {VehicleZones.Count} vehicle zones, and {MacroRoads.Count} macro roads.");
    }
    ...
    {
        BuildRtsCamera(root, "Camera_GC17_RtsOverview", new Vector3(-620f, 520f, -820f), BlueprintPoint(388f, 430f), 38f);
        BuildRtsCamera(root, "Camera_GC17_RtsCityReadable", new Vector3(-930f, 330f, -430f), BlueprintPoint(172f, 366f), 34f);
        BuildRtsCamera(root, "Camera_GC17_RtsAirfieldCommand", new Vector3(260f, 430f, -620f), BlueprintPoint(660f, 320f), 36f);

        Camera top = CameraObject(root, "Camera_GC17_TopDownContractVisualProof");
    ...
    private static void CaptureScene()
    {
        foreach (Camera camera in Object.FindObjectsByType<Camera>(FindObjectsInactive.Include, FindObjectsSortMode.None))
        {
            ...
            if (camera.name == "Camera_GC17_RtsCityReadable")
                Render(camera, ProjectPath(CaptureRoot + "/gc17_rts_city_readable_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC17_RtsAirfieldCommand")
                Render(camera, ProjectPath(CaptureRoot + "/gc17_rts_airfield_command_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC17_TopDownContractVisualProof")
                RenderContractProof(camera, ProjectPath(CaptureRoot + "/gc17_topdown_contract_visual_proof_2048x2048.png"));
        }
    }
    ...
    private static void RenderContractProof(Camera camera, string path)
    {
        ...
        GameObject visuals = FindSceneObjectIncludingInactive("Visuals_PlacedOnBlockerZones_NotWalkableZones");
    ...
    private static void WriteContractJson()
    {
        ...
        {
            generatedBy = nameof(WarlineCaptureGc17WalkabilityFirstVisualBuilder),
            scene = ScenePath
        };
    ...
    private static void WriteReport()
    {
        ...
        report.AppendLine("# GC17 Walkability-First Visual Scene");
        report.AppendLine();
        report.AppendLine("Lane: Gameplay");
        report.AppendLine("Task: Build the next scene pass by placing visuals from the GC16 walkability contract, keeping macro roads and city/camp local walk zones as the source of truth.");
        report.AppendLine();
        report.AppendLine("Files changed:");
        report.AppendLine("- `Assets/Game/Scripts/Editor/WarlineCaptureGc17WalkabilityFirstVisualBuilder.cs`");
        report.AppendLine("- `Assets/Game/Scenes/Generated/GC17_WalkabilityFirstVisual_2048.unity`");
        report.AppendLine("- `Design/AgentReports/Data/GeneratedScenes/GC17_WalkabilityFirstVisual_2048/gc17_walkability_visual_contract.json`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC17_WalkabilityFirstVisual_2048/gc17_rts_overview_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC17_WalkabilityFirstVisual_2048/gc17_rts_city_readable_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC17_WalkabilityFirstVisual_2048/gc17_rts_airfield_command_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC17_WalkabilityFirstVisual_2048/gc17_topdown_contract_visual_proof_2048x2048.png`");
        report.AppendLine();
        report.AppendLine("Contracts touched: GC16 walkability contract is promoted into visual placement input; buildings/props are anchored to blocker zones, not placed over walkable lanes.");
        report.AppendLine("User-visible behavior: no shipped runtime behavior changed; generated scene proves visuals can be built around walkability instead of the reverse.");
        report.AppendLine("Validation run: Unity batchmode `WarlineCaptureGc17WalkabilityFirstVisualBuilder.BuildGc17WalkabilityFirstVisual2048`.");
        report.AppendLine("Validation result: " + (ValidationLog.Any(line => line.StartsWith("ERROR:", StringComparison.Ordinal)) ? "failed; see validation log below." : "passed walkability-first visual validation."));
        report.AppendLine("Known gaps: This is a first walkability-first visual pass, not final art quality. Roads still need authored curved road meshes and local yards need richer dressing once the contract is accepted.");
        report.AppendLine("Cross-lane impacts: Design/Art should now compose high-quality modules around blocker/walkable masks instead of producing opaque city clusters with unknown internal walkability.");
        report.AppendLine("Next recommended task: replace rectangle placeholder roads/yards with authored art-road meshes while preserving the exported GC17 walkability visual contract.");
        report.AppendLine();
