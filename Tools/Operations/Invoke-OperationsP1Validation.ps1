# Runs Operations P1 Unity validation against the shadow project only.
[CmdletBinding()]
param(
    [string] $ShadowPath = "D:\Projects\WarlineCapture-Operations",
    [string] $SharedPath = "D:\Projects\WarlineCapture",
    [string] $LogFile = "$env:TEMP\operations-p1-shadow.log",
...
if (Test-Path -LiteralPath $sharedLibrary) {
    Write-Host "[OperationsP1] leaving_shared_library_untouched=$sharedLibrary"
}
...
Write-Host "[OperationsP1] project=$shadow"
Write-Host "[OperationsP1] unity=$unityExe"
Write-Host "[OperationsP1] log=$LogFile"
...
& $validateScript `
    -UnityExe $unityExe `
    -ProjectPath $shadow `
    -ExecuteMethod "Game.Tests.Editor.Operations.OperationsP1Validation.RunFocusedValidation" `
    -LogFile $LogFile `
    -RequiredPassMarker "[OperationsP1Validation] result=Passed checks=16" `
    -GuiLicensing:$gui `
    -TimeoutSeconds $TimeoutSeconds

if ($LASTEXITCODE -ne 0) {
    throw "Operations P1 shadow validation failed."
}

Write-Host "[OperationsP1] result=Passed project=$shadow"
