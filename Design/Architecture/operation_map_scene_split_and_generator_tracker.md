...
Overall implementation progress: 35% (62/177 checklist items complete).
...
| 2. Per-map static presentation ownership | In progress | 7 | 14 | 50% | Bake input, compatibility routing, schema-2 identity, deterministic map-owned output root, map-namespaced scene filenames, per-map integrity, and map-scoped stale cleanup are accepted; remaining transaction/wiring ownership stays fail-closed. |
