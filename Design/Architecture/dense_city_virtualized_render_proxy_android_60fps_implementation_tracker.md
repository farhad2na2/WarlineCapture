...
Date: 2026-07-28
Status: Candidate implementation in progress through bounded Burst selection, deterministic slot assignment, an asynchronously ordered parallel slot-apply boundary, explicit persistent-owner/blob lifetime fences, and an exact frozen render-only pilot; upstream runtime scheduling, pilot enablement, persisted candidate, and production cutover remain disabled
...
| Phase 3: Candidate pool baking | Complete for the authorized unsaved direct-bake gate; two complete dense candidate conversions produced identical fingerprint `4d859a9a...2d17d`, one schema-1 database (`22` prototypes, `26` parts, `9,721` placements, `1,635` cells, two buckets), `704` fixed disabled leaf slots, `11,299` exact eligible render-entity rows marked baking-only, and `70,710` exact resident render-entity exceptions from `82,009` independently counted stable-owner renderer rows. The historical `82,797` packed inventory remains the renderer/material-row count; its `788` additional rows are resident multi-material expansion and are not source-ownership buffer rows. The persisted dense candidate definition remains `EntityScene + ResidentEntities`, production remains `StaticSceneChunks + ResidentEntities`, production cutover is `0`, and protected candidate/accepted/frozen/Addressables ownership hashes remained unchanged after both passes. No EntityScene/package was persisted by this gate; later pilot admission/runtime/package steps own that mutation. |
| Phase 4: Jobified runtime assignment | Checklist complete; the ordered camera boundary, exact map-generation native initialization, pure guard-envelope decision, bounded selection, deterministic assignment, parallel apply, apply-to-Entities-Graphics dependency boundary, stable apply no-op gate, bounded instrumentation contract, and explicit persistent-owner/database-blob lifetime proof are accepted. Initialization publishes immutable placement/logical-part/resident/capacity counts. A Burst metrics projection validates and publishes slot/activity/assignment/overflow/deficit/version/rebuild counters without allocation; disabled formatting returns before constructing even a fixed string. Exact profiler markers wrap initialization and the accepted selection/assignment/apply jobs, with the exact sync marker registered for its still-open state-sync system. Selection/assignment runtime-system scheduling and dynamic metrics hookup remain open integration work owned by the pilot path; no pilot or persisted candidate is enabled by Phase 4 completion. |
| Phase 5: Render-only pilot | In progress; the largest accepted stable-owner-joined repeated render-only Vegetation/Prop set is frozen at 9,721 exact placement identities, 22 repeated prototypes/26 parts, and 11,299 render rows. All 5,423 Vegetation rows and 5,876 eligible Prop rows are selected; the three excluded Prop rows remain resident. The selection manifest is byte-deterministic, while mutation authorization and pilot enablement remain false pending VRP-051. |
...
Checklist progress: `42 / 80` complete.
