...
- selection runtime cached query construction, selected-move query handles, selected-tag query handles, and grid-config query handles belong in `SelectionRuntimeQuerySystem`; `SelectionRuntimeContextSystem` must not own `CreateEntityQuery` calls or cached `EntityQuery` fields.
