...
| Current target | Selection allocation attribution has been measured with an editor-only subphase probe and classified as report/profiler attribution noise for this capture. Building-placement visual instantiate attribution is now measured too: `BuildingPlacementVisualPresentationSystemHelper.CreateBuildingVisualInstance` reported `0` bytes across `0` create calls in the 300-frame battle capture, so the next exact action is to instrument/classify the remaining production transport attribution under `BuildingPlacementRuntimeTick.UpdateActiveProductionTransports` / `BuildingProductionRuntimeTick.UpdateActiveProductionTransports` before migrating BuildingPlacement/transports. |
| Compiler status | Latest building-visual allocation-probe slice passed `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, and `git diff --check` with 0 errors. Unity 6.5.2 battle capture passed through the documented non-headless batchmode workaround at `/private/tmp/warline-arch-followup-building-visual-probe-windowed-battle.log` with 0 project compiler errors. |
| Android baseline status | Not started |
| GC baseline status | Latest completed 2026-07-04 20:15:28 UTC battle-state Match GC capture after building visual allocation probe: 300 requested frames after 180 warmup frames, 301 scanned profiler frames, 11,672 raw GC.Alloc samples / 774,296 raw bytes, 7,820 player-relevant samples / 504,604 bytes after excluding editor/tooling rows, and 3,852 editor/tooling samples / 269,692 bytes. Runtime allocation probe assertion passed. `UIShellEcsPresentationSystem.Update`, `MenuBootstrapView.Update`, selection subphases, and `BuildingPlacementVisualPresentationSystemHelper.CreateBuildingVisualInstance` all reported `0` bytes; the building visual probe also reported `0` create calls, `0` pooled hits, and `0` prefab instantiates during capture. |
