...
| Phase 6: Building and attachment state | In progress through VRP-061. VRP-060 requires every virtualized gameplay-building state-owner index to be the contiguous zero-based position of its collision-checked owner identity in unsigned `(Low, High)` order; the replacement gate rejects arbitrary, sparse, or reordered indices before removing resident render-root ownership. `OperationMapBuildingDestructionSystem` now has disjoint resident and virtualized Burst jobs. Resident buildings retain the accepted intact/destroyed root-scale behavior. Virtualized buildings retain canonical `OperationMapBuildingDestroyedComponent` authority, enable it once at zero health, and append exactly one destroyed-state event carrying the deterministic owner index and monotonic nonzero change version to the unique map-owned buffer; that job has no transform/proxy lookup, creates no entity or visual, and a missing/duplicate buffer, presentation overlap, or version overflow fails closed before scheduling. Focused coverage preserves blocker/gameplay ownership, entity count, and unrelated proxy transforms. The current render-only pilot still has no state owners and no building family has been virtualized. Bounded event consumption/state synchronization remains VRP-062. |
...
Checklist progress: `48 / 80` complete.
