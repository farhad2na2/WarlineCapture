...
public static class WarlineCaptureGc22AuthoredClusterModuleBuilder
{
    private const string ScenePath = "Assets/Game/Scenes/Generated/GC22_AuthoredClusterModules_2048.unity";
    private const string CaptureRoot = "Design/AgentReports/Captures/GeneratedScenes/GC22_AuthoredClusterModules_2048";
    private const string ReportPath = "Design/AgentReports/2026-05-21_gameplay_gc22-authored-cluster-modules.md";
    private const string DataPath = "Design/AgentReports/Data/GeneratedScenes/GC22_AuthoredClusterModules_2048/gc22_authored_cluster_modules_contract.json";
    ...
    [MenuItem("WarlineCapture/Design/Build GC22 Authored Cluster Modules 2048")]
    public static void BuildGc22AuthoredClusterModules2048()
    {
        MacroRoads.Clear();
        ...
        GameObject root = new("GC22_AuthoredClusterModules_2048_Root");
        ...
        Debug.Log($"WARLINECAPTURE_GC22_AUTHORED_CLUSTER_MODULES_BUILT scene={ScenePath} report={ReportPath}");
    ...
    private static void BuildEnvironment(GameObject root)
    {
        ...
        Volume volume = Child(root, "GC22_RTS_PresentationVolume").AddComponent<Volume>();
    ...
    {
        Material baseMat = CreateMaterial("GC22_SandBase", new Color(0.61f, 0.50f, 0.31f, 1f));
        Material roadMat = CreateMaterial("GC22_AuthoredCompactedRoad", new Color(0.64f, 0.53f, 0.35f, 1f));
        Material shoulderMat = CreateMaterial("GC22_AuthoredRoadShoulder", new Color(0.53f, 0.43f, 0.27f, 1f));
        Material trackMat = CreateMaterial("GC22_DirtTireTrack", new Color(0.42f, 0.32f, 0.18f, 1f));
        Material runwayMat = CreateMaterial("GC22_RunwayWeatheredAsphalt", new Color(0.36f, 0.35f, 0.31f, 1f));
        Material plazaMat = CreateMaterial("GC22_LocalPackedPlaza", new Color(0.70f, 0.58f, 0.37f, 1f));
        Material vehicleMat = CreateMaterial("GC22_VehicleYardDust", new Color(0.55f, 0.46f, 0.33f, 1f));
    ...
    private static void PlaceAuthoredCluster(GameObject parent, MaskedModule module, Zone mask, string path)
    {
        ...
        instance.name = "GC22_" + module.Name + "_AuthoredCluster";
    ...
    private static void BuildDebugOverlay(GameObject root)
    {
        ...
        Material green = CreateMaterial("GC22_DebugSoldierWalkable", new Color(0f, 1f, 0.1f, 0.6f));
        Material cyan = CreateMaterial("GC22_DebugVehicleWalkable", new Color(0f, 0.8f, 1f, 0.6f));
        Material red = CreateMaterial("GC22_DebugBlocker", new Color(1f, 0f, 0f, 0.7f));
        foreach (Zone zone in SoldierLocalZones)
    ...
    private static void BuildGeneratorAuthoringOverlay(GameObject root)
    {
        ...
        Material city = CreateMaterial("GC22_AuthoringDistrict_City", new Color(0.20f, 0.42f, 0.80f, 0.85f));
        Material baseMat = CreateMaterial("GC22_AuthoringDistrict_Base", new Color(0.30f, 0.62f, 0.28f, 0.85f));
        Material logistics = CreateMaterial("GC22_AuthoringDistrict_Logistics", new Color(0.84f, 0.58f, 0.18f, 0.85f));
        Material airfield = CreateMaterial("GC22_AuthoringDistrict_Airfield", new Color(0.68f, 0.68f, 0.74f, 0.85f));
    ...
    private static void BuildMaskedModuleOverlay(GameObject root)
    {
        ...
        Material moduleMat = CreateMaterial("GC22_ModuleMask_DistrictEnvelope", new Color(0.88f, 0.18f, 0.92f, 0.82f));
        Material blockerMat = CreateMaterial("GC22_ModuleMask_BlockerOwned", new Color(1f, 0.08f, 0.08f, 0.9f));
    ...
    private static void Validate()
    {
        ...
        if (detailBlockerPlacements < 60)
            ValidationLog.Add($"ERROR: GC22 placed only {detailBlockerPlacements} legal detail props; densification target is at least 60.");

        if (Districts.Count < 7)
            ValidationLog.Add($"ERROR: GC22 exported only {Districts.Count} districts; procedural authoring target is at least 7 named districts.");
        ...
        if (Modules.Count < 7)
            ValidationLog.Add($"ERROR: GC22 exported only {Modules.Count} reusable masked modules; target is at least 7.");

        if (authoredClusterPlacements < Modules.Count)
            ValidationLog.Add($"ERROR: GC22 placed only {authoredClusterPlacements} authored Demo clusters for {Modules.Count} reusable modules.");
        ...
        {
            Zone mask = BlockerZones.FirstOrDefault(zone => zone.Name == cluster.MaskName);
            if (mask.Name == null)
    ...
    {
        BuildRtsCamera(root, "Camera_GC22_RtsOverview", new Vector3(-620f, 520f, -820f), BlueprintPoint(388f, 430f), 38f);
        BuildRtsCamera(root, "Camera_GC22_RtsCityReadable", new Vector3(-930f, 330f, -430f), BlueprintPoint(172f, 366f), 34f);
        BuildRtsCamera(root, "Camera_GC22_RtsAirfieldCommand", new Vector3(260f, 430f, -620f), BlueprintPoint(660f, 320f), 36f);
        BuildRtsCamera(root, "Camera_GC22_RtsDenseCityClose", new Vector3(-700f, 210f, -250f), BlueprintPoint(176f, 392f), 30f);

        Camera top = CameraObject(root, "Camera_GC22_TopDownContractVisualProof");
    ...
    private static void CaptureScene()
    {
        ...
        {
            if (camera.name == "Camera_GC22_RtsOverview")
                Render(camera, ProjectPath(CaptureRoot + "/gc22_rts_overview_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC22_RtsCityReadable")
                Render(camera, ProjectPath(CaptureRoot + "/gc22_rts_city_readable_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC22_RtsAirfieldCommand")
                Render(camera, ProjectPath(CaptureRoot + "/gc22_rts_airfield_command_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC22_RtsDenseCityClose")
                Render(camera, ProjectPath(CaptureRoot + "/gc22_rts_dense_city_close_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC22_TopDownContractVisualProof")
            {
                RenderContractProof(camera, ProjectPath(CaptureRoot + "/gc22_topdown_contract_visual_proof_2048x2048.png"));
                RenderGeneratorBlueprint(camera, ProjectPath(CaptureRoot + "/gc22_topdown_generator_blueprint_2048x2048.png"));
                RenderModuleMaskProof(camera, ProjectPath(CaptureRoot + "/gc22_topdown_module_mask_proof_2048x2048.png"));
            }
        }
    }
    ...
    private static void WriteContractJson()
    {
        ...
        {
            generatedBy = nameof(WarlineCaptureGc22AuthoredClusterModuleBuilder),
            scene = ScenePath,
    ...
    private static void WriteReport()
    {
        ...
        report.AppendLine("# GC22 Authored Cluster Modules Scene");
        report.AppendLine();
        ...
        report.AppendLine("- `Assets/Game/Scripts/Editor/WarlineCaptureGc22AuthoredClusterModuleBuilder.cs`");
        report.AppendLine("- `Assets/Game/Scenes/Generated/GC22_AuthoredClusterModules_2048.unity`");
        report.AppendLine("- `Design/AgentReports/Data/GeneratedScenes/GC22_AuthoredClusterModules_2048/gc22_authored_cluster_modules_contract.json`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC22_AuthoredClusterModules_2048/gc22_rts_overview_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC22_AuthoredClusterModules_2048/gc22_rts_city_readable_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC22_AuthoredClusterModules_2048/gc22_rts_airfield_command_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC22_AuthoredClusterModules_2048/gc22_rts_dense_city_close_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC22_AuthoredClusterModules_2048/gc22_topdown_contract_visual_proof_2048x2048.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC22_AuthoredClusterModules_2048/gc22_topdown_generator_blueprint_2048x2048.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC22_AuthoredClusterModules_2048/gc22_topdown_module_mask_proof_2048x2048.png`");
        report.AppendLine();
        report.AppendLine("Contracts touched: GC17/GC18 walkability visual contract remains the source of truth; GC22 adds fitted authored-cluster placements to the reusable module contract.");
        ...
        report.AppendLine("Validation run: Unity batchmode `WarlineCaptureGc22AuthoredClusterModuleBuilder.BuildGc22AuthoredClusterModules2048`.");
