...
| Current task | `AM-025` remains blocked from acceptance but active for remediation. Selection-outline materials and mesh are now owned and disposed by their ECS World, closing four lifecycle warnings. The safer GameText reset remains tracked static-registry debt. The `575` historical rows contain `461` non-debt and `114` genuine-debt rows grouped into `45` unique file/rule items. Separately owned FirstLaunch and operation-map helper paths still block the canonical source-growth gate. |
...
| Blockers | AM-025 source-growth is externally blocked by four separately owned FirstLaunch helpers and four operation-map-owned helpers. Three operation-map helpers are new since the five-path closure policy was published. Row-bound evidence still exposes `114` genuine-debt rows in `45` unique file/rule items that must be remediated to zero. Release-only certification remains intentionally deferred. |
| Latest validation | Selection-outline lifecycle validation passes `1 / 1` and confirms World disposal destroys its runtime material; Unity compiles with zero errors. The broader visual suite reached an unrelated existing health-bar assertion after the changed outline tests passed. Phase 2 evidence passes `19 / 19`, ownership-delta regeneration is byte-stable, and `git diff --check` passes. The AM-021 ownership snapshot remains refreshed at implementation commit `2107ff75b` with the same `575 / 553 / 22 / 0` resource, explicit-owner, protected-owner, and gap result. |
| Latest evidence | Schema-2 `am025_phase2_ownership_delta.json/.md` records one decision for each of `575` historical intake rows: `453` resolved, `8` protected-deferred, and `114` genuine debt grouped into `45` unique file/rule items. The selection-outline lifecycle owner closes four historical mutable-static rows. `am025_phase2_ownership_delta_intake.json` preserves the immutable v1 intake, and policy schema 2 lists all five exact source-growth blockers |
