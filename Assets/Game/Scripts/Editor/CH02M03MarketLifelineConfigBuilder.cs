...
namespace Game.Editor
{
    public static class CH02M03MarketLifelineConfigBuilder
    {
        ...
        private static void BuildMap()
        {
            ...
            using(blob)
            {
                (string name,int x,int z,OperationMapAnchorKind kind,int faction,float radius)[] anchors={
                    ("delivery",797,451,OperationMapAnchorKind.Objective,1,8),("manifest",748,462,OperationMapAnchorKind.Objective,1,6),
                    ("convoy",684,426,OperationMapAnchorKind.Deployment,1,4),("squad_a",699,430,OperationMapAnchorKind.Deployment,1,3),("squad_b",715,430,OperationMapAnchorKind.Deployment,1,3),
        ...
        private static void BuildScenario()
        {
            ...
            A(data.FindProperty("ambientPresentations"),0,null);A(data.FindProperty("unitGroups"),5,Group);
            A(data.FindProperty("patrolRoutes"),2,(e,i)=>{S(e,"routeId","route.ch02.m03.hostile."+i);S(e,"unitGroupId","group.ch02.m03.hostile_"+(i==0?"a":"b"));S(e,"startDelayMilliseconds",35000+i*35000);A(e.FindPropertyRelative("anchorIds"),2,(a,n)=>a.stringValue=Prefix+(n==0?(i==0?"hostile_a":"hostile_b"):(i==0?"delivery":"manifest")));});
            var market=data.FindProperty("marketLifeline");S(market,"enabled",true);S(market,"deliveryAnchorId",Prefix+"delivery");S(market,"manifestAnchorId",Prefix+"manifest");S(market,"requiredDeliveries",3);S(market,"manifestHoldMilliseconds",6000);S(market,"victoryHoldMilliseconds",10000);S(market,"deadlineMilliseconds",600000);market.FindPropertyRelative("deliveryRadius").floatValue=8;market.FindPropertyRelative("manifestRadius").floatValue=6;
        ...
        {
            string[] names={"squad_a","squad_b","convoy","hostile_a","hostile_b"};S(group,"groupId","group.ch02.m03."+names[i]);S(group,"factionIndex",i>=3?2:1);
            string[] units=i switch {0 or 1=>new[]{"Chr_Soldier_Male_02_Alt_02","Chr_Soldier_Male_02_Alt_04","Chr_Soldier_Female_01_Alt_01","Chr_Soldier_Female_02_Alt_01"},2=>new[]{"Veh_Truck_Tray","Veh_Truck_Tray","Veh_Truck_Canopy"},_=>new[]{"Chr_Insurgent_Male_03","Chr_Insurgent_Female_01","Chr_Insurgent_Female_02"}};
            A(group.FindPropertyRelative("units"),units.Length,(unit,n)=>{string key="Unit_"+units[n],path="Assets/Game/Prefabs/"+(i==2?"Vehicles/":"Characters/")+key+".prefab";Require(Load<GameObject>(path)!=null,path);S(unit,"unitConfigKey","unit.market."+units[n].ToLowerInvariant()+"."+n);S(unit,"runtimePrefabSourceKey",key);S(unit,"expectedAssetGuid",AssetDatabase.AssetPathToGUID(path));S(unit,"count",1);S(unit,"spawnAnchorId",Prefix+names[i]);S(unit,"missionRoleId",i==2?"role.market.relief_convoy":i>=3?"role.market.hostile":"role.market.rifle");});
        }
