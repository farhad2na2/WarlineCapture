...
| Current task | `AM-025` remains blocked from acceptance but active for remediation. Gameplay and camera mode state now belongs only to the active match World instead of being mirrored into process-wide static fields. The `575` historical rows contain `550` non-debt and `25` genuine-debt rows grouped into `21` unique file/rule items. Separately owned FirstLaunch and operation-map helper paths still block the canonical source-growth gate. |
...
| Blockers | AM-025 source-growth is externally blocked by four separately owned FirstLaunch helpers and four operation-map-owned helpers. Three operation-map helpers are new since the five-path closure policy was published. Row-bound evidence still exposes `25` genuine-debt rows in `21` unique file/rule items that must be remediated to zero. Release-only certification remains intentionally deferred. |
| Latest validation | Unity compiles with zero errors; focused gameplay-state validation proves ECS writes, external ECS reads, match-start and shutdown resets, camera-focus consumption, replacement-World rebinding, and two-World isolation. Phase 2 evidence regeneration is byte-stable and `git diff --check` passes. The AM-021 ownership snapshot remains refreshed at implementation commit `2107ff75b` with the same `575 / 553 / 22 / 0` resource, explicit-owner, protected-owner, and gap result. |
| Latest evidence | Schema-2 `am025_phase2_ownership_delta.json/.md` records one decision for each of `575` historical intake rows: `542` resolved, `8` protected-deferred, and `25` genuine debt grouped into `21` unique file/rule items. Focused Unity validation proves gameplay and camera state remain isolated per match World and reset cleanly. `am025_phase2_ownership_delta_intake.json` preserves the immutable v1 intake, and policy schema 2 lists all five exact source-growth blockers |
