...
public sealed class BuildingProductionQueueCompositionSystemHelperTests
{
    ...
    public static void RunOperationMapProducerQueueConsumerValidation()
    {
        try
        {
            var tests = new BuildingProductionQueueCompositionSystemHelperTests();
        ...
        {
            Debug.LogException(ex);
            Debug.LogError("[OperationMapProducerQueueConsumerValidation] result=Failed");
            ValidationExit.Failed();
        }
    }

    public static void RunOperationMapProducerSpawnTransactionValidation()
    {
        try
        {
            var tests = new BuildingProductionQueueCompositionSystemHelperTests();
            tests.OperationMapProducerSpawnTransaction_InstantiatesConfiguredUnitAndCompletesRequest();
            tests.OperationMapProducerSpawnTransaction_RejectsEarlyOrInvalidPlacement();
            tests.OperationMapProducerSpawnTransaction_RejectsPrefabWithoutUnitContract();
            Debug.Log("[OperationMapProducerSpawnTransactionValidation] result=Passed tests=3");
            ValidationExit.Passed();
        }
        catch (Exception ex)
        {
            Debug.LogException(ex);
            Debug.LogError("[OperationMapProducerSpawnTransactionValidation] result=Failed");
            ValidationExit.Failed();
        }
    }
    ...
    public void OperationMapProducerQueue_EnqueuesDeterministicPendingRows()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(72.5f, queue[0].ReadyAt);
    ...
    [Test]
    public void OperationMapProducerQueueConsumer_WaitsForReadyTime()
    {
        using var world = new World(nameof(OperationMapProducerQueueConsumer_WaitsForReadyTime));
        EntityManager em = world.EntityManager;
        GameObject unitPrefab = new("Rifleman");
        try
        {
            ...
            BuildingProductionRequestSystemHelper.Context context = CreateProducerSelectionContext(
                ...
                em);
            Assert.IsTrue(requestSystem.TryEnqueueFriendlyOperationMapProduction(
                context, unitPrefab, 10f, out _, out int requestId, out _));

            Assert.IsFalse(requestSystem.TryPeekReadyOperationMapProduction(em, producer, 69.99f, out _));
            Assert.IsTrue(requestSystem.TryPeekReadyOperationMapProduction(em, producer, 70f, out OperationMapBuildingUnitProductionRequest ready));
            Assert.AreEqual(requestId, ready.RequestId);
            Assert.AreEqual(70f, ready.ReadyAt);
        }
        finally
        {
            UnityEngine.Object.DestroyImmediate(unitPrefab);
    ...
    [Test]
    public void OperationMapProducerQueueConsumer_CompletesStrictFifoOnce()
    {
        using var world = new World(nameof(OperationMapProducerQueueConsumer_CompletesStrictFifoOnce));
        EntityManager em = world.EntityManager;
        GameObject unitPrefab = new("Rifleman");
        try
        {
            ...
            Assert.IsFalse(requestSystem.TryCompleteReadyOperationMapProduction(em, producer, secondRequestId, 100f));
            Assert.IsTrue(requestSystem.TryCompleteReadyOperationMapProduction(em, producer, firstRequestId, 100f));
    ...
    [Test]
    public void OperationMapProducerQueueConsumer_RejectsDestroyedProducerAndMissingPrefab()
    {
        using var world = new World(nameof(OperationMapProducerQueueConsumer_RejectsDestroyedProducerAndMissingPrefab));
        EntityManager em = world.EntityManager;
        GameObject unitPrefab = new("Rifleman");
        try
        {
            ...
            em.SetComponentEnabled<OperationMapBuildingDestroyedComponent>(producer, true);
            Assert.IsFalse(requestSystem.TryPeekReadyOperationMapProduction(em, producer, 60f, out _));
            em.SetComponentEnabled<OperationMapBuildingDestroyedComponent>(producer, false);
            em.DestroyEntity(prefabEntity);
            Assert.IsFalse(requestSystem.TryPeekReadyOperationMapProduction(em, producer, 60f, out _));
    ...
    [Test]
    public void OperationMapProducerSpawnTransaction_InstantiatesConfiguredUnitAndCompletesRequest()
    {
        using var world = new World(nameof(OperationMapProducerSpawnTransaction_InstantiatesConfiguredUnitAndCompletesRequest));
        EntityManager em = world.EntityManager;
        GameObject unitPrefab = new("Rifleman");
        try
        {
            ...
            Assert.AreEqual(spawnPosition, em.GetComponentData<LocalTransform>(spawned).Position);
            Assert.AreEqual(spawnPosition, em.GetComponentData<UnitPrevWorldPos>(spawned).Value);
            Assert.AreEqual(FactionIdentity.PlayerFactionId, em.GetComponentData<Faction>(spawned).Id);
    ...
    [Test]
    public void OperationMapProducerSpawnTransaction_RejectsEarlyOrInvalidPlacement()
    {
        using var world = new World(nameof(OperationMapProducerSpawnTransaction_RejectsEarlyOrInvalidPlacement));
        EntityManager em = world.EntityManager;
        GameObject unitPrefab = new("Rifleman");
        try
        {
            ...
            Assert.IsFalse(requestSystem.TrySpawnReadyOperationMapProduction(
                em, producer, requestId, 69f, new int2(1, 1), float3.zero, out _));
            Assert.IsFalse(requestSystem.TrySpawnReadyOperationMapProduction(
                em, producer, requestId, 70f, new int2(-1, 1), float3.zero, out _));
            Assert.IsFalse(requestSystem.TrySpawnReadyOperationMapProduction(
                em, producer, requestId, 70f, new int2(1, 1), new float3(float.NaN), out _));
            Assert.AreEqual(1, em.GetBuffer<OperationMapBuildingUnitProductionRequest>(producer, true).Length);
        }
        finally
        {
    ...
    [Test]
    public void OperationMapProducerSpawnTransaction_RejectsPrefabWithoutUnitContract()
    {
        using var world = new World(nameof(OperationMapProducerSpawnTransaction_RejectsPrefabWithoutUnitContract));
        EntityManager em = world.EntityManager;
        GameObject unitPrefab = new("Rifleman");
        try
        {
            ...
            Assert.IsFalse(requestSystem.TrySpawnReadyOperationMapProduction(
                em, producer, requestId, 60f, new int2(1, 1), float3.zero, out _));
            Assert.AreEqual(1, em.GetBuffer<OperationMapBuildingUnitProductionRequest>(producer, true).Length);
        }
        finally
        {
    ...
    [Test]
    public void OperationMapProductionRuntimeScheduler_ReportsAmbiguousReadyStallOncePerInterval()
    {
        NativeArray<int> blockerCounts = default;
        NativeBitArray blocked = default;
        NativeBitArray occupied = default;
        NativeArray<byte> friendlyPassFactionIds = default;
        World world = new(nameof(OperationMapProductionRuntimeScheduler_ReportsAmbiguousReadyStallOncePerInterval));
        EntityManager em = world.EntityManager;
        GameObject unitPrefab = new("Rifleman");
        try
        {
            ...
            Entity duplicateGrid = em.CreateEntity(
                ...
                typeof(DynamicBlockerComponent),
                typeof(DynamicOccupancyComponent));
            em.SetComponentData(duplicateGrid, em.GetComponentData<GridConfig>(authoredGrid));
            em.SetComponentData(duplicateGrid, em.GetComponentData<DynamicBlockerComponent>(authoredGrid));
            em.SetComponentData(duplicateGrid, em.GetComponentData<DynamicOccupancyComponent>(authoredGrid));
            DynamicBuffer<GridWalkable> duplicateWalkable = em.AddBuffer<GridWalkable>(duplicateGrid);
            DynamicBuffer<GridWalkable> authoredWalkable = em.GetBuffer<GridWalkable>(authoredGrid, true);
            duplicateWalkable.ResizeUninitialized(authoredWalkable.Length);
            for (int index = 0; index < authoredWalkable.Length; index++)
                duplicateWalkable[index] = authoredWalkable[index];
            ...
            Assert.AreEqual(0, requestSystem.ProcessReadyOperationMapProductions(
                ...
                60f,
                warnings.Add,
                out BuildingProductionRequestSystemHelper.OperationMapProductionSchedulerDiagnostics first));
            Assert.AreEqual(
                BuildingProductionRequestSystemHelper.OperationMapProductionSchedulerOutcome.AmbiguousAuthoredGrid,
                first.Outcome);
            Assert.AreEqual(1, first.PendingRequestCount);
            Assert.AreEqual(1, first.ReadyProducerCount);
            Assert.AreEqual(2, first.AuthoredGridCount);
            Assert.AreEqual(1, warnings.Count);
            StringAssert.Contains("outcome=AmbiguousAuthoredGrid", warnings[0]);

            Assert.AreEqual(0, requestSystem.ProcessReadyOperationMapProductions(em, 61f, warnings.Add, out _));
            Assert.AreEqual(1, warnings.Count);
            Assert.AreEqual(0, requestSystem.ProcessReadyOperationMapProductions(em, 65f, warnings.Add, out _));
    ...
    private static Entity CreateOperationMapUnitPrefab(EntityManager entityManager)
    {
        Entity prefab = entityManager.CreateEntity(
            ...
            typeof(UnitSourcePrefabKey));
        entityManager.SetComponentData(prefab, new UnitHealth { Current = 100, Max = 100 });
