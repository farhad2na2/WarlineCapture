...
| Current task | `AM-025` remains blocked from acceptance but active for remediation. Child AMFR-015 is accepted at `15 / 22` and AMFR-016 is dependency-ready. The last governed scan has `430` reviewable rows: `412` resolved, `5` protected, and `13` genuine-debt rows grouped into `12` unique file/rule items; AMFR-015's validated projection is `6` / `7`, pending governed Phase D regeneration. The original `575` rows remain historical intake. Five exact operation-map paths are handed to the AM-025 child while the broader map owner remains active. |
...
| Blockers | The governed `13` genuine-debt rows in `12` unique items must be remediated to zero; validated slices through AMFR-015 project `6` / `7` pending governed regeneration. The source-growth helper intake is cleared, while `45` broader exact-head source review items and the Resource Exchange/UI shell responsibility ratchet remain fail-closed inputs for AMFR-017. All other operation-map paths remain protected. Release-only certification remains intentionally deferred. |
...
| Latest evidence | Unchanged visual-quality helper `198` lines / `8,170` bytes is bound to immutable pushed commit `e172b6bb899ce5a39966cb48528ef4c971f1f7c2` with no spare headroom and no production edit. AMFR-003 evidence still classifies all `430` rows once (`412 / 5 / 13`) with zero unclassified. |
...
### 2026-07-18 - AM-025 - UI gateway lifecycle evidence

- Lifecycle result: Unity subsystem registration replaces any stale UI shell gateway, and the existing ECS adapter discards queries from a replaced World before reading the new match state.
- Debt result: one historical mutable-static row is now resolved. The remaining Phase 2 projection is `511` non-debt rows, `64` genuine-debt rows, and `29` unique remediation items.
- Validation: focused Unity lifecycle validation passed `2 / 2`; project compilation completed with zero compiler errors. Release-only device, thermal, and sustained tests remained deferred.
- Scope preserved: no operation-map, FirstLaunch, audio, UI visual-lock, scene, prefab, package, or project-settings file was edited by this slice.
- Next action: move the menu startup-settings World marker from process-wide storage into the existing menu composition instance, then validate shutdown and replacement-World behavior.

### 2026-07-18 - AM-025 - Menu startup settings ownership

- Lifecycle result: the startup-settings World marker belongs to each menu composition instead of the whole process, and menu shutdown releases it before any early return.
- Debt result: one more historical mutable-static row is resolved. The remaining Phase 2 projection is `512` non-debt rows, `63` genuine-debt rows, and `28` unique remediation items.
- Validation: focused Unity ownership validation passed `2 / 2` and Unity reported zero compiler errors. Release-only device, thermal, and sustained tests remained deferred.
- Scope preserved: the existing audio-owned behavior test was not changed; no operation-map, FirstLaunch, audio, UI visual-lock, scene, prefab, package, or project-settings file was edited by this slice.
- Next action: replace the global quick-game AI settings snapshot with menu-owned state and an ECS-owned match handoff.

### 2026-07-18 - AM-025 - AI settings and duplicate build UI ownership

- AI result: quick-game settings belong to each menu config store, cross the existing match-start boundary as ECS data, and are consumed once by match startup. The global AI settings class and implicit startup fallbacks are removed.
- UI result: the unused duplicate ECS build drawer and placement projection path is removed. The active Canvas build drawer and placement confirmation views remain unchanged and authoritative.
- Debt result: 26 historical rows across seven remediation items are resolved. The remaining projection is `538` non-debt rows, `37` genuine-debt rows, and `21` unique remediation items.
- Validation: Unity compiled with zero compiler errors; AI ownership passed `2 / 2`, AI startup passed `1 / 1`, and the obsolete build UI regression guard passed `1 / 1`. The active drawer's migrated materials/fuel checks passed before its broader suite reached an unrelated existing price-fixture mismatch.
- Scope preserved: no operation-map, FirstLaunch, audio, UI visual-lock, scene, prefab, package, project-settings, or release-certification file was edited by this slice.
- Next action: remove the legacy process-wide gameplay/camera mirror in favor of the existing ECS-owned runtime state components.

### 2026-07-19 - AM-025 - Match gameplay and camera state ownership

- Ownership result: play, simulation, selection, build, fullscreen-map, pointer suppression, player auto-mode, zoom, and initial-camera-focus state now live only in the active match World's ECS components. The legacy process-wide mirror and its mirror component are removed.
- Lifecycle result: initial base focus is published directly to ECS; gameplay start and match shutdown reset the World-owned state; replacement Worlds begin clean and cannot read the previous match's values.
- Debt result: 12 historical mutable-static rows are resolved. The remaining projection is `550` non-debt rows, `25` genuine-debt rows, and `21` unique remediation items; the three diagnostic switches in `InitialUnitsRuntimeState` remain tracked.
- Validation: Unity compiled with zero compiler errors and focused gameplay-state validation passed all ownership, reset, focus-consumption, and two-World isolation cases.
- Scope preserved: no operation-map, FirstLaunch, audio, UI visual-lock, scene, prefab, package, project-settings, or release-certification file was edited by this slice.
- Next action: remove the two unprotected scene-hierarchy discovery paths through explicit ECS composition references, then return to the remaining mutable diagnostic state.

### 2026-07-19 - AM-025 - Match-scene reference ownership

- Ownership result: match startup registers its `MatchSceneView` once in the active World. Menu presentation, match UI binding, and deferred gameplay start resolve that exact reference instead of searching scene roots.
- Lifecycle result: two Worlds remain isolated, a previous owner cannot clear a replacement owner, and disposed Worlds cannot leak a stale view into a later match.
- Performance result: the repeated `GetRootGameObjects` loop and MatchStart scene-manager dependency are removed from this path.
- Debt result: one runtime-discovery row and one unique remediation item are resolved. The remaining projection is `551` non-debt rows, `24` genuine-debt rows, and `20` unique remediation items.
- Validation: Unity compiled with zero compiler errors and focused match-reference validation passed registration, isolation, replacement, and disposal cases.
- Scope preserved: no operation-map, FirstLaunch, audio, UI visual-lock, scene, prefab, package, project-settings, or release-certification file was edited by this slice.
- Next action: replace the remaining menu-to-match performance-diagnostics scene scan with an explicit composition reference.

### 2026-07-19 - AM-025 - Performance-diagnostics reference ownership

- Ownership result: the initialized menu diagnostics instance is registered once in the active World and match startup resolves it directly. The managed ECS boundary is explicit and does not require `SystemBase`.
- Lifecycle result: separate Worlds cannot share diagnostics, an older menu cannot clear a replacement owner, and match startup retains its local fallback when no menu registration exists.
- Performance result: the Menu-scene `GetRootGameObjects` allocation and hierarchy walk are removed.
- Debt result: one runtime-discovery row and one unique remediation item are resolved. The remaining projection is `552` non-debt rows, `23` genuine-debt rows, and `19` unique remediation items.
- Validation: Unity compiled with zero compiler errors and focused diagnostics validation passed ownership, isolation, fallback, allocation, and replacement-World cases.
- Scope preserved: no operation-map, FirstLaunch, audio, UI visual-lock, scene, prefab, package, project-settings, or release-certification file was edited by this slice.
- Next action: remove the remaining three process-wide initial-unit diagnostic switches or replace them with explicit Editor/runtime diagnostics ownership.

### 2026-07-19 - AM-025 - World-owned runtime diagnostics

- Ownership result: AI verbosity, transport boarding diagnostics, and building runtime slice diagnostics now live only in `RuntimeDiagnosticsStateComponent`. The obsolete process-wide `InitialUnitsRuntimeState` file is removed.
- Runtime result: AI systems retain their existing allocation-free ECS queries; transport and building diagnostics read the same World-owned state; Editor performance controls write through the diagnostics facade.
- Isolation result: direct ECS changes remain authoritative, replacement Worlds begin clean, and selection diagnostics route to the supplied match World.
- Debt result: three mutable-static rows and one unique remediation item are resolved. The remaining projection is `555` non-debt rows, `20` genuine-debt rows, and `18` unique remediation items.
- Validation: Unity compiled with zero compiler errors and focused runtime-diagnostics validation passed ECS write/read, replacement-World, and match-routing cases. The broader AI end-to-end fixture reached its existing economy expectation mismatch (`70,000` expected versus `99,980` current credits); diagnostics ownership does not change economy values.
- Scope preserved: the separately owned operation-map probe still contains stale historical text fingerprints and was not edited. No FirstLaunch, audio, UI visual-lock, scene, prefab, package, project-settings, or release-certification file was changed.
- Next action: reconcile the remaining unprotected runtime-discovery and lifecycle/cache debt items while owner-controlled source-growth blockers remain external.

### 2026-07-19 - AM-025 - Explicit pathfinding World ownership

- Ownership result: building and citizen startup now give the pending-path reader their exact ECS World. The reader no longer searches for the process-wide default World.
- Lifecycle result: rebinding releases the old query and creates a query for the replacement match, preventing one match from reading another match's pending path state.
- Debt result: one global-World row and one unique remediation item are resolved. The remaining projection is `556` non-debt rows, `19` genuine-debt rows, and `17` unique file/rule items.
- Validation: Unity compiled with zero compiler errors and the focused replacement-World check passed. The first sandboxed licensing attempt timed out; the documented escalated macOS licensing workaround completed successfully.
- Scope preserved: operation-map and UI visual-lock work remained untouched. Release-only device, thermal, cold/warm, and sustained tests remained deferred.
- Next action: move the remaining audio request state out of process-wide mutable storage, now that audio is available for architecture remediation.

### 2026-07-19 - AM-025 - Audio request World ownership

- Ownership result: audio requests, results, settings, cooldowns, music state, and listener state remain in each match World's ECS singleton. The process-wide cached audio World and entity are removed.
- Isolation result: focused validation creates two Worlds and proves a request added to one match never appears in the other match.
- Performance result: the existing high-volume request, cooldown, history bound, and source-pool stress checks remain green. No new update system, managed gameplay owner, or `SystemBase` was introduced.
- Debt result: three genuine-debt rows and two unique remediation items are resolved; two previously protected static rows are now also resolved. The remaining projection is `559` non-debt rows, `16` genuine-debt rows, and `15` unique file/rule items.
- Validation: Unity compiled with zero compiler errors; audio behavior passed `7 / 7`, audio stress/pooling passed `4 / 4`, deterministic Phase 2 evidence passed, and `git diff --check` passed.
- Scope preserved: operation-map and UI visual-lock work remained untouched. Release-only device, thermal, cold/warm, and sustained tests remained deferred.
- Next action: continue with remaining non-map ECS ownership issues while operation-map work stays protected.

### 2026-07-19 - AM-025 - Explicit gameplay-state World binding

- Ownership result: match bootstrap and selection startup now supply their exact EntityManager to the gameplay-state facade. The facade no longer discovers `World.DefaultGameObjectInjectionWorld`.
- Isolation result: a replacement match begins with fresh gameplay, camera-input, and focus-request state only after an explicit rebind; one facade cannot silently switch to an unrelated World.
- Integration result: existing Editor validation and camera/selection fixtures now construct the facade with their own test EntityManager, making test ownership match production ownership.
- Debt result: one global-World row and one unique remediation item are resolved. The remaining projection is `560` non-debt rows, `15` genuine-debt rows, and `14` unique file/rule items.
- Validation: Unity compiled with zero compiler errors; gameplay-state behavior passed `8 / 8`, match startup passed `5 / 5`, selection-startup lifecycle passed `1 / 1`, deterministic Phase 2 evidence passed, and `git diff --check` passed.
- Scope preserved: operation-map and UI visual-lock work remained untouched. Release-only device, thermal, cold/warm, and sustained tests remained deferred.
- Next action: continue with the remaining non-map selection ownership paths.

### 2026-07-19 - AM-025 - Shared selection-input World ownership

- Ownership result: match startup creates the selection-input owner with its exact EntityManager and shares that owner with gameplay input, UI commands, and selection-rectangle reads. The input-state helper no longer finds `World.DefaultGameObjectInjectionWorld`.
- Isolation result: replacement-World validation explicitly rebinds the owner and proves the new match starts with fresh input state and empty command queues.
- Behavior result: move, attack, scan, boarding, selection modes, camera commands, and HUD command routing remain unchanged across the complete focused suite.
- Debt result: one global-World row and one unique remediation item are resolved. The remaining projection is `561` non-debt rows, `14` genuine-debt rows, and `13` unique file/rule items.
- Validation: Unity compiled with zero compiler errors; selection World replacement passed `1 / 1`, the complete selection command suite passed `61 / 61`, deterministic Phase 2 evidence passed, and `git diff --check` passed.
- Scope preserved: operation-map and UI visual-lock work remained untouched. Release-only device, thermal, cold/warm, and sustained tests remained deferred.
- Next action: continue with the remaining non-map selection composition boundaries.

### 2026-07-19 - AM-025 - Explicit selection composition World ownership

- Ownership result: selection startup resolves managed camera systems from its supplied EntityManager, captures that manager for runtime query delegates, and gives it to building interaction. No selection startup or building-interaction path discovers the global default World.
- Behavior result: building selection, movement, attack, scan, boarding, camera, and HUD command behavior remains unchanged across the complete focused suite.
- Debt result: two global-World rows and two unique remediation items are resolved. The remaining projection is `563` non-debt rows, `12` genuine-debt rows, and `11` unique file/rule items.
- Validation: Unity compiled with zero compiler errors; the complete selection command suite passed `61 / 61`, deterministic Phase 2 evidence passed, and `git diff --check` passed.
- Scope preserved: operation-map and UI visual-lock work remained untouched. Release-only device, thermal, cold/warm, and sustained tests remained deferred.
- Next action: reassess the remaining debt after the latest operation-map integration and continue only with unprotected ownership paths.

### 2026-07-19 - AM-025 - Non-map source-growth closure

- FirstLaunch result: localized voice indexing and selection now belong to the existing portrait/voice presentation owner instead of the sequence orchestrator. The sequence owner is back below 500 lines.
- Naming result: the locale text type now uses a composition suffix that reflects its managed locale-overlay responsibility; its Unity meta GUID is preserved.
- Guardrail result: four FirstLaunch and three non-map composition failures are resolved with exact measured ceilings and no spare growth allowance. The canonical source-growth check now reports only four operation-map-owned paths.
- Validation: Unity compiled with zero compiler errors; integrated FirstLaunch architecture and behavior passed `56 / 56`; `git diff --check` passed. The source-growth suite remains red only on four separately owned operation-map paths. The source-responsibility evidence test passes `4 / 5`; its only failure is the separately owned `MainMenuNavigationView` visual-lock ceiling (`152` current lines versus `146` accepted).
- Scope preserved: operation-map source, scenes, documents, prefabs, packages, `ProjectSettings`, UI visual-lock work, and release certification were not edited.
- Next action: continue row-bound remediation only where ownership is free; otherwise wait for the operation-map handoff before final Phase 2 capture.

### 2026-07-19 - AM-025 - UI lifecycle ownership

- Minimap result: marker images stay with one minimap owner while its view is rebuilt, then are destroyed when that owner is disposed. The large map raster buffer is no longer shared across matches, and viewport projection no longer keeps a mutable global array.
- Menu result: the selected main-menu tab has one resettable session owner, so rebuilding the view preserves the current tab while a fresh play session returns to the default tab.
- Resource Exchange result: the recently merged popup lifecycle behavior remains unchanged, while its runtime view is back within the accepted source-size limit.
- Debt result: three genuine-debt rows and three unique remediation items are resolved. The remaining projection is `566` non-debt rows, `9` genuine-debt rows, and `8` unique file/rule items.
- Validation: Unity compiled with zero errors; menu navigation and minimap lifecycle focused checks passed. The source-growth check is blocked only by four separately owned operation-map files.
- Scope preserved: operation-map source, scenes, documents, prefabs, packages, `ProjectSettings`, and release certification were not edited.
- Next action: continue the remaining non-map debt items while the operation-map owner completes its separate work.

### 2026-08-11 - AM-025 - AMFR-004 scene-loading responsibility closure

- Implementation: pushed commit `e92f16815ff871e8ba0a04481a8e2abd28551d2b` reduces the combined scene-loading owner from `927` lines / `34,174` bytes to one `465` / `16,734` transition owner plus four bounded non-polling Addressables, packed-EntityScene ownership, request-validation, and manifest-validation sources.
- Behavior: checked focused validation passes scene loading/unloading/failure/reset/sequential-switch behavior `19 / 19` and current static/EntityScene wiring plus teardown ordering `6 / 6`, with zero compiler errors and no production asset edit.
- Authority: one exact `AMFR-004` `system-helper` authorization binds the retained owner to the pushed blob at `465` / `16,734`; canonical authorization validation passes `1 / 1`. No extracted source can adopt transition authority or the helper suffix under this entry.
- Global disposition: the unchanged 17-test source-growth entrypoint now names seven later helper blockers instead of eight. A separate broader guardrail run also exposes pre-existing Resource Exchange/UI shell ratchet drift; it remains a fail-closed later reconciliation input and is not claimed by this slice.
- Result: AMFR-004 accepted without parent checklist credit. AMFR-005 is next; AM-025 remains unchecked.

### 2026-08-11 - AM-025 - AMFR-005 runtime-bootstrap responsibility closure

- Implementation: the runtime metadata/bootstrap helper is reduced from `395` lines / `15,565` bytes to one `284` / `10,241` created-World and owned-blob lifecycle owner, plus stateless `65` / `2,851` root-contract and `92` / `3,684` readiness-projection sources.
- Behavior: checked focused validation passes catalog publication, transactional blob replacement/disposal, monotonic generation, readiness/status projection, map-surface reuse, compatibility integration, typed failures, clear/dispose, and decomposition boundaries `15 / 15`; current static/EntityScene wiring and teardown ordering pass `6 / 6`, with zero C# errors.
- Authority: the retained helper is below the existing exact `295` / `11,398` ceiling, so no new exception, ceiling, or spare headroom was added. Extracted sources retain no World, Entity, blob, generation, transition, scene-loading, Addressables, or recurring-update authority.
- Global disposition: the unchanged 17-test source-growth entrypoint now names six later helper blockers instead of seven and no longer lists the runtime-bootstrap helper. Licensing recovery followed the authorized verified-client and version-matched-client ladder without changing wrappers, Hub, project settings, assets, or thresholds.
- Result: AMFR-005 accepted without parent checklist credit. AMFR-006 is next; AM-025 remains unchecked.

### 2026-08-11 - AM-025 - AMFR-006 building production UI responsibility closure

- Implementation: the building production entries helper is reduced from `437` lines / `18,403` bytes to a `349` / `13,935` produced-unit, pending-production, and friendly-runtime-building read-model boundary plus one stateless `116` / `5,380` operation-map producer-queue projection.
- Behavior: checked building UI query validation passes `12 / 12`; the real Build Drawer prefab queue snapshot passes `1 / 1`; both accepted logs contain zero C# errors. Produced-unit pruning/read-model precedence, pending progress, friendly filtering, operation-map queue visibility, preview/source-key resolution, producer naming, and append/clear semantics are preserved.
- Phase 3 boundary: the extracted source is supplied its World-bound context and caller-owned output, retains no state/query/container/cache/version, and adds no loop or Canvas application authority. AM-027/AM-028/AM-030 still own semantic versions, canonical projection-cache identity, duplicate-writer retirement, polling removal, and allocation acceptance after Phase 2.
- Global disposition: the unchanged 17-test source-growth entrypoint now names five later helper blockers instead of six and no longer lists the building-production helper. No source exception, ceiling, threshold, asset, or Phase 3 runtime authority was added.
- Result: AMFR-006 accepted without parent checklist credit. AMFR-007 is next; AM-025 remains unchecked.

### 2026-08-11 - AM-025 - AMFR-007 scene-reference authority closure

- Review: the unchanged `67` / `2,223` helper owns only bounded reusable root/candidate scratch and exactly-one loaded-scene view resolution. It retains no Scene/World/view state, load/unload/retry/readiness policy, static mutable state, recurring update, or polling loop; the scene-loading transition owner has exactly one call site.
- Behavior: checked focused validation passes exactly-one success, deterministic invalid/unloaded, multiple-view, and identity failures, a structural single-call-site/reused-storage ratchet, and 300 warm lookups with zero current-thread managed allocation `6 / 6`, with zero C# errors.
- Authority: one exact `AMFR-007` `system-helper` authorization binds the source to pushed commit `18ebc842961b31e33239f9c510c613a25902b89e` at `67` / `2,223`; canonical authorization passes `1 / 1`. No production source correction was necessary.
- Global disposition: the unchanged 17-test source-growth entrypoint now names four later helper blockers instead of five. No source threshold, asset, wrapper, or unrelated authority changed.
- Result: AMFR-007 accepted without parent checklist credit. AMFR-008 is next; AM-025 remains unchecked.

### 2026-08-11 - AM-025 - AMFR-008 runtime-grid persistent-storage authority closure

- Review: the unchanged `199` / `8,699` helper remains the sole production allocator/resizer/disposer for the five AM-021-governed native containers. Replacement allocation precedes old-storage disposal, disposed component aliases are defaulted, `DynamicBlockerInitSystem.OnDestroy` owns normal World teardown, and the helper retains no World/default-World/static/update authority.
- Behavior: checked focused validation passes initialization, same-size reuse, resize/replacement, idempotent alias-clearing disposal, replacement-World recreation, and structural single-owner/teardown checks `6 / 6`, with zero C# errors.
- Authority: one exact `AMFR-008` `system-helper` authorization binds the source to pushed commit `4e6eeecc00846b2464e423892e522d4e423013f2` at `199` / `8,699`; canonical authorization passes `1 / 1`. No production correction was necessary.
- Global disposition: the unchanged 17-test source-growth entrypoint now names three later helper blockers instead of four. No source threshold, asset, wrapper, or unrelated authority changed.
- Result: AMFR-008 accepted without parent checklist credit. AMFR-009 is next; AM-025 remains unchecked.

### 2026-08-11 - AM-025 - AMFR-009 road-visual prototype authority closure

- Review: the unchanged `286` / `10,976` R&D helper’s overrun is limited to explicit clone-versus-borrow material ownership. One composition owner constructs/configures/disposes it, one generated child root owns all visual slabs, normalized cell/edge sets prevent repeated/reversed-stroke growth, and disposal destroys the root and only owned materials while clearing retained counters/sets.
- Behavior: checked city-generation validation passes the existing suite plus road continuity, exact duplicate-generation bounds, root and borrowed-material teardown, and structural one-owner/no-ECS/no-update checks `31 / 31`, with zero C# errors.
- Authority: one exact `AMFR-009` `system-helper` authorization replaces the historical ceiling and binds the unchanged source to pushed commit `367e427336c845732fe04410ea1a81eb738c9376` at `286` / `10,976`; canonical authorization passes `1 / 1`. A first native shutdown crash was fail-closed and the fresh-log bounded retry passed; no failed run was credited.
- Global disposition: the unchanged 17-test source-growth entrypoint now names two later helper blockers instead of three. No production source, asset/material, threshold, wrapper, or unrelated authority changed.
- Result: AMFR-009 accepted without parent checklist credit. AMFR-010 is next; AM-025 remains unchecked.

### 2026-08-11 - AM-025 - AMFR-010 visual-quality authority closure

- Review: the unchanged `198` / `8,170` helper’s overrun is limited to explicit clone/borrow foundation material ownership and deterministic immediate EditMode collider cleanup. Clearance remains explicit-input, district-generation-only work; absent cleanup is a no-op and the helper has no Update, coroutine, World, entity, or steady-state authority.
- Behavior: checked city-generation validation passes the existing suite plus borrowed-material survival, collider removal, counter teardown, and structural two-owner generation-only checks `33 / 33`, with zero C# errors.
- Authority: one exact `AMFR-010` `system-helper` authorization replaces the historical ceiling and binds the unchanged source to pushed commit `e172b6bb899ce5a39966cb48528ef4c971f1f7c2` at `198` / `8,170`; canonical authorization passes `1 / 1`.
- Global disposition: the unchanged 17-test source-growth entrypoint now names only the later building-definition footprint clone helper. No production source, asset/material, threshold, wrapper, or unrelated authority changed.
- Result: AMFR-010 accepted without parent checklist credit. AMFR-011 is next; AM-025 remains unchecked.

### 2026-08-11 - AM-025 - AMFR-011 footprint-clone authority closure

- Review: the unchanged `72` / `4,230` pure clone preserves every current public building-definition field exactly once except the sole footprint override, including both later portrait references. It retains no GameObject, asset, native, ECS, World, lifecycle, or scheduling authority and has one production call site.
- Behavior: checked canonical validation passes the original authored building-cost/config/read-model cases plus exhaustive reflection-backed clone behavior and field-map structure `5 / 5`, with zero C# errors. Its fail-closed first run exposed and the slice corrected one existing one-line defect where configured `Price` used materials instead of credits.
- Authority: one exact `AMFR-011` `system-helper` authorization replaces the historical ceiling and binds the unchanged clone source to pushed commit `f50dc9c6f57c53fa3624d7c530adda903e987187` at `72` / `4,230`; canonical authorization passes `1 / 1` with no speculative headroom.
- Global disposition: all eight Phase B/C helper blockers are now cleared. The unchanged 17-test entrypoint advances to the broader baseline ratchet and honestly rejects `45` later review items (`12` unreviewed, `33` grown), retained for AMFR-017 rather than hidden or auto-baselined.
- Result: AMFR-011 accepted at child `11 / 22` without parent checklist credit. AMFR-012 is next; AM-025 remains unchecked.

### 2026-08-11 - AM-025 - AMFR-012 runtime-city explicit-World ownership closure

- Ownership: runtime-city composition resolves the exact match EntityManager through its existing building-spawn context and supplies that World to readiness queries and road-size fallback. Both helpers clear the binding at teardown, readiness disposes its cached query before replacement, and neither source reads `World.DefaultGameObjectInjectionWorld`.
- Isolation: checked runtime-city validation passes `36 / 36`, including first/replacement World cases while an unrelated default World exists; exact final-source startup wiring passes `6 / 6`; both contain zero C# errors. The dependency-hazard analyzer passes its six-unit suite and reports zero findings for both target paths.
- Authority: four exact `AMFR-012` scopes bind the pushed `acc7b52a799fa1929f9434452b659310faaf5a06` source blobs with no headroom; canonical authorization passes `1 / 1`. The unchanged global source-growth diagnostic retains the same `45` later baseline-review items and no AMFR-012 path.
- Debt disposition: `AM025-GLOBAL-WORLD-001/002` disappear from current source, while explicit replacement/clear evidence resolves overlapping `AM025-WORLD-003/004`. The governed scan remains unchanged until Phase D regeneration; its validated projection is `9` genuine-debt rows / `10` unique items.
- Result: AMFR-012 accepted at child `12 / 22` without parent checklist credit. AMFR-013 is next; AM-025 remains unchecked.

### 2026-08-11 - AM-025 - AMFR-013 GridAuthoring runtime-debug boundary closure

- Ownership: runtime-debug Gizmo reads now compile only in Editor builds, use six cached queries against the exact World supplied by the existing active-match building-spawn context, and clear/dispose on replacement or disable. `GridAuthoring` no longer discovers the default World or creates queries during a draw.
- Isolation: focused validation passes `3 / 3`, including first/replacement Worlds while an unrelated default World exists and dead-World teardown; exact map/startup wiring passes `6 / 6`. The immutable dependency scan reports zero findings for the authoring path.
- Authority: three exact `AMFR-013` scopes bind pushed implementation `d1e055591d042390bd5fabfac7edf81bd353af87` at the exact current ceilings with no spare headroom. Player builds receive no mutable authoring World/query owner.
- Result: AMFR-013 accepted at child `13 / 22` without parent checklist credit. The validated projection is `8` genuine-debt rows / `9` unique items. AMFR-014 is next; AM-025 remains unchecked.

### 2026-08-11 - AM-025 - AMFR-014 map-surface created-World lifecycle closure

- Ownership: the existing created-World helper retains exactly its created persistent surface blob, disposes the prior identity on same-World replacement, is disposed by normal match shutdown before World loss, and can still dispose its owned blob after the supplied World has died. It implements `IDisposable`; the compatibility teardown API forwards to the same owner.
- Behavior: checked focused validation passes first publication, replacement, normal owner shutdown, World-first teardown, idempotence, metadata preservation, and structural ownership `8 / 8`; exact map/startup wiring passes `6 / 6`, with zero C# errors.
- Classification: the exact-head lifecycle inventory classifies one target World owner with readonly `createdWorld`, standard `Dispose`, and zero default-World access. The exact-head dependency scan reports zero target findings. The helper is `346` lines / `13,619` bytes, below baseline, so no source authority or headroom is added.
- Global disposition: the exact-head 17-test diagnostic retains the same `45` broader review items and does not name the helper. Those fail-closed inputs remain owned by AMFR-017.
- Result: AMFR-014 accepted at child `14 / 22` without parent checklist credit. The validated projection is `7` genuine-debt rows / `8` unique items. AMFR-015 is next; AM-025 remains unchecked.

### 2026-08-11 - AM-025 - AMFR-015 road-preview pool lifecycle closure

- Ownership: the plain presentation helper now has one exact total retained-object capacity, reuses released objects by visual type, destroys releases beyond capacity without truncating active previews, and implements one standard idempotent `Dispose` owner. Teardown destroys active and pooled GameObjects plus only the material copies created by the helper; borrowed source assets and road gameplay owners are preserved.
...
### 2026-08-11 - AM-025 - AMFR-017 source-growth reconciliation closure

- Review: the first canonical failure emitted 44 exact tuples; the unchanged next layer emitted 50 below-500/helper ratchets. AMFR-017 therefore retains 94 exact no-headroom tuples over 75 immutable `d767bcf1e...` paths: 35 500-line review, 11 strict 1,000-line no-growth, and 48 helper-growth scopes. No production source, baseline, threshold, feature, or historical decision changed.
- Ownership contract: each exact path remains its named existing owner, keeps its accepted responsibility and focused behavior evidence, is re-gated by the complete AMFR-018 architecture suite, is reviewed on any tuple/blob change, and is removed when decomposed below its historical ceiling or retired. No path-only or inherited authorization is valid.
- Enforcement: the prior 20 explicit assertions remain, while all 114 ordered tuples are frozen by one exact SHA-256 plus per-entry path/scope/task/commit/line/byte and immutable-blob validation. The provisional completed row is valid only if canonical authorization and the unchanged source-growth `17 / 17` both pass.
- Ratchet retirement: the masked unused `AIStartupSystem` historical exception is superseded by an exact `537 / 24,773` guarded responsibility entry at the source's current historical minimum, so it grants no retained growth headroom.
- Historical/owner ratchets: the unchanged gate subsequently exposed the shrunken AI startup review exception, one exact deleted build-drawer helper exception, the shrunken match-start helper exception, and two stale guarded UI owner measurements. Exact post-hardening guardrails now freeze AI startup at `537 / 24,773`, match start at `249 / 9,938`, the sole retired helper path, `ResourceExchangeShellBinding` at `94 / 3,201`, and `UIShellContentView` at `951 / 40,958` with exact responsibility/state/symbol constraints. The two guarded UI files are the only permitted existing owner paths; a new/recreated/replacement owner still fails closed.
- Validation: authorization log `Build/Logs/amfr017-source-authorization-owner-ratchet.log`, SHA-256 `0c9a6a6746adc7a93d5e52765348c35351aa807d7eec5c1513dd7429a6914439`, passes `1 / 1`; source-growth log `Build/Logs/amfr017-source-growth-owner-ratchet.log`, SHA-256 `97a7b738be30aa7d5ac812ffaadf52113f80edbe1d1de5830641712ffbe36f33`, passes the exact `17 / 17` marker with zero C# errors. Production/protected paths have no diff and `git diff --check` passes.
- Result: AMFR-017 is accepted at child `17 / 22` without parent credit. AMFR-018 is next; AM-025 remains unchecked.

### 2026-08-11 - AM-025 - AMFR-018 complete architecture validation claim
