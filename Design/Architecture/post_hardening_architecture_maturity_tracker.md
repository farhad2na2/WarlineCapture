...
| Parallel preparatory work | `AM-026` remains dependency-blocked and receives no completion credit; `ui_projection_allocation_inventory.md` inventories 29 surfaces and reconciles every declared route/popup contract; drafts `AM-WP-001` through `AM-WP-018` bind the currently characterized UI surfaces to explicit ownership, lifecycle, semantic identity, and real-path performance/allocation gates; `AM-WP-007` is additionally blocked on explicit mission/objective owner handoff; `AM-WP-009` preserves separately owned audio/FirstLaunch consumers as read-only dependencies; `AM-WP-010` through `AM-WP-014` cover Armory, Match resources, Quick Custom, Main Menu resources, and Commander Profile; `AM-WP-015` preserves one measured managed animation bridge and requires a GUID-preserving rename away from the reserved bare `*System` suffix; `AM-WP-016` requires one canonical Diagnostics path, one bounded log history, zero disabled work, and separately measured enabled cost; `AM-WP-017` fails closed until gameplay-result, reward/save, routing, and prefab owners provide the five-state Mission Result contract; `AM-WP-018` requires caller and input/accessibility policy proof before either removing the unused Tooltip layer contract or implementing one event-driven shell owner |
