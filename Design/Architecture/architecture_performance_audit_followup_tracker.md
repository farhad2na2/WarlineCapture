...
| Field | Status |
|---|---|
| Checklist complete | 58 / 103 |
| Checklist percent complete | 56.3% |
| Current phase | Phase 8 - Fuel/Oil ECS Drift Correction |
| Quick wins complete | 6 / 6 |
| Current target | Fuel/Oil behavior safety-net slice complete: focused tests now lock current oil extraction clamps, refinery conversion clamps, destroyed-building skips, fuel/oil cargo switching, blocked unload preservation, and matching-resource load reverts before moving authoritative production/storage/transfer/conversion into ECS. Next step is the first behavior-preserving ECS ownership slice for production/storage state. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, Unity 6.5.2 focused faction resource validation log `/private/tmp/warline-arch-followup-fuel-oil-faction-validation.log`, and Unity 6.5.2 focused hauler validation log `/private/tmp/warline-arch-followup-fuel-oil-hauler-validation.log` passed with 0 compiler errors after the Fuel/Oil behavior safety-net tests. Unity MCP is usable for inspection but currently adds tracing/logging traffic to performance captures, so batchmode/editor-log validation remains the compiler path. |
...
| Fuel/Oil drift status | Drift confirmed and safety-net tests added: authoritative production, conversion, load/unload, and storage mutation are still in managed helper/runtime-building paths; UI/header paths are read-only display consumers. Focused tests now protect the existing edge-case behavior before moving ownership into Burst-capable ECS systems. |
| Validation status | Phase 0 inventory completed; Phase 1 config edited; Phase 2 release OnGUI strip implemented; Phase 3 release diagnostic system gate implemented; Phase 4 GC quick wins completed; Phase 5 Burst inventory/classification report added and guardrail implemented; Phase 6 instantiate classification report added and first placement-visual pooling slice implemented; Phase 8 Fuel/Oil behavior safety-net tests added. Latest validations passed: `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, `/private/tmp/warline-arch-followup-fuel-oil-faction-validation.log` with `[FactionResourceFocusedValidation] result=Passed tests=9`, and `/private/tmp/warline-arch-followup-fuel-oil-hauler-validation.log` with `[ResourceHaulerFocusedValidation] result=Passed tests=12`. |
| Still wrong / next iteration | No visual defects are known from code/compile validation, but manual visual smoke checks for placement-preview pooling and command behavior are still pending. Fuel/Oil authoritative gameplay still lives in managed helper/runtime-building paths; next slice should start moving production/storage mutation into ECS while preserving the newly locked behavior. Android baseline is still not captured, and HDR/soft-shadow tier ownership remains unresolved. |
...
- Next iteration: start the first Fuel/Oil ECS ownership slice for production/storage mutation, preserving the newly locked behavior tests.
