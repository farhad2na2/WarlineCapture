...
Date: 2026-07-28
Status: Candidate implementation in progress through bounded Burst selection, deterministic slot assignment, and an asynchronously ordered parallel slot-apply boundary; upstream runtime scheduling, persisted candidate, and production cutover remain disabled
...
| Phase 4: Jobified runtime assignment | In progress; the ordered camera boundary, exact map-generation native initialization, pure guard-envelope decision, bounded selection, deterministic assignment, parallel apply, apply-to-Entities-Graphics dependency boundary, and stable apply no-op gate are accepted. The database owner bakes one fixed command and one version owner. A Burst presentation `ISystem` returns before slot count/query/type-handle updates or job scheduling when owner/version is unchanged; changed nonzero versions schedule the parallel apply through `state.Dependency` before presentation update and Entities Graphics. Focused measurement proves the warmed stable call allocates `0 B`, schedules zero additional apply jobs, writes zero slot components/enable bits, and changes zero rebuild state; the contained guard-envelope decision remains `None`. Selection/assignment runtime-system scheduling, metrics publication, and full unload/job-lifetime proof remain open. |
...
Checklist progress: `39 / 80` complete.
