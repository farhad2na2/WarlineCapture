...
| Last verified commit | `0c3df66d2` on `main`; the compact APH-704 candidate remains disabled after exact `9/9` equivalence, `74.88%` benchmark improvement, zero warmed allocation, and affected architecture/build gates. |
| Last update | 2026-07-14 - The building UI query facade split into bounded material-fabrication read-model and production-entry UI helpers without changing pull ownership or adding a runtime loop. The original owner dropped from 868 to 411 lines; both helpers remain below 500 lines. Focused behavior passes `58/58`, APH-700 passes `7/7`, five affected assemblies build with zero compiler errors, and Menu-to-Match reaches `MatchHudReady`, completes Match Intro, and unlocks input. The global oversized inventory remains 21 because an upstream 644-line material-scenario helper entered the gate as this owner left it. |
