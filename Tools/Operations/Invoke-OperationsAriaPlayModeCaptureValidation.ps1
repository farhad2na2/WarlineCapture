# Runs Operations ARIA Play Mode capture wiring validation against the shadow project only.
# Does NOT claim AriaWon / playable. Distinct from live capture markers.
[CmdletBinding()]
param(
    [string] $ShadowPath = "D:\Projects\WarlineCapture-Operations",
    [string] $SharedPath = "D:\Projects\WarlineCapture",
    [string] $LogFile = "$env:TEMP\operations-aria-playmode-capture-wiring.log",
...
if (Test-Path -LiteralPath $sharedLibrary) {
    Write-Host "[OperationsAriaPlayModeCaptureValidation] leaving_shared_library_untouched=$sharedLibrary"
}
...
Write-Host "[OperationsAriaPlayModeCaptureValidation] project=$shadow"
Write-Host "[OperationsAriaPlayModeCaptureValidation] unity=$unityExe"
Write-Host "[OperationsAriaPlayModeCaptureValidation] log=$LogFile"
...
& $validateScript `
    -UnityExe $unityExe `
    -ProjectPath $shadow `
    -ExecuteMethod "Game.Tests.Editor.Operations.OperationsAriaPlayModeCaptureValidation.RunFocusedValidation" `
    -LogFile $LogFile `
    -RequiredPassMarker "[OperationsAriaPlayModeCaptureValidation] result=Passed checks=7" `
    -GuiLicensing:$gui `
    -TimeoutSeconds $TimeoutSeconds

if ($LASTEXITCODE -ne 0) {
    throw "Operations ARIA Play Mode capture wiring validation failed."
}

Write-Host "[OperationsAriaPlayModeCaptureValidation] result=Passed project=$shadow"
