...
The Saga Map selects a Mission node, Mission Briefing previews the Mission and its referenced ScenarioSetup, and tactical HUD/result surfaces bind to the active ScenarioSetup and Level / Map runtime data. Do not use `Level` as a synonym for a player-facing Mission label.
...
| Key art / background art | Must represent the active route or gameplay context and follow the premium 2D isometric direction for gameplay-facing content. |
...
| Map / minimap / district region | Must bind to map definition, `IsoMapId`, macro-tile metadata, mission node config, district state, camera/minimap state, or operation event data. |
...
| Resource strip | Show wallet/match/operation resources. | `ResourceWallet`, canonical resource definitions, `FactionResources`, `OperationState.ResourceState`. | Resource names must come from economy config; third account slot is Command Authority. |
| Mode card image | Preview route gameplay fantasy. | `GameModeDefinition`, art id. | Gameplay-facing art follows 2D isometric direction. |
| Mission/key art image | Preview scenario content. | `MissionConfig.MapPreviewArtId`, `ScenarioSetup.IsoMapId`, macro-tile map preview. | Replaceable per mission; no baked objectives/rewards text. Terrain art may show roads/pads, but interactive buildings/objectives must remain runtime/state data. |
...
Gameplay-facing map art can be a baked macro-tile terrain image, but UI must treat it as a preview of metadata-backed gameplay.

- Chapter 1 tactical-map UI surfaces must follow `WarlineCapture_Chapter01_Tactical_Production_Implementation_Plan.md` for `IsoMapId`, `MapPreviewArtId`, `MinimapArtId`, objective anchors, route anchors, and runtime marker ownership.
- Strategic/tactical map behavior must follow `WarlineCapture_Strategic_Tactical_Map_Gameplay_Alignment.md`.
- The first concrete tactical UI validation target is `WarlineCapture_M01_FirstContact_Production_Contract.md`.
- Missing close-up tactical controls must follow `WarlineCapture_Tactical_UI_Missing_Parts_Work_Order.md`; this includes selected entity feedback, command mode feedback, world command markers, invalid command toasts, minimap camera jumps, and metadata-backed build placement.
- Strategic/zoomed-out art is preview/minimap only. The tactical/zoomed-in ground plate is the real playable map behind units, vehicles, buildings, selection rings, movement markers, attack markers, and build placement overlays.
- Build placement UI must show approved sockets/pads clearly.
...
- Destruction state belongs to runtime building entities and VFX overlays, not to the terrain background.
...
| SCN-02 Main Menu | Profile block, level badge, resource strip, mode card images/titles/subtitles, side-nav icons, notification badges, footer/status strip. | Profile, Credits, Materials, Command Authority, mode definitions, unread counts, live event state, social feed state. |
...
| SCN-05 Saga Map | Chapter selectors, map viewport, route line, mission nodes, node stars, selected/locked badges, difficulty selector, chapter rewards counter. | Saga progress, chapter config, mission node configs, mission ids, scenario setup ids, level/map ids, star counts, difficulty unlocks, reward thresholds. |
| SCN-06 Mission Briefing | Mission art, level/map preview, mission title, briefing text, objective rows, star rows, enemy intel tiles, reward strip, CTA state. | Mission config, scenario setup, LevelId/IsoMapId, MapPreviewArtId, objective configs, star goals, AI profile/intel data, reward config. |
...
| SCN-13 Quick Custom | Preset selector, AI/rules controls, sliders/steppers/dropdowns/toggles, map preview, special-rule/dev-only group, launch state. | QuickGameConfig, AI profiles, rule configs, map definitions, debug permission, scenario validation. |
...
| POP-03 Build Placement | Building preview, footprint label, socket/pad status, cost row, rotate/cancel/confirm buttons. | Building definition, footprint, macro-tile building socket, rotation, resources, build validity. |
...
| Control | Gameplay Purpose | Route Or Effect | Gameplay Data | Enable Rule |
|---|---|---|---|---|
| `Button_Game` | Development fallback to current tactical match. | Calls current no-argument `GameBootstrap.BeginGameplay()` path. | Current scene defaults and AI runtime state. | `DevOnly` after Quick Custom launch flow exists; hidden in release builds. |
...
| Control | Gameplay Purpose | Route Or Effect | Gameplay Data | Enable Rule |
|---|---|---|---|---|
| Saga Campaign card | Enter curated campaign progression. | Route to SCN-05. | `SagaProgress`, `ChapterConfig`, `ModeUnlockState`. | Enabled when campaign data exists; `DesignedUnavailable` state opens a modal explaining Saga unlock path. |
| Persistent Operation card | Enter long-running district operation. | Route to SCN-11. | `OperationState`, `DistrictState[]`, `ModeUnlockState`. | Enabled when Operation save/default state exists; `DesignedUnavailable` state opens a modal explaining Operation unlock path. |
| Quick Custom Game card | Configure and launch skirmish. | Route to SCN-13. | `QuickGameConfig`, `AIProfileConfig[]`, last-used setup. | First fully playable mode path. |
...
Alignment note: the Main Menu resource strip is `Credits`, `Materials`, and `Command Authority`.
...
| Chapter dropdown | Select available chapter. | Reload map nodes. | `SagaProgress`, `ChapterConfig`. | Only unlocked chapters selectable. |
...
| Mission node | Select mission. | Opens SCN-06 for playable node. | `SagaMissionNodeConfig`, `SagaProgress`. | Locked nodes show requirements; completed nodes remain replayable. |
| Map preview/node art | Preview the mission's battlefield context. | Updates selected node panel and briefing handoff. | `ScenarioSetup.LevelId`, `ScenarioSetup.IsoMapId`, `MissionConfig.MapPreviewArtId`. | Read-only; shows authored empty art state only if a mission intentionally has no unique preview. |
...
| Chapter Rewards | Inspect/claim star-threshold rewards. | Opens reward detail/POP-04 claim. | `RewardConfig`, `SagaProgress`. | Claim enabled only at met thresholds. |
...
| Level / Map preview | Explain where the mission plays and verify the selected ScenarioSetup. | Opens map detail tooltip or remains read-only. | `ScenarioSetup.LevelId`, `ScenarioSetup.IsoMapId`, `MissionConfig.MapPreviewArtId`, minimap art id. | Always visible for Saga missions; uses a designed empty state only for generated modes without unique preview art. |
...
| Preset dropdown | Load recommended AI/rule setup. | Replaces config fields. | `QuickGamePreset`. | Enabled when presets exist. |
...
| Map selector/preview | Choose map preset/art. | Writes `IsoMapId`, preview/minimap ids. | Map definitions. | Enabled for validated maps only. |
| Launch Mission | Start skirmish. | Creates `GameLaunchPayload(QuickCustom)` and starts match. | `QuickGameConfig`, `ScenarioSetup`. | Enabled only when config maps to valid scenario. |
...
| Item detail | Explain selected item. | Opens `POP-09`. | AbilityConfig or UpgradeTrackConfig. | Enabled for every ability and upgrade track with an availability spec. |
...
- The active gameplay art direction is 2D isometric; new gameplay-facing thumbnails, maps, minimaps, and key art should not be generated in the old low-poly/desert direction.
