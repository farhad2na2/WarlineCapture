...
- Operation-map UI additions for `SCN-09`, `SCN-10`, `POP-01`, `POP-03`, and `POP-05` are tracked in `Design/WarlineCapture_UIUX_Gameplay_Element_Alignment.md`. The first concrete target is `Design/WarlineCapture_M01_FirstContact_Production_Contract.md`. The relevant missing pieces are command target hints, explicit move/attack mode feedback, disabled command reasons, build availability reasons, metadata-backed footprint overlays, threat jump-to-camera behavior, and result binding to the active Mission / ScenarioSetup / OperationMap ids.
- Before revisiting tactical popups, use `Design/WarlineCapture_3D_SingleMap_Gameplay_Direction.md` and these refreshed state targets:
...
- Tactical command/build/threat/result UI additions from `WarlineCapture_UIUX_Gameplay_Element_Alignment.md` have prefab/controller/test owners before M01 production gameplay is marked ready.
