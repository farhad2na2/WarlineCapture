...
- Checklist progress: `40 / 142 complete (28.2%)`.
- In progress: `0`.
- Remaining open: `102`.
- Current target: `Blocked: POP-06 Settings and POP-07 Inbox require saved Target Lock reference PNGs before visual matching can continue`.
- Iteration loop status: `SCN-02 approved and reused as shared chrome baseline; SCN-02 runtime scaling now uses UI Toolkit Scale With Screen Size against the 4800x2160 reference with Expand-style aspect handling so 1920x1080 does not render raw oversized text; SCN-01 Loading already matched the V04 reference closely with existing TargetLockV04 assets, and slice 01 added only an editor static-preview menu hook plus shadow UI Builder validation; SCN-08 Match HUD slice 08 was rejected because the selected state was a cheap inner overlay and the five squad cards had asymmetric tray padding; slice 09 removes the squad overlay, swaps squad selected/hover/focus to a dedicated selected squad-card chrome sprite, balances the squad-card outer margins, gives the command rail selected/hover/focus buttons a dedicated selected square-button chrome sprite matching the mockup Move button, and adds restrained lift/scale transitions so both command buttons and squad cards visibly sit forward when highlighted; slice 10 continues the panel-by-panel pass beyond the bottom rail by replacing the selected-unit portrait with a squad-group reference crop and adding chrome-level hover/focus/selected treatment to the header menu, selected-unit action buttons, passenger controls, threat jump button, right quick rail, minimap controls, and feedback actions; slice 11 improves the objective panel typography/separators, removes the unsupported `picking-mode` USS warning, and changes minimap side controls to a correctly sliced square-panel frame so the panel reads cleanly in focused shadow UI Builder crops; SCN-08 Build Placement slices 01-04 replace the legacy thin rail with a full-width Target Lock confirmation rail, preview card, status chip, separate cost/time chips, section dividers, readable instruction module, and large non-overlapping cancel/rotate/confirm buttons with hover/focus/press impact states while preserving runtime-bound names; SCN-09 Build Drawer slices 01-05 enlarge the modal, restore a dark popup fill, replace collapsing catalog template instances with runtime-bindable explicit card buttons, add full selected/hover/focus/press card/button states, and rebalance the right detail/queue stack; SCN-19 slice 13 fixes the rejected right action-button scale and the right stats label/value collision; SCN-03 slice 02 fixes artifacting panel frames and readability; POP-05 slices 01-03 rebuild the mission result as live Target Lock UI Toolkit panels with a large modal frame, mission-result header, close chrome, victory/defeat result selectors, rating stars, horizontal stat rail, separate objectives/rewards/casualties/score panels, and three visible footer actions while preserving runtime-bound names; Shell iteration 01 verifies required shell slots and the 4800x2160 Scale With Screen Size contract in the shadow project without changing shell visuals`.
- Button-state rule: `for every remaining button/control family, add selected/active, hover, and focus states with chrome-level treatment plus restrained lift/scale impact, tuned to avoid overlap in dense layouts`.
- Panel cleanliness rule: `for every remaining surface, validate panel by panel with focused crops; do not leave SCN-08 until header/resources/current order, left selected/objectives stack, right threat/quick rail, minimap, feedback panel, footer command rail, and squad tray are aligned and clean or have a user-approved exception`.
- Surfaces target-matched: `3 / 11`.
...
- Shadow project validation status: `SCN-08 Match HUD slice 11 synced to /Users/farhad/Projects/WarlineCapture-CodexUnity1; shadow Unity import/compile passed in /private/tmp/warline-scn08-panel-cleanup-shadow-batch-v2.log, shadow GUI static preview opened from the shadow project with playMode=False, and final focused UI Builder crops were saved from shadow capture. SCN-08 Build Placement slice 04 UXML/USS synced to the shadow project; shadow Unity batch import/compile passed in /private/tmp/warline-build-placement-shadow-batch.log, visible shadow UI Builder preview opened from /Users/farhad/Projects/WarlineCapture-CodexUnity1 with Match Game View enabled and Fit Viewport at 19%, and focused rail/canvas crops were saved. SCN-09 Build Drawer slice 05 UXML/USS synced to the shadow project; visible shadow UI Builder preview opened from /Users/farhad/Projects/WarlineCapture-CodexUnity1 with Match Game View enabled and Fit Viewport at 19%, and final window capture was saved. SCN-01 Loading static preview tooling synced to the shadow project; visible shadow UI Builder preview opened from /Users/farhad/Projects/WarlineCapture-CodexUnity1 with Match Game View enabled and Fit Viewport at 19%, and baseline/current-pass capture was saved. POP-05 Mission Result static preview tooling and UXML/USS synced to the shadow project; visible shadow UI Builder preview opened from /Users/farhad/Projects/WarlineCapture-CodexUnity1 with Match Game View enabled and Fit Viewport at 19%, and full-window plus focused modal/footer crops were saved. Shell iteration 01 synced `UIShellAppCanvas.uxml`, `UIShellAppCanvas.uss`, and static preview tooling to the shadow project; shadow shell UXML/USS matched main copies, required shell names were present, and shadow `RuntimePanelSettings.asset` kept the approved scaling contract`.
- Validation status: `git diff --check passed after POP-06/POP-07 blocker notes; POP-06 and POP-07 remain blocked by missing saved target references`.
...
- [x] SCN-01 Loading: match loading panel chrome, logo scale, progress bar, status chips, spinner, background, and footer/header spacing for the current static pass.
- [x] SCN-02 Main Menu: match header, navigation, resource strip, mode cards, commander panel, deploy CTA, background, and footer spacing.
...
- [x] SCN-08 Build Placement Bar: match confirmation rail, action buttons, instruction text, warning state, and edge anchoring.
- [x] SCN-09 Build Drawer Popup: match popup frame, left catalog, right details/queue panel, tabs, cards, footer buttons, and close/header chrome for the current readable static pass.
...
- [x] POP-05 Mission Result: match modal frame, result title, rewards/stats, CTA buttons, close/back behavior, and backdrop.
...
- [x] Shell: verify `HeaderRegion`, `LeftRegion`, `MiddleRegion`, `RightRegion`, and `FooterRegion` still mount the same screen content.
...
| --- | --- | ---: | --- | --- |
| Shell | Satisfied for structural/static pass | 1 | Required shell regions and runtime slots remain present, shell UXML/USS were synced to the shadow project, and both main and shadow `RuntimePanelSettings.asset` keep the approved 4800x2160 Scale With Screen Size contract. The already-open shadow editor had not imported the new shell preview menu during this slice, so shell validation used synced-file and PanelSettings checks instead of a UI Builder shell screenshot; no shell visuals changed. | `Design/VisualLockLayered/_UIToolkitVisualMatch/Shell/iteration_01/notes.md`; `Assets/Game/UI Toolkit/UIShellAppCanvas/UIShellAppCanvas.uxml`; `Assets/Game/UI Toolkit/UIShellAppCanvas/UIShellAppCanvas.uss`; `Assets/Game/UI Toolkit/RuntimePanelSettings.asset`; `Assets/Game/Scripts/Editor/UiToolkitTargetLockStaticPreview.cs` |
| SCN-01 Loading | Satisfied for current pass | 1 | Existing TargetLockV04 UI Toolkit layout already matches the saved reference closely: background, corner brackets, logo lockup, command-system chip, loading panel, progress bar, and bottom status chip all align cleanly in shadow UI Builder. No SCN-01 visual file changes were needed; slice 01 only added the editor-only static preview hook for shadow validation. | `Design/VisualLockLayered/_UIToolkitVisualMatch/SCN-01_SplashLoading/iteration_01/notes.md`; `Design/VisualLockLayered/_UIToolkitVisualMatch/SCN-01_SplashLoading/iteration_01/shadow_ui_builder_scn01_baseline_window.png`; `Assets/Game/Scripts/Editor/UiToolkitTargetLockStaticPreview.cs` |
...
| SCN-08 Match HUD | Satisfied for current pass | 11 | Reopened after user review: slice 08 removed the top health/progress strips but still used a weak inner selected overlay and asymmetric squad tray padding. Slice 09 replaces squad selected/hover/focus with a dedicated selected squad-card chrome sprite, balances the five-card tray margins, replaces command-button selected/hover/focus tinting with a dedicated selected square-button chrome sprite for the Move-button example, and adds short lift/scale transitions for both squad-card and command-button hover/focus/selected impact. Slice 10 continues panel-by-panel cleanup outside the bottom rail: selected-unit portrait now uses a squad-group reference crop, and header menu, selected-unit action buttons, passenger controls, threat jump, right quick rail, minimap controls, and feedback actions now have chrome-level hover/focus/selected treatment. Slice 11 raises objective readability, adds objective separators, removes the unsupported USS warning, and changes minimap controls to the square-panel frame with matching slices. Focused shadow UI Builder crops now show left stack, right edge controls, minimap controls, and footer command/squad panels clean enough for the current pass. Existing runtime bindings remain intact. | `Design/VisualLockLayered/_UIToolkitVisualMatch/SCN-08_MatchHud/iteration_01/notes.md`; `Design/VisualLockLayered/_UIToolkitVisualMatch/SCN-08_MatchHud/iteration_01/shadow_ui_builder_scn08_slice11b_zoomed_fullscreen.png`; `Design/VisualLockLayered/_UIToolkitVisualMatch/SCN-08_MatchHud/iteration_01/shadow_ui_builder_scn08_slice11_left_stack_crop_retina.png`; `Design/VisualLockLayered/_UIToolkitVisualMatch/SCN-08_MatchHud/iteration_01/shadow_ui_builder_scn08_slice11_right_edge_controls_wide_crop_retina.png`; `Design/VisualLockLayered/_UIToolkitVisualMatch/SCN-08_MatchHud/iteration_01/shadow_ui_builder_scn08_slice11_minimap_controls_wide_crop_retina.png`; `Design/VisualLockLayered/_UIToolkitVisualMatch/SCN-08_MatchHud/iteration_01/shadow_ui_builder_scn08_slice11_footer_crop_retina.png`; `Assets/Game/UI Toolkit/SCN08_MatchHudContent/SCN08_MatchHudContent.uss`; `Assets/Game/Art/UI/Generated/MatchHUD/TargetLockV02/scn08_v02_selected_squad_group_portrait.png` |
| SCN-08 Build Placement Bar | Satisfied for current pass | 4 | Replaced the legacy thin confirmation bar with a Target Lock bottom rail: full-width frame, building preview card, status chip, separate cost/time metric chips, section dividers, readable instruction module, and large cancel/rotate/confirm buttons with hover/focus/press impact states. Existing runtime-bound names remain intact (`Title`, `Status`, `Cost`, `Duration`, `Instruction`, `CancelButton`, `RotateButton`, `ConfirmButton`). Slice 04 fixed the clipped instruction and rotate label by shortening the static instruction copy and separating action-button widths/positions so no button overlaps. | `Design/VisualLockLayered/_UIToolkitVisualMatch/SCN-08_BuildPlacementConfirmationBar/iteration_01/notes.md`; `Design/VisualLockLayered/_UIToolkitVisualMatch/SCN-08_BuildPlacementConfirmationBar/iteration_01/shadow_ui_builder_scn08_build_placement_slice08_rail_crop.png`; `Design/VisualLockLayered/_UIToolkitVisualMatch/SCN-08_BuildPlacementConfirmationBar/iteration_01/shadow_ui_builder_scn08_build_placement_slice08_canvas.png`; `Assets/Game/UI Toolkit/SCN08_BuildPlacementConfirmationBar/SCN08_BuildPlacementConfirmationBar.uxml`; `Assets/Game/UI Toolkit/SCN08_BuildPlacementConfirmationBar/SCN08_BuildPlacementConfirmationBar.uss` |
| SCN-09 Build Drawer Popup | Satisfied for current pass | 5 | Slice 05 fixes the major SCN-09 static-preview defects: the popup now uses a large centered Target Lock modal, the transparent frame has a dark fill, the left catalog no longer collapses template text, catalog cards are explicit runtime-bindable buttons with readable thumbnails/costs and chrome-level selected/hover/focus/press states, and the right detail/queue stack has larger readable panels and buttons. A small top strip of the secondary catalog row remains visible as scroll depth, but no panel/text/button is collapsed or unreadable in the latest shadow UI Builder pass. | `Design/VisualLockLayered/_UIToolkitVisualMatch/SCN-09_BuildDrawer/iteration_01/notes.md`; `Design/VisualLockLayered/_UIToolkitVisualMatch/SCN-09_BuildDrawer/iteration_01/shadow_ui_builder_scn09_slice05_window.png`; `Assets/Game/UI Toolkit/SCN09_BuildDrawerPopup/SCN09_BuildDrawerPopup.uxml`; `Assets/Game/UI Toolkit/SCN09_BuildDrawerPopup/SCN09_BuildDrawerPopup.uss` |
...
| POP-05 Mission Result | Target matched for current static pass | 3 | Slices 01-03 rebuild the mission-result popup as live UI Toolkit panels instead of the previous compact summary card: large Target Lock modal, `MISSION RESULT` header, close chrome, selected Victory and inactive Defeat panels, mission-rating stars, horizontal stat rail, separate objectives/rewards/casualties/score panels, and three visible footer actions. Runtime-bound names remain intact: `Title`, `Subtitle`, `SummaryBody`, `ResultBadge`, `ContinueButton`, and `ReplayButton`. | `Design/VisualLockLayered/_UIToolkitVisualMatch/POP-05_MissionResult/iteration_01/notes.md`; `Design/VisualLockLayered/_UIToolkitVisualMatch/POP-05_MissionResult/iteration_01/shadow_ui_builder_pop05_slice03_window.png`; `Design/VisualLockLayered/_UIToolkitVisualMatch/POP-05_MissionResult/iteration_01/shadow_ui_builder_pop05_slice03_modal_crop.png`; `Design/VisualLockLayered/_UIToolkitVisualMatch/POP-05_MissionResult/iteration_01/shadow_ui_builder_pop05_slice03_footer_crop.png`; `Design/VisualLockLayered/POP-05_MissionResult/reference/POP-05_MissionResult_NewMainMenuArtDirection_TargetLock_V01.png`; `Assets/Game/UI Toolkit/POP05_MissionResultPopup/POP05_MissionResultPopup.uxml`; `Assets/Game/UI Toolkit/POP05_MissionResultPopup/POP05_MissionResultPopup.uss` |
| POP-06 Settings | Blocked - needs target | 0 | Saved Target Lock reference missing. Do not edit popup visuals until a canonical Target Lock reference PNG exists. | `Design/VisualLockLayered/_UIToolkitVisualMatch/POP-06_Settings/blocked_missing_reference.md` |
| POP-07 Inbox | Blocked - needs target | 0 | Saved Target Lock reference missing. Do not edit popup visuals until a canonical Target Lock reference PNG exists. | `Design/VisualLockLayered/_UIToolkitVisualMatch/POP-07_Inbox/blocked_missing_reference.md` |
