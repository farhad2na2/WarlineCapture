...
- Menu/camp UI money reads, camp catalog reads, camp request validation/commands, friendly pending-production UI read models, selected-building display/health/preview reads, selected-building modal confirm/destroy commands, building minimap flags, visible-building selection checks, live-unit preview prefab resolution, and placement/session UI commands belong behind `BuildingUiCommandSystem`; `MenuView` must not hold a `BuildingPlacementSystem` facade instance.
