...
| Phase 2: Deterministic database builder | In progress; generated bake-config schema, exact output ownership, deterministic source-row inventory, shared compound prototype recipes, exact render-only logical placements, accepted-origin-aligned 32 m spatial index, provisional per-policy capacity/entity/active-slot budgets, and the transactionally persisted candidate config/report are accepted; unchanged-input serialized-byte determinism, camera/projection acceptance, and building state linkage remain open |
...
Checklist progress: `18 / 80` complete.
