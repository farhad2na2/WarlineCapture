...
Audio must follow the 3D single-map direction in `3D_SingleMap_Gameplay_Direction.md`. Planning, briefing, minimap, deployment, threat, and battle views are UI/camera states over one operation map.
...
These cues support the reusable feedback primitives in `Visual_Feedback_VFX_Recommendations.md`. Prefer these shared ids for common feedback so screens do not invent one-off sounds for the same interaction.
...
The authored FTUE and ARIA assistant behavior that drives these cue and VO needs is defined in `FTUE_And_Command_Assistant_Design.md`.
...
- Shared UI motion and VFX feedback follows `Visual_Feedback_VFX_Recommendations.md`, with audio reinforcing but not replacing visible feedback.
