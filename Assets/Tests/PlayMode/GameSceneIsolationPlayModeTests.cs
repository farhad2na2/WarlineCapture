...
public sealed class GameSceneIsolationPlayModeTests
{
    ...
    public async Task GameScene_PlayUsesPromotedDefaultCanvasWithoutOld2DRoute()
    {
        ...
        Assert.IsTrue(defaultScene.IsValid(), "Game scene should be loaded by the editor PlayMode path.");
        ...
        Assert.IsFalse(WarlineCaptureMissionSession.HasActiveMission, "Default Game scene Play must not create a production M01 mission session.");
        Assert.IsFalse(Chapter01M01PlayableRuntime.IsActiveMission(), "Default Game scene Play must not enter the old 2D/isometric M01 runtime.");
