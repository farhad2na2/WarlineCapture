...
| `SCAN` | Selected scan-capable unit/source passes cooldown/charge/resource checks, or mission/global scan rules allow scanning without selection. Selected scanners take priority over global scan. | Mission does not allow scan, scan unavailable, selected unit cannot scan, cooldown, no charges, insufficient resources, invalid target. |
