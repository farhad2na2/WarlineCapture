...
| Last verified commit | `095db4f6f` on `main`; the compact APH-704 candidate remains disabled after exact `9/9` equivalence, `74.88%` benchmark improvement, zero warmed allocation, and affected architecture/build gates. |
| Last update | 2026-07-14 - The Build Drawer catalog runtime view split into bounded catalog-presentation and production-queue UI helpers while retaining its existing 0.2-second presentation schedule. The original owner dropped from 1,051 to 477 lines, both helpers remain below 500 lines, and the global oversized inventory dropped from 22 to 21. Combined Build Drawer, dual-cost read-model, and localization behavior passes `35/35`; the six affected assemblies and final Editor-test build pass with zero compiler errors; the code-equivalent Menu-to-Match smoke reaches `MatchHudReady`, completes Match Intro, and unlocks input. |
