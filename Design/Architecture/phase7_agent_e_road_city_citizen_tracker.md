...
- In progress: `0`.
- Remaining open: `0`.
- Current target: `Agent E naming-only Batch 199 CitizenScheduleCompositionSystemHelper complete; continue road/city/citizen helper naming in small batches`.
- Converted to `ISystem`: `0`.
- Split passive/managed boundaries: `0`.
- Retired/folded helpers: `109`.
- Managed `SystemBase` exceptions created: `0`.
- Validation status: `CitizenScheduleCompositionSystemHelper naming slice completed after prior helper fold. Compile, inventory regeneration, citizen visible-unit focused validation, git diff --check, and Phase 7 architecture guard passed. Latest logs include /private/tmp/warline-non-ecs-helper-naming-batch199-citizen-visible.log with [CitizenVisibleUnitFocusedValidation] result=Passed tests=3 and /private/tmp/warline-non-ecs-helper-naming-batch199-architecture.log with [NonEcsSystemConversionArchitectureValidation] result=Passed tests=9 and runtimeNonEcsDenominator=30. Previous CitizenResourceCompositionSystemHelper naming logs remain /private/tmp/warline-non-ecs-helper-naming-batch198-citizen-visible.log and /private/tmp/warline-non-ecs-helper-naming-batch198-architecture.log.`
...
- [x] Rename `P7-0195 CitizenPopulationReadModelSystem` to `CitizenPopulationReadModelCompositionSystemHelper` with preserved `.meta` GUID and focused citizen/architecture validation.
...
- [x] Fold `P7-0181 RuntimeCityYardGateUtilitySystemHelper` from a disabled `SystemBase` wrapper into a plain runtime-city yard-gate helper; gate-side/opening calculations, state access, and runtime-city composition callers stayed unchanged.
...
- [x] Fold and rename `P7-0226 RoadBuildSessionCompositionSystemHelper` from a disabled `SystemBase` wrapper into a plain road-build session helper; build mode/session state, delete prompt state, command callbacks, and road composition callers stayed unchanged.
- [x] Fold and rename `P7-0231 RoadNetworkCompositionSystemHelper` from a disabled `SystemBase` wrapper into a plain road network graph helper; stroke graph state, road tile data, snapshot/restore behavior, and road composition callers stayed unchanged.
- [x] Fold and rename `P7-0235 RoadRuntimeGenerationCompositionSystemHelper` from a disabled `SystemBase` wrapper into a plain runtime road generation helper; runtime road stroke creation, deferred ECS sync callbacks, special visual bridge calls, and runtime-city composition callers stayed unchanged.
...
- [x] Fold `P7-0182 RuntimeCityYardWallPlanUtilitySystemHelper` from a disabled `SystemBase` wrapper into a plain runtime-city yard-wall plan helper; house-plan shuffling, yard rectangle search, state access, and runtime-city composition callers stayed unchanged.
...
- [x] Rename `P7-0193 CitizenPopulationEventSystem` to `CitizenPopulationEventCompositionSystemHelper` with preserved `.meta` GUID and focused citizen/architecture validation.
...
- [x] Rename `P7-0194 CitizenPopulationLifecycleSystem` to `CitizenPopulationLifecycleCompositionSystemHelper` with preserved `.meta` GUID and focused citizen/architecture validation.
...
- [x] Rename `P7-0196 CitizenPopulationRuntimeUpdateSystem` to `CitizenPopulationRuntimeUpdateCompositionSystemHelper` with preserved `.meta` GUID and focused citizen/architecture validation.
...
- [x] Fold `P7-0161 RuntimeCityHallSpawnPrefabSystemHelper` from a disabled `SystemBase` wrapper into a plain runtime-city hall spawn helper; civic-center placement, hall-prefab shuffling, landmark offset checks, spawn-and-reserve behavior, and generation callers stayed unchanged.
- [x] Fold `P7-0162 RuntimeCityHouseYardWallPrefabSystemHelper` from a disabled `SystemBase` wrapper into a plain runtime-city house-yard-wall helper; yard wall plan selection, yard rectangle search, gate-side selection, wall prefab selection, boundary visual calls, footprint reservation, and generation callers stayed unchanged.
- [x] Fold `P7-0165 RuntimeCityLandmarkSpawnPrefabSystemHelper` from a disabled `SystemBase` wrapper into a plain runtime-city landmark spawn helper; clock tower, fountain, monument, and pillar selection, hall-distance rejection, spawn-and-reserve behavior, and generation callers stayed unchanged.
...
- [x] Rename `P7-0186 CitizenDangerSystem` to `CitizenDangerCompositionSystemHelper` with preserved `.meta` GUID and focused citizen/architecture validation.
...
- [x] Fold and rename `P7-0236 RoadRuntimeRootSceneSystemHelper` from a disabled `SystemBase` wrapper into a plain road runtime root scene helper; runtime road/building root creation and disposal, child-root naming, and road build composition callers stayed unchanged.
- [x] Fold and rename `P7-0210 RoadBuildCompositionLifecycleCompositionSystemHelper` from a disabled `SystemBase` wrapper into a plain road build lifecycle helper; initialization, dependency binding, disposal, exit-build-mode fallback, and road composition callers stayed unchanged.
- [x] Fold and rename `P7-0209 RoadBuildCompositionContextCompositionSystemHelper` from a disabled `SystemBase` wrapper into a plain road build context factory helper; road footprint, runtime generation, read-model, input, command, delete prompt, disposal, ECS, visual, mutation, and placement context creation stayed unchanged.
...
- [x] Fold and rename `P7-0221 RoadBuildInteractionCompositionSystemHelper` from a disabled `SystemBase` wrapper into a plain road build interaction helper; building placement commit, selection hit tests, building selection, deletion, ECS entity cleanup, and storage callbacks stayed unchanged.
- [x] Fold and rename `P7-0223 RoadBuildPlacementStorageCompositionSystemHelper` from a disabled `SystemBase` wrapper into a plain road build placement storage helper; runtime building collection state, active placement storage, building id allocation, selection state, and road composition callers stayed unchanged.
...
- [x] Fold `P7-0166 RuntimeCityLayoutUtilitySystemHelper` from a disabled `SystemBase` wrapper into a plain runtime-city layout algorithm helper; city layout types, state access, and runtime-city composition callers stayed unchanged.
...
- [x] Rename `P7-0187 CitizenHouseholdRegistrationSystem` to `CitizenHouseholdRegistrationCompositionSystemHelper` with preserved `.meta` GUID and focused citizen/architecture validation.
...
- [x] Rename `P7-0192 CitizenPopulationEcsProjectionSystem` to `CitizenPopulationEcsProjectionCompositionSystemHelper` with preserved `.meta` GUID and focused citizen/architecture validation.
...
- [x] Rename `P7-0201 CitizenRefugeeSystem` to `CitizenRefugeeCompositionSystemHelper` with preserved `.meta` GUID and focused citizen/architecture validation.
