...
Date: 2026-07-21
Status: Implementation started; Phase 0 complete; Phase 0A transactional candidate Bake All complete and accepted-source-to-candidate render-only transform/bounds parity repaired (production still StaticSceneChunks; production Addressables untouched); baked/runtime transform parity, fixed-camera parity, Editor lifecycle, and Android acceptance next; previously generated dense city cleared by project owner
...
| Phase 0A: Existing map ECS presentation migration | Transactional candidate Bake All complete; stable per-owner identity contract landed; transform/visual parity and runtime acceptance open |
...
- [ ] Add deterministic source-to-candidate-to-baked transform parity records and fail-closed Editor validation for every gameplay/render-only owner, including nested prefab, hierarchy, pivot, negative/non-uniform scale, `Parent`, and `PostTransformMatrix` cases.
  ...
  - Source/candidate/baked owner-matrix manifest landed fail-closed on 2026-07-21: all 9,544 stable identities join exactly once and every accepted-source → candidate → baked owner world matrix passes the `0.0001` residual after explicit ECS `Parent * LocalTransform * PostTransformMatrix` reconstruction. The same manifest rejects 938 renderer-bounds rows (9 gameplay vehicles and 929 render-only owners): 777 transformed-bounds residuals and 161 Editor-renderer/baked-`RenderBounds` presence mismatches. No tolerance was weakened and no exception was approved. Next work is renderer/submesh-to-baked-render-entity identity and bounds matching, followed by local Addressables-loaded runtime comparison. Android parity is user-triggered and deferred when no device/build is available; it does not block other Editor work.
