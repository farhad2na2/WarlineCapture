...
| Current task | `AM-025` blocked from acceptance but active for remediation; helicopter diagnostics and tactical attack-camera obstruction checks no longer share state across matches, while tactical-camera and building-command caches have verified lifecycle ownership. The `575` historical rows now contain `411` non-debt and `164` genuine-debt rows grouped into `76` unique file/rule items; five external helper paths remain |
