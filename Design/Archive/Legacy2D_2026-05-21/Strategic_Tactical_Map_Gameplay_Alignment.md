...
This document is no longer the active gameplay/map contract.

The active direction is full 3D single-map gameplay: each mission or operation uses one large 3D operation map, and planning, briefing, minimap, deployment, threat alerts, objective focus, and battle view are UI/camera layers on that same world. Do not use this document to justify new 2.5D isometric work or a separate strategic-map/tactical-map product split.

This file is retained for historical context and for migration notes where older UI, FTUE, art, and validation docs still reference the previous split.
...
Tactical gameplay maps must follow `Chapter01_Tactical_Production_Implementation_Plan.md` and `Tactical_Map_AI_Workflow.md`.

For the first playable slice, use `M01_FirstContact_Production_Contract.md` as the concrete mission/map contract.
...
UI work follows `Tactical_UI_Missing_Parts_Work_Order.md`.
