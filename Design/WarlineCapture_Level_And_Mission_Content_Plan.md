...
For every mission or level that uses map/world art, also read `WarlineCapture_3D_SingleMap_Gameplay_Direction.md`. Planning, briefing, minimap, deployment, threat alerts, and battle view are UI/camera layers on the same 3D operation map. Do not author new missions around separate strategic and tactical maps.
...
| `ScenarioSetup` | Operation map id, district id, spawn rules, starting units/buildings/resources, enemy setup, allowed catalog ids from `BalanceConfigs/WarlineCapture_Combat_Balance_Config_v0_1.json` and prefab display data from `Assets/Game/Configs/Prefabs`. |
