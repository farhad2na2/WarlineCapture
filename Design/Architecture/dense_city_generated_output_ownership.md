...
| Virtualized render database bake config | Future `OperationMapRenderDatabaseBuilder` through the candidate-only transaction authorized by `VRP-026` | `Assets/Game/GeneratedOperationMapEntityPresentationCandidate/VirtualizedPresentation/OperationMapRenderDatabaseBakeConfig.asset` | The asset does not exist during `VRP-020`. Before first creation, the builder must journal the exact asset and `.meta`, remove a partial first creation on failure, and restore prior accepted bytes on replacement failure. | Regenerate only from accepted stable records and the deterministic builder. Never hand-create it from a menu, edit it as source, reference it from player runtime after baking, or promote it outside the candidate transaction. |
| Virtualized render database report | Future `OperationMapRenderVirtualizationReportBuilder` in the same candidate transaction | `Design/AgentReports/2026-07-28_dense_city_render_virtualization_database.json` | The report does not exist during `VRP-020`. Its future writer must publish atomically, validate persisted schema/content bytes, and restore or delete it with the matching bake-config transaction on failure. | Regenerate with the exact config/content hash and revision. Never repair counts, identities, policy capacities, or hashes manually, and never accept it when its config asset is absent or differs. |
