...
| 2026-07-16 | `D-163` | Register the operation-map runtime bootstrap boundary | One load-strategy-neutral composition boundary publishes validated immutable operation-map metadata into the existing ECS lifecycle contract, owns only its blob lifetime, and has no recurring update loop or scene-loading policy | `source-growth-exception(path=Assets/Game/Scripts/Composition/OperationMapRuntimeBootstrapSceneSystemHelper.cs;scope=system-helper;maxLines=257;maxBytes=10183;task=APH-710)` |
