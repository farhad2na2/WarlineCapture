...
| Status | Task | Notes |
| --- | --- | --- |
| Complete | Add serialized popup prefab reference to `UIShellContentView`. | Single `settingsPopupPrefab`; shell configures menu or match context at install time. |
| Complete | Add settings popup instance tracking. | Mirrors build drawer/full map popup cleanup. |
| Complete | Add `InstallSettingsPopup`. | Installs the shared settings prefab and applies menu or match context from the active route. |
...
| Status | Task | Notes |
| --- | --- | --- |
| Complete | Create `SCN_SettingsPopup.prefab`. | Generated by `SettingsPopupPrefabBuilder`; replaces the duplicated menu and match popup prefabs. |
| Complete | Remove duplicate settings popup prefabs. | `SCN02_MenuSettingsPopup` and `SCN08_MatchSettingsPopup` are legacy-deleted by the builder and covered by validation. |
...
- Menu settings click opens shared `SCN_SettingsPopup` with menu context.
- Match settings click opens shared `SCN_SettingsPopup` with match context.
...
/Applications/Unity/Hub/Editor/6000.4.0f1/Unity.app/Contents/MacOS/Unity -batchmode -nographics -quit -projectPath /Users/farhad/Projects/WarlineCapture-Clone -executeMethod SettingsPopupValidationTests.RunFocusedValidation -logFile /private/tmp/warline-settings-popup-shared-validation.log
...
- `UIShellContentView` installs shared `SCN_SettingsPopup` under `PopupLayer` and configures menu or match context from the active route.
