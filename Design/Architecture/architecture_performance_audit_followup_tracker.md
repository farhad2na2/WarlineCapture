...
| Current target | Phase 6 first pooling slice for `BuildingPlacementVisualPresentationSystemHelper.CreateBuildingVisualInstance` is compiled and GC-captured; the placement visual create stack no longer appears in searched/top allocation rows. Next measured allocation target is `RtsSelectionAttackTargetModeCommandSystem.ProcessPendingRequests`, which should follow the earlier board/scan cached-query pattern. |
...
| GC baseline status | Latest 2026-07-03 17:37:31 UTC steady-state Match GC capture after placement-visual pooling: 300 requested frames after 180 warmup frames, 301 scanned profiler frames, 21,883 raw GC.Alloc samples / 1,907,835 raw bytes, 21,629 player-relevant samples / 1,880,085 bytes after excluding editor/tooling rows, 254 editor/tooling samples / 27,750 bytes, and runtime allocation probe assertion passed for `UIShellEcsPresentationSystem.Update` plus `MenuBootstrapView.Update`. `BuildingPlacementVisualPresentationSystemHelper.CreateBuildingVisualInstance` no longer appears in searched/top rows; `RtsSelectionAttackTargetModeCommandSystem.ProcessPendingRequests` is now the measured recurring target. |
...
| Validation status | Phase 0 inventory completed; Phase 1 config edited; Phase 2 release OnGUI strip implemented; Phase 3 release diagnostic system gate implemented; Phase 4 GC quick wins completed; Phase 5 Burst inventory/classification report added and guardrail implemented; Phase 6 instantiate classification report added and first placement-visual pooling slice implemented. Latest validations passed: `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, Unity 6.5.2 compile log `/private/tmp/warline-arch-followup-placement-pool-compile.log` with `Exiting batchmode successfully now!`, Match GC capture log `/private/tmp/warline-arch-followup-gc-placement-pool.log` with `[MatchGcAllocationCallstackCapture] result=Passed frames=300`, and earlier focused architecture validation `/private/tmp/warline-arch-followup-toarray-focused.log` with `[EcsBurstHotPathArchitectureValidation] result=Passed tests=10`. |
| Still wrong / next iteration | No visual defects are known from code/compile validation, but a manual visual smoke check for building placement preview reuse is still pending. The placement visual instantiate stack is no longer in the GC report, but player-relevant allocations are still high and now dominated by `RtsSelectionAttackTargetModeCommandSystem.ProcessPendingRequests`; next slice should cache/reuse its query path the same way board/scan command paths were fixed. |
