...
| Remaining genuine-debt rows | 147 |
| Remaining unique debt items | 71 |
| Projected unclassified rows | 0 |
| Source-growth blockers | 5 |

The row-bound evidence now records `428` non-debt rows and `147` remaining genuine-debt rows, grouped into `71` unique file/rule remediation items. It remains non-accepting because every genuine-debt item must be closed before Phase 2 can pass.
...
| World-owner candidates | 27 lifecycle rows after exact boundary/protected classifications | Reconcile overlaps with the hazard lane and avoid double-counting one production defect as multiple remediation items. |
