...
{
    public static class BattleScenarioAd001Runner
    {
        public const string ScenarioId = "AD-001_AirMissileLauncher_InterceptIncomingGroundMissile_RadarComparison";

        private const int Seed = 12345;
        private const float FixedDeltaTime = 0.05f;
        private const float MaxDurationSeconds = 12f;
        ...
        public static BattleScenarioVariant[] CreateDefaultVariants()
        {
            return new[]
            {
                new BattleScenarioVariant
                {
                    ...
                    IncomingThreatStartDistance = 130f,
                ...
                new BattleScenarioVariant
                {
                    ...
                    Label = "No Support / Fast Threat",
                    SupportMode = BattleScenarioSupportMode.None,
                    IncomingThreatKind = BattleScenarioIncomingThreatKind.GroundMissile,
                    IncomingThreatSpeedMultiplier = 1.8f,
                    IncomingThreatStartDistance = 210f,
                    IncomingThreatAltitude = 10f,
                    LauncherCount = 1,
                    RadarDistanceFromLauncher = 0f,
                    ExpectedOutcome = BattleScenarioExpectedOutcome.MayFail
                },
                new BattleScenarioVariant
                {
                    VariantId = "AD-001-D-RadarNear-FastThreat",
                    Label = "Radar Near / Fast Threat",
                    SupportMode = BattleScenarioSupportMode.RadarNear,
                    IncomingThreatKind = BattleScenarioIncomingThreatKind.GroundMissile,
                    IncomingThreatSpeedMultiplier = 1.8f,
                    IncomingThreatStartDistance = 210f,
        ...
        public static BattleScenarioResult RunDefinition(BattleScenarioDefinition definition)
        {
            ...
            if (definition.ScenarioId != ScenarioId)
                throw new ArgumentException($"Unsupported AD-001 scenario id '{definition.ScenarioId}'.", nameof(definition));
            ...
            float maxDurationSeconds)
        {
            ...
            result.Comparisons = new[]
            {
                ...
                BattleScenarioResultComparison.CompareRadarSupport(result.Variants[2], result.Variants[3])
            };
        ...
        public static BattleScenarioMetrics RunVariant(BattleScenarioVariant variant)
        {
            return RunVariant(variant, Seed, FixedDeltaTime, MaxDurationSeconds);
        }
        ...
        {
            if (result.Variants.Length < 4 || result.Comparisons.Length < 2)
                return false;

            BattleScenarioMetrics normalSupported = result.Variants[1];
            BattleScenarioComparison normalComparison = result.Comparisons[0];
            BattleScenarioComparison fastComparison = result.Comparisons[1];
            return normalSupported.Intercepted &&
                   normalComparison.RadarImprovedLockTime &&
                   normalComparison.RadarImprovedOrMatchedOutcome &&
                   fastComparison.RadarImprovedDetectionTime &&
                   fastComparison.RadarImprovedOrMatchedOutcome;
        }
            ...
            BattleScenarioMetrics metrics)
        {
            ...
            using Unity.Collections.NativeArray<LocalTransform> transforms = projectileQuery.ToComponentDataArray<LocalTransform>(Unity.Collections.Allocator.Temp);
