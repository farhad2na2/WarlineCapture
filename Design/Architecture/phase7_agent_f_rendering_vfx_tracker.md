...
- `2026-06-21` - `P7-0259` `BuildingMarkerVisualPresentationSystemHelper`: folded disabled marker-property-block `SystemBase` into a plain helper. `BuildingGameplayCompositionSystem` now owns the helper directly instead of resolving it from the ECS World. This preserves shared marker property-block reuse while removing one non-UI production `SystemBase` declaration from the inventory.
