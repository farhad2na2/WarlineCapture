...
| Current task | Unblock exact-revision Android builds through the operation-map-owned static presentation manifest refresh, then collect clean same-artifact development, release, thermal, memory, startup, and visual evidence |
...
- [~] `APH-803` Add an Android development-build gate for p95/p99 frame, peak memory, startup time, and thermal status on the reference device profile.
  ...
  - Current device preflight: Xiaomi `24090RA29G` (`R4M7PZEQZ58T59ZH`) is visible to ADB with thermal status `0`. The clean exact-revision profiler APK build is blocked before artifact production because `StaticMapAndroidBuildSceneResolver` reports that the canonical Match static presentation manifest is stale. This fail-closed result is accepted behavior; APH-803 must not bypass the resolver or install an older APK. The operation-map owner must refresh and validate the generated manifest before collection resumes.
- Evidence: `Design/AgentReports/2026-07-11_aph-803_android_development_gate_plan.md`, `Design/AgentReports/2026-07-12_aph-803_android_runtime_recorder.md`, `Tools/CI/android_development_performance_gate.py`, and `Tools/CI/android_development_device_collection.py`. Focused recorder `5/5`, diagnostics allocation `4/4`, assembly boundary `31/31`, Python gate `18/18`, development collector `10/10`, combined collector/gate regression `41/41`, and three project builds pass. Remaining work is accepted p99/startup limits and live clean-reference-device proof.
- [~] `APH-804` Add Android release-build acceptance for the documented 30 FPS tier; keep 60 FPS as a separate high-end target.
  ...
  - Current blocker: release artifact collection shares the exact-revision static presentation manifest prerequisite described under APH-803. No release build or device evidence is admissible until the operation-map owner refreshes that generated manifest and the clean build succeeds.
...
### 2026-07-15 - APH-803 / APH-804 / APH-809 - Connected-device and visual preflight blockers

- Status: blocked on operation-map-owned generated evidence; no architecture or map production files changed
- Device: Xiaomi `24090RA29G` (`R4M7PZEQZ58T59ZH`) is connected, USB powered, and reports Android thermal status `0`
- Android build: `BuildScript.BuildAndroidProfilerApk` failed closed before producing an APK because `StaticMapAndroidBuildSceneResolver` detected a stale canonical Match static presentation manifest; no bypass or stale installation was used
