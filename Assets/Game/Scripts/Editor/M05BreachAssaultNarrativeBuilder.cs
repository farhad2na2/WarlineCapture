...
{
    public static class M05BreachAssaultNarrativeBuilder
    {
        public const string Path = "Assets/Game/Configs/Narrative/Chapter01/M05_BreachAssault_Narrative.asset";
        [MenuItem("Game/Campaign/M05/Build Final Narrative")]
        ...
        {
            M05BreachAssaultMediaImporter.ConfigureArt(); if(voices) M05BreachAssaultMediaImporter.ConfigureVoices();
            M05NarrativeLine[][] lines = {M05BreachAssaultCopyCatalog.Brief,M05BreachAssaultCopyCatalog.Comms,M05BreachAssaultCopyCatalog.Comms.Concat(M05BreachAssaultCopyCatalog.Debrief).ToArray()};
            ...
            var merged = (bootstrap.CampaignMissionNarrativeConfigs ?? System.Array.Empty<NarrativeSequenceConfig>())
                .Where(s => s != null && !s.SequenceId.StartsWith("seq.ch01.m05.",StringComparison.Ordinal))
            ...
            Debug.Log("[M05BreachAssaultNarrativeBuilder] result=Passed sequences=3 artSources=7 crops=14 locales=en,fa-IR voices="+voices);
        }

        private static void Configure(string stage,M05NarrativeLine[] lines,NarrativeSequenceConfig basis)
        {
            string sequenceId = "seq.ch01.m05."+stage;
            ...
            if(sequence == null)
            {
                sequence = ScriptableObject.CreateInstance<NarrativeSequenceConfig>(); sequence.name = "M05_BreachAssault_"+stage;
            ...
            sequence.name="M05_BreachAssault_"+stage;
            SerializedObject data = new(sequence);
            data.FindProperty("sequenceId").stringValue = sequenceId;
            data.FindProperty("entryStateId").stringValue = "M05-"+stage+"-0";
            data.FindProperty("defaultSkipDestinationId").stringValue = "M05-"+stage+"-complete";
            ...
            for(int i=0;i<states.arraySize;i++)
            {
                ...
                string completionId = "M05-"+stage+"-complete";
                S(state,"stateId",complete ? completionId : "M05-"+stage+"-"+i);
                ...
                S(state,"continueStateId",complete ? string.Empty : i+1==lines.Length ? completionId : "M05-"+stage+"-"+(i+1));
                S(state,"skipStateId",complete ? string.Empty : completionId);
                S(state,"completionPayloadId",complete ? "request.m05."+(stage=="brief" ? "interactive_brief" : stage)+".complete" : string.Empty);
                ...
                SerializedProperty line = authored.GetArrayElementAtIndex(0); M05NarrativeLine copy = lines[i];
                ...
                line.FindPropertyRelative("voiceClip").objectReferenceValue = M05BreachAssaultMediaImporter.Voice(copy.Id,false);
        ...
        private static string PanelId(string stage,int index)=>stage=="debrief"?(index==0?"C01":"D"+index.ToString("00")):(stage=="brief"?"B":"C")+(index+1).ToString("00");
        ...
        private static void SetPanelReference(NarrativeStateRecord state,string field,string panel,bool wide)
        {
            ...
            if(panel!=null)
            {
                ...
                var group=settings.FindGroup("M05 Narrative") ?? settings.CreateGroup("M05 Narrative",false,false,false,null,
                    typeof(BundledAssetGroupSchema),typeof(ContentUpdateGroupSchema));
                var schema=group.GetSchema<BundledAssetGroupSchema>();
                schema.BundleMode=BundledAssetGroupSchema.BundlePackingMode.PackSeparately;
                var sprite=M05BreachAssaultMediaImporter.Panel(panel,wide);
                string guid=AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(sprite));
                AddressableAssetEntry entry=settings.CreateOrMoveEntry(guid,group,false,false);
                entry.SetAddress("narrative.m05."+panel,false);
        ...
        private static void AddPersian()
        {
            ...
            for(int i=entries.arraySize-1;i>=0;i--)
                if(entries.GetArrayElementAtIndex(i).FindPropertyRelative("key").stringValue.StartsWith("narrative.m05.",StringComparison.Ordinal)) entries.DeleteArrayElementAtIndex(i);
            SerializedProperty voices=data.FindProperty("voices");
            for(int i=voices.arraySize-1;i>=0;i--)
                if(voices.GetArrayElementAtIndex(i).FindPropertyRelative("lineId").stringValue.StartsWith("m05-",StringComparison.Ordinal)) voices.DeleteArrayElementAtIndex(i);
            foreach(M05NarrativeLine line in M05BreachAssaultMediaImporter.Lines)
            {
                ...
                voice.FindPropertyRelative("voiceClip").objectReferenceValue=M05BreachAssaultMediaImporter.Voice(line.Id,true);
        ...
        private static float Duration(in M05NarrativeLine line)=>Mathf.Max(8,
            Mathf.Max(M05BreachAssaultMediaImporter.Voice(line.Id,false)?.length ?? 0,M05BreachAssaultMediaImporter.Voice(line.Id,true)?.length ?? 0)+1,
