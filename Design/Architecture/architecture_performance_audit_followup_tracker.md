...
| Current target | GC capture report clarity slice complete: player-relevant allocation rows, editor/tooling rows, and raw rows are now separated so Unity AI/MCP/Tracing contamination remains visible but no longer drives gameplay cleanup priority. Next step is either a MCP-suppressed clean capture or continuing from the highest remaining player-relevant row if it becomes meaningful. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, and Unity 6.5.2 batchmode compile log `/private/tmp/warline-arch-followup-gc-report-tooling-table-compile.log` passed with 0 compiler errors after the GC report table split. Unity MCP is usable for inspection but currently adds tracing/logging traffic to performance captures, so batchmode/editor-log validation remains the compiler path. |
...
| Validation status | Phase 0 inventory completed; Phase 1 config edited; Phase 2 release OnGUI strip implemented; Phase 3 release diagnostic system gate implemented; Phase 4 GC quick wins completed; Phase 5 Burst inventory/classification report added and guardrail implemented; Phase 6 instantiate classification report added and first placement-visual pooling slice implemented. Latest validations passed: `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, Unity 6.5.2 compile log `/private/tmp/warline-arch-followup-gc-report-tooling-table-compile.log` with `Exiting batchmode successfully now!`, Match GC capture log `/private/tmp/warline-arch-followup-gc-report-tooling-table.log` with `[MatchGcAllocationCallstackCapture] result=Passed frames=300`, and earlier focused architecture validation `/private/tmp/warline-arch-followup-toarray-focused.log` with `[EcsBurstHotPathArchitectureValidation] result=Passed tests=10`. |
| Still wrong / next iteration | No visual defects are known from code/compile validation, but manual visual smoke checks for placement-preview pooling and command behavior are still pending. The capture has only `736` player-relevant bytes after filtering; raw rows are still dominated by Unity AI/MCP/Tracing logging, but the report now separates those rows. Next slice should either suppress MCP/editor bridge traffic for a cleaner raw capture or move on to Android baseline / Fuel-Oil ECS drift once the player-relevant table remains negligible. |
...
- Next iteration: either suppress/disconnect Unity MCP/editor bridge traffic for a cleaner raw GC capture, or proceed to Android baseline / Fuel-Oil ECS drift correction now that the player-relevant GC table is negligible.
