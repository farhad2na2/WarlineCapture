...
Status: milestone 1 implementation/internal QA complete and committed as `b9d17a700`; milestone 2 Base Assault is ready for internal Editor play in both languages. Device/player acceptance and pacing evaluation remain separate. The existing Android candidate predates these changes. This roadmap orders future work; it does not authorize starting every milestone now.
...
| 2 — internally playable | Small skirmish prototype | One Base Assault preset, existing map/roster, one AI opponent and clear win/loss rules | Functional Editor checks complete; evaluate comprehension, enjoyment and pacing with unfamiliar players before further investment |
...
See the [skirmish completion report](Skirmish_Prototype/COMPLETION.md), [detailed plan](Skirmish_Prototype/PLAN.md) and [acceptance matrix](Skirmish_Prototype/ACCEPTANCE.md). The next decision is an unfamiliar-player review of this playable prototype, followed by targeted corrections and device testing. The original 8–12-minute pacing target has not been demonstrated. After that decision, plan milestone 3 progression/rewards without enlarging the skirmish roster prematurely.
