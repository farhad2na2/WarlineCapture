...
public sealed class TacticalFollowCameraModeCommandSystemHelperTests
{
    ...
    public static void RunFocusedValidation()
    {
        ...
        try
        {
            ...
            passed++;
            RunCase(test => test.AttackCinematicContinuesWithCachedLaunchWhenSourceIsDestroyed());
            passed++;
    ...
    [Test]
    public void ActiveAttackCinematicPreservesTemporaryPoseDuringBaseRefresh()
    {
        Entity aircraft = CreateSelectedAirUnit(new float3(0f, 12f, 0f), quaternion.identity);
        Entity target = _em.CreateEntity(typeof(LocalTransform));
        float3 targetPosition = new float3(24f, 0f, 10f);
        ...
        Assert.IsTrue(_system.RefreshActiveTargetAndPose(_em, default, 10.1f));
        Assert.IsTrue(_system.TryReadPose(_em, out TacticalFollowCameraPoseComponent cinematicPose));
        Assert.AreEqual(TacticalFollowCameraPoseSource.TemporaryMissile, cinematicPose.Source);

        _em.SetComponentData(aircraft, LocalTransform.FromPosition(new float3(200f, 12f, 200f)));
        Assert.IsTrue(_system.RefreshActiveTargetAndPose(_em, default, 10.2f));

        Assert.IsTrue(_system.TryReadMode(_em, out TacticalFollowCameraModeComponent mode));
        Assert.AreEqual(1, mode.HasTemporaryTarget);
        Assert.AreEqual(TacticalFollowCameraTargetKind.AttackImpact, mode.TemporaryTargetKind);
        Assert.IsTrue(_system.TryReadPose(_em, out TacticalFollowCameraPoseComponent preservedPose));
    ...
    [Test]
    public void CompletedAttackCinematicRespectsRetriggerCooldown()
    {
        Entity aircraft = CreateSelectedAirUnit(new float3(0f, 12f, 0f), quaternion.identity);
        Entity target = _em.CreateEntity(typeof(LocalTransform));
        float3 targetPosition = new float3(24f, 0f, 10f);
        ...
        Entity request = CreateAttackVfxRequest(aircraft, target, targetPosition);
        SystemHandle cinematicSystem = _world.CreateSystem<TacticalFollowAttackCinematicSystem>();
        _world.SetTime(new TimeData(10d, 0.016f));
        cinematicSystem.Update(_world.Unmanaged);
    ...
    [Test]
    public void AttackCinematicAbortCleansStateWhenTemporaryTargetCleared()
    {
        Entity aircraft = CreateSelectedAirUnit(new float3(0f, 12f, 0f), quaternion.identity);
        Entity target = _em.CreateEntity(typeof(LocalTransform));
        float3 targetPosition = new float3(24f, 0f, 10f);
        ...
        TacticalFollowCameraModeComponent mode = _em.GetComponentData<TacticalFollowCameraModeComponent>(GetModeEntity());
        ...
        _world.SetTime(new TimeData(10.25d, 0.016f));
        cinematicSystem.Update(_world.Unmanaged);

        TacticalFollowAttackCinematicStateComponent cinematic = ReadCinematicState();
        Assert.AreEqual(0, cinematic.Active);
        Assert.AreEqual(1, cinematic.HasEnded);
        Assert.AreEqual(TacticalFollowAttackCinematicAbortReason.TemporaryTargetCleared, cinematic.AbortReason);
        Assert.AreEqual(10.25f, cinematic.LastEndedElapsedTime, 0.001f);
        Assert.IsTrue(_system.TryReadMode(_em, out mode));
        Assert.AreEqual(0, mode.HasTemporaryTarget);
    }

    [Test]
    public void AttackCinematicContinuesWithCachedImpactWhenTargetIsDestroyed()
    {
        Entity aircraft = CreateSelectedAirUnit(new float3(0f, 12f, 0f), quaternion.identity);
        Entity target = _em.CreateEntity(typeof(LocalTransform));
        float3 targetPosition = new float3(24f, 0f, 10f);
        ...
        TacticalFollowAttackCinematicStateComponent cinematic = ReadCinematicState();
        Assert.AreEqual(1, cinematic.Active);
        Assert.AreEqual(Entity.Null, cinematic.TargetEntity);
        Assert.AreEqual(TacticalFollowAttackCinematicAbortReason.None, cinematic.AbortReason);
        Assert.AreEqual(targetPosition, cinematic.ImpactPosition);
        Assert.IsTrue(_system.TryReadMode(_em, out TacticalFollowCameraModeComponent mode));
        Assert.AreEqual(1, mode.HasTemporaryTarget);
        Assert.AreEqual(TacticalFollowCameraTargetKind.AttackImpact, mode.TemporaryTargetKind);
        ...
        Assert.AreEqual(0, cinematic.Active);
        Assert.AreEqual(TacticalFollowAttackCinematicAbortReason.Completed, cinematic.AbortReason);
    }

    [Test]
    public void AttackCinematicContinuesWithCachedLaunchWhenSourceIsDestroyed()
    {
        Entity aircraft = CreateSelectedAirUnit(new float3(0f, 12f, 0f), quaternion.identity);
        Entity target = _em.CreateEntity(typeof(LocalTransform));
        float3 targetPosition = new float3(24f, 0f, 10f);
        ...
        _em.DestroyEntity(aircraft);
        _world.SetTime(new TimeData(10.25d, 0.016f));
        cinematicSystem.Update(_world.Unmanaged);

        TacticalFollowAttackCinematicStateComponent cinematic = ReadCinematicState();
        Assert.AreEqual(1, cinematic.Active);
        Assert.AreEqual(Entity.Null, cinematic.SourceEntity);
        Assert.AreEqual(TacticalFollowAttackCinematicAbortReason.None, cinematic.AbortReason);
        Assert.AreEqual(new float3(0f, 12f, 1f), cinematic.LaunchPosition);
        Assert.IsTrue(_system.TryReadPose(_em, out TacticalFollowCameraPoseComponent pose));
        Assert.AreEqual(TacticalFollowCameraPoseSource.TemporaryMissile, pose.Source);
    ...
    private Entity CreateAttackVfxRequest(Entity aircraft, Entity target, float3 targetPosition)
    {
        Entity request = _em.CreateEntity(typeof(UnitAttackVfxRequest));
        _em.SetComponentData(request, new UnitAttackVfxRequest
        {
    ...
    private void AssertActiveCinematic(byte active)
    {
        TacticalFollowAttackCinematicStateComponent cinematic = ReadCinematicState();
        Assert.AreEqual(active, cinematic.Active);
    }

    private TacticalFollowAttackCinematicStateComponent ReadCinematicState()
    {
        using EntityQuery cinematicQuery =
            _em.CreateEntityQuery(ComponentType.ReadOnly<TacticalFollowAttackCinematicStateComponent>());
        Assert.IsFalse(cinematicQuery.IsEmptyIgnoreFilter);
        return _em.GetComponentData<TacticalFollowAttackCinematicStateComponent>(cinematicQuery.GetSingletonEntity());
    }
