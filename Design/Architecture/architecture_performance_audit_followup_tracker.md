...
| Field | Status |
|---|---|
| Checklist complete | 82 / 119 |
| Checklist percent complete | 68.9% |
| Current phase | Phase 8 - Managed Helper Hot-path Migration |
| Quick wins complete | 6 / 6 |
| Current target | The AttackVfx request-resolution slice has been re-measured with battle capture. `UnitAttackVfxRequestSystem`, `CombatGameObjectVfxPlaybackSystem`, and combat VFX playback no longer appear in the top battle allocation rows. The next measured targets are recurring selection/building/transport runtime lanes, led by `SelectionGameplayStartupSystemHelper.UpdateSelectionRuntimePhases` attribution under `BuildingPlacementRuntimeTick.UpdateBuildingRuntimeState`, `MenuBootstrapView.Update`, and `TransportBoardingCommandSystem`. |
| Compiler status | Latest capture-tooling and AttackVfx measurement slice passed `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, and Unity 6.5.2 battle GC capture `/private/tmp/warline-arch-followup-gc-battle-tooling-filter.log` with 0 project compiler errors. Earlier AttackVfx request-resolution slice passed `Game.Runtime`, `Game.Editor`, `Game.Tests.Editor`, and focused `UnitCombatFocusedEditModeTests.RunFocusedValidation`. |
| Android baseline status | Not started |
| GC baseline status | Latest 2026-07-04 11:59:45 UTC battle-state Match GC capture after AttackVfx request-resolution: 300 requested frames after 180 warmup frames, 301 scanned profiler frames, 6,630 raw GC.Alloc samples / 420,990 raw bytes, 5,691 player-relevant samples / 355,518 bytes after excluding editor/tooling rows, 939 editor/tooling samples / 65,472 bytes, and runtime allocation probe assertion passed for `UIShellEcsPresentationSystem.Update` plus `MenuBootstrapView.Update`. The battle capture prewarmed 9 VFX prefabs and armed 64 attackers; AttackVfx does not appear in top allocation rows. The capture tool now ignores Unity AI/MCP relay connection errors during play-mode validation and classifies main-thread Burst JIT hierarchy rows as editor/tooling. |
