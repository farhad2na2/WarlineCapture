...
| Current task | `AM-025` blocked from acceptance but active for remediation; the first production fix moved helicopter diagnostic state from process-wide storage into its ECS World owner. The `575` historical rows now contain `408` non-debt and `167` genuine-debt rows grouped into `79` unique file/rule items; five external helper paths remain |
