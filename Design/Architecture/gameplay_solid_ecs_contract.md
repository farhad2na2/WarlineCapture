...
- Building runtime tick orchestration, per-phase timing, runtime boundary publish tick, and placement pointer/click frame flow belong in `BuildingPlacementRuntimeTickSystem`; runtime tick context assembly belongs in `BuildingPlacementRuntimeTickContextSystem`, not `BuildingPlacementSystem`; `BuildingRuntimeUpdateSystem` must not receive or invoke a `BuildingPlacementSystem.Update` delegate.
