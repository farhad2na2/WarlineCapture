...
- `UIUX_Implementation_High_Level_Spec.md`
- `UIUX_Implementation_Detailed_Spec.md`
- `Economy_Reward_Design.md`
- `Visual_Feedback_VFX_Recommendations.md`
- `FTUE_And_Command_Assistant_Design.md`
- `3D_SingleMap_Gameplay_Direction.md`
- `M01_FirstContact_Production_Contract.md`
- `Match_HUD_And_Gameplay_Implementation_Spec.md`
- `Match_Selection_Implementation_Spec.md`
...
| `Feedback` | Toast, tooltip, modal, sound, error, selected state, result update, motion, or VFX. Use `Visual_Feedback_VFX_Recommendations.md` for shared feedback patterns and `Audio_Design_Guidelines.md` for paired audio event ids. |
...
| Header/resource counter | Must bind to profile, wallet, match resources, operation resources, or explicit disabled state. Use only the canonical names in `Economy_Reward_Design.md`. |
...
| Reward icon/tile | Preview or show fixed grant. | `RewardConfig`, `RewardItemConfig`, `RewardGrantResult`. | Uses only canonical reward types from `Economy_Reward_Design.md`; no hidden odds or unexplained containers. |
...
- Chapter 1 operation-map UI surfaces must follow `3D_SingleMap_Gameplay_Direction.md` for `OperationMapId`, planning camera ids, minimap projection ids, objective anchors, route anchors, deployment zones, civilian zones, and runtime marker ownership.
- The first concrete tactical UI validation target is `M01_FirstContact_Production_Contract.md`.
...
Implementation source of truth: `Match_HUD_And_Gameplay_Implementation_Spec.md` owns the complete live match HUD contract. `Match_Selection_Implementation_Spec.md` owns selection-specific behavior.
