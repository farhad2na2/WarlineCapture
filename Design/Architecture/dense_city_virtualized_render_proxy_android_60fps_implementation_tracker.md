...
| Phase 4: Jobified runtime assignment | In progress; the ordered camera boundary, exact map-generation native initialization, pure guard-envelope decision, bounded selection, deterministic assignment, parallel apply, and apply-to-Entities-Graphics dependency boundary are accepted. The database owner now bakes one fixed external-buffer command per immutable slot. A Burst presentation `ISystem` validates that exact slot order at lifecycle allocation, schedules the chunk apply through `state.Dependency`, and is explicitly ordered before both `UpdatePresentationSystemGroup` and `EntitiesGraphicsSystem`; it never completes or reads job output in recurring `OnUpdate`. Equal-generation commands receive no component writes, while changed generations apply or fully release one unique slot. Selection/assignment runtime-system scheduling, stable-camera zero-schedule gating, metrics publication, and full unload/job-lifetime proof remain open. |
...
Checklist progress: `38 / 80` complete.
