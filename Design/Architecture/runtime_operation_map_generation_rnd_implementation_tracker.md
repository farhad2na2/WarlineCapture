...
**Status:** Active. R77 was rejected by the project owner because the generated scene had a civilian hall inside the military area, roads and ground entering buildings, buried or unsupported content, and a composition far below the first editor prototype and Match reference. R78 is the current pre-Android owner-review candidate. It restores the three complete hand-authored district modules at their original editor transforms, removes only major-road/runway content, removes the civilian compound hall and oversized authored compound overlay, retains one `32.12 m` two-segment `3.2 m` connector, disables large rock/cliff owners that overlap primary building footprints, and grounds only assemblies left without valid authored support. The editor close audit passes with zero structure, road, burial, floating, or support violations; completed runtime perspective and top-down compositions match the editor source. Owner visual acceptance remains open; Android profiling, gameplay integration, and production adoption remain open.
...
|---|---|---|
| R77 reduces the dense cropped Old Market foreground and brings the central route, compacted compound, and east ruins into one readable overview. Some open right/foreground terrain remains intentional breathing room pending owner review. | M01 authored composition and camera presentation | Owner review of the R77 overview at runtime scale, with feedback on whether the remaining open ground reads as tactical space or dead space. |
| R76 moves the entire utility module and authored compound `15 m` west / `8 m` south, so its west edge now meets the Old Market terrain and the central silhouette is less fragmented. The south residential lobe and outer ruins still require broader composition review. | M01 authored district composition | Owner review of the R76 top-down capture, followed by a south/core balance pass that preserves intentional tactical open space. |
...
| R76 removes the compound's detached northeast-island placement by moving the complete imported/authored utility composition as one unit. Its rectilinear military material and spacing language remains deliberately distinct and still needs owner direction. | Palette and M01 composition problem | Owner decision on whether the now-integrated compound should remain a deliberate landmark or receive an internal shape/material pass. |
| The narrow route no longer dominates. R74 replaces its rectangular end overhangs with exact-length ribbons and bounded round dirt/shoulder caps; owner review still decides whether the remaining straight side edges fit the imported dirt-road language. | Reusable road-surface presentation rule | R74 close, overview, and top-down captures show grounded endpoints with no new wide slabs; owner confirmation remains open. |
| R75 removes the `58` isolated rocks, cacti, bushes, and pebble accents previously scattered around the outer rectangular boundary. Five rear-only dune assemblies now provide restrained skyline closure; owner review still decides whether the ridge needs further art direction. | Presentation and horizon-closure rule | R75 gameplay and top-down captures show no perimeter outlier ring or foreground horizon props; owner confirmation remains open. |
...
Internal post-handover evidence review confirms that R78 must not be described as strong or Match-quality. `Design/ArtReview/OperationMaps/M01/m01_top_down_plan.png` visibly proves the disconnected macro-layout above. The original stored Demo/Demo2 PNGs were uniform blank frames; R79 adds a validated editor-only offscreen capture entry point and successfully replaces them with distinct nonblank `1600x900` images. Direct inspection establishes the missing quality criteria: Demo uses continuous street-wall density, circulation-facing entrances, layered rooftops, and localized prop storytelling; Demo2 uses a clearly bounded military perimeter, readable internal zones, differentiated ground, and purposeful vehicle/prop placement. R78 uses related assets but does not yet bind them with those composition rules. Evidence: `Logs/M01_R78_QualityReferenceRecapture.log`, `Logs/M01_QualityReference_Demo.png`, and `Logs/M01_QualityReference_Demo2.png`.
...
### 2026-07-17 R72 Local-Route Topology Guard

- Kept the R71 scene, visual recipe, runtime marker, manifests, and captures unchanged so the project owner can review one fixed candidate.
- Extended the editor-generation road policy beyond per-segment dimensions. Regeneration now requires at most four segments, at most `90 m` total route length, `0.05 m` endpoint continuity, and no non-adjacent XZ self-intersection; the existing `3.2 m` maximum width, `2-36 m` segment length, and road/solid-content clearance gates remain active.
- Kept the topology calculation in the narrow `M01VisualMapPrototypeEditorUtility.RoutePolicy.cs` partial editor policy instead of growing runtime ownership or the scene-authoring flow. Added the current route policy to focused M01 validation so a disconnected spur, oversized route, or cross-shaped topology cannot silently replace the accepted narrow lane in a later regeneration.
- `Game.Editor.csproj` and `Game.Tests.Editor.csproj` compile with zero errors and warnings. Final Unity validation passes `4/4` in `Logs/M01_R72_RoadTopologyGuardTests_Final.log`; the production source-growth architecture suite passes `17/17` in `Logs/M01_R72_SourceGrowthTests.log`.
- Read-only review of the fixed R71 source inventories `48` inherited building assemblies. Only `TownMarket_DemoAuthored_SM_Bld_Village_House_06 (5)` lies within `15 m` of the authored lane (`8.91 m`); the other `47` are internal district buildings and are not candidates for blanket route-facing rotation. Its serialized yaw is `270 degrees` while the nearest-road bearing from its pivot is `41.5 degrees`, but prefab-forward identity is not sufficient evidence to rotate it without a close visual review. Evidence: `Logs/M01_R72_OrientationInventory.log`.
- The broad terrain/structure inventory records `204` meaningful AABB contacts: `129` in Old Market, `75` in Residential, and `0` in Utility. Of these, `127` contacts involve `41` unique primary structures; many are intentionally rock-founded buildings inherited from the hand-authored quality references. The strict high-confidence penetration gate remains zero, but the broader terrain-clearance checkbox stays open until close captures distinguish authored foundations from visible rock-through-building defects. Evidence: `Logs/M01_R72_TerrainContactInventory.log`.
- This is a non-visual regression guard. No Android optimization, gameplay integration, production adoption, or owner visual acceptance is claimed.

### 2026-07-17 R73 Terrain-Contact Visual Review Evidence

- Kept the R71 source scene, runtime recipe, version markers, manifests, and owner-review captures unchanged. Git records no scene or recipe mutation from this evidence pass.
- Added the narrow editor-only `M01VisualMapPrototypeEditorUtility.TerrainReview.cs` partial. It ranks primary structures by meaningful rock/cliff contact, frames every candidate with deterministic review cameras, validates every `1600x900` capture for visible pixels, and writes individual images plus a manifest and six paged contact sheets under `Logs/M01_R73_TerrainContactReview`.
- Captured and directly inspected all `41` ranked primary structures successfully. The contacts read as intentionally rock-founded or cliff-supported assemblies; no rock visibly passes through a wall or roof in the complete reviewed set. This closes the visible penetration sub-gate and prevents the earlier `204` broad bounds contacts from being mass-cleared and damaging the hand-authored quality reference.
- Evidence: `Logs/M01_R73_AllTerrainContactsCapture.log`, `Logs/M01_R73_TerrainContactReview/m01_r73_terrain_contact_review.md`, `Logs/M01_R73_TerrainContactReview/m01_r73_terrain_contact_review_page_01.png` through `page_06.png`, and `Logs/M01_R73_TerrainContactReview/contact_01.png` through `contact_41.png`.
- `Game.Editor.csproj` and `Game.Tests.Editor.csproj` compile with zero errors and warnings. Production source-growth architecture validation passes `17/17` in `Logs/M01_R73_SourceGrowthTests_Final.log`; focused M01 validation passes `4/4` in `Logs/M01_R73_VisualFocusedTests.log`.
- This is evidence tooling only. No runtime frame ownership, generation recipe, Android optimization, gameplay integration, production adoption, or owner visual acceptance is claimed.

### 2026-07-17 R74 Soft Local-Road Edge Candidate

- Preserved the hard no-autobahn topology: one connected three-segment dusty lane, `3.2 m` maximum surface width, `36 m` maximum segment length, `90 m` maximum total length, no branch, no non-adjacent self-intersection, and zero imported or authored solid-content intersections.
- Removed the artificial `1.2 m` length overhang from every road and shoulder box. Each segment now ends at its authored node; four bounded round dirt caps and transition-shoulder caps ground the two endpoints and two bends without creating intersections or widening the route beyond `3.2 m` surface / `4.8 m` shoulder envelopes.
- Kept each cap pair under its existing road owner and taught the editor exporter to snapshot composite non-prefab mesh entries as one runtime prefab. This preserves recipe entry count and avoids dropping child geometry at runtime without adding frame ownership, scene searches, per-frame allocations, or a runtime role ending in `Builder`.
- Regenerated the shared editor source as `M01VisualPrototype_2026-07-17_v27_soft_road_edges`, fingerprint `9339EA02697ADC50B99D1FF4FAB8E1CC477F561C6822C71F35DBF28894A38D`, and exported `M01RuntimeVisualRecipe_2026-07-17_v27_soft_road_edges` under marker `RuntimeCityM01RoadEdges_R74_2026-07-17`.
- Direct inspection of the editor and actual runtime overview, Old Market approach, and top-down captures confirms the lane remains visually subordinate and the node caps do not read as circular plazas or another major road.
- Editor/runtime manifests match at `10302` renderers, `12` lights, `71` particle renderers, and SHA-256 `1900CEDB130C1DA1E6A5A24BAD98BF53CD1E9D8BFF2643C61AC237109150E11C`. Same-camera comparison measures perspective RGB MAE `0.018` with `0.032%` outliers and top-down RGB MAE `0.001` with `0.003%` outliers.
- Runtime smoke passes with `7049` recipe entries, one foundation, `8.307 ms` worst visual batch, `7` budget yields, and `18.687s` elapsed generation. The desktop construction-batch ceiling remains satisfied; total generation remains above the `5s` pilot target and is deferred until owner visual review.
- Focused validation passes `49/49`: `28` runtime-city tests, `4` M01 source/evidence tests, and `17` production source-growth architecture checks. `Game.Editor.csproj` and `Game.Runtime.csproj` compile with zero errors and warnings.
- Evidence: `Logs/M01_R74_GenerateCapture_Final.log`, `Logs/M01_R74_RuntimeBuild_Composite.log`, `Logs/M01_R74_RuntimeSmoke.log`, `Logs/M01_R74_VisualFocusedTests.log`, `Logs/M01_R74_RuntimeCityFocusedTests.log`, `Logs/M01_R74_SourceGrowthTests.log`, `Design/ArtReview/OperationMaps/M01/m01_gameplay_overview.png`, `Design/ArtReview/OperationMaps/M01/m01_old_market_approach.png`, `Design/ArtReview/OperationMaps/M01/m01_top_down_plan.png`, `Logs/M01_RuntimeGenerationBaseline.png`, and `Logs/M01_RuntimeGenerationTopDown.png`.
- No Android optimization, gameplay integration, production adoption, or owner visual acceptance is claimed.

### 2026-07-17 R75 Clean Horizon Belt Candidate

- Preserved the R74 road, frontage, district placement, terrain-clearance, ground-coverage, staged-reveal, and exact-replay behavior without modification.
- Removed the generator policy that placed `58` cacti, bushes, rocks, and pebble prefabs around a rectangular `286 x 202 m` perimeter. Direct R74 capture review showed these as isolated floating-looking edge dots rather than useful horizon closure.
- Replaced the outlier ring with five overlapping rear-only dune assemblies between approximately `z=152-168 m`, lowered below grade and sharing the transition-ground material. No generated horizon prop remains on the front or side boundaries.
- Replaced the old arbitrary `150` top-level recipe-entry proxy with a named `128` composition floor while retaining the `900` renderer floor, exact editor/runtime manifest hash, stage coverage, foundation, zero-suppression, and same-camera image checks as the authoritative density and parity evidence. The accepted R75 source exports `142` top-level entries and remains far above the renderer floor.
- Regenerated the shared editor source as `M01VisualPrototype_2026-07-17_v28_horizon_belt`, fingerprint `D81C7AC31220D4894BC23A83BF3CFCBB5EC3C7FE846D3B5EB0BD024CBA69DC09`, and exported `M01RuntimeVisualRecipe_2026-07-17_v28_horizon_belt` under marker `RuntimeCityM01Horizon_R75_2026-07-17`.
- Direct inspection of editor and actual runtime overview, Old Market approach, and top-down captures confirms the rectangular outlier ring is absent, the skyline is cleaner, and the five dunes do not form a wall or intrude through foreground structures.
- Editor/runtime manifests match at `10246` renderers, `12` lights, `71` particle renderers, and SHA-256 `10FBCC1A325888F82808F882A9AA705A17A5FCF1A047912DDE089377B4639775`. Same-camera comparison measures perspective RGB MAE `0.018` with `0.032%` outliers and top-down RGB MAE `0.001` with `0.003%` outliers.
- Runtime smoke passes with `6993` realized recipe entries, one foundation, `8.517 ms` worst visual batch, `5` budget yields, and `18.238s` elapsed generation. Lifecycle recovery passes one cancellation and one deterministic missing-config fallback with one attempt and no incomplete cleanup.
- Focused validation passes `49/49`: `28` runtime-city tests, `4` M01 source/evidence tests, and `17` production source-growth architecture checks. `Game.Editor.csproj` and `Game.Runtime.csproj` compile with zero errors and warnings.
- Evidence: `Logs/M01_R75_GenerateCapture.log`, `Logs/M01_R75_RuntimeBuild_Final.log`, `Logs/M01_R75_RuntimeSmoke.log`, `Logs/M01_R75_LifecycleRecovery.log`, `Logs/M01_R75_VisualFocusedTests.log`, `Logs/M01_R75_RuntimeCityFocusedTests.log`, `Logs/M01_R75_SourceGrowthTests.log`, `Design/ArtReview/OperationMaps/M01/m01_gameplay_overview.png`, `Design/ArtReview/OperationMaps/M01/m01_old_market_approach.png`, `Design/ArtReview/OperationMaps/M01/m01_top_down_plan.png`, `Logs/M01_RuntimeGenerationBaseline.png`, and `Logs/M01_RuntimeGenerationTopDown.png`.
- No Android optimization, gameplay integration, production adoption, or owner visual acceptance is claimed.

### 2026-07-17 R76 Compact Utility Core Candidate

- Rejected two preliminary civilian-edge experiments because isolated bright houses outside the compound made the composition worse. No rejected placement was exported or retained.
- Moved `UtilityCompound_East_DemoAuthored` and every authored compound surface, structure, wall, prop, light, and gate together `15 m` west and `8 m` south. The utility curation envelope moves with the module from `(0, 6)->(44, 52)` to `(-15, -2)->(29, 44)`, preserving source-owner curation rather than applying runtime patches.
- Retargeted only the final `CompoundApproach` endpoint from `(15.5, 18)` to `(0.5, 10)`. The road remains one connected three-segment `3.2 m` dusty simple chain with no branch or crossing; total length decreases from `80.88 m` to `64.85 m`, and the `36 m` per-segment / `90 m` total hard limits still pass.
- Direct perspective and top-down inspection confirms the compound now participates in the central settlement silhouette instead of floating as a detached northeast rectangle. The mosque/operations silhouette remains readable, the Old Market and Residential placements are unchanged, and no isolated edge prefabs were added.
- Regenerated the shared editor scene as `M01VisualPrototype_2026-07-17_v29_compact_utility_core`, fingerprint `68F156D98C4FCCB048CB567E9D3E50A3085758DD648161B06773544388E3A6A3`, and exported `M01RuntimeVisualRecipe_2026-07-17_v29_compact_utility_core` under marker `RuntimeCityM01CompactCore_R76_2026-07-17`.
- Editor/runtime manifests match at `10246` renderers, `12` lights, `71` particle renderers, and SHA-256 `9FD56124B4D599032EA2E4968E0D1DA1C6885AC84EC007EF09CF8B3BE1A40330`. Same-camera comparison measures perspective RGB MAE `0.017` with `0.030%` outliers and top-down RGB MAE `0.001` with `0.002%` outliers.
- Runtime smoke passes with `6993` realized recipe entries, `10175` runtime renderers, one foundation, `8.975 ms` worst visual batch, `5` budget yields, and `18.530s` elapsed smoke time. The desktop construction-batch ceiling remains satisfied; total generation remains above the `5s` pilot target and optimization remains deferred until owner review.
- Lifecycle recovery passes one cancellation and one deterministic missing-config fallback with one attempt, `6993` recipe entries, and one foundation. Focused validation passes `49/49`: `28` runtime-city tests, `4` M01 source/evidence tests, and `17` production source-growth architecture checks. `Game.Editor.csproj` and `Game.Runtime.csproj` compile with zero errors and warnings.
- Evidence: `Logs/M01_R76_CompactUtility_Tight_GenerateCapture.log`, `Logs/M01_R76_RuntimeBuild.log`, `Logs/M01_R76_RuntimeSmoke.log`, `Logs/M01_R76_LifecycleRecovery.log`, `Logs/M01_R76_VisualFocusedTests.xml`, `Logs/M01_R76_RuntimeCityFocusedTests.xml`, `Logs/M01_R76_SourceGrowthTests.xml`, `Design/ArtReview/OperationMaps/M01/m01_gameplay_overview.png`, `Design/ArtReview/OperationMaps/M01/m01_old_market_approach.png`, `Design/ArtReview/OperationMaps/M01/m01_top_down_plan.png`, `Logs/M01_RuntimeGenerationBaseline.png`, and `Logs/M01_RuntimeGenerationTopDown.png`.
- No Android optimization, gameplay integration, production adoption, or owner visual acceptance is claimed.

### 2026-07-17 R77 Balanced Final Framing And Reveal Alignment

- Reframed the accepted gameplay overview from position `(-88, 31, -52)`, target `(-23, 4, 0)`, FOV `46` to position `(-82, 38, -64)`, target `(-14, 3, -1)`, FOV `44`. Direct inspection confirms the central route, relocated utility compound, west market, and east ruins now read together while the foreground remains close enough for building inspection.
- Corrected the staged Compound reveal that still targeted the pre-R76 east-side compound. The serialized runtime pose now arrives at position `(-80, 52, -92)`, target `(25, 4, 2)`, FOV `48`; the runtime smoke directly observes the Compound-stage arrival at that position.
- Kept all R76 geometry, terrain, road topology, structure placement, curation, and generation ownership unchanged. The final road remains a `64.85 m`, three-segment, `3.2 m` simple chain with no branch or crossing.
- Added a focused editor regression test that requires the runtime Horizon pose to match the generated editor gameplay camera and pins the Compound reveal to the relocated core. This prevents later composition moves from silently leaving staged cameras on stale coordinates.
- Regenerated the editor scene as `M01VisualPrototype_2026-07-17_v30_balanced_gameplay_framing`, fingerprint `9129AF2B7BF3BB89F74AD8B3C14EC88D18310615D5E4A59D4B56022E4757E945`, and exported `M01RuntimeVisualRecipe_2026-07-17_v30_balanced_gameplay_framing` under marker `RuntimeCityM01BalancedFrame_R77_2026-07-17`.
- Geometry manifests remain identical at `10246` renderers, `12` lights, `71` particle renderers, and SHA-256 `9FD56124B4D599032EA2E4968E0D1DA1C6885AC84EC007EF09CF8B3BE1A40330`. Same-camera runtime comparison improves slightly to perspective RGB MAE `0.016` with `0.022%` outliers and top-down RGB MAE `0.001` with `0.002%` outliers.
- Runtime smoke passes with `6993` realized recipe entries, `10175` runtime renderers, one foundation, `8.493 ms` worst visual batch, `10` budget yields, and `17.886s` elapsed smoke time. R76 lifecycle recovery remains applicable because R77 changes serialized camera poses and editor validation only, not generation, cleanup, cancellation, or fallback behavior.
- Focused validation passes `50/50`: `28` runtime-city tests, `5` M01 source/route/camera tests, and `17` production source-growth architecture checks. `Game.Editor.csproj`, `Game.Runtime.csproj`, and `Game.Tests.Editor.csproj` compile with zero errors and warnings.
- Evidence: `Logs/M01_R77_BalancedFraming_GenerateCapture.log`, `Logs/M01_R77_RuntimeBuild.log`, `Logs/M01_R77_RuntimeSmoke.log`, `Logs/M01_R77_VisualFocusedTests.xml`, `Logs/M01_R77_RuntimeCityFocusedTests.xml`, `Logs/M01_R77_SourceGrowthTests.xml`, `Design/ArtReview/OperationMaps/M01/m01_gameplay_overview.png`, `Design/ArtReview/OperationMaps/M01/m01_old_market_approach.png`, `Design/ArtReview/OperationMaps/M01/m01_top_down_plan.png`, `Logs/M01_RuntimeGenerationBaseline.png`, and `Logs/M01_RuntimeGenerationTopDown.png`.
- No Android optimization, gameplay integration, production adoption, or owner visual acceptance is claimed.

### 2026-07-17 R78 Intact Authored District Parity Candidate

- Explicitly rejected R77 as a valid handover baseline after owner review. Its compact curation envelopes and road-driven deletion policy had fragmented the Demo/Demo 2 compositions into isolated asset islands; its authored compound overlay also mixed inappropriate civilian and military content. Earlier R73 visual-acceptance wording for rock contacts is withdrawn because direct close review later showed visible building penetration.
- Restored `TownMarket_DemoAuthored`, `BaseCommand_DemoAuthored`, and `TownBlock_SouthCenter_DemoAuthored` to the original editor-prototype transforms and broad composition envelopes. Curation no longer deletes buildings or terrain merely because an authored connector intersects them; placement must pass explicit clearance validation instead. Imported major-road and airfield/runway content remains excluded.
- Removed the authored compound courtyard, walls, barracks, and hall replacement experiment. The military district again derives from the hand-authored Base Command module with only the restrained first-prototype checkpoint, tower, tank, supplies, lights, and markings. `CompoundOperationsHall` is forbidden by focused validation.
- Replaced the R77 route with one non-branching `32.12 m`, two-segment, `3.2 m` dusty connector. Route policy and close audit report zero structure intersections; no autobahn, plus, perpendicular branch, or road-driven building deletion is present.
- Added building-footprint terrain clearance and selective support-aware grounding. Large rock, boulder, and cliff owners overlapping primary building footprints are disabled; only building assemblies left without an active authored ground/road support surface move to the continuous base datum. Regeneration records `126` terrain removals and `28` selective building grounding corrections across the three districts.
- Added `M01VisualMapPrototypeEditorUtility.QualityAudit.cs`, with eight deterministic close cameras and automated checks for authored/imported overlap, authored/authored overlap, road/structure overlap, buried structures, floating authored structures, and unsupported primary assemblies. Final result: `559` inspected structures and zero violations in every category.
- Regenerated the shared editor source as `M01VisualPrototype_2026-07-17_v32_intact_authored_districts`, deterministic fingerprint `2AF6E8813D396870F8A8CAB7AE82B3ADF9B30E98424B12655BA0DCE5FBCB3324`, and exported `M01RuntimeVisualRecipe_2026-07-17_v32_intact_authored_districts` under marker `RuntimeCityM01IntactMap_R78_2026-07-17`.
- Completed runtime smoke passes with `6976` realized recipe entries, `10146` renderers, one foundation, visual-manifest SHA-256 `D9EF3E546047187741C69F9CF18B74993EE727F1AD0A6DD2FE6091C4DB87DDB1`, perspective MAE `0.009`, top-down MAE `0.001`, `12.344 ms` worst visual batch, `10` budget yields, and `19.628s` elapsed generation. Direct inspection confirms completed runtime and editor compositions are visually identical; the staged loading capture displays the R78 marker.
- Validation passes `51/51`: `6` M01 source/route/camera/quality tests, `28` runtime-city determinism/cleanup/GC tests, and `17` production source-growth architecture checks. `Game.Editor.csproj`, `Game.Runtime.csproj`, and `Game.Tests.Editor.csproj` compile with zero errors and warnings.
- Evidence: `Logs/M01_R78_SelectiveGrounding_Generate.log`, `Logs/M01_R78_FinalCloseAudit.log`, `Logs/M01_QualityAudit/m01_quality_audit.md`, `Logs/M01_R78_Determinism.log`, `Logs/M01_R78_RuntimeBuild2.log`, `Logs/M01_R78_RuntimeSmoke2.log`, `Logs/M01_R78_VisualTests3.xml`, `Logs/M01_R78_RuntimeCityTests.xml`, `Logs/M01_R78_ArchitectureTests2.xml`, `Logs/M01_EditorCurrentReference.png`, `Logs/M01_RuntimeGenerationBaseline.png`, `Logs/M01_EditorCurrentTopDown.png`, and `Logs/M01_RuntimeGenerationTopDown.png`.
- R78 is ready for project-owner desktop visual review before Android optimization. No owner visual acceptance, Android result, gameplay integration, production adoption, or Match-quality completion is claimed.
