...
| Current target | Phase 5 runtime `ISystem` no-Burst classification and guardrail are in place; next slice should handle Burst in small refactor-backed batches instead of blind attribute additions. |
...
| Validation status | Phase 0 inventory completed; Phase 1 config edited; Phase 2 release OnGUI strip implemented; Phase 3 release diagnostic system gate implemented; Phase 4 GC quick wins completed; Phase 5 Burst inventory/classification report added and guardrail implemented. Latest guardrail validations passed: `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, and Unity 6.5.2 `/private/tmp/warline-arch-followup-burst-guardrail-method.log` with `[EcsNoBurstISystemClassificationValidation] result=Passed tests=1` and `noBurst=71 classified=71 unclassified=0`. Full `EcsBurstHotPathArchitectureTests.RunFocusedValidation` reached Unity and compiled in `/private/tmp/warline-arch-followup-burst-guardrail-escalated.log`, but failed on pre-existing ToArray debt: current `8`, ceiling `0`. |
| Still wrong / next iteration | No visual defects are known in this non-visual guardrail slice. Phase 5 now has the no-Burst runtime `ISystem` classification and a passing guardrail, but no Burst attributes have been added yet because the first pass found no truthful blind-attribute candidates. The broader architecture suite still has pre-existing ToArray debt in `TacticalFollowCameraModeSystemHelper`, `AirMissileLauncherSystems`, `BuildingProductionTransportPresentationSystemHelper`, and `RtsSelectionImmediateSelectedUnitCommandSystem`; next slice should either address that quick debt or begin refactor-backed Burst work on the highest-value `needs refactor` systems. |
