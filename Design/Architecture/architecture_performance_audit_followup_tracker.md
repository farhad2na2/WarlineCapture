...
| Field | Status |
|---|---|
| Checklist complete | 97 / 129 |
| Checklist percent complete | 75.2% |
| Current phase | Phase 7 - Android Ground Truth Baseline |
| Quick wins complete | 6 / 6 |
| Current target | First current-branch Android profiler baseline is captured/exported and compared against the earlier reference capture. The exporter now records saved-capture render counters where Unity exposes them, and the live runtime diagnostics helper now resolves render `ProfilerRecorder` names through exact names plus Unity's available recorder list so draw-call/batch logs are less likely to degrade to `-1` after Unity-version/platform counter drift. A fresh profiler APK with that change was built and installed, but the clean reinstall launched into the menu/startup path and did not emit match `[FrameRateDiag]` / `[RenderSceneDiag]` samples. The next exact action is to add/use a deterministic match-start validation entry path, then capture live Android draw-call/batch logs or run the longer 10-minute/thermal-soak capture before deciding whether HDR/soft-shadow tier wiring needs another config slice. |
| Compiler status | Latest live-render-recorder hardening slice passed `git diff --check`, `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, and `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly` with 0 errors. Fresh Unity 6.5.2 Android profiler APK rebuild passed in `/private/tmp/warline-arch-followup-live-render-counter-apk-build.log` with `26` warnings and 0 errors. |
| Android baseline status | Device profile selected: USB device `R4M7PZEQZ58T59ZH`, Xiaomi `24090RA29G`, codename `malachite`, MediaTek `MT6878`, Android `16`. Current-branch profiler APK built successfully at `Build/AndroidProfiler/WarlineCapture-Profiler.apk` (`405M` on disk, Unity-reported size `3736467585` bytes, `28` warnings), installed with `adb install -r`, launched with profiler capture args, and exported from `/sdcard/Android/data/com.warlinecapture.game/files/WarlineCapture_2026-07-05_11-19-56.data.raw`. First steady report `Design/AgentReports/2026-07-05_perf_WarlineCapture_current_branch_android_11-19-56_steady_summary.md` scans frames `300..1999`: avg `16.64 ms`, p50 `16.64 ms`, p95 `17.57 ms`, p99 `18.30 ms`, max `20.89 ms`, p95 CPU active `9.03 ms`, p95 GPU `5.74 ms`, and `84,780` GC bytes. Compared with the reference-only `WarlineCapture-Clone` steady report, current branch improves avg frame time by `32.7%`, p95 by `55.0%`, p99 by `63.6%`, max steady frame by `69.4%`, p95 CPU active by `50.2%`, and GC by `97.6%`; frames over budget dropped from `1634/1700` to `797/1700`. Thermal snapshot after capture: thermal status `0`, current HAL CPU/GPU `37.455C`, battery `32.0C`, skin `34.126C`, cooling device values `0`; this is a first `2,000`-frame baseline, not the full 10-minute run. |
