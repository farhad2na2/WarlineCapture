...
- In progress: `0`.
- Remaining open: `18`.
- Current target: `Agent B naming-only Batch 211 PerformanceDiagnosticsSystemHelper complete; no open Agent B SystemBase rows remain in the authoritative inventory`.
- Converted to `ISystem`: `8`.
- Retired/folded helpers: `5`.
- Managed `SystemBase` exceptions created: `0`.
- Validation status: `PerformanceDiagnosticsSystemHelper naming slice completed after inventory regeneration, compile, git diff --check, focused performance diagnostics validation, and Phase 7 architecture guard passed. Latest logs include /private/tmp/warline-non-ecs-helper-naming-batch211-performance-diagnostics.log with [PerformanceDiagnosticsAllocationValidation] result=Passed tests=3 and /private/tmp/warline-non-ecs-helper-naming-batch211-architecture.log with [NonEcsSystemConversionArchitectureValidation] result=Passed tests=9 and runtimeNonEcsDenominator=18. CustomGameStartupSystemHelper naming slice completed after inventory regeneration, compile, git diff --check, focused custom-game startup validation, and Phase 7 architecture guard passed. Latest logs include /private/tmp/warline-non-ecs-helper-naming-batch202-custom-game-startup.log with [CustomGameStartupFocusedValidation] result=Passed tests=4 and /private/tmp/warline-non-ecs-helper-naming-batch202-architecture.log with [NonEcsSystemConversionArchitectureValidation] result=Passed tests=9 and runtimeNonEcsDenominator=27. P7-0019 PerformanceDiagnosticsReferenceBoundarySystem and P7-0003 MatchSceneReferenceBoundarySystem retired/folded out of ECS in prior completed slices; prior Agent B logs remain /private/tmp/warline-phase7-agent-b-performance-diagnostics-reference.log, /private/tmp/warline-phase7-agent-b-match-scene-reference.log, /private/tmp/warline-phase7-agent-b-gameplay-scene-binding-helper-fold-bootstrap.log, /private/tmp/warline-phase7-agent-b-gameplay-scene-binding-helper-fold-assembly-boundary.log, and /private/tmp/warline-phase7-agent-a-architecture.log.`
...
rg -n "GameplaySceneBindingSystem|MapSurfaceRuntimeBootstrapSceneSystemHelper|MatchSceneReferenceBoundarySystem|AIFactionControlStartupSystem|AIPlanEntryStartupSystem|AIStartupSystem|FactionEconomyStartupSystem|InitialFactionSpawnCellSystem|PerformanceDiagnosticsReferenceBoundarySystem|PerformanceDiagnosticsSystemHelper|RuntimeDiagnosticsSystem|RuntimeGameplayStateSystem" Assets/Game/Scripts Assets/Tests -g '*.cs'
```
...
| --- | --- | --- | --- | --- | --- | --- |
| `P7-0003` | `MatchSceneReferenceBoundarySystem` | `PlainClass` | `RetiredFolded` | `Folded` | Done | Removed disabled ECS reference store; `MatchSceneReferenceSystem` now resolves the loaded scene-root `MatchSceneView` directly and callers no longer register/clear a world-scoped managed system. |
| `P7-0019` | `PerformanceDiagnosticsReferenceBoundarySystem` | `PlainClass` | `RetiredFolded` | `Folded` | Done | Removed disabled ECS diagnostics reference store; `PerformanceDiagnosticsReferenceSystem` now resolves initialized menu-owned diagnostics directly from the loaded Menu scene roots. |
| `P7-0020` | `PerformanceDiagnosticsSystemHelper` | `PlainClass` | `RetiredFolded` | `Folded` | Done | Folded out of ECS inheritance; remains a manually owned diagnostics helper for menu/bootstrap composition and no longer counts in the ECS system inventory. |
...
| `P7-0002` | `MapSurfaceRuntimeBootstrapSceneSystemHelper` | `PlainClass` | `RetiredFolded` | `Folded` | Done | Folded out of ECS inheritance; remains an explicit composition helper because it must scan scene authoring `MeshFilter`/`Renderer` overlays while installing runtime map-surface ECS data. |
...
- `2026-06-22` - `P7-0001` `GameplaySceneBindingSystem`: folded disabled scene-binding `SystemBase` wrapper into a plain direct-owned helper. `MatchBootstrapSystem` still owns the helper directly and `GameplayFeatureStartupCompositionSystemHelper` still invokes runtime grid blocker debug-view binding; no scene authoring lookup behavior changed.
...
- `MatchSceneReferenceBoundarySystem`, `MapSurfaceRuntimeBootstrapSceneSystemHelper`, `AIStartupSystem`, `InitialFactionSpawnCellSystem`, `GameplaySceneBindingSystem`, and `PerformanceDiagnosticsSystemHelper` are called primarily by `Assets/Game/Scripts/Composition/MatchBootstrapSystem.cs`.
...
| `P7-0020` | `PerformanceDiagnosticsSystemHelper` | Retired/folded from `SystemBase` into a plain diagnostics helper; removed disabled ECS lifecycle inheritance while preserving manual bootstrap/menu ownership and allocation behavior. | `dotnet build Assembly-CSharp-Editor.csproj --no-restore -v:minimal` passed; `git diff --check` passed; performance diagnostics allocation validation passed in `/private/tmp/warline-phase7-agent-b-performance-diagnostics.log`; inventory regenerated to `Design/Architecture/systembase_to_isystem_inventory.md`; architecture guard passed in `/private/tmp/warline-phase7-agent-a-architecture.log`. |
| `P7-0002` | `MapSurfaceRuntimeBootstrapSceneSystemHelper` | Retired/folded from `SystemBase` into a plain composition helper; removed disabled ECS lifecycle inheritance while preserving runtime blob install/disposal and managed scene-overlay extraction. | `dotnet build Assembly-CSharp-Editor.csproj --no-restore -v:minimal` passed; `git diff --check` passed; map-surface runtime bootstrap validation passed in `/private/tmp/warline-phase7-agent-b-map-surface-runtime-bootstrap.log`; inventory regenerated to `Design/Architecture/systembase_to_isystem_inventory.md`; architecture guard passed in `/private/tmp/warline-phase7-agent-a-architecture.log`. |
| `P7-0003` | `MatchSceneReferenceBoundarySystem` | Retired/folded from `SystemBase` by deleting the disabled world-scoped `MatchSceneView` storage boundary; `MatchSceneReferenceSystem` now resolves the loaded scene-root `MatchSceneView` directly for menu/start callers. | `dotnet build Assembly-CSharp-Editor.csproj --no-restore -v:minimal` passed; `git diff --check` passed; Match scene reference validation passed in `/private/tmp/warline-phase7-agent-b-match-scene-reference.log`; inventory regenerated to `Design/Architecture/systembase_to_isystem_inventory.md`; architecture guard passed in `/private/tmp/warline-phase7-agent-a-architecture.log`. |
| `P7-0019` | `PerformanceDiagnosticsReferenceBoundarySystem` | Retired/folded from `SystemBase` by deleting the disabled world-scoped diagnostics storage boundary; `PerformanceDiagnosticsReferenceSystem` now resolves initialized menu-owned diagnostics from loaded Menu scene roots. | `dotnet build Assembly-CSharp-Editor.csproj --no-restore -v:minimal` passed; `git diff --check` passed; PerformanceDiagnostics validation passed in `/private/tmp/warline-phase7-agent-b-performance-diagnostics-reference.log`; inventory regenerated to `Design/Architecture/systembase_to_isystem_inventory.md`; architecture guard passed in `/private/tmp/warline-phase7-agent-a-architecture.log`. |
