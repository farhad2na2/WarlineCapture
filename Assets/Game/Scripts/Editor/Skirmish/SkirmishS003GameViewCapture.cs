...
{
    [InitializeOnLoad]
    public static class SkirmishS003GameViewCapture
    {
        private const string Key = "Warline.S003.GameViewCapture";
        ...
        static SkirmishS003GameViewCapture()
        {
            if (SessionState.GetBool(Key, false))
                EditorApplication.update += Tick;
        }

        [MenuItem("Tools/Warline/Skirmish/Launch S003 Regular Standard Game View")]
        ...
        public static void LaunchRegularStandard()
        {
            if (EditorApplication.isPlaying)
                throw new InvalidOperationException("Exit play mode before launching S003 Regular Standard.");
        ...
        {
            string root = Path.GetFullPath(Path.Combine(Application.dataPath, ".."));
            return Path.Combine(root, SkirmishAcceptanceScaffold.S003RelativeReportEvidenceDirectory);
        }

        public static string DescribeLaunchPayload()
        {
            if (!SkirmishAriaAcceptancePayload.TryCreateFirstVisitS003(
                    SkirmishS003FirstVisit.SeedA,
                    GameLocalization.EnglishLocaleCode,
                    out SkirmishAriaAcceptancePayload payload,
                    out string error))
                throw new InvalidOperationException(error);
            return "[SkirmishS003GameView] selected " + payload.FormatSelectedConfiguration();
        }
        ...
        private static void Tick()
        {
            ...
            try
            {
                ...
                if (stage == 1)
                {
                    if (match.Seed != SkirmishS003FirstVisit.SeedA)
                    {
        ...
        private static bool TryQueueExpanded(EntityManager em)
        {
            ...
            {
                RequiredFeatureIds = authored.DefinitionS003.RequiredFeatureIds
            };
            ...
            if (!SkirmishExpandedLaunchResolver.TryCompileAndQueue(
                    ...
                    registry))
                throw new InvalidOperationException(reasons == null || reasons.Count == 0 ? "S003 queue failed" : reasons[0].ToString());

            if (setup.CatalogId != SkirmishAcceptanceCensusCapture.S003CatalogId ||
                setup.DefinitionId != SkirmishAcceptanceCensusCapture.S003DefinitionId ||
                setup.Seed != SkirmishS003FirstVisit.SeedA ||
                setup.DifficultyId != SkirmishDifficultyId.Regular ||
                setup.SizeId != SkirmishSizeId.Standard)
                throw new InvalidOperationException("S003 Game View queue drifted from Regular Standard seed 104732.");

            Debug.Log(string.Format(
                CultureInfo.InvariantCulture,
                "[SkirmishS003GameView] queued catalog={0} seed={1} hash={2:X8} playable=0",
                setup.CatalogId,
                setup.Seed,
                setup.SetupHash));
            return true;
        ...
        private static string CapturePlayingFrame(in SkirmishMatchState match)
        {
            ...
            string png = Path.Combine(primary, SkirmishAcceptanceScaffold.S003PlayingPngFileName);
            ScreenCapture.CaptureScreenshot(png);
            WriteSidecar(primary, in match);
            WriteSidecar(report, in match);
            Debug.Log("[SkirmishS003GameView] captured " + png);
            return png;
        }
        ...
        {
            File.WriteAllText(
                Path.Combine(directory, SkirmishAcceptanceScaffold.S003GameViewSidecarFileName),
                string.Format(
                    CultureInfo.InvariantCulture,
        ...
        {
            string source = Path.Combine(EvidenceDirectory(), SkirmishAcceptanceScaffold.S003PlayingPngFileName);
            if (!File.Exists(source))
                return;
            Directory.CreateDirectory(ReportEvidenceDirectory());
            File.Copy(
                source,
                Path.Combine(ReportEvidenceDirectory(), SkirmishAcceptanceScaffold.S003PlayingPngFileName),
                true);
        }
        ...
        private static void Finish(string reason)
        {
            ...
            Debug.Log("[SkirmishS003GameView] result=" +
