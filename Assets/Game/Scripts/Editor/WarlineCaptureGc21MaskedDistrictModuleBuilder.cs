...
public static class WarlineCaptureGc21MaskedDistrictModuleBuilder
{
    private const string ScenePath = "Assets/Game/Scenes/Generated/GC21_MaskedDistrictModules_2048.unity";
    private const string CaptureRoot = "Design/AgentReports/Captures/GeneratedScenes/GC21_MaskedDistrictModules_2048";
    private const string ReportPath = "Design/AgentReports/2026-05-21_gameplay_gc21-masked-district-modules.md";
    private const string DataPath = "Design/AgentReports/Data/GeneratedScenes/GC21_MaskedDistrictModules_2048/gc21_masked_district_modules_contract.json";
    ...
    [MenuItem("WarlineCapture/Design/Build GC21 Masked District Modules 2048")]
    public static void BuildGc21MaskedDistrictModules2048()
    {
        MacroRoads.Clear();
        ...
        GameObject root = new("GC21_MaskedDistrictModules_2048_Root");
        ...
        Debug.Log($"WARLINECAPTURE_GC21_MASKED_DISTRICT_MODULES_BUILT scene={ScenePath} report={ReportPath}");
    ...
    private static void BuildEnvironment(GameObject root)
    {
        ...
        Volume volume = Child(root, "GC21_RTS_PresentationVolume").AddComponent<Volume>();
    ...
    {
        Material baseMat = CreateMaterial("GC21_SandBase", new Color(0.61f, 0.50f, 0.31f, 1f));
        Material roadMat = CreateMaterial("GC21_AuthoredCompactedRoad", new Color(0.64f, 0.53f, 0.35f, 1f));
        Material shoulderMat = CreateMaterial("GC21_AuthoredRoadShoulder", new Color(0.53f, 0.43f, 0.27f, 1f));
        Material trackMat = CreateMaterial("GC21_DirtTireTrack", new Color(0.42f, 0.32f, 0.18f, 1f));
        Material runwayMat = CreateMaterial("GC21_RunwayWeatheredAsphalt", new Color(0.36f, 0.35f, 0.31f, 1f));
        Material plazaMat = CreateMaterial("GC21_LocalPackedPlaza", new Color(0.70f, 0.58f, 0.37f, 1f));
        Material vehicleMat = CreateMaterial("GC21_VehicleYardDust", new Color(0.55f, 0.46f, 0.33f, 1f));
    ...
    private static void BuildDebugOverlay(GameObject root)
    {
        ...
        Material green = CreateMaterial("GC21_DebugSoldierWalkable", new Color(0f, 1f, 0.1f, 0.6f));
        Material cyan = CreateMaterial("GC21_DebugVehicleWalkable", new Color(0f, 0.8f, 1f, 0.6f));
        Material red = CreateMaterial("GC21_DebugBlocker", new Color(1f, 0f, 0f, 0.7f));
        foreach (Zone zone in SoldierLocalZones)
    ...
    private static void BuildGeneratorAuthoringOverlay(GameObject root)
    {
        ...
        Material city = CreateMaterial("GC21_AuthoringDistrict_City", new Color(0.20f, 0.42f, 0.80f, 0.85f));
        Material baseMat = CreateMaterial("GC21_AuthoringDistrict_Base", new Color(0.30f, 0.62f, 0.28f, 0.85f));
        Material logistics = CreateMaterial("GC21_AuthoringDistrict_Logistics", new Color(0.84f, 0.58f, 0.18f, 0.85f));
        Material airfield = CreateMaterial("GC21_AuthoringDistrict_Airfield", new Color(0.68f, 0.68f, 0.74f, 0.85f));
    ...
    private static void BuildMaskedModuleOverlay(GameObject root)
    {
        ...
        Material moduleMat = CreateMaterial("GC21_ModuleMask_DistrictEnvelope", new Color(0.88f, 0.18f, 0.92f, 0.82f));
        Material blockerMat = CreateMaterial("GC21_ModuleMask_BlockerOwned", new Color(1f, 0.08f, 0.08f, 0.9f));
    ...
    private static void Validate()
    {
        ...
        if (detailBlockerPlacements < 60)
            ValidationLog.Add($"ERROR: GC21 placed only {detailBlockerPlacements} legal detail props; densification target is at least 60.");

        if (Districts.Count < 7)
            ValidationLog.Add($"ERROR: GC21 exported only {Districts.Count} districts; procedural authoring target is at least 7 named districts.");
        ...
        if (Modules.Count < 7)
            ValidationLog.Add($"ERROR: GC21 exported only {Modules.Count} reusable masked modules; target is at least 7.");
    ...
    {
        BuildRtsCamera(root, "Camera_GC21_RtsOverview", new Vector3(-620f, 520f, -820f), BlueprintPoint(388f, 430f), 38f);
        BuildRtsCamera(root, "Camera_GC21_RtsCityReadable", new Vector3(-930f, 330f, -430f), BlueprintPoint(172f, 366f), 34f);
        BuildRtsCamera(root, "Camera_GC21_RtsAirfieldCommand", new Vector3(260f, 430f, -620f), BlueprintPoint(660f, 320f), 36f);
        BuildRtsCamera(root, "Camera_GC21_RtsDenseCityClose", new Vector3(-700f, 210f, -250f), BlueprintPoint(176f, 392f), 30f);

        Camera top = CameraObject(root, "Camera_GC21_TopDownContractVisualProof");
    ...
    private static void CaptureScene()
    {
        foreach (Camera camera in Object.FindObjectsByType<Camera>(FindObjectsInactive.Include, FindObjectsSortMode.None))
        {
            ...
            if (camera.name == "Camera_GC21_RtsCityReadable")
                Render(camera, ProjectPath(CaptureRoot + "/gc21_rts_city_readable_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC21_RtsAirfieldCommand")
                Render(camera, ProjectPath(CaptureRoot + "/gc21_rts_airfield_command_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC21_RtsDenseCityClose")
                Render(camera, ProjectPath(CaptureRoot + "/gc21_rts_dense_city_close_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC21_TopDownContractVisualProof")
            {
                RenderContractProof(camera, ProjectPath(CaptureRoot + "/gc21_topdown_contract_visual_proof_2048x2048.png"));
                RenderGeneratorBlueprint(camera, ProjectPath(CaptureRoot + "/gc21_topdown_generator_blueprint_2048x2048.png"));
                RenderModuleMaskProof(camera, ProjectPath(CaptureRoot + "/gc21_topdown_module_mask_proof_2048x2048.png"));
            }
        }
    }
    ...
    private static void WriteContractJson()
    {
        ...
        {
            generatedBy = nameof(WarlineCaptureGc21MaskedDistrictModuleBuilder),
            scene = ScenePath,
    ...
    private static void WriteReport()
    {
        ...
        report.AppendLine("# GC21 Masked District Modules Scene");
        report.AppendLine();
        report.AppendLine("Lane: Gameplay");
        report.AppendLine("Task: Continue from GC20 by converting district intent into reusable masked district modules. Each module names its district, intended authored replacement style, and the exact blocker masks it owns so Demo-quality clusters can be swapped in without breaking RTS walkability.");
        report.AppendLine();
        report.AppendLine("Files changed:");
        report.AppendLine("- `Assets/Game/Scripts/Editor/WarlineCaptureGc21MaskedDistrictModuleBuilder.cs`");
        report.AppendLine("- `Assets/Game/Scenes/Generated/GC21_MaskedDistrictModules_2048.unity`");
        report.AppendLine("- `Design/AgentReports/Data/GeneratedScenes/GC21_MaskedDistrictModules_2048/gc21_masked_district_modules_contract.json`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC21_MaskedDistrictModules_2048/gc21_rts_overview_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC21_MaskedDistrictModules_2048/gc21_rts_city_readable_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC21_MaskedDistrictModules_2048/gc21_rts_airfield_command_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC21_MaskedDistrictModules_2048/gc21_rts_dense_city_close_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC21_MaskedDistrictModules_2048/gc21_topdown_contract_visual_proof_2048x2048.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC21_MaskedDistrictModules_2048/gc21_topdown_generator_blueprint_2048x2048.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC21_MaskedDistrictModules_2048/gc21_topdown_module_mask_proof_2048x2048.png`");
        report.AppendLine();
        report.AppendLine("Contracts touched: GC17/GC18 walkability visual contract remains the source of truth; GC21 adds reusable module ownership over blocker masks.");
        report.AppendLine("User-visible behavior: no shipped runtime behavior changed; generated scene now includes module-mask proof for replacing placeholder visuals with authored Demo-quality modules.");
        report.AppendLine("Validation run: Unity batchmode `WarlineCaptureGc21MaskedDistrictModuleBuilder.BuildGc21MaskedDistrictModules2048`.");
        report.AppendLine("Validation result: " + (ValidationLog.Any(line => line.StartsWith("ERROR:", StringComparison.Ordinal)) ? "failed; see validation log below." : "passed masked-module validation."));
        report.AppendLine("Known gaps: This pass exports module contracts and proof overlays, but it still uses the existing placeholder scene visuals. It does not yet instantiate full Demo clusters.");
        report.AppendLine("Cross-lane impacts: Art/Design can review module boundaries before Gameplay promotes full Demo clusters into these masks.");
        report.AppendLine("Next recommended task: GC22 should instantiate selected Demo-authored clusters inside the GC21 module masks and reject any cluster whose internal geometry leaks onto walkable roads/yards.");
        report.AppendLine();
