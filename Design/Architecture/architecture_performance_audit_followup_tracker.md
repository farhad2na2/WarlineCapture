...
| Current target | Building runtime boundary entity lookup now follows the earlier singleton-entity cache pattern. Latest GC capture is player-clean after editor/tooling filtering, but raw rows are dominated by `System.Text.StringBuilder` stacks that include `Unity.AI.MCP.Editor.Bridge`; next step is to keep MCP/editor-tooling contamination out of performance captures before chasing raw hierarchy rows. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, and Unity 6.5.2 batchmode compile log `/private/tmp/warline-arch-followup-building-boundary-cache-compile.log` passed with 0 compiler errors after caching the building runtime boundary entity. Unity MCP is usable for inspection but is currently contaminating GC capture call stacks, so batchmode/editor-log validation remains the compiler path. |
| Android baseline status | Not started |
| GC baseline status | Latest 2026-07-03 18:15:33 UTC steady-state Match GC capture after building-boundary entity cache: 300 requested frames after 180 warmup frames, 301 scanned profiler frames, 12,779 raw GC.Alloc samples / 903,267 raw bytes, 3 player-relevant samples / 272 bytes after excluding editor/tooling rows, 12,776 editor/tooling samples / 902,995 bytes, and runtime allocation probe assertion passed for `UIShellEcsPresentationSystem.Update` plus `MenuBootstrapView.Update`. Raw top rows still name gameplay hierarchy markers, but their managed call stacks include `Unity.AI.MCP.Editor.Bridge` and are therefore classified as tooling-contaminated capture overhead. |
...
| Validation status | Phase 0 inventory completed; Phase 1 config edited; Phase 2 release OnGUI strip implemented; Phase 3 release diagnostic system gate implemented; Phase 4 GC quick wins completed; Phase 5 Burst inventory/classification report added and guardrail implemented; Phase 6 instantiate classification report added and first placement-visual pooling slice implemented. Latest validations passed: `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, Unity 6.5.2 compile log `/private/tmp/warline-arch-followup-building-boundary-cache-compile.log` with `Exiting batchmode successfully now!`, Match GC capture log `/private/tmp/warline-arch-followup-gc-building-boundary-cache.log` with `[MatchGcAllocationCallstackCapture] result=Passed frames=300`, and earlier focused architecture validation `/private/tmp/warline-arch-followup-toarray-focused.log` with `[EcsBurstHotPathArchitectureValidation] result=Passed tests=10`. |
| Still wrong / next iteration | No visual defects are known from code/compile validation, but manual visual smoke checks for placement-preview pooling and command behavior are still pending. The capture has only `272` player-relevant bytes after filtering, but raw top rows are contaminated by `Unity.AI.MCP.Editor.Bridge` call stacks; next slice should make the GC capture path explicitly suppress or isolate Unity MCP/editor bridge traffic before treating raw `System.Text.StringBuilder` hierarchy rows as gameplay allocations. |
...
- Known unresolved: Unity MCP/editor bridge traffic contaminates raw GC call stacks, visual/playmode smoke validation is pending, HDR/soft-shadow tier ownership is still unresolved, Android baseline is not captured, Fuel/Oil authoritative gameplay still lives in managed helper/runtime-building paths, and Phase 5 still has no truthful blind Burst candidates.
- Next iteration: update the GC capture workflow to suppress, disconnect, or separately bucket Unity MCP/editor bridge traffic before using raw `System.Text.StringBuilder` hierarchy rows as gameplay targets; after that, rerun the focused GC capture and continue from the highest remaining player-relevant row.
