...
| Current task | `AM-025` blocked from acceptance but active for remediation; tactical camera state is isolated, Editor-only switches have reset ownership, and match-start, building-AI, diagnostics, logistics telemetry, unit lookup, selected-unit lifecycle, selected-unit details and order snapshots, squad-tray selection, selection startup, building-selection interaction, follow-camera mode, pathfinding pending state, central match state, scene-transition queue, diagnostics state, startup readiness counts, player-input state, visible-unit selection, pointer-target queries, building queries, the building runtime boundary, selection diagnostic routing, plus Scenario Lab action-boundary ownership now have replacement-World proof. The `575` historical rows now contain `448` non-debt and `127` genuine-debt rows grouped into `51` unique file/rule items; five external helper paths remain |
...
| Blockers | AM-025 source-growth is externally blocked by four separately owned FirstLaunch helper paths plus operation-map-owned `RuntimeGridPersistentStorageUtilitySystemHelper.cs`. Row-bound evidence exposes `127` genuine-debt rows in `51` unique file/rule items that must be remediated to zero. Final broad-contract acceptance remains gated by separately owned operation-map, FirstLaunch/UI, and package-bound violations. Release-only certification remains intentionally deferred |
| Latest validation | AM-025 cleanup: squad-tray replacement-World validation passes `4 / 4`, selected-unit details replacement-World validation passes inside UI read-model validation `7 / 7`, building-selection interaction replacement-World validation passes `1 / 1`, selection startup replacement-World validation passes `1 / 1`, pointer-target replacement-World validation passes inside selection input validation `61 / 61`, Scenario Lab action-boundary ownership validation passes `1 / 1`, selection diagnostics explicit-world validation passes inside runtime diagnostics validation `6 / 6`, selected-unit order snapshot replacement-World validation passes `2 / 2`, building runtime boundary replacement-World validation passes inside building production request validation `31 / 31`, building-query replacement-World validation passes `55 / 55`, visible-unit selection replacement-World validation passes `12 / 12`, player-input state replacement-World validation passes `1 / 1`, startup readiness replacement-World validation passes `1 / 1`, diagnostics-state replacement-World validation passes `5 / 5`, scene-transition queue replacement-World validation passes `1 / 1`, central match-state replacement-World validation passes `8 / 8`, pathfinding pending-state replacement-World validation passes `1 / 1`, follow-camera replacement-World validation passes `46 / 46`, selected-unit lifecycle replacement-World validation passes `11 / 11`, unit lookup replacement-World validation passes `5 / 5`, logistics telemetry replacement-World validation passes `54 / 54`, diagnostics replacement-World validation passes `6 / 6`, building-AI two-World validation passes `53 / 53`, match-start two-World validation passes `2 / 2`, Editor performance-switch ownership validation passes `14 / 14`, tactical attack-camera validation passes `20 / 20`, helicopter validation passes `9 / 9`, tactical camera lifecycle validation passes `2 / 2`, and Unity reports zero compiler errors; closure/delta plus source-growth authority checks pass `24 / 24`; integrated architecture passes `176 / 176`; byte-identical regeneration, current source/authority hashes, JSON, and diff checks pass. Canonical Unity source-growth remains fail-closed on five exact external helper paths |
| Latest evidence | Schema-2 `am025_phase2_ownership_delta.json/.md` records one decision for each of `575` historical intake rows: `440` resolved, `8` protected-deferred, and `127` genuine debt grouped into `51` unique file/rule items. `am025_phase2_ownership_delta_intake.json` preserves the immutable v1 intake, and policy schema 2 lists all five exact source-growth blockers |
