...
**Status:** Active. R75 is the stable pre-Android narrow-frontage and clean-horizon candidate. The plus-shaped autobahn, perpendicular crossing, and imported major-road/runway content remain removed; the only authored inter-district route is one connected three-segment `3.2 m` dusty lane with no segment longer than `36 m` and zero imported or authored solid-prop intersections. R72 guards its short simple-chain topology; R74 removes rectangular end overhangs and grounds all four route nodes inside `4.8 m` transition shoulders. R75 removes the `58` randomly scattered perimeter accents and replaces them with five rear-only dune assemblies, eliminating the rectangular outlier ring without changing districts, frontage, road topology, or terrain clearance. Exact editor/runtime manifests and same-camera renders match, lifecycle recovery passes, and all `49` focused checks pass. Overall owner visual acceptance remains open; desktop total-generation-time improvement, Android profiling, gameplay integration, and production adoption also remain open.
...
R75 is the fixed owner-review baseline while this table is reviewed. No Android optimization or procedural-variation work starts from these classifications before visual direction is confirmed.
