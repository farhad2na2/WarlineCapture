...
public static class WarlineCaptureGc24AuthoredDistrictVisualUpgradeBuilder
{
    private const string ScenePath = "Assets/Game/Scenes/Generated/GC24_AuthoredDistrictVisualUpgrade_2048.unity";
    private const string CaptureRoot = "Design/AgentReports/Captures/GeneratedScenes/GC24_AuthoredDistrictVisualUpgrade_2048";
    private const string ReportPath = "Design/AgentReports/2026-05-21_gameplay_gc24-authored-district-visual-upgrade.md";
    private const string DataPath = "Design/AgentReports/Data/GeneratedScenes/GC24_AuthoredDistrictVisualUpgrade_2048/gc24_authored_district_visual_upgrade_contract.json";
    ...
    [MenuItem("WarlineCapture/Design/Build GC24 Authored District Visual Upgrade 2048")]
    public static void BuildGc24AuthoredDistrictVisualUpgrade2048()
    {
        MacroRoads.Clear();
        ...
        GameObject root = new("GC24_AuthoredDistrictVisualUpgrade_2048_Root");
        ...
        Debug.Log($"WARLINECAPTURE_GC24_AUTHORED_DISTRICT_VISUAL_UPGRADE_BUILT scene={ScenePath} report={ReportPath}");
    ...
    private static void BuildEnvironment(GameObject root)
    {
        ...
        Volume volume = Child(root, "GC24_RTS_PresentationVolume").AddComponent<Volume>();
    ...
    {
        Material baseMat = CreateMaterial("GC24_SandBase", new Color(0.58f, 0.49f, 0.34f, 1f));
        Material sandLightMat = CreateMaterial("GC24_SandWindLight", new Color(0.66f, 0.57f, 0.40f, 1f));
        Material sandDarkMat = CreateMaterial("GC24_SandCompactedDark", new Color(0.46f, 0.38f, 0.25f, 1f));
        Material roadMat = CreateMaterial("GC24_AuthoredCompactedRoad", new Color(0.50f, 0.40f, 0.25f, 1f));
        Material roadEdgeMat = CreateMaterial("GC24_RoadRaisedEdge", new Color(0.34f, 0.27f, 0.17f, 1f));
        Material shoulderMat = CreateMaterial("GC24_AuthoredRoadShoulder", new Color(0.47f, 0.38f, 0.24f, 1f));
        Material trackMat = CreateMaterial("GC24_DirtTireTrack", new Color(0.26f, 0.20f, 0.12f, 1f));
        Material runwayMat = CreateMaterial("GC24_RunwayWeatheredAsphalt", new Color(0.24f, 0.24f, 0.22f, 1f));
        Material runwayMarkMat = CreateMaterial("GC24_RunwayFadedMarking", new Color(0.72f, 0.68f, 0.58f, 1f));
        Material plazaMat = CreateMaterial("GC24_LocalPackedPlaza", new Color(0.63f, 0.52f, 0.35f, 1f));
        Material vehicleMat = CreateMaterial("GC24_VehicleYardDust", new Color(0.50f, 0.42f, 0.31f, 1f));
        Material districtPadMat = CreateMaterial("GC24_DistrictPackedSandPad", new Color(0.61f, 0.51f, 0.34f, 1f));
    ...
    {
        GameObject variation = Child(root, "GC24_VisualSandVariationTiles");
    ...
    {
        GameObject pads = Child(root, "GC24_AuthoredDistrictPads_VisualOnly");
    ...
    private static void PlaceChildMaskedAuthoredModule(GameObject parent, MaskedModule module, string path)
    {
        ...
        source.name = "GC24_" + module.Name + "_ChildMaskSource";
        ...
        GameObject moduleRoot = Child(parent, "GC24_" + module.Name + "_ChildMaskedPieces");
        int accepted = 0;
        int pieceIndex = 0;
    ...
    private static bool TryPlaceChildMaskedPiece(GameObject parent, MaskedModule module, Zone mask, string sourcePath, GameObject sourceChild, int index)
    {
        ...
        instance.name = "GC24_" + module.Name + "_ChildPiece_" + index + "_" + sourceChild.name;
        ...
        float scale = Mathf.Min(mask.Rect.width * 0.78f / width, mask.Rect.height * 0.78f / depth);
    ...
    private static void PlaceAuthoredCluster(GameObject parent, MaskedModule module, Zone mask, string path)
    {
        ...
        instance.name = "GC24_" + module.Name + "_AuthoredCluster";
        instance.transform.SetParent(parent.transform, true);
        instance.transform.position = Center(mask.Rect, 0f);
    ...
    private static void BuildDebugOverlay(GameObject root)
    {
        ...
        Material green = CreateMaterial("GC24_DebugSoldierWalkable", new Color(0f, 1f, 0.1f, 0.6f));
        Material cyan = CreateMaterial("GC24_DebugVehicleWalkable", new Color(0f, 0.8f, 1f, 0.6f));
        Material red = CreateMaterial("GC24_DebugBlocker", new Color(1f, 0f, 0f, 0.7f));
        foreach (Zone zone in SoldierLocalZones)
    ...
    private static void BuildGeneratorAuthoringOverlay(GameObject root)
    {
        ...
        Material city = CreateMaterial("GC24_AuthoringDistrict_City", new Color(0.20f, 0.42f, 0.80f, 0.85f));
        Material baseMat = CreateMaterial("GC24_AuthoringDistrict_Base", new Color(0.30f, 0.62f, 0.28f, 0.85f));
        Material logistics = CreateMaterial("GC24_AuthoringDistrict_Logistics", new Color(0.84f, 0.58f, 0.18f, 0.85f));
        Material airfield = CreateMaterial("GC24_AuthoringDistrict_Airfield", new Color(0.68f, 0.68f, 0.74f, 0.85f));
    ...
    private static void BuildMaskedModuleOverlay(GameObject root)
    {
        ...
        Material moduleMat = CreateMaterial("GC24_ModuleMask_DistrictEnvelope", new Color(0.88f, 0.18f, 0.92f, 0.82f));
        Material blockerMat = CreateMaterial("GC24_ModuleMask_BlockerOwned", new Color(1f, 0.08f, 0.08f, 0.9f));
    ...
    private static void Validate()
    {
        ...
        if (detailBlockerPlacements < 60)
            ValidationLog.Add($"ERROR: GC24 placed only {detailBlockerPlacements} legal detail props; densification target is at least 60.");

        if (Districts.Count < 7)
            ValidationLog.Add($"ERROR: GC24 exported only {Districts.Count} districts; procedural authoring target is at least 7 named districts.");
        ...
        if (Modules.Count < 7)
            ValidationLog.Add($"ERROR: GC24 exported only {Modules.Count} reusable masked modules; target is at least 7.");

        if (authoredClusterPlacements < Modules.Count)
            ValidationLog.Add($"ERROR: GC24 placed only {authoredClusterPlacements} authored Demo clusters for {Modules.Count} reusable modules.");
    ...
    {
        BuildRtsCamera(root, "Camera_GC24_RtsOverview", new Vector3(-620f, 520f, -820f), BlueprintPoint(388f, 430f), 38f);
        BuildRtsCamera(root, "Camera_GC24_RtsCityReadable", new Vector3(-930f, 330f, -430f), BlueprintPoint(172f, 366f), 34f);
        BuildRtsCamera(root, "Camera_GC24_RtsAirfieldCommand", new Vector3(260f, 430f, -620f), BlueprintPoint(660f, 320f), 36f);
        BuildRtsCamera(root, "Camera_GC24_RtsDenseCityClose", new Vector3(-700f, 210f, -250f), BlueprintPoint(176f, 392f), 30f);

        Camera top = CameraObject(root, "Camera_GC24_TopDownContractVisualProof");
    ...
    private static void CaptureScene()
    {
        foreach (Camera camera in Object.FindObjectsByType<Camera>(FindObjectsInactive.Include))
        {
            ...
            if (camera.name == "Camera_GC24_RtsCityReadable")
                Render(camera, ProjectPath(CaptureRoot + "/gc24_rts_city_readable_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC24_RtsAirfieldCommand")
                Render(camera, ProjectPath(CaptureRoot + "/gc24_rts_airfield_command_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC24_RtsDenseCityClose")
                Render(camera, ProjectPath(CaptureRoot + "/gc24_rts_dense_city_close_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC24_TopDownContractVisualProof")
            {
                RenderContractProof(camera, ProjectPath(CaptureRoot + "/gc24_topdown_contract_visual_proof_2048x2048.png"));
                RenderGeneratorBlueprint(camera, ProjectPath(CaptureRoot + "/gc24_topdown_generator_blueprint_2048x2048.png"));
                RenderModuleMaskProof(camera, ProjectPath(CaptureRoot + "/gc24_topdown_module_mask_proof_2048x2048.png"));
            }
        }
    }
    ...
    private static void WriteContractJson()
    {
        ...
        {
            generatedBy = nameof(WarlineCaptureGc24AuthoredDistrictVisualUpgradeBuilder),
            scene = ScenePath,
    ...
    private static void WriteReport()
    {
        ...
        report.AppendLine("# GC24 Authored District Visual Upgrade Scene");
        report.AppendLine();
        report.AppendLine("Lane: Gameplay");
        report.AppendLine("Task: Continue from GC23 by upgrading the visual presentation of authored districts while preserving the GC17/GC18 walkability and blocker contract.");
        report.AppendLine();
        report.AppendLine("Files changed:");
        report.AppendLine("- `Assets/Game/Scripts/Editor/WarlineCaptureGc24AuthoredDistrictVisualUpgradeBuilder.cs`");
        report.AppendLine("- `Assets/Game/Scenes/Generated/GC24_AuthoredDistrictVisualUpgrade_2048.unity`");
        report.AppendLine("- `Design/AgentReports/Data/GeneratedScenes/GC24_AuthoredDistrictVisualUpgrade_2048/gc24_authored_district_visual_upgrade_contract.json`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC24_AuthoredDistrictVisualUpgrade_2048/gc24_rts_overview_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC24_AuthoredDistrictVisualUpgrade_2048/gc24_rts_city_readable_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC24_AuthoredDistrictVisualUpgrade_2048/gc24_rts_airfield_command_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC24_AuthoredDistrictVisualUpgrade_2048/gc24_rts_dense_city_close_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC24_AuthoredDistrictVisualUpgrade_2048/gc24_topdown_contract_visual_proof_2048x2048.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC24_AuthoredDistrictVisualUpgrade_2048/gc24_topdown_generator_blueprint_2048x2048.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC24_AuthoredDistrictVisualUpgrade_2048/gc24_topdown_module_mask_proof_2048x2048.png`");
        report.AppendLine();
        report.AppendLine("Contracts touched: GC17/GC18 walkability visual contract remains the source of truth; GC24 keeps GC23 child-masked authored placements and adds visual-only road, district pad, runway marking, and sand-variation presentation layers.");
        report.AppendLine("User-visible behavior: no shipped runtime behavior changed; generated scene has less mask-like roads/ground, stronger visual road boundaries, denser city child pieces, and clearer district pads.");
        report.AppendLine("Validation run: Unity batchmode `WarlineCaptureGc24AuthoredDistrictVisualUpgradeBuilder.BuildGc24AuthoredDistrictVisualUpgrade2048`.");
        ...
        report.AppendLine("Known gaps: GC24 improves presentation but still uses simple generated surface meshes rather than final art roads/terrain decals; districts are not yet full handcrafted multi-mask city chunks.");
        report.AppendLine("Cross-lane impacts: Art/Design can review the upgraded road/ground readability and decide which surface kit pieces or authored district chunks should replace the generated visual-only meshes.");
        report.AppendLine("Next recommended task: GC25 should replace generated surface strips with real Polygon/Demo road, wall, curb, and terrain dressing modules for the city/base/airfield districts.");
        report.AppendLine();
