# Runs Operations P0 Unity validation against the shadow project only.
[CmdletBinding()]
param(
    [string] $ShadowPath = "D:\Projects\WarlineCapture-Operations",
    [string] $SharedPath = "D:\Projects\WarlineCapture",
    [string] $LogFile = "$env:TEMP\operations-p0-shadow.log",
...
if (-not (Test-Path -LiteralPath $shadow -PathType Container)) {
    throw "Shadow project $shadow does not exist. Run Tools/Operations/Ensure-OperationsShadowWorktree.ps1 first."
}

$sharedLibrary = Join-Path $shared "Library"
if (Test-Path -LiteralPath $sharedLibrary) {
    Write-Host "[OperationsP0] leaving_shared_library_untouched=$sharedLibrary"
}
...
Write-Host "[OperationsP0] project=$shadow"
Write-Host "[OperationsP0] unity=$unityExe"
Write-Host "[OperationsP0] log=$LogFile"
...
if ($LASTEXITCODE -ne 0) {
    throw "Operations P0 shadow validation failed."
}

Write-Host "[OperationsP0] result=Passed project=$shadow"
