...
Overall implementation progress: 36% (63/177 checklist items complete).
...
| 2. Per-map static presentation ownership | In progress | 8 | 14 | 57% | Bake input, compatibility routing, schema-2 identity, deterministic map-owned output root, map-namespaced scene filenames, per-map integrity, stale cleanup, and transaction rollback are accepted; remaining hashing/no-op/wiring ownership stays fail-closed. |
