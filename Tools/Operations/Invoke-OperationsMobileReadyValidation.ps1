...
& $validateScript `
    -UnityExe $unityExe `
    -ProjectPath $shadow `
    -ExecuteMethod "Game.Tests.Editor.Operations.OperationsMobileReadyValidation.RunFocusedValidation" `
    -LogFile $LogFile `
    -RequiredPassMarker "[OperationsMobileReadyValidation] result=Passed checks=8" `
    -GuiLicensing:$gui `
