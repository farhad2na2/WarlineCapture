...
- camera drag state, smooth focus state, zoom transition state, camera mode math, camera ground projection, camera pan/zoom mutation, and camera mode interpolation belong in `RtsCameraSystem`; camera control requests must flow through ECS data such as `RtsCameraRequestElement`, `RtsCameraRequestQueueComponent`, and `RtsCameraStateComponent`, processed by `RtsCameraRequestSystem`; `RTSSelectionSystem` may temporarily enqueue and flush camera requests for compatibility, but it must not directly call camera mutation/control APIs or reintroduce private one-line camera wrappers.
