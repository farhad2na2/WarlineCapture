...
For the operational step-by-step workflow, use `Design/UIUX_Target_To_Canvas_Workflow_Guide.md`. That guide records the accepted layered target-to-canvas process from the Main Menu, Settings, Skirmish, Splash, and corrected MatchOverlay passes.
...
The updated gameplay design removes the former split between separate strategic and tactical maps. Use `Design/3D_SingleMap_Gameplay_Direction.md`, `Design/UIUX_MainMenu_Visual_Contract.md`, and `Design/UIUX_Gameplay_Element_Alignment.md` before continuing Phase 6 or M01 UI work.
