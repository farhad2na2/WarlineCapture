...
| Current task | `AM-025` blocked from acceptance but active for remediation; helicopter diagnostic state now belongs to its ECS World, and the tactical camera cache has verified shutdown ownership. The `575` historical rows now contain `409` non-debt and `166` genuine-debt rows grouped into `78` unique file/rule items; five external helper paths remain |
