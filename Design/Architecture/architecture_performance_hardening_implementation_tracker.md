...
| Current task | Rebuild the package-compliant APH-804 release APK with the now-enforced Frame Timing Stats contract, then repeat unplugged device evidence and APH-809 visual acceptance |
...
| Last update | 2026-07-16 - the exact clean APK completed five cold/five warm starts plus the 60+600-second unplugged release capture, but failed closed because its release Player had Frame Timing Stats disabled and therefore emitted zero CPU/GPU timing samples; the build/report/CI/device contracts now reject that incapable artifact before ADB |
...
- Status: stable build/collector contract repair; replacement release artifact and acceptance rerun remain active
...
- Next action: create one clean package-compliant release APK from the timing-capable revision, verify its BuildReport capability field/hash/size, install it, and repeat unplugged APH-804 on isolated ADB port `5038`
