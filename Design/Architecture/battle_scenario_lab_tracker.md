...
- Checklist progress: `93 / 94 complete (98.9%)`.
- In progress: `0`.
- Remaining open: `1`.
- Current target: `AD-001 manual scene now resolves production baked launcher prefabs, validates live ECS near-contact interception, keeps the visible ground rocket synced to the logical projectile, homes against the rendered ground rocket visual when present, requires visual air-missile-to-ground-rocket contact before automated pass, and clears the spent ground rocket visual after intercept; user editor/cinematic verification remains open`.
...
- Isolated scene status: `scene shell created at Assets/Game/Scenes/ScenarioLab/BattleScenarioLab.unity with neutral ground, camera, root reference holder, defended target marker, passive metrics overlay, Play bootstrap, restart button event, AD-001 manual variant selector, a baked Scenario Lab production-prefab registry SubScene, and live ECS visual playback that instantiates production launcher/radar prefabs and seeds the real launcher systems instead of animating proxy objects`.
- Automated metrics runner status: `AD-001 through AD-010 plus GM-001 and DR-001 scenario assets, asset-backed fixed-step ECS runners, report writer, batch execute methods, suite runner, suite index report, focused tests, and Phase 6 deterministic radar-support tuning implemented`.
- Manual verification status: `scene opens in batchmode smoke validation, has required references, can run all AD-001 variants or one selected AD-001 variant on Play, can be rerun through a passive restart button event, displays pass/fail plus variant/comparison metrics, resolves the baked production prefab registry in Play Mode, instantiates production ECS launcher/radar entities, observes live ground and air missile projectiles, validates the visible ground rocket against the logical projectile, validates rendered visual contact between the air missile and ground rocket before accepting the intercept, and validates that the spent ground rocket visual is cleared after the intercept. User visual verification remains open for battery open/rotate timing, actual rocket detach/flight, air missile launch, trail/VFX visibility, camera framing quality, and Run Again behavior in the interactive editor.`
...
- Validation status: `git diff --check passed after visible/logical missile sync corrections; static old-value scan found no remaining old radar-support magic values in the relevant runtime/config paths; Unity EditMode BattleScenarioLabModelTests passed 5/5; Unity EditMode BattleScenarioAd001RunnerTests passed 2/2 including asset-backed run after overlay/bootstrap compile; Unity EditMode BattleScenarioAd002RunnerTests passed 2/2; Unity EditMode BattleScenarioAd003RunnerTests passed 2/2; Unity EditMode BattleScenarioEcsSpawnHelpersTests passed 4/4 before the GM-001 helper additions; latest broad BattleScenario EditMode filter passed 15/15 before AD-004; latest post-tuning batch executeMethod BattleScenarioLabSuiteRunner.RunScenarioSuite passed with AD-001 through AD-010 plus GM-001 and DR-001 non-skipped; latest shadow-project batch executeMethod BattleScenarioLabSceneBuilder.CreateManualSceneShell regenerated the production-prefab live ECS visual scene and baked registry SubScene; latest shadow-project batch executeMethod BattleScenarioLabValidationRunner.ValidateManualSceneSmoke passed with production prefab registry, SubScene, selector/dropdown, restart listener, and live playback reference checks; latest shadow-project Play Mode executeMethod BattleScenarioLabValidationRunner.ValidateManualSceneLiveEcsPlayback passed with rendered visual-contact and clear-after-intercept validation after observing production registry, instantiated launcher entities, ground and air missile projectiles, synced visible ground rocket distance `0.00m`, closest sampled missile distance `0.28m`, visual missile contact distance `0.20m`, and max ground missile altitude `8.36m`; the production air-missile homing target now follows the rendered incoming ground rocket visual when present, the intercept event records visual separation, and the spent ground rocket visual is hidden/restored after intercept so automated validation fails if the visuals pass each other, explode late, or the incoming visual keeps flying. Known non-blocking batchmode noise remains from Entities Graphics shutdown under -nographics. Latest suite index: /private/tmp/warline-scenario-lab-suite-index.json; latest suite log: /private/tmp/warline-scenario-lab-suite-phase6-tuning-rerun.log; latest production visual scene build log: /private/tmp/warline-scenario-lab-baked-prefab-build.log; latest production visual scene smoke log: /private/tmp/warline-scenario-lab-baked-prefab-smoke.log; latest production visual Play Mode log: /private/tmp/warline-scenario-lab-live-ecs-clear-ground-rocket.log.`
...
- `incomingThreatKind`: ground missile, jet, drone
...
- [ ] User Play Mode visual verification confirms battery opening/rotation, actual rocket launch, air missile launch, VFX/trails, camera framing, and Run Again behavior.
