...
| P4 — vertical slice | **Implemented in `Game.Operations.Content` + authored O001–O003:** localized EN/FA copy, two approaches, district changes, day report, ARIA objective planner/input skills on D01. Clear/Protect verbs enabled in tactical for O002/O003. See [P4_VERTICAL_SLICE.md](P4_VERTICAL_SLICE.md). | **Bookmark implemented:** host marker `[OperationsP4Validation] result=Passed checks=14` via `Tools/Operations/check_p4.py`. Manual and ARIA visible-control win paths wired for O001–O003; failures/partial/withdraw/recovery and multi-day scaffolding covered in host checks. **Playable/complete is not claimed** until Programmer 2 / QA record ARIA win evidence. Windows Editor validation later on `D:\Projects\WarlineCapture-Operations` via `Tools/Operations/Invoke-OperationsP4Validation.ps1`. Do not open `D:\Projects\WarlineCapture`. |
