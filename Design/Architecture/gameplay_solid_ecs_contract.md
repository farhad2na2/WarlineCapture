...
- RoadBuildSystem refactor is tracked in `Design/Architecture/road_build_system_refactor_roadmap.md`; `RoadBuildSystem.cs` must not exist, and temporary road runtime state belongs in `RoadBuildRuntimeStateSystem` only while callers finish moving to narrow road boundaries.
