...
{
    public sealed partial class MissionExtractionHudView:MonoBehaviour
    {
        ...
        public void RefreshCinematicVisibility()
        {
            if(!UiShellRuntimeGateway.TryReadMissionCameraTour()) return;
            if(actions.activeSelf) actions.SetActive(false);
            if(navigation!=null && navigation.activeSelf) navigation.SetActive(false);
            DestroyWorldMarkers();
        }
