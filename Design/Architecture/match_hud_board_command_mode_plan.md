...
- 2026-06-10: Fixed passenger-first Board mode trapping the player when no transport is visible. Passenger-first Board now allows camera pan while it is waiting for a transport target; transport-first Board still reserves drag for selecting passenger candidates. Added secondary-pointer cancel for active world-target command modes so right-click exits Board/Move/Attack/Scan targeting without issuing world orders. `git diff --check` passed, no forbidden find/camera/log calls were added in the touched code files, and shadow Unity compile passed with `/private/tmp/warline-board-pan-cancel-compile.log`. Focused EditMode command again exited without writing `/private/tmp/warline-board-pan-cancel-tests.xml`, so focused test-result validation remains open.
