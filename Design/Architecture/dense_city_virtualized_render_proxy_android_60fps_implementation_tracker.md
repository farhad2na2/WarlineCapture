...
| Phase 6: Building and attachment state | In progress through VRP-064. Deterministic contiguous state-owner indices remain owned by canonical gameplay buildings, resident destruction behavior remains unchanged, and the first complete generated House family is candidate-virtualized. Its 3,638 canonical source owners each own exactly one intact and one destroyed recipe (7,276 state-linked placements), while 9,721 render-only Vegetation/Prop placements retain self-owned identities. The source-owner/logical-placement split produces 16,997 placements, 7,291 prototypes, 10,778 parts, 1,651 cells, and 929 fixed slots from 22,058 eligible renderer/material rows; 59,951 independently counted source render rows remain resident and 7,572 non-House building-family rows remain explicitly deferred. Focused stateful compound coverage now forces two distant owners through one three-slot bucket, proves atomic three-part intact to two-part destroyed swaps, retains off-camera destruction until return, reproduces exact slot reuse after recycle/return, and prevents intact/destroyed state leakage between owners. `OperationMapRenderStateSyncSystem` continues to initialize exact canonical state once per map generation, apply bounded changes in Burst, and mark only affected placements/cells. Production remains `StaticSceneChunks + ResidentEntities` with `productionCutover=0`; canonical gameplay-consumer independence remains VRP-065, broader-family expansion remains VRP-066, and Android destruction/recycling acceptance remains VRP-095. |
...
Checklist progress: `51 / 80` complete.
