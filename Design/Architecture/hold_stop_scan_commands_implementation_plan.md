...
- `ISelectionUiReadModel` is the UI-facing read-model contract. `UIShellContentView` passes it into `MatchOverlayCommandInputSystem`, which updates Hold, Stop, and command-wheel Stop interactability from the same capability source. Scan stays pressable so unavailable selections can show typed rejection feedback instead of silently swallowing the click.
- Scan priority is selected scanner first. No global/faction scan rules are currently exposed to the focused-unit read model, so Scan is rejected without an eligible selected scanner until a mission/faction global scan source is added.
...
- Scan debug instrumentation now writes `[ScanCommandTrace]` lines for HUD binding, button click, capability decision, command enqueue, scan target-mode activation, target tap resolution, scan execution, source resolution, and scan results. Latest validation after instrumentation: `MatchHudCommandFeedbackPanelTests.RunFocusedValidation` at `/private/tmp/warline-scan-debug-feedback.log` and `UIShellCurrentContentLoadTests.RunFocusedValidation` at `/private/tmp/warline-scan-debug-shell.log`.
- Command strip pointer diagnostics now write `[ScanCommandTrace] commandPointerProbe ...` on pointer down/up, including `tabHit`, `scanRect`, `holdRect`, `stopRect`, `topHit`, and the top UI raycast hits. If a pointer down/up lands inside the Scan rect but the button click does not fire, the probe invokes the Scan command path as a one-frame-delayed fallback. The legacy `SupportCommand` tab is also routed as a Scan alias and validated with `MatchHudCommandControlsCurrentPrefabTests.RunFocusedValidation` at `/private/tmp/warline-scan-command-controls.log`.
- The live `SCN08_MatchHudContent` prefab had a Scan button hit-target mismatch: `ScanCommand.Button.targetGraphic` pointed at the Build tab frame and the Scan root lacked the transparent root `Image` used by the working command buttons. The prefab now has its own transparent root raycast target, and `MatchOverlayCommandInputSystem` repairs the same issue defensively at bind time for stale instances. Latest validation: `MatchHudCommandControlsCurrentPrefabTests.RunFocusedValidation` at `/private/tmp/warline-scan-command-controls.log`, `MatchHudCommandFeedbackPanelTests.RunFocusedValidation` at `/private/tmp/warline-scan-debug-feedback.log`, and `UIShellCurrentContentLoadTests.RunFocusedValidation` at `/private/tmp/warline-scan-debug-shell.log`.
