...
| Last update | 2026-07-14 - Build Drawer localization, formatting, and bounded fixed-string projection moved into a 188-line stateless helper, reducing the read-model owner from 610 to 436 lines without changing its unmanaged `ISystem`, queries, request routing, or update ownership. The owned read-model fixture passes `6/6`; naming passes `9/9`; exact source authorization passes `2/2`; Runtime and Editor-test builds pass with zero errors; Menu-to-Match reaches `MatchHudReady`, completes Match Intro, and unlocks input. The global inventory is 25 unrelated oversized owners. |
...
- Artifacts: `/private/tmp/warline-build-drawer-projection-focused.xml`, `/private/tmp/warline-build-drawer-projection-source-growth-full.log`, `/private/tmp/warline-build-drawer-projection-match-smoke.log`, and standalone exact-policy/naming validation output in the active task transcript
