...
| Field | Status |
|---|---|
| Checklist complete | 87 / 124 |
| Checklist percent complete | 70.2% |
| Current phase | Phase 8 - Managed Helper Hot-path Migration |
| Quick wins complete | 6 / 6 |
| Current target | Selection, building-placement visual creation, production transport visuals, and `TransportBoardingCommandSystem` have now been measured with editor-only allocation probes. `TransportBoardingCommandSystem` reported `0` bytes across `300` updates, `0` handled updates, and `0` command calls; the report detail shows the recurring transport-boarding hierarchy row is actually `PerformanceDiagnosticsSystemHelper.LogNoStackTrace` frame-rate diagnostic logging. The next exact action is to filter or gate those diagnostics in the capture path before using the remaining GC table to choose another gameplay migration. |
| Compiler status | Latest transport-boarding allocation-probe slice passed `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, and `git diff --check` with 0 errors. Unity 6.5.2 battle capture passed through the documented non-headless batchmode workaround at `/private/tmp/warline-arch-followup-transport-boarding-probe-windowed-battle.log` with 0 project compiler errors. |
| Android baseline status | Not started |
| GC baseline status | Latest completed 2026-07-04 21:11:08 UTC battle-state Match GC capture after transport-boarding allocation probe: 300 requested frames after 180 warmup frames, 301 scanned profiler frames, 16,783 raw GC.Alloc samples / 1,758,350 raw bytes, 8,740 player-relevant samples / 530,819 bytes after excluding editor/tooling rows, and 8,043 editor/tooling samples / 1,227,531 bytes. Runtime allocation probe assertion passed. `UIShellEcsPresentationSystem.Update`, `MenuBootstrapView.Update`, selection subphases, `BuildingPlacementVisualPresentationSystemHelper.CreateBuildingVisualInstance`, `BuildingProductionRuntimeTick.UpdateActiveProductionTransports`, and `TransportBoardingCommandSystem` all reported `0` bytes. The remaining top player-relevant rows are dominated by `PerformanceDiagnosticsSystemHelper.LogNoStackTrace` diagnostic logging stacks. |
