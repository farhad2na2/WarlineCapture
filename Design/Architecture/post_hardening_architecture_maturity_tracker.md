...
| Current task | `AM-025` blocked from acceptance but active for remediation; tactical camera state is isolated, Editor-only switches have reset ownership, and match-start, building-AI, diagnostics, logistics telemetry, unit lookup, selected-unit lifecycle, follow-camera mode, pathfinding pending state, central match state, scene-transition queue, plus diagnostics state now have replacement-World proof. The `575` historical rows now contain `435` non-debt and `140` genuine-debt rows grouped into `64` unique file/rule items; five external helper paths remain |
