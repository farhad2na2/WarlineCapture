...
Cross-lane impacts: UI can replace placeholder assistant panel content by binding `WarlineCaptureAssistantService.CreatePresentationData()` into `AssistantPanelController`. Gameplay remains owner of typed command execution for select/move/attack. PM should track the remaining checklist in `WarlineCapture_AssistantRuntime_M01_Wiring_Plan.md`; I also observed untracked assistant command-runtime files in the shared workspace and left them untouched because they appear to belong to the gameplay wrapper lane.
