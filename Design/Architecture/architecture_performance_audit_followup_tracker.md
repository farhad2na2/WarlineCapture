...
| Current target | First current-branch Android profiler baseline is captured/exported and compared against the earlier reference capture. The exporter now records saved-capture render counters where Unity exposes them; the next exact action is to capture draw-call/batch counters through a live recorder/log path or run the longer 10-minute/thermal-soak capture before deciding whether HDR/soft-shadow tier wiring needs another config slice. |
| Compiler status | Latest render-counter exporter slice passed `git diff --check`, `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, and `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly` with 0 errors. Unity 6.5.2 profiler export reruns passed. |
