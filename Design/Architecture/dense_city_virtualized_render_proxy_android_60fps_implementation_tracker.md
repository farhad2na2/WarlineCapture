...
**Independent correctness queue (does not advance the 80-item VRP checklist):** candidate canal water has a regeneration-stable, clearly blue transparent presentation override with its Windows/Editor asset contract accepted; Android visual confirmation remains pending. The building-road clearance generator correction is Windows/Editor accepted, but persisted-candidate regeneration/bake and Android visual confirmation remain pending before the reported overlap defect can close. Road-over-water-without-bridge remains a separate open defect.
...
| Phase 7: Coverage, LOD, and GPU headroom | In progress through VRP-071. The candidate virtualizes all 38,675 component-safe, supported-policy generated Infrastructure rows under 21,868 owners with eligible render rows. Exact material-surface classification records 74,123 opaque rows (`59,950` eligible), 3,769 alpha-clipped rows (`3,629` eligible), and 509 transparent rows (`0` eligible). All transparent rows currently cast shadows and remain resident under the closed unsupported-render-policy reason; no transparent bucket is materialized. The complete database remains 63,579 eligible logical rows, 41,482 placements, 9,909 prototypes, 13,673 parts, 1,934 cells, two materialized buckets, and 7,075 fixed slots. The schema-2 measured-envelope sweep retains at least 20% per-bucket headroom and stays below the 8,000 provisional slot limit. The provisional logical-placement MMI proxy is 41,482 and therefore does not satisfy the later 24,000 target; VRP-075 still owns the exact packed runtime `MaterialMeshInfo` gate or an evidence-backed amendment. VRP-072 is dependency-ready. |
