...
- Checklist progress: `28 / 105 complete (26.7%)`.
- In progress: `0`.
- Remaining open: `61`.
- Current target: `Agent F naming slice complete for P7-0280 SelectionOrderMarkerPresentationSystemHelper; continue remaining Agent F visual/helper naming candidates`.
- Converted to `ISystem`: `4`.
- Split passive/managed boundaries: `2`.
- Managed presentation `SystemBase` exceptions: `8`.
- Retired/folded helpers: `18`.
- Validation status: `P7-0280 SelectionOrderMarkerPresentationSystemHelper naming slice completed after prior helper fold. Compile, inventory regeneration, selection order marker focused validation, git diff --check, and Phase 7 architecture guard passed. Latest logs include /private/tmp/warline-non-ecs-helper-naming-batch177-selection-order-marker.log with [SelectionOrderMarkerFocusedValidation] result=Passed tests=15 and /private/tmp/warline-non-ecs-helper-naming-batch177-architecture.log with [NonEcsSystemConversionArchitectureValidation] result=Passed tests=9 and runtimeNonEcsDenominator=52. Prior Agent F logs include /private/tmp/warline-phase7-agent-f-selection-order-marker-helper-fold.log, /private/tmp/warline-phase7-agent-f-road-build-visual-context-helper-fold-road-build-command.log, /private/tmp/warline-phase7-agent-f-building-selection-marker-helper-fold.log, /private/tmp/warline-phase7-agent-f-building-runtime-visual-helper-fold-selection-marker.log, /private/tmp/warline-phase7-agent-f-building-placement-visual-helper-fold-placement-runtime.log, /private/tmp/warline-phase7-agent-f-building-foundation-visual-helper-fold-runtime-building.log, /private/tmp/warline-phase7-agent-f-building-destroyed-visual-helper-fold.log, /private/tmp/warline-phase7-agent-f-unit-impostor-helper-fold-render-budget.log, /private/tmp/warline-phase7-agent-f-unit-attack-trace-helper-fold-unit-combat.log, /private/tmp/warline-phase7-agent-f-unit-selection-marker-outline-split-vehicle-visual.log, /private/tmp/warline-phase7-agent-f-runtime-city-yard-wall-visual-helper-fold-runtime-city-generation.log, /private/tmp/warline-phase7-agent-f-runtime-city-visual-helper-fold-runtime-city-generation.log, /private/tmp/warline-phase7-agent-f-building-placement-visual-update-helper-fold-placement-runtime.log, /private/tmp/warline-phase7-agent-f-building-placement-visual-composition-helper-fold-placement-runtime.log, /private/tmp/warline-phase7-agent-f-road-visual-refresh-helper-fold-road-build-command.log, /private/tmp/warline-phase7-agent-f-camera-helper-fold-rts-camera.log, and /private/tmp/warline-phase7-agent-f-camera-helper-fold-runtime-camera-reference.log. Residual unrelated validation note: /private/tmp/warline-phase7-agent-f-building-marker-composition-placement-runtime.log failed an existing runtime-tick cadence assertion (Expected 2, But was 1).`

Completed slices:

- `2026-06-21` - `P7-0281` `SelectionScreenMarkerUiSystemHelper`: folded disabled event-only `SystemBase` into a plain request relay. `SelectionGameplayStartupSystem` now creates the relay directly instead of registering it in the ECS World. This preserves the existing move/attack/hide screen marker events while removing one non-UI production `SystemBase` declaration from the inventory.
- `2026-06-21` - `P7-0259` `BuildingMarkerVisualPresentationSystemHelper`: folded disabled marker-property-block `SystemBase` into a plain helper. `BuildingGameplayCompositionSystemHelper` now owns the helper directly instead of resolving it from the ECS World. This preserves shared marker property-block reuse while removing one non-UI production `SystemBase` declaration from the inventory.
...
- `2026-06-22` - `P7-0278` `RtsSelectionRuntimeCameraSystemHelper` and `P7-0282` `SelectionUiCameraSystemHelper`: folded disabled camera coordination `SystemBase` wrappers into plain direct-owned helpers. The actual Unity camera/reference/render quality boundaries remain in counted managed `SystemBase` exceptions, while the plain helpers keep request queuing, UI camera controls, and match-intro camera behavior unchanged.
- `2026-06-22` - `P7-0273` `RoadVisualRefreshPresentationSystemHelper`: folded disabled static-helper `SystemBase` wrapper into a plain direct-owned helper. Road visual refresh, road ECS sync, dirty chunk rebuild, and special-road rebuild behavior stayed unchanged.
- `2026-06-22` - `P7-0260` `BuildingPlacementVisualCompositionPresentationSystemHelper`: folded disabled placement visual composition `SystemBase` wrapper into a plain direct-owned helper. Building placement visual update, validation, focus, and commit callback wiring stayed unchanged.
- `2026-06-22` - `P7-0262` `BuildingPlacementVisualUpdateCompositionSystemHelper`: folded disabled placement visual update `SystemBase` wrapper into a plain direct-owned helper. Placement visual update, wall preview rebuild, validation, focus, and placement commit behavior stayed unchanged.
- `2026-06-22` - `P7-0241` `RuntimeCityVisualPresentationSystemHelper`: folded disabled runtime city visual `SystemBase` wrapper into a plain direct-owned helper. Runtime city visual root creation, visual-only prefab spawning, surface integration, and disposal behavior stayed unchanged.
- `2026-06-22` - `P7-0242` `RuntimeCityYardWallVisualPresentationSystemHelper`: folded disabled runtime city yard-wall visual `SystemBase` wrapper into a plain direct-owned helper. Yard boundary visual state, wall/gate/pillar spawning, and runtime city visual helper calls stayed unchanged.
...
- `2026-06-22` - `P7-0253` `UnitAttackTracePresentationSystemHelper`: folded disabled attack-trace `SystemBase` wrapper into a plain direct-owned renderer helper. `MatchBootstrapSystem` now instantiates the helper directly; trace config, ECS trace reads, `Graphics.DrawMeshInstanced` batching, and disposal behavior stayed unchanged.
- `2026-06-22` - `P7-0254` `UnitImpostorPresentationSystemHelper`: folded disabled impostor-render `SystemBase` wrapper into a plain direct-owned renderer helper. `MatchBootstrapSystem` now instantiates the helper directly; registry prefab lookup, atlas/material setup, query-driven impostor candidate selection, `Graphics.RenderMeshInstanced` batching, and disposal behavior stayed unchanged.
- `2026-06-22` - `P7-0256` `BuildingDestroyedVisualPresentationSystemHelper`: folded disabled destroyed-building visual `SystemBase` wrapper into a plain direct-owned helper. `BuildingGameplaySourceCompositionSystemHelper` now instantiates the helper directly; destroyed-prefab spawning, alive visual root hiding, existing destroyed instance reuse, and cleanup behavior stayed unchanged.
- `2026-06-22` - `P7-0258` `BuildingFoundationVisualPresentationSystemHelper`: folded disabled foundation visual `SystemBase` wrapper into a plain direct-owned helper. `BuildingRuntimeCreationCompositionSystemHelper` now instantiates the helper directly; runtime building foundation-height GameObject adjustment and combat-entity surface component updates stayed unchanged.
- `2026-06-22` - `P7-0261` `BuildingPlacementVisualPresentationSystemHelper`: folded disabled building placement visual `SystemBase` wrapper into a plain direct-owned helper. `BuildingGameplaySourceCompositionSystemHelper` now instantiates the helper directly; visual wrapper creation, prefab instantiation, combined-mesh renderer filtering, placement positioning, and gate alignment behavior stayed unchanged.
- `2026-06-22` - `P7-0263` `BuildingRuntimeVisualPresentationSystemHelper`: folded disabled runtime building visual `SystemBase` wrapper into a plain direct-owned helper. `BuildingGameplaySourceCompositionSystemHelper` now instantiates the helper directly; runtime visual initialization, Door_Z/barrier setup, animated parts, faction renderer caching/application, resource visual animation updates, and destroyed-building marker visibility refresh stayed unchanged.
- `2026-06-22` - `P7-0264` `BuildingSelectionMarkerPresentationSystemHelper`: folded disabled building selection marker `SystemBase` wrapper into a plain direct-owned helper. `BuildingGameplaySourceCompositionSystemHelper` now instantiates the helper directly; marker prefab instantiation, active-building selection resolution, marker scale/positioning, premium boundary view setup, object outline setup, hide, and dispose behavior stayed unchanged.
- `2026-06-22` - `P7-0270` `RoadBuildVisualContextPresentationSystemHelper`: folded and renamed disabled road build visual context `SystemBase` wrapper into a plain direct-owned presentation helper. `RoadBuildCompositionSourceSystem` now instantiates the helper directly; chunk, preview, special-road, road visual resolution context construction, and prefab resolution stayed unchanged.
- `2026-06-22` - `P7-0280` `SelectionOrderMarkerPresentationSystemHelper`: folded disabled selection order marker `SystemBase` wrapper into a plain direct-owned helper. `SelectionGameplayStartupSystem` already creates it directly; move/attack/scan markers, attack-target selection marker, attack/board preview marker pools, command result marker handling, visibility expiry, and disposal stayed unchanged.
...
- [x] Fold disabled camera coordination wrappers `P7-0278 RtsSelectionRuntimeCameraSystemHelper` and `P7-0282 SelectionUiCameraSystemHelper` into plain direct-owned helpers while preserving request queue and UI camera behavior.
...
- [x] Fold disabled road visual helper wrapper `P7-0273 RoadVisualRefreshPresentationSystemHelper` into a plain direct-owned helper after checking road-build composition source/context call sites.
- [x] Fold disabled building placement visual composition wrapper `P7-0260 BuildingPlacementVisualCompositionPresentationSystemHelper` into a plain direct-owned helper after checking composition source and placement callback call sites.
- [x] Fold disabled building placement visual update wrapper `P7-0262 BuildingPlacementVisualUpdateCompositionSystemHelper` into a plain direct-owned helper after checking composition source and placement visual callback call sites.
- [x] Fold disabled runtime city visual wrapper `P7-0241 RuntimeCityVisualPresentationSystemHelper` into a plain direct-owned helper after checking runtime city composition owner, spawn context, yard-wall call sites, and disposal.
- [x] Fold disabled runtime city yard-wall visual wrapper `P7-0242 RuntimeCityYardWallVisualPresentationSystemHelper` into a plain direct-owned helper after checking runtime city composition owner, spawn context, house yard-wall call sites, and visual state ownership.
