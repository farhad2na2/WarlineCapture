...
### Continued Editor validation — 22 September, evening

- Fixed the checkpoint test fixture's CS1612 buffer assignment. The first new test run exited 3; it is failed evidence.
- `/private/tmp/o001-editor-recovery-tests-3.log` and XML: wrapper exit 0, **27 passed**. Includes the pending-transform writer regression and checkpoint/profile tests.
- Hardened checkpoint validation for version/identity, invalid references and non-finite clocks/transforms. Preflight checks the complete destination roster before any actor is changed. `/private/tmp/o001-editor-recovery-tests-4.log` and XML: wrapper exit 0, **29 passed**, including rejection without partial mutation.
- `/private/tmp/o001-editor-content-2.log`: wrapper exit 0, 370 configured localization entries, `hudMissingScripts=0`, and finite-roster/reachability validation passed. The shipping English/Persian scan instructions are now rebuilt.
- A second normal mouse-input session (`/private/tmp/o001-interactive-2.log`) visibly completed Signal A at 15 seconds without the original objective job exception. A lone scout and then a four-person group died before Signal B. The player explicitly withdrew; the result showed one scan, eleven infantry at exit, a saved result, and return to Operations with AP 2. This is a real withdrawal journey, **not a victory or tactical acceptance pass**.
- The live session exposed overlapping scan guidance and the shared four-soldier quick-select behavior. Added enough HUD text height and Operations-only original-roster selection through the existing squad tray. A subsequent normal deployment visibly selected all 16 infantry and displayed the guidance without clock overlap. Its complete outcome and the new selection regression test remain pending.
- The shared HUD deliberately leaves unavailable unit categories visible in grayscale, matching Campaign behavior; `HideUnrelatedControls` does not mean those authored cards are removed.

Active-world recovery is still **not wired for players**; neutral-city state and resume/restart choices remain unfinished. No Editor-ready certification is claimed.

The next normal-input deployment selected the complete force, completed A and B, and ended in a durably saved Defeat at 287.29 simulated seconds with one original survivor. No objective/runtime exception was found in the session log. This is not a win. Post-outcome inspection showed the shared manual MOVE path intentionally suppresses automatic retaliation during travel. ATTACK on empty terrain issues an interruptible attack-move order; direct unit/building taps retain target validation. Replaced the mission's MOVE-only guidance with full-force selection and ATTACK-on-open-ground guidance. This guidance update still needs catalog rebuild and another normal-input tactical attempt. Enemy stats/force counts were not changed to compensate for the failed attempt.

Checkpoint hardening also rejects unknown archive schemas before writing, preserving the exact existing profile bytes. Capture maps only actors with restorable health/transform state, so stale references to deleted state resolve to no actor. These final edits await the next focused run.

Final focused run `/private/tmp/o001-editor-recovery-tests-5.log` and XML exited **0**: **35 passed, 0 failed**. This includes the final archive safeguards, bounded interaction clocks, Operations roster selection and existing Campaign/shared quick-select tests. Sanitized XML is published in `O001_Integration_20260922/editor-readiness-regressions.xml`; the earlier 29-test foundation report is also retained. The interactive wrapper exited **124** at its scheduled 1500-second timeout after the observed playthroughs had ended and Play Mode was stopped. It is not an automated acceptance pass; the manual observations above remain separately identified.

Final content rebuild `/private/tmp/o001-editor-content-3.log` exited **0** with `ConfiguredUiTables result=Passed configEntries=370`, `hudMissingScripts=0`, and mission validation `result=Passed player=16 enemy=20 scans=3 reachable=8`. The ATTACK-on-open-ground guidance is now in the shipping English/Persian catalog. A normal-input win following that guidance and Persian visual QA remain open.

## Full normal-input Victory and explicit interrupted-attempt recovery

The corrected C forecourt and ADVANCE objective controls close the observed navigation blocker. The normal English/Regular seed 1102 route now wins in 588.137 seconds, with three scans, carried evidence, sixteen surviving original infantry and three at exit when Victory latches. Result save, Continue and AP 2 were verified; wrapper exit 0. Chronological records and selected screenshots: [manual victory evidence](O001_Integration_20260922/manual-victory-1102/).

An interrupted reservation no longer silently restarts through Deploy. The briefing offers explicit Restart Attempt or confirmed Withdraw; restarting preserves the session/seed/AP reservation and records a durable, idempotent restart count. Changed content is rejected and refunded without replacing the recovery journal; existing tactical checkpoint bytes are preserved rather than silently reset. The live button-event smoke passed interrupted restart → saved withdrawal → actor cleanup → menu → paid redeploy with AP 1. Active-world Resume is still **not implemented** and player copy says so. The checkpoint foundation now includes selection/manual-control flags and uses schema 2 so incomplete older images are not silently accepted.

O001 is now manually completable in the Editor. It is **not yet certified player-ready**: active-world resume, the real-input ARIA matrix, full FA journey, a second reviewed tactical approach, cross-mode playthroughs and unfamiliar-player review remain. Known Editor teardown allocation/preview-scene warnings also remain under investigation. Android is excluded by the user's scope.

The subsequent Farsi normal-control journey also passed Victory/result/return at 439.264 seconds (7:19), seed 1102, all 16 survivors, AP 2 and no reservation. Its chronological evidence is in [manual-victory-1102-fa](O001_Integration_20260922/manual-victory-1102-fa/). This closes the previously pending full FA mission journey, but not broader shared Settings layout review or ARIA coverage. The stale Settings language label is corrected through `UiLocalizedText`; final regression evidence follows in the integration report.
