...
| Field | Status |
|---|---|
| Checklist complete | 67 / 109 |
| Checklist percent complete | 61.5% |
| Current phase | Phase 8 - Fuel/Oil ECS Drift Correction |
| Quick wins complete | 6 / 6 |
| Current target | A disabled Burst-capable `BuildingResourceHaulerTransferEcsSystem` now exposes explicit ECS-owned load/unload entry points over `BuildingResourceStorageComponent` and `UnitResourceHauler`. It is `[DisableAutoCreation]` and disabled on create, so this slice validates the ECS transfer surface without adding a parallel live hauler owner. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, focused Unity 6.5.2 ECS mirror validation `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-ecs-validation.log`, focused Unity 6.5.2 faction resource validation `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-faction-validation.log`, focused Unity 6.5.2 hauler mirror validation `/private/tmp/warline-arch-followup-fuel-oil-hauler-mirror-sync-validation.log`, focused Unity 6.5.2 resource transfer validation `/private/tmp/warline-arch-followup-resource-transfer-helper-validation.log`, and focused Unity 6.5.2 hauler ECS transfer validation `/private/tmp/warline-arch-followup-hauler-transfer-ecs-validation.log` passed with 0 compiler errors after the Fuel/Oil mirror-sync and transfer-surface slices. |
...
- Known unresolved: visual/playmode smoke validation is pending, HDR/soft-shadow tier ownership is still unresolved, Android baseline is not captured, Fuel/Oil live storage is still mirrored rather than fully authoritative on ECS entities, live hauler load/unload still enters from the managed caller, and Phase 5 still has no truthful blind Burst candidates.
- Next iteration: switch the live hauler handoff to the ECS transfer system path without creating a parallel owner, or pause Fuel/Oil drift work and return to Android baseline capture.
