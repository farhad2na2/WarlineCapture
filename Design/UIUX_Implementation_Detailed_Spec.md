...
For interaction polish, use `Design/Visual_Feedback_VFX_Recommendations.md` as the shared implementation checklist for UI motion, locked/invalid feedback, reward flyouts, popup/drawer transitions, tactical command markers, critical warning feedback, and paired audio cues from `Design/Audio_Design_Guidelines.md`.

For FTUE, contextual help, assistant recommendations, tutorial cards, highlights, and assistant takeover UI, use `Design/FTUE_And_Command_Assistant_Design.md` together with `Design/UIUX_Gameplay_Element_Alignment.md`.
...
- Direction doc: `Design/3D_SingleMap_Gameplay_Direction.md`
...
- `Design/3D_SingleMap_Gameplay_Direction.md` unless the task is explicitly a design-source update
...
Before a visible UI element is implemented, it must be checked against `Design/UIUX_Gameplay_Element_Alignment.md`. An element is not ready for production prefab work unless it has a documented gameplay purpose, route/effect or read-only/decorative role, gameplay data source, state rule, and feedback state. Elements without runtime systems must be explicitly marked `DesignedUnavailable`, `Locked`, `DevOnly`, or `ReadOnly`.
Assistant-specific visible elements must also satisfy `FTUE_And_Command_Assistant_Design.md`, especially the rules for explicit player permission, visible takeover ownership, and player-input cancellation.
...
Use `Design/Skirmish_Mode_Implementation_Spec.md` as the active source for player-facing Skirmish flow, setup controls, launch behavior, presets, result routing, and 3D single-map alignment.
...
Chapter 1 content is owned by `SagaChapters/Saga_Chapter01_First_Response.md`. UI implementation must bind nodes from that chapter document instead of hard-coded demo labels.
