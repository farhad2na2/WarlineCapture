...
{
    public static class CH02M01GridlockMediaImporter
    {
        public const string ArtRoot="Assets/Game/Art/Narrative/CH02M01Gridlock/Final";
        public static readonly string[] Panels={"O01","O02","O03","O04","O05","B01","B02","B03","C01","D01","D02","D03"};
        public const string VoiceRoot="Assets/Game/Audio/Narrative/CH02M01Gridlock/Voice";
        public static IEnumerable<GridlockNarrativeLine> Lines => CH02M01GridlockNarrativeCopy.Opening
            .Concat(CH02M01GridlockNarrativeCopy.Brief).Concat(CH02M01GridlockNarrativeCopy.Comms).Concat(CH02M01GridlockNarrativeCopy.Debrief);
        ...
        public static Sprite Panel(string id,bool wide)=>AssetDatabase.LoadAllAssetsAtPath($"{ArtRoot}/CH02-{id}.png")
            .OfType<Sprite>().Single(s=>s.name==$"CH02-{id}-{(wide ? "20x9" : "16x9")}");
        public static void ConfigureArt()
        {
            AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport);
            foreach(string id in Panels)
            {
                string path=$"{ArtRoot}/CH02-{id}.png";
                ...
                var crops=new List<SpriteMetaData>{
                    new SpriteMetaData{name=$"CH02-{id}-16x9",rect=Crop(width,height,16f/9f),alignment=0,pivot=new Vector2(.5f,.5f)},
                    new SpriteMetaData{name=$"CH02-{id}-20x9",rect=Crop(width,height,20f/9f),alignment=0,pivot=new Vector2(.5f,.5f)}};
            ...
            Debug.Log("[GridlockComicArt] result=Passed sources=12 authoredCrops=24 bilingualTextFree=1");
        }
        ...
        public static void ValidateStableArtImports()
        {
            ...
            Debug.Log("[GridlockComicArtStability] result=Passed sources=12 stableSpriteIds=24");
        }
        ...
        public static void ConfigureVoices()
        {
            ...
            for(int step=1;step<=10;step++) foreach(bool persian in new[]{false,true}) ConfigureVoice(VoicePath($"tutorial-gridlock-{step:00}",persian),persian);
            AudioRuntimeConfigAssetBuilder.BuildDefaultAssets();
            Debug.Log("[GridlockVoiceImports] result=Passed clips=44 locales=2 preload=0 runtimeNetworkTts=0");
        }
        ...
        private static void ConfigureVoice(string path,bool persian)
        {
            ...
            importer.userData="status=ELEVENLABS_PAID_CREATOR_COMMERCIAL_LICENSE; provider=ElevenLabs; model=eleven_v3; locale="+
                (persian ? "fa-IR" : "en-US")+"; manifest=gridlock_voice_manifest.json; runtimeNetworkTts=false";
