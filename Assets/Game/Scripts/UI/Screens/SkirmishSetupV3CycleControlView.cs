...
namespace Game.UI.Runtime
{
    ...
    public sealed class SkirmishSetupV3CycleControlView : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if(segmented != null) segmented.SelectionChanged += RefreshDisplay;
            if(sliderRow?.Slider != null) sliderRow.Slider.onValueChanged.AddListener(OnSliderChanged);
            RefreshDisplay();
        }
        ...
        {
            if(segmented != null) segmented.SelectionChanged -= RefreshDisplay;
            if(sliderRow?.Slider != null) sliderRow.Slider.onValueChanged.RemoveListener(OnSliderChanged);
            if (cycleButton != null)
                cycleButton.onClick.RemoveListener(Cycle);
        }

        private void OnSliderChanged(float _) => RefreshDisplay();
