...
{
    public static class BattleScenarioAd005Runner
    {
        public const string ScenarioId = "AD-005_TwoAirMissileLaunchers_InterceptTwoIncomingGroundMissiles_RadarComparison";

        private const int Seed = 52345;
        private const float FixedDeltaTime = 0.05f;
        private const float MaxDurationSeconds = 16f;
        private static readonly float3 LauncherLeftPosition = new(0f, 0f, -16f);
        private static readonly float3 LauncherRightPosition = new(0f, 0f, 16f);
        private static readonly float3 MetricsCenterPosition = new(0f, 0f, 0f);
        ...
        public static BattleScenarioVariant[] CreateDefaultVariants()
        {
            return new[]
            {
                ...
                {
                    VariantId = "AD-005-A-NoSupport-TwoLaunchersTwoGroundMissiles",
                    Label = "No Support / Two Launchers / Two Ground Missiles",
                    ...
                    IncomingThreatStartDistance = 140f,
                ...
                new BattleScenarioVariant
                {
                    VariantId = "AD-005-B-RadarNear-TwoLaunchersTwoGroundMissiles",
                    Label = "Radar Near / Two Launchers / Two Ground Missiles",
        ...
        public static BattleScenarioResult RunDefinition(BattleScenarioDefinition definition)
        {
            ...
            if (definition.ScenarioId != ScenarioId)
                throw new ArgumentException($"Unsupported AD-005 scenario id '{definition.ScenarioId}'.", nameof(definition));
            ...
            float maxDurationSeconds)
        {
            ...
            Entity[] launchers =
            {
                CreateLauncher(em, LauncherLeftPosition),
                CreateLauncher(em, LauncherRightPosition)
            };
            ...
            return BattleScenarioFixedStepRunner.RunVariant(
                ...
                (state, metrics) =>
                {
                    ...
                    CaptureLauncherMetrics(em, launchers, incomingMissiles, metrics, state);
        ...
        private static Entity[] CreateIncomingGroundMissiles(EntityManager em, BattleScenarioVariant variant)
        {
            ...
            float3 firstStart = new(variant.IncomingThreatStartDistance, variant.IncomingThreatAltitude, -18f);
            float3 secondStart = new(variant.IncomingThreatStartDistance + 46f, variant.IncomingThreatAltitude + 2f, 18f);
            return new[]
            {
        ...
        private static void CreateRadarSupportProvider(EntityManager em, float distanceFromLauncher)
        {
            BattleScenarioEcsSpawnHelpers.CreateAirDefenseSupportProvider(
                em,
                MetricsCenterPosition + new float3(distanceFromLauncher, 0f, 0f),
                FactionIdentity.PlayerFactionId,
            ...
            BattleScenarioFixedStepState state)
        {
            for (int i = 0; i < launchers.Length; i++)
            {
                ...
                metrics.LauncherEffectiveRange = math.max(metrics.LauncherEffectiveRange, launcherState.EffectiveRange);
                metrics.LauncherEffectiveLockSeconds = metrics.LauncherEffectiveLockSeconds <= 0f
                    ? launcherState.EffectiveLockSeconds
                    : math.min(metrics.LauncherEffectiveLockSeconds, launcherState.EffectiveLockSeconds);
                metrics.LauncherEffectiveTrackingQuality = math.max(
                    metrics.LauncherEffectiveTrackingQuality,
                    launcherState.EffectiveTrackingQuality);
                metrics.LauncherEffectiveTurnRateDegreesPerSecond = math.max(
                    metrics.LauncherEffectiveTurnRateDegreesPerSecond,
                    launcherState.EffectiveTurnRateDegreesPerSecond);
        ...
        private static float FindClosestActiveMissileDistance(EntityManager em, Entity[] incomingMissiles)
        {
            ...
            for (int i = 0; i < incomingMissiles.Length; i++)
            {
                ...
                float distance = math.distance(MetricsCenterPosition, missilePosition);
