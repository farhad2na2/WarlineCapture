...
- `Assets/Game/Scripts/UI/BuildingPlacementSystem.cs`: 2139 lines.
...
3. Move runtime/manual building and wall spawn wrapper methods out of the facade.
4. Move placement grid/input/preview/commit context wiring out of the facade.
5. Move UI command/query compatibility wrappers out of the facade.
6. Move selection and interaction compatibility wrappers out of the facade.
7. Replace `BuildingGameplayCompositionSystem` construction so it no longer calls `new BuildingPlacementSystem()`.
8. Migrate remaining editor validation tests to a narrow building gameplay harness.
9. Delete `Assets/Game/Scripts/UI/BuildingPlacementSystem.cs` and its `.meta`.
10. Remove facade allowlists and update the architecture contract to require zero facade references.

## Drift Guard

Until deletion:
- `BuildingPlacementSystem.cs` must not grow beyond 2139 lines.
