...
| Field | Status |
|---|---|
| Checklist complete | 77 / 119 |
| Checklist percent complete | 64.7% |
| Current phase | Phase 8 - Fuel/Oil ECS Drift Correction |
| Quick wins complete | 6 / 6 |
| Current target | Production/conversion, hauler load/revert/unload, hauler bridge source-storage reads/fuel-source selection, resource visual production flags, faction resource summaries, resource sell/drain requests, selected-building resource display, selected unit/hauler cargo display, and faction-summary publish invalidation now prefer live ECS component/read-model data where available. Runtime-building fallback/mirroring remains for compatibility. |
...
| Validation status | Phase 0 inventory completed; Phase 1 config edited; Phase 2 release OnGUI strip implemented; Phase 3 release diagnostic system gate implemented; Phase 4 GC quick wins completed; Phase 5 Burst inventory/classification report added and guardrail implemented; Phase 6 instantiate classification report added and first placement-visual pooling slice implemented; Phase 8 Fuel/Oil behavior safety-net tests added, production math extracted into a pure system-helper surface, runtime-created building combat entities receive their runtime building ids, per-building ECS resource storage mirrors are populated for resource buildings, the disabled ECS resource-production tick/apply surface has focused validation, managed production applies through the ECS surface without double-ticking, production/hauler ticks sync managed results back to `BuildingResourceStorageComponent`, hauler transfer mutation uses the component-based transfer helper, the disabled ECS hauler transfer surface has focused validation, live hauler load/revert/unload enters through that ECS transfer surface, live hauler mutation now prefers the combat entity storage component, live production/conversion now prefers the combat entity storage component too, live faction summaries/resource sell-drains now prefer the combat entity storage component, selected-building resource display now prefers the combat entity storage component, selected unit/hauler cargo display has a focused ECS cargo regression, faction-summary publish invalidation now prefers ECS storage, hauler bridge source-storage reads and fuel-source selection now prefer ECS storage, and resource visual production flags now prefer ECS storage. Latest validations passed: `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Tests.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, and `git diff --check`; Unity focused validation attempts `/private/tmp/warline-arch-followup-hauler-stored-resource-ecs-validation.log`, `/private/tmp/warline-arch-followup-resource-visual-ecs-storage-validation.log`, and `/private/tmp/warline-arch-followup-hauler-fuel-source-ecs-validation.log` were blocked before project execution by Unity 6.5.2 licensing initialization failure, not compiler errors. |
| Still wrong / next iteration | No visual defects are known from code/compile validation, but manual visual smoke checks for placement-preview pooling and command behavior are still pending. Unity focused validation is currently blocked by licensing initialization. Fuel/Oil still keeps runtime-building storage mirrors for compatibility. Next exact action: retry Unity validation when licensing is available, then continue Fuel/Oil by removing one compatibility mirror dependency or return to Android baseline capture. Android baseline is still not captured, and HDR/soft-shadow tier ownership remains unresolved. |
