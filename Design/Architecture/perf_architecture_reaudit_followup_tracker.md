...
| Field | Status |
|---|---|
| Checklist complete | 30 / 67 active, 0 skipped |
| Checklist percent complete | 44.8% active |
| Optional backlog | 0 / 5, not counted in active total |
| Current phase | Phase 3 - Mobile Visual Quality Verification |
| Current target | Run candidate mobile visual capture after the project editor is closed, then collect Android profiler evidence before changing defaults. |
...
| Mobile visual status | Current mobile tier is performance-safe but may be over-corrected visually. Phase 3 evidence plan added in `Design/AgentReports/2026-07-07_mobile_visual_quality_verification_plan.md`; current settings are `runtimeMode=High`, render scale `0.5`, shadow distance `16`, one cascade, HDR off, MSAA off/effectively 1x. Candidate config `Assets/Game/Rendering/VisualQualityConfig_MobileCandidate.asset` and candidate URP asset `Assets/Game/Rendering/Mobile_RPAsset_MobileCandidate.asset` exist for deliberate capture/testing only with render scale `0.75`, shadow distance `48`, one cascade, HDR off, and MSAA off/effectively 1x; Match remains bound to the production config. Editor-only capture utility `Assets/Game/Scripts/Editor/MobileVisualQualityPlayModeCapture.cs` captured current-tier proof through the existing Menu -> Match route: `Design/AgentReports/Captures/MobileVisualQuality/current_gameplay_zoom.png`, `current_max_zoom_out.png`, `current_night_phase.png`, and `current_manifest.md`. Candidate capture, Android profiler evidence, side-by-side review, and user sign-off remain required before default changes. |
...
| Validation status | Phase 0 `git diff --check` passed before rebase. Phase 1 rebased validation: `git diff --check` passed, `Game.Tests.Editor.csproj` passed with 0 errors, `Game.Runtime.csproj` passed with 0 errors, Unity no-Burst classification validation passed with `noBurst=73 classified=73 unclassified=0`. Phase 2 inventory found no code-change hoist targets. Phase 3 current-setting inventory/report slice passed `git diff --check`; candidate-config slice passed `git diff --check`; candidate URP asset slice passed `git diff --check`; PlayMode capture utility slice passed `git diff --check` and `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly` with 0 errors. Current capture passed in `/private/tmp/warline-mobile-visual-current-capture.log` with `result=Passed`; candidate capture was blocked before execution because another Unity editor instance had this project open. |
| Still wrong / next iteration | Fetched/rebased onto `origin/main` at `c218b124a` after the Burst inventory slice. No low-risk `Burstable as-is` no-Burst systems were found. Phase 2 found 26 direct interpolated Debug calls, all already gated, editor/development-only, warning/error, or intentional one-shot startup markers. Next iteration should run `Game.Editor.MobileVisualQualityPlayModeCapture.CaptureCandidate` after the open editor releases the project lock, inspect current/candidate screenshots side by side, then run a candidate Android profiler capture before any visual-quality default is changed. |
...
- [x] Capture current mobile settings at three fixed device viewpoints: gameplay zoom, max zoom-out, and night phase: `Design/AgentReports/Captures/MobileVisualQuality/current_gameplay_zoom.png`, `current_max_zoom_out.png`, `current_night_phase.png`, and `current_manifest.md`.
- [x] Prepare a recommended visual tier variant in config/assets without committing it as default: `Assets/Game/Rendering/VisualQualityConfig_MobileCandidate.asset` plus `Assets/Game/Rendering/Mobile_RPAsset_MobileCandidate.asset`.
- [x] Recommended starting point: renderScale `0.7-0.8`, shadowDistance `40-60`, 1-2 cascades, still no HDR/MSAA unless evidence supports it: candidate uses render scale `0.75`, shadow distance `48`, one cascade, HDR off, and MSAA off/effectively 1x.
- [ ] Capture the same three viewpoints for the recommended tier: first attempt was blocked before execution because another Unity editor instance had this project open.
