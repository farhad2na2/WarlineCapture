...
| Phase 6: Building and attachment state | In progress through VRP-062. Deterministic contiguous state-owner indices remain owned by canonical gameplay buildings, and resident destruction behavior remains unchanged. Virtualized destruction publishes globally monotonic nonzero versions through one map-owned sequence while the bounded event buffer can hold at most one transition per state owner. `OperationMapRenderStateSyncSystem` initializes and validates an exact canonical-state buffer once per map generation from authoritative destroyed-state enablement; the no-event steady-state path does not rescan placements or cells. Missing/duplicate/noncontiguous owners, invalid cell memberships, duplicate owners within a batch, gaps/divergence/overflow, and buffer counts above the owner bound fail closed, while valid changes apply in a Burst job. It marks only placements owned by changed buildings and every cell containing those placements, clears consumed events through the scheduled ECS dependency chain, and advances a revision that rebuilds the current materialized envelope with reason `VisualStateChanged`; a simultaneous camera exit uses the new camera envelope rather than losing the move. Off-camera changes persist without allocating a slot, and future selection reads current canonical state. Focused coverage proves exact initialization, bounded cleanup, off-camera retention, visible intact-to-destroyed reassignment, and fail-closed sequencing. The current render-only pilot still has zero state owners and no building family has been virtualized; complete-family ownership remains VRP-063. |
...
Checklist progress: `49 / 80` complete.
