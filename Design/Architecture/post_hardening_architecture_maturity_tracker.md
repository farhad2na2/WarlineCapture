...
| Current task | `AM-025` remains blocked from acceptance but active for remediation. Child AMFR-012 is accepted at `12 / 22` and AMFR-013 is dependency-ready. The last governed scan has `430` reviewable rows: `412` resolved, `5` protected, and `13` genuine-debt rows grouped into `12` unique file/rule items; AMFR-012's validated projection is `9` / `10`, pending governed Phase D regeneration. The original `575` rows remain historical intake. Five exact operation-map paths are handed to the AM-025 child while the broader map owner remains active. |
