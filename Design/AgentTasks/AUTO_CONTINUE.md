...
For macOS WarlineCapture workspaces, prefer `Tools/CI/invoke_unity_macos.sh` over raw `Unity -batchmode`. The wrapper quits Unity Hub's generic Licensing Client, resets stale Unity Licensing Client and Package Manager IPC before/after the run, and kills timed-out Unity process trees, preventing Codex validation attempts from leaving the user's editor or Package Manager stuck until a Mac restart. If the editor is already stuck, run `Tools/CI/reset_unity_macos_ipc.sh --quit-hub` once before retrying.
