...
| Last verified commit | `da7375f4a` on `main`; the compact APH-704 candidate remains disabled after exact `9/9` equivalence, `74.88%` benchmark improvement, zero warmed allocation, and affected architecture/build gates. |
| Last update | 2026-07-14 - Typed route, popup, build, placement, tactical-command, squad-tray, diagnostics, drawer-audio, and Resource Exchange command handling moved out of the existing unmanaged UI action request `ISystem`. The owner dropped from 885 to 300 lines without changing queries or update ownership. Focused UI behavior passes `26/26`; naming passes `9/9`; exact source authorization passes `2/2`; Runtime and Editor-test builds pass with zero errors; Menu-to-Match reaches `MatchHudReady`, completes Match Intro, and unlocks input. The global inventory is 23 unrelated oversized owners. |
