...
| Phase 6: Building and attachment state | In progress through VRP-066. Deterministic contiguous state-owner indices remain owned by 4,933 canonical gameplay buildings: 3,638 complete House owners and 1,295 complete Shop owners. Their 9,866 intact/destroyed state-linked placements coexist with 9,721 render-only Vegetation/Prop placements, producing 19,587 placements, 9,875 prototypes, 17,982 parts, 1,660 cells, and 1,364 fixed slots from 29,268 eligible renderer/material rows. Exact multi-submesh reconciliation distinguishes 29,268 logical rows from 29,081 physical source renderers; direct bake removes every selected renderer and leaves 52,928 of 82,009 physical source renderers resident. CityHall is excluded atomically: all 406 rows remain resident because 44 rows use unsupported render policy; no generated Tent or Military family exists in the current dense candidate. Focused state/destruction contracts remain green and canonical gameplay consumers remain independent of proxy assignment. Production remains `StaticSceneChunks + ResidentEntities` with `productionCutover=0`; Android House/Shop destruction-transition evidence is VRP-067 and the final 16.667 ms/60 FPS gate remains open. |
