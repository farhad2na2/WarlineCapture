...
It must stay aligned with `WarlineCapture_Strategic_Tactical_Map_Gameplay_Alignment.md`; that document owns the shared strategic-preview versus tactical-playable-map contract.

For the first production slice, the UI agent must also read `WarlineCapture_M01_FirstContact_Production_Contract.md`. That document owns the concrete M01 ids, anchors, command reason codes, FTUE targets, UI feedback, and validation gates.
...
1. Use `WarlineCapture_M01_FirstContact_Production_Contract.md` as the first concrete UI target.
