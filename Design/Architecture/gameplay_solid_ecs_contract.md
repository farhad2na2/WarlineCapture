...
- Citizen population building read paths must use `BuildingRuntimeQuerySystem` for building lists, positions, destroyed state, refugee settings, and approach cells. Citizen upkeep spending belongs in `CitizenResourceSystem`, and citizen configured prefab/entity resolution belongs in `CitizenPrefabSystem`; `BuildingPlacementSystem` may remain only as temporary composition context debt until those contexts move to a non-facade runtime boundary.
