...
namespace Game.UI.Runtime
{
    public sealed partial class CampaignOperationsScreenView
    {
        ...
        private void ApplyMissionGoals(string missionId)
        {
            ...
            bool route=missionId==Game.Missions.Contracts.CampaignMissionSequence.RouteReopened;
            bool power=missionId==Game.Missions.Contracts.CampaignMissionSequence.PowerRelay;
            if(m3 || m4 || m5 || gridlock || supply || market || power || route)
            {
                string prefix = route ? "mission.route_reopened." : power ? "mission.power_relay." : market ? "mission.market_lifeline." : supply ? "mission.supply_line." : gridlock ? "mission.gridlock." : m5 ? "mission.m05." : m3 ? "mission.m03." : "mission.m04.";
                string[] objectives = route ? new[] { "lifelines", "link", "records" } : power ? new[] { "shelter", "restore", "secure" } : market ? new[] { "delivery", "manifest", "market" } : supply ? new[] { "oil", "fuel", "reserve" } : gridlock ? new[] { "site_a", "site_b", "delivery" } : m5 ? new[] { "gate", "core", "archive" } : m3 ? new[] { "stop_convoy", "protect_post", "prevent_breach" } : new[] { "extract", "transport", "landing" };
