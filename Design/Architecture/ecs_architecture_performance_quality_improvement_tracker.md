...
- Runtime callers to keep behavior-compatible: `SelectionGameplayStartupSystem` owns the command-system instance, `RtsSelectionCommandResultFlushCompositionSystemHelper` drains command buffers through `ProcessCommandIntentRequests`, `RtsSelectionPointerTargetCommandSystem` uses click/preview/capacity helpers, `RtsSelectionBoardTargetModeCommandSystem` uses target classification helpers, `UnitTransportDeployOrderSystem` uses deploy-disembark and direct-cell helpers, and `UnitTransportAirPickupSystem` uses soldier and air-pickup helpers.
