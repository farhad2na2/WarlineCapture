# Runs Operations P3 Unity validation against the shadow project only.
[CmdletBinding()]
param(
    [string] $ShadowPath = "D:\Projects\WarlineCapture-Operations",
    [string] $SharedPath = "D:\Projects\WarlineCapture",
    [string] $LogFile = "$env:TEMP\operations-p3-shadow.log",
...
if (Test-Path -LiteralPath $sharedLibrary) {
    Write-Host "[OperationsP3] leaving_shared_library_untouched=$sharedLibrary"
}
...
Write-Host "[OperationsP3] project=$shadow"
Write-Host "[OperationsP3] unity=$unityExe"
Write-Host "[OperationsP3] log=$LogFile"
...
& $validateScript `
    -UnityExe $unityExe `
    -ProjectPath $shadow `
    -ExecuteMethod "Game.Tests.Editor.Operations.OperationsP3Validation.RunFocusedValidation" `
    -LogFile $LogFile `
    -RequiredPassMarker "[OperationsP3Validation] result=Passed checks=12" `
    -GuiLicensing:$gui `
    -TimeoutSeconds $TimeoutSeconds

if ($LASTEXITCODE -ne 0) {
    throw "Operations P3 shadow validation failed."
}

Write-Host "[OperationsP3] result=Passed project=$shadow"
