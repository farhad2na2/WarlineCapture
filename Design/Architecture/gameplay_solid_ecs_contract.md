...
- Building visual helper behavior, animated-part discovery, and animated-part updates belong in `BuildingVisualSystem`; runtime building visual initialization and runtime resource animation updates belong in `BuildingRuntimeVisualSystem`; building selection marker projection belongs in `BuildingSelectionMarkerSystem`. Building prefabs under `Assets/Game/Prefabs/Buildings` must not contain `SelectionMarker` children; unit selection marker authoring remains allowed for character prefabs.
