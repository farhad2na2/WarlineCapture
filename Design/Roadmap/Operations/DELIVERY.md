...
P0 contracts, the P1 city/profile transaction core, and the P2 tactical rules are **implemented**. P3 and later stay **Planned**. Host markers are not Unity Editor, device, or ARIA validation. The implementation lead owns integration, scope and evidence; content agents must not independently modify shared contracts.
