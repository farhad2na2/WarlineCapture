...
| Field | Status |
|---|---|
| Checklist complete | 79 / 119 |
| Checklist percent complete | 66.4% |
| Current phase | Phase 8 - Managed Helper Hot-path Migration |
| Quick wins complete | 6 / 6 |
| Current target | Fuel/Oil live production/storage/transfer/conversion now uses ECS-preferred storage with runtime-building mirrors retained for compatibility, and production/runtime helper tick markers are standardized below the existing placement-tick markers for pending production, active transports, resource production, resource-storage mirror sync, resource haulers, and spawn reservation cleanup. |
| Compiler status | Latest marker slice passed `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Tests.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, and Unity 6.5.2 focused validation `BuildingResourceProductionEcsSystemTests.RunFocusedValidation` in `/private/tmp/warline-arch-followup-production-tick-markers-validation-escalated.log` with 0 compiler errors. Earlier Fuel/Oil ECS mirror, transfer, display, and publish-invalidation validations also passed. |
...
| Validation status | Phase 0 inventory completed; Phase 1 config edited; Phase 2 release OnGUI strip implemented; Phase 3 release diagnostic system gate implemented; Phase 4 GC quick wins completed; Phase 5 Burst inventory/classification report added and guardrail implemented; Phase 6 instantiate classification report added and first placement-visual pooling slice implemented; Phase 8 Fuel/Oil behavior safety-net tests added, production math extracted into a pure system-helper surface, ECS storage preference added across production, haulers, faction summaries, selection display, resource visuals, and publish invalidation, and production/runtime helper profiler markers standardized. Latest validations passed: `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Tests.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, and Unity focused validation `/private/tmp/warline-arch-followup-production-tick-markers-validation-escalated.log`. |
| Still wrong / next iteration | No visual defects are known from code/compile validation, but manual visual smoke checks for placement-preview pooling and command behavior are still pending. Fuel/Oil still keeps runtime-building storage mirrors for compatibility. Next exact action: start the measured AttackVfx pass only after confirming it is still the largest spike, then move request processing toward ECS/jobs while keeping visual spawning at the presentation edge. Android baseline is still not captured, and HDR/soft-shadow tier ownership remains unresolved. |
