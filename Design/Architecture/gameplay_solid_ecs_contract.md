...
Managed gameplay runtime update orchestration is owned by `GameplayRuntimeUpdateSystem`. `GameBootstrap` may call that system from Unity lifecycle methods and pass current composed references into it, but it must not own the long managed `Update`, `LateUpdate`, or `OnGUI` step sequence directly. Building runtime updates inside that loop must go through `BuildingRuntimeUpdateSystem`, not a direct `BuildingPlacementSystem` facade parameter. `BuildingRuntimeUpdateSystem` ownership and context construction belong in managed composition, not inside `BuildingPlacementSystem`. `GameBootstrap` must not expose a public `BuildingPlacementSystem` facade property; temporary facade access is private composition debt only until callers migrate to narrower boundaries.
Managed building gameplay composition is owned by `BuildingGameplayCompositionSystem`. `ManagedGameplayStartupSystem` may consume that composition result, but it must not reach through `BuildingPlacementSystem` to retrieve child systems, contexts, runtime update delegates, interaction boundaries, citizen resource contexts, or prefab contexts.
