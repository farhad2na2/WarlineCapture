...
| Virtualized render database bake config | `OperationMapRenderDatabaseBuilder` | `Assets/Game/GeneratedOperationMapEntityPresentationCandidate/VirtualizedPresentation/OperationMapRenderDatabaseBakeConfig.asset` | `OperationMapEntitySceneCandidateBakeAll.CandidateFileTransaction` journals the config, `.meta`, and report together. Failure calls transaction rollback and revalidates the protected-production snapshot, including the frozen rollback root. | Regenerate only from accepted stable records and the deterministic builder. Never hand-create/edit it as source, reference the ScriptableObject from player runtime after baking, or promote it outside the candidate transaction. Accepted serialized SHA-256 is `047e07cc77d3a576df6c2ee25c78f93bdebdb57d3c1ffda2da6424a24ffc2a06`. |
| Virtualized render database report | `OperationMapRenderDatabaseBuilder` with `OperationMapRenderVirtualizationReportSerializer` | `Design/AgentReports/2026-07-28_dense_city_render_virtualization_database.json` | Same file transaction as the bake config; the writer publishes atomically and validates the persisted report against the config before commit. | Regenerate with the exact config/content hash and revision. Never repair counts, identities, policy capacities, or hashes manually. Accepted content/order hashes are `bfb350f0c8d1474aa05252dc04c87eede4c1210adcee9c92dcdbecc35897896e` / `a43040ee38b9e8cfe752f1e52848cfa523e453fa2ebcde0cadc4142510d79318`. |
| Virtualized presentation authoring root | `OperationMapRenderVirtualizationPilotEnabler` | Exactly one `OperationMapVirtualizedPresentationAuthoring` in the dense candidate EntityScene authoring scene | The enablement transaction journals the dense candidate definition and candidate scene together and restores both on failure. | Re-run the checked enabler/validator. Do not add a second root, bind a different database by hand, or change the production definition. |
...
### Accepted Virtualized Output Identity

The accepted current candidate is schema `1`, operation-map id `opmap.skirmish.desert_base_01`, and contains `88` meshes, `24` materials, `9,107` prototypes, `12,293` parts, `40,460` placements, `1,934` cells, `60,922` cell-placement indices, four fixed policy buckets, and `7,784` slots. The persisted nine-file logical set is `131,560,240` bytes with SHA-256 `efe4a13cc8ca7212dc6dcacf0615cb9d7b479a02545873cdd36ee667f1498c21`; two complete Bake All passes reproduce packed fingerprint `23bebb4580aef31d7cd666b7f300f207bc8ccca1df5ce732d132b703400a655a`. Materialized parity covers `14,592` canonical envelopes in two visual states (`29,184` samples), peaks at `4,773/7,784` slots, and has fingerprint `7674a2c08142bb2fa41969de8d39c67ac16076fd746566f4bc218713ec7e0b18`.

These identities describe candidate output only. A mismatch invalidates the candidate evidence; it is not permission to copy old generated bytes forward. Regenerate through the owner and repeat dependent validation.
