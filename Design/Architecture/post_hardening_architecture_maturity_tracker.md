...
| Current task | `AM-025` blocked from acceptance but active for remediation; all `575` historical intake rows now have row-bound authority, producing `407` non-debt and `168` genuine-debt rows grouped into `80` unique file/rule items, while source-growth exposes five exact external helper paths |
