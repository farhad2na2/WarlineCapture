...
- In progress: `0`.
- Remaining open: `0`.
- Current target: `Phase 7 single-thread domain execution: Integration P7-0374 VisibleUnitSelectionCameraSystemHelper split completed; only held Agent B RetireFold rows P7-0003/P7-0019 remain pending an explicit managed-reference boundary guardrail/model change because direct per-instance folding would break world-scoped sharing`.
...
- [x] Replace `BuildingUiQueryUiSystemHelper` runtime-boundary `ToEntityArray` snapshot with first-chunk entity lookup.
...
- Selection/order/building marker pooling slice: building selection already reuses one marker, move/attack command markers already cache one marker each, attack target selection/ring markers remain cached, and attack/board preview markers now prewarm the full `MaxAttackTargetPreviewMarkers` pool during `SelectionOrderMarkerPresentationSystemHelper.Initialize` instead of instantiating during hover preview updates.
- Marker reuse coverage was added to `SelectionOrderMarkerPresentationSystemHelperTests.Initialize_PrewarmsAttackTargetPreviewMarkerPool`; focused order-marker validation passed with `/private/tmp/warline-ecs-audit-selection-order-marker-pool-validation.log` (`SelectionOrderMarkerFocusedValidation`, `tests=15`).
...
- Runtime callers to keep behavior-compatible: `SelectionGameplayStartupSystemHelper` owns the command-system instance, `RtsSelectionCommandResultFlushCompositionSystemHelper` drains command buffers through `ProcessCommandIntentRequests`, `RtsSelectionPointerTargetCommandCompositionSystemHelper` uses click/preview/capacity helpers, `RtsSelectionBoardTargetModeCommandSystem` uses target classification helpers, `UnitTransportDeployOrderSystem` uses deploy-disembark and direct-cell helpers, and `UnitTransportAirPickupSystem` uses soldier and air-pickup helpers.
...
| C | Selection and command data | `RtsSelection*`, `SelectionStateCompositionSystemHelper`, `FocusableUnitLookupSystem`, command-result flush/read models. | Keep pointer/camera/UI input passive; convert command/result processing and selected-state mutations. |
...
- [ ] Treat `Design/Architecture/five_systembase_to_isystem_conversion_tracker.md` as the required sub-track for `BuildingSpawnCompositionSystemHelper`, `BuildingProductionTransportBridgeCompositionSystemHelper`, `CitizenVisibleUnitPresentationSystemHelper`, `MapVehiclePlacementSpawnPrefabSystemHelper`, and `CustomGameStartupSystem`.
...
- [x] Fold and rename Agent E `P7-0232 RoadPathPlanningUtilitySystemHelper` from a disabled `SystemBase` wrapper into a plain direct-owned road-path planning helper and validate road-build command behavior, compile, and Phase 7 architecture guard.
- [x] Fold and rename Agent E `P7-0237 RoadSurfacePlacementUtilitySystemHelper` from a disabled `SystemBase` wrapper into a plain direct-owned road-surface placement helper and validate road-build command behavior, compile, and Phase 7 architecture guard.
...
- [x] Fold Agent E `P7-0186 CitizenDangerCompositionSystemHelper` from a disabled `SystemBase` wrapper into a plain direct-owned citizen danger helper and validate citizen movement, visible citizen, compile, inventory regeneration, `git diff --check`, and Phase 7 architecture guard.
...
- [x] Fold and rename Agent E `P7-0236 RoadRuntimeRootSceneSystemHelper` from a disabled `SystemBase` wrapper into a plain road runtime root helper and validate road build command behavior, compile, inventory regeneration, `git diff --check`, and Phase 7 architecture guard.
...
- [x] Fold Agent F final camera helpers `P7-0278 RtsSelectionRuntimeCameraSystemHelper` and `P7-0282 SelectionUiCameraSystemHelper` from disabled `SystemBase` wrappers into plain camera coordination helpers and validate RTS camera behavior, runtime camera references, compile, inventory regeneration, `git diff --check`, and Phase 7 architecture guard.
- [x] Fold Agent F `P7-0273 RoadVisualRefreshPresentationSystemHelper` from a disabled static-helper `SystemBase` wrapper into a plain road visual refresh helper and validate road-build command behavior, compile, inventory regeneration, `git diff --check`, and Phase 7 architecture guard.
...
- [x] Fold Agent F `P7-0264 BuildingSelectionMarkerPresentationSystemHelper` from a disabled `SystemBase` wrapper into a plain direct-owned building selection marker helper and validate building selection marker behavior, compile, inventory regeneration, `git diff --check`, and Phase 7 architecture guard.
- [x] Fold and rename Agent F `P7-0270 RoadBuildVisualContextPresentationSystemHelper` from a disabled `SystemBase` wrapper into a plain direct-owned road build visual context helper and validate road-build command behavior, compile, inventory regeneration, `git diff --check`, and Phase 7 architecture guard.
- [x] Fold Agent F `P7-0280 SelectionOrderMarkerPresentationSystemHelper` from a disabled `SystemBase` wrapper into a plain direct-owned selection order marker helper and validate selection order marker behavior, compile, inventory regeneration, `git diff --check`, and Phase 7 architecture guard.
...
- [x] Rename Integration `P7-0325 RuntimeRootSystem` to `RuntimeRootSceneSystemHelper` with preserved `.meta` GUID, compile validation, and architecture validation.
- [x] Split Integration `P7-0374 VisibleUnitSelectionCameraSystemHelper` by moving visible unit candidate collection into `VisibleUnitSelectionCandidateSystem : ISystem`, keeping managed camera screen filtering in the direct helper, and validating focused selection-state behavior, compile, inventory regeneration, `git diff --check`, and Phase 7 architecture guard.
...
- [x] Rename Integration `P7-0324 RuntimeResourceSystem` to `RuntimeResourceUtilitySystemHelper` with preserved `.meta` GUID, focused building gameplay composition validation, and architecture validation.
...
- [x] Rename Integration `P7-0328 SceneLifecycleSystem` to `SceneLifecycleSceneSystemHelper` with preserved `.meta` GUID, focused scene lifecycle queue validation, and architecture validation.
- [x] Rename Agent C `P7-0044 SelectedUnitOrderSnapshotSystem` to `SelectedUnitOrderSnapshotCompositionSystemHelper` with preserved `.meta` GUID, focused selected-order snapshot validation, and architecture validation.
- [x] Rename Agent C `P7-0045 SelectionBuildingInteractionSystem` to `SelectionBuildingInteractionCompositionSystemHelper` with preserved `.meta` GUID, focused selection/input validation, and architecture validation.
- [x] Rename Agent C `P7-0046 SelectionGameplayStartupSystem` to `SelectionGameplayStartupSystemHelper` with preserved `.meta` GUID, focused selection/input validation, and architecture validation.
...
- [x] Fold and rename Agent E `P7-0235 RoadRuntimeGenerationCompositionSystemHelper` from a disabled `SystemBase` wrapper into a plain direct-owned runtime road generation helper and validate runtime city generation, road-build command behavior, compile, and Phase 7 architecture guard.
