...
| Current target | First current-branch Android profiler baseline is captured/exported and compared against the earlier reference capture. The exporter now records saved-capture render counters where Unity exposes them, and the live runtime diagnostics helper now resolves render `ProfilerRecorder` names through exact names plus Unity's available recorder list so draw-call/batch logs are less likely to degrade to `-1` after Unity-version/platform counter drift. The next exact action is to capture a live Android `[FrameRateDiag]` / `[RenderSceneDiag]` sample for draw calls and batches, or run the longer 10-minute/thermal-soak capture before deciding whether HDR/soft-shadow tier wiring needs another config slice. |
| Compiler status | Latest live-render-recorder hardening slice passed `git diff --check`, `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, and `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly` with 0 errors. Unity 6.5.2 profiler export reruns passed in the prior saved-capture exporter slice. |
...
- Current limitation: this makes the live diagnostic capture path more reliable but does not itself satisfy Phase 7's draw-call/batch evidence requirement. The open validation remains a live Android log/sample showing non-invalid draw-call and batch values during a match.
- Validation: `git diff --check`, `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, and `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly` passed with 0 errors.
