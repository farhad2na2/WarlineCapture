...
| Current task | `AM-025` remains blocked from acceptance but active for remediation. Selection gameplay and UI commands now share one input-state owner supplied by the active match instead of finding a global World. The `575` historical rows contain `561` non-debt and `14` genuine-debt rows grouped into `13` unique file/rule items. Operation-map work remains separately owned. |
...
| Blockers | Operation-map source growth remains externally owned while that work is active. Row-bound evidence still exposes `14` genuine-debt rows in `13` unique file/rule items that must be remediated to zero. Release-only certification remains intentionally deferred. |
