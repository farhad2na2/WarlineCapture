...
| Field | Status |
|---|---|
| Checklist complete | 70 / 112 |
| Checklist percent complete | 62.5% |
| Current phase | Phase 8 - Fuel/Oil ECS Drift Correction |
| Quick wins complete | 6 / 6 |
| Current target | Production/conversion and hauler load/revert/unload now prefer the live `BuildingResourceStorageComponent` on each runtime building combat entity, commit mutation to that ECS component, and mirror the result back to the runtime-building object for compatibility. Runtime-building fallback remains for buildings without an ECS storage component. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, focused Unity 6.5.2 ECS mirror validation `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-ecs-validation.log`, focused Unity 6.5.2 faction resource validation `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-faction-validation.log`, focused Unity 6.5.2 hauler mirror validation `/private/tmp/warline-arch-followup-fuel-oil-hauler-mirror-sync-validation.log`, focused Unity 6.5.2 resource transfer validation `/private/tmp/warline-arch-followup-resource-transfer-helper-validation.log`, focused Unity 6.5.2 hauler ECS transfer validation `/private/tmp/warline-arch-followup-hauler-transfer-ecs-validation.log`, focused Unity 6.5.2 live hauler ECS transfer validation `/private/tmp/warline-arch-followup-hauler-live-ecs-transfer-validation.log`, focused Unity 6.5.2 live ECS-storage-preferred hauler validation `/private/tmp/warline-arch-followup-hauler-live-storage-prefer-ecs-validation.log`, and focused Unity 6.5.2 live ECS-storage-preferred production validation `/private/tmp/warline-arch-followup-production-live-storage-prefer-ecs-validation.log` passed with 0 compiler errors after the Fuel/Oil mirror-sync and transfer-surface slices. |
...
- Next iteration: audit faction resource drains/totals/UI projection reads for ECS preference, or pause Fuel/Oil drift work and return to Android baseline capture.
