...
- SCN-08 command button interaction repair completed after the command-size fix: Hold and Stop remain interactable so hover/click feedback is not suppressed by the read model, while the existing `TryAcceptCapability` rejection path still handles unavailable units. Hold, Stop, and Scan visible frame Images are raycast targets so the selected/hover target stays on the premium chrome instead of falling back to an invisible hit Image. Shadow captures passed for `HoldCommand`, `StopCommand`, and `ScanCommand` under `Design/VisualLockLayered/_CanvasTargetLockVisualMatch/SCN-08_MatchHUD/command_button_interaction_fix/`.
- SCN-08 selected-unit portrait black-panel guard added: `MatchHudSelectionPanelView.SetSelectionVisible(true, portraitSprite)` now falls back to the configured generic squad portrait when the immediate selection feedback path receives a null portrait from the prefab resolver, so selecting units or multi-selecting cannot leave the left-side portrait Image disabled over the black backing while the richer selection model catches up. Shadow Match HUD route capture passed after the change under `Design/VisualLockLayered/_CanvasTargetLockVisualMatch/SCN-08_MatchHUD/selection_portrait_fix/`.
- SCN-08 Board command relocation slice: the existing serialized `BoardButton`/`boardAction` binding was moved from `LeftContent/SelectedSquadPanel/Frame/CommandButtons` into `FooterContent/CommandRail/Frame` immediately after `ScanCommand`. The button now uses the same 214x265 command rail footprint, full-frame Target Lock command chrome, command-sized icon/label rhythm, visible-frame Button target graphic, and highlighted/pressed/selected/disabled SpriteSwap state pair as the bottom command buttons. Graphics-enabled shadow capture selected `BoardButton` and passed at `Design/VisualLockLayered/_CanvasTargetLockVisualMatch/SCN-08_MatchHUD/board_command_relocation/shadow_canvas_scn08_board_button_relocated_graphics_1920x1080.png`.
- SCN-08 Board command feedback repair completed after relocation: the rail `BoardButton` remains interactable even when the selection panel is hidden so the command system can show no-selection feedback instead of silently doing nothing. No selected unit now rejects with `Select a unit first.`; passenger/unit board mode prompts `Select a transport.`; transport board mode prompts `Select units to board or use BOARD ALL.` and preserves the existing ECS follow-up boarding path. First validation missed a live shell issue because it clicked the whole source prefab; the runtime clones the Footer section separately. The footer command controls now serialize and bind `BoardButton` directly through `MatchOverlayCommandInputUiSystemHelper`, while the old selection-panel board binding remains only as compatibility fallback. Focused shadow validations passed: `/private/tmp/warline-matchhud-board-rail-command-fix.log` (`MatchHudCommandControlsCurrentPrefabValidation`, `6` checks including the section-installed Footer BoardButton), `/private/tmp/warline-matchhud-board-feedback-panel-after-rail-fix.log` (`MatchHudCommandFeedbackValidation`, `13` checks), and `/private/tmp/warline-selection-command-board-feedback-after-rail-fix.log` (`SelectionCommandBoardFeedbackValidation`, `1` check).
...
| SCN-01 Loading | `Assets/Game/Prefabs/UI/Shell/Content/SCN01_LoadingContent.prefab` | `Design/VisualLockLayered/SCN-01_SplashLoading/reference/SCN-01_SplashLoadingV3_Final_Target.png` |
...
| SCN-09 Build Drawer Popup | `Assets/Game/Prefabs/UI/Shell/Popups/SCN09_BuildDrawerPopup.prefab` | `Design/VisualLockLayered/SCN-09_BuildDrawer/reference/SCN-09_BuildDrawer_MatchBuildPopupV3_SharpSolid_Target.png` |
