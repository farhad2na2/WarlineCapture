    ...
    public sealed class InboxV3View : UIScreenView
    {
        ...
        private void MarkAttachmentReady(int slot)
        {
            if (attachmentStates == null || slot < 0 || slot >= attachmentStates.Length || attachmentStates[slot] == null)
                return;
            attachmentStates[slot].text = UiShellRuntimeGateway.Localization.Get("ui.inbox.open_via_intel", "OPEN VIA INTEL");
        }
        ...
        private void RefreshList(bool resetSelection)
        {
            ...
            for (int i = 0; i < Messages.Length; i++)
            {
                ...
                if (_unreadOnly && !_unread[i])
                    continue;
                string localizedTitle = UiShellRuntimeGateway.Localization.GetBySource(message.Title);
                string localizedFrom = UiShellRuntimeGateway.Localization.GetBySource(message.From);
                string localizedBody = UiShellRuntimeGateway.Localization.GetBySource(message.Body);
                if (query.Length > 0 &&
            ...
            for (int i = 0; i < (messageButtons?.Length ?? 0); i++)
            {
                ...
                if (messageTitles != null && i < messageTitles.Length && messageTitles[i] != null) messageTitles[i].text = UiShellRuntimeGateway.Localization.GetBySource(message.Title);
                if (messageSenders != null && i < messageSenders.Length && messageSenders[i] != null)
                    messageSenders[i].text = UiShellRuntimeGateway.Localization.Format("ui.inbox.from", "From: {0}", message.From);
                if (messageTimes != null && i < messageTimes.Length && messageTimes[i] != null)
                {
                    ...
                    messageTimes[i].text = separator >= 0
                        ...
                        : UiShellRuntimeGateway.Localization.GetBySource(message.Time);
                }
            ...
            if (sortLabel != null)
                sortLabel.text = _oldestFirst
                    ? UiShellRuntimeGateway.Localization.Get("ui.common.oldest", "OLDEST")
                    : UiShellRuntimeGateway.Localization.GetBySource("NEWEST");
            if (filterLabel != null)
                filterLabel.text = _unreadOnly
                    ? UiShellRuntimeGateway.Localization.Get("ui.common.unread", "UNREAD")
                    : UiShellRuntimeGateway.Localization.Get("ui.common.filters", "FILTERS");
            RefreshCategories();
            RefreshDetail();
        }
        ...
        private void RefreshDetail()
        {
            ...
            if (detailTitle != null) detailTitle.text = UiShellRuntimeGateway.Localization.GetBySource(selected.Title);
            if (detailFrom != null)
                detailFrom.text = UiShellRuntimeGateway.Localization.Format(
                    "ui.inbox.from_highlight",
                    "From: <color=#77B936>{0}</color>",
                    selected.From);
            if (detailDate != null) detailDate.text = UiShellRuntimeGateway.Localization.GetBySource(selected.Time);
            if (detailBody != null) detailBody.text = UiShellRuntimeGateway.Localization.GetBySource(selected.Body);
            ...
            if (markReadLabel != null)
                markReadLabel.text = _unread[_selectedMessageIndex]
                    ? UiShellRuntimeGateway.Localization.Get("ui.common.mark_read", "MARK READ")
                    : UiShellRuntimeGateway.Localization.Get("ui.common.marked_read", "MARKED READ");
            ...
            for (int i = 0; i < 2; i++)
            {
                if (attachmentTitles != null && i < attachmentTitles.Length && attachmentTitles[i] != null) attachmentTitles[i].text = UiShellRuntimeGateway.Localization.GetBySource(titles[i]);
