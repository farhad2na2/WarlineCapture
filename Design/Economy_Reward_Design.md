...
| `Materials` | Construction, repair, infrastructure, and upgrade resource. | Local Field Fabrication Depot conversion from tactical Oil, mission rewards, construction objectives, Operation repair/aide routes, chapter rewards, store bundles. | Building/outpost construction, district repair, gear/module upgrades. | Match resource strip, Main Menu resource strip, Build Drawer, Operation Dashboard, District Detail, End of Day, Store. |
...
- Tactical `Materials` is one faction ECS value used by fabrication, construction, repair, Exchange, AI, and the Match HUD. It is distinct from persistent profile Materials.
...
- `Field_Fabrication_Materials_Design.md` owns the tactical Oil -> Materials branch, the Field Fabrication Depot role, one canonical faction tactical Materials value, and Credits + Materials battlefield construction costs.
- `Resource_Logistics_Exchange_Design.md` owns the optional timed match Resource Exchange popup. It can export surplus tactical Oil/Materials/Fuel into Credits or import Materials/Fuel with Credits through authored recipes, queue timers, Rush Ticket acceleration, and economy-event logging. Materials import is expensive emergency recovery, not the primary Materials source. It is not the Store and it is not instant free conversion.
...
| `Materials` | Tactical Field Fabrication Depot conversion from delivered Oil, mission rewards, build/construction objectives, Operation repair routes, chapter rewards, profile milestones, season nodes, event nodes, store resource bundles. | Building/outpost construction, district repair, gear/module upgrades, repair convoy crafting or action costs. No passive account decay. | Construction, repair, infrastructure recovery, upgrade crafting. | Tactical Oil converts into tactical Materials only through an authored Field Fabrication Depot. Materials can convert into `OperationInfrastructure` only by authored Operation repair actions. Materials do not convert directly into Command Authority. | Store may sell capped account Material bundles. Active-match Exchange import remains an authored expensive recovery route and does not replace local fabrication. | `FabricationOilPerCycle`, `FabricationMaterialsPerCycle`, `FabricationCycleSeconds`, `TacticalMaterialsCapacity`, `BaseMaterialGrant`, `ConstructionObjectiveBonus`, `RepairActionMaterialCost`, `UpgradeMaterialCurve`, `MaterialBundleCap`, `InfrastructureDeltaPerMaterialBand`. |
...
| `Credits` | Tactical `Materials` or `Fuel` | Resource Logistics Exchange import job. | Allowed only when the active match enables `Resource_Logistics_Exchange_Design.md`; Credits are spent at confirmation and output is granted on timed completion, respecting storage/capacity rules. Materials import is priced and timed as emergency recovery and must be less efficient than local fabrication. |
