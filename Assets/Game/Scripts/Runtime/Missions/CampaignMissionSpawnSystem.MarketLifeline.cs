...
namespace Game.Runtime
{
    public partial struct CampaignMissionSpawnSystem
    {
        private static void InitializeMarketLifelineAttempt(EntityManager em,Entity root,ref CampaignMissionDefinitionBlob definition,ref OperationMapBlob map,in CampaignMissionRuntimeComponent runtime)
        {
            ...
            SetOrAdd(em,root,new CampaignMissionMarketLifelineState {DeliveryCell=ToGridCell(delivery.Position,map.Grid),ManifestCell=ToGridCell(manifest.Position,map.Grid),SessionToken=runtime.SessionToken,AttemptOrdinal=runtime.AttemptOrdinal,SourceVersion=runtime.SourceVersion});
        ...
        {
            if(definition.MarketLifeline.Enabled==0)return;
            byte kind=group.FactionId==2?(byte)2:unit.MissionRoleId.Equals(new FixedString64Bytes("role.market.relief_convoy"))?(byte)1:(byte)0;
            if(kind==1 && em.HasComponent<UnitFuelConsumption>(instance)){var fuel=em.GetComponentData<UnitFuelConsumption>(instance);fuel.Enabled=0;em.SetComponentData(instance,fuel);}
