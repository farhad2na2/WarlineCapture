# Runs Operations P2 Unity validation against the shadow project only.
[CmdletBinding()]
param(
    [string] $ShadowPath = "D:\Projects\WarlineCapture-Operations",
    [string] $SharedPath = "D:\Projects\WarlineCapture",
    [string] $LogFile = "$env:TEMP\operations-p2-shadow.log",
...
if (Test-Path -LiteralPath $sharedLibrary) {
    Write-Host "[OperationsP2] leaving_shared_library_untouched=$sharedLibrary"
}
...
Write-Host "[OperationsP2] project=$shadow"
Write-Host "[OperationsP2] unity=$unityExe"
Write-Host "[OperationsP2] log=$LogFile"
...
& $validateScript `
    -UnityExe $unityExe `
    -ProjectPath $shadow `
    -ExecuteMethod "Game.Tests.Editor.Operations.OperationsP2Validation.RunFocusedValidation" `
    -LogFile $LogFile `
    -RequiredPassMarker "[OperationsP2Validation] result=Passed checks=16" `
    -GuiLicensing:$gui `
    -TimeoutSeconds $TimeoutSeconds

if ($LASTEXITCODE -ne 0) {
    throw "Operations P2 shadow validation failed."
}

Write-Host "[OperationsP2] result=Passed project=$shadow"
