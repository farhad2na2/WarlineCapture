...
Date: 2026-07-28
Status: Candidate implementation in progress through bounded Burst cell selection, deduplicated canonical-state filtering, and deterministic logical-row expansion; persisted candidate and production cutover remain disabled
...
| Phase 4: Jobified runtime assignment | In progress; the ordered camera boundary, exact map-generation native initialization, and pure guard-envelope decision are accepted. Two Burst `IJob` stages now scan only the precomputed cell table for an inclusive required envelope, traverse only selected cell ranges, deduplicate multi-cell placements with a preallocated bitset, filter state-owned placements against supplied canonical intact/destroyed state, globally sort placement indices, and expand selected prototypes into deterministic placement/part/pool-bucket logical-row keys. Explicit cell, placement, and logical-row maxima prevent allocator capacity rounding from becoming an unbounded policy; invalid blobs, envelopes, cell ranges, placement/state ownership, prototype ranges, pool buckets, or capacity deficits clear output and return a closed failure code. Runtime scheduling/wiring, deterministic retain/release/assignment, apply, metrics publication, and full unload/job-lifetime proof remain open. |
...
Checklist progress: `35 / 80` complete.
