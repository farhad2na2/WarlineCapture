    ...
    internal sealed partial class SelectionGameplayStartupSystemHelper
    {
            ...
            IMatchIntroStateQuery matchIntroStateQuery)
        {
            ...
            void UpdateSelectionRuntimePhases()
            {
                ...
                using (SelectionTacticalCameraMarker.Auto())
                {
                    if (runtimeGameplayStateSystem.InitialCameraFocusRequested &&
                        TryGetEntityManager(out EntityManager cameraPresentationEntityManager))
                    {
                        tacticalFollowCameraModeSystem.YieldToExternalCameraPresentation(
                            cameraPresentationEntityManager,
            ...
            bool ShouldRefreshSelectionUi()
            {
                ...
                return selectionStateSystem.SelectionVersion != lastSelectionUiRefreshVersion ||
                       now >= nextSelectionUiRefreshAt;
            }

            void MarkSelectionUiRefreshed()
            {
                lastSelectionUiRefreshVersion = selectionStateSystem.SelectionVersion;
