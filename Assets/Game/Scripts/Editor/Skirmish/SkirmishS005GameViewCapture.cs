...
{
    [InitializeOnLoad]
    public static class SkirmishS005GameViewCapture
    {
        private const string Key = "Warline.S005.GameViewCapture";
        ...
        static SkirmishS005GameViewCapture()
        {
            if (SessionState.GetBool(Key, false))
                EditorApplication.update += Tick;
        }

        [MenuItem("Tools/Warline/Skirmish/Launch S005 Regular Standard Game View")]
        ...
        public static void LaunchRegularStandard()
        {
            if (EditorApplication.isPlaying)
                throw new InvalidOperationException("Exit play mode before launching S005 Regular Standard.");
        ...
        {
            string root = Path.GetFullPath(Path.Combine(Application.dataPath, ".."));
            return Path.Combine(root, SkirmishAcceptanceScaffold.S005RelativeReportEvidenceDirectory);
        }

        public static string DescribeLaunchPayload()
        {
            if (!SkirmishAriaAcceptancePayload.TryCreateFirstVisitS005(
                    SkirmishS005FirstVisit.SeedA,
                    GameLocalization.EnglishLocaleCode,
                    out SkirmishAriaAcceptancePayload payload,
                    out string error))
                throw new InvalidOperationException(error);
            return "[SkirmishS005GameView] selected " + payload.FormatSelectedConfiguration();
        }
        ...
        private static void Tick()
        {
            ...
            try
            {
                ...
                if (stage == 1)
                {
                    if (match.Seed != SkirmishS005FirstVisit.SeedA)
                    {
        ...
        private static bool TryQueueExpanded(EntityManager em)
        {
            ...
            {
                RequiredFeatureIds = authored.DefinitionS005.RequiredFeatureIds
            };
            ...
            if (!SkirmishExpandedLaunchResolver.TryCompileAndQueue(
                    ...
                    registry))
                throw new InvalidOperationException(reasons == null || reasons.Count == 0 ? "S005 queue failed" : reasons[0].ToString());

            if (setup.CatalogId != SkirmishAcceptanceCensusCapture.S005CatalogId ||
                setup.DefinitionId != SkirmishAcceptanceCensusCapture.S005DefinitionId ||
                setup.Seed != SkirmishS005FirstVisit.SeedA ||
                setup.DifficultyId != SkirmishDifficultyId.Regular ||
                setup.SizeId != SkirmishSizeId.Standard)
                throw new InvalidOperationException("S005 Game View queue drifted from Regular Standard seed 104734.");

            Debug.Log(string.Format(
                CultureInfo.InvariantCulture,
                "[SkirmishS005GameView] queued catalog={0} seed={1} hash={2:X8} playable=0",
                setup.CatalogId,
                setup.Seed,
                setup.SetupHash));
            return true;
        ...
        private static string CapturePlayingFrame(in SkirmishMatchState match)
        {
            ...
            string png = Path.Combine(primary, SkirmishAcceptanceScaffold.S005PlayingPngFileName);
            ScreenCapture.CaptureScreenshot(png);
            WriteSidecar(primary, in match);
            WriteSidecar(report, in match);
            Debug.Log("[SkirmishS005GameView] captured " + png);
            return png;
        }
        ...
        {
            File.WriteAllText(
                Path.Combine(directory, SkirmishAcceptanceScaffold.S005GameViewSidecarFileName),
                string.Format(
                    CultureInfo.InvariantCulture,
        ...
        {
            string source = Path.Combine(EvidenceDirectory(), SkirmishAcceptanceScaffold.S005PlayingPngFileName);
            if (!File.Exists(source))
                return;
            Directory.CreateDirectory(ReportEvidenceDirectory());
            File.Copy(
                source,
                Path.Combine(ReportEvidenceDirectory(), SkirmishAcceptanceScaffold.S005PlayingPngFileName),
                true);
        }
        ...
        private static void Finish(string reason)
        {
            ...
            Debug.Log("[SkirmishS005GameView] result=" +
