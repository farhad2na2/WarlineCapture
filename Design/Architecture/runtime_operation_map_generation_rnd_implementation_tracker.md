...
**Status:** Active. R74 is the stable pre-Android narrow-frontage candidate. The plus-shaped autobahn, perpendicular crossing, and imported major-road/runway content remain removed; the only authored inter-district route is one connected three-segment `3.2 m` dusty lane with no segment longer than `36 m` and zero imported or authored solid-prop intersections. R72 guards its short simple-chain topology, bounded total length, endpoint continuity, and lack of non-adjacent self-intersection. R74 removes the remaining rectangular end overhangs, uses exact-length ribbons, and grounds all four route nodes with bounded `3.2 m` dirt caps inside `4.8 m` transition shoulders. Three road-facing civilian houses and bounded courtyards reduce the central seam, while continuous frontage clearance removes penetrating rocks and oversized vegetation without exposing ground. Exact editor/runtime manifests and same-camera renders match; all 28 focused runtime-city tests, all 4 M01 source/evidence tests, and all 17 production source-growth checks pass. Overall owner visual acceptance remains open; desktop total-generation-time improvement, Android profiling, gameplay integration, and production adoption also remain open.
...
R74 is the fixed owner-review baseline while this table is reviewed. No Android optimization or procedural-variation work starts from these classifications before visual direction is confirmed.
...
9. [~] After Gate 1 passes, replace the inappropriate plus-shaped autobahn, remove its building intersections, fix penetrating rocks and uncovered ground in the shared editor/runtime source, and prove parity again. R74 removes every imported major road, constrains the replacement to one connected three-segment `3.2 m` dusty route with no segment above `36 m`, and fails generation on road/building or large-terrain intersections. R72 additionally fails regeneration when the route is disconnected, self-intersecting, longer than `90 m` in total, or contains more than four segments. R74 replaces rectangular route overhangs with exact-length surfaces and four bounded node caps, while the three road-facing civilian houses, bounded courtyards, continuous frontage clearance, horizon-scale foundation, and overlapping authored ground remain intact. Exact editor/runtime parity passes with `8.307 ms` worst visual batch; focused validation passes `49/49`. The R74 desktop candidate is ready for owner review; owner visual acceptance and broader composition work remain open.
