...
| Field | Status |
|---|---|
| Checklist complete | 81 / 119 |
| Checklist percent complete | 68.1% |
| Current phase | Phase 8 - Managed Helper Hot-path Migration |
| Quick wins complete | 6 / 6 |
| Current target | AttackVfx is confirmed from the current audit as the largest measured spike (`UnitAttackVfxRequestSystem` 42.8 ms). The first migration slice moved per-request prefab/position/rotation/origin resolution into the ECS attack request creation path and left `UnitAttackVfxRequestSystem` as a thin managed playback/drain edge. |
| Compiler status | Latest AttackVfx slice passed `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Tests.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, and Unity 6.5.2 focused validation `UnitCombatFocusedEditModeTests.RunFocusedValidation` in `/private/tmp/warline-arch-followup-attack-vfx-resolution-validation.log` with 0 compiler errors. Earlier production tick marker and Fuel/Oil ECS validations also passed. |
...
| Validation status | Phase 0 inventory completed; Phase 1 config edited; Phase 2 release OnGUI strip implemented; Phase 3 release diagnostic system gate implemented; Phase 4 GC quick wins completed; Phase 5 Burst inventory/classification report added and guardrail implemented; Phase 6 instantiate classification report added and first placement-visual pooling slice implemented; Phase 8 Fuel/Oil behavior safety-net tests added, production math extracted into a pure system-helper surface, ECS storage preference added across production, haulers, faction summaries, selection display, resource visuals, and publish invalidation, production/runtime helper profiler markers standardized, and the first AttackVfx request-processing slice completed with focused request-resolution tests. Latest validations passed: `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Tests.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, and Unity focused validation `/private/tmp/warline-arch-followup-attack-vfx-resolution-validation.log`. |
| Still wrong / next iteration | No visual defects are known from code/compile validation, but manual visual smoke checks for placement-preview pooling, command behavior, and combat VFX timing are still pending. Fuel/Oil still keeps runtime-building storage mirrors for compatibility. Next exact action: re-run GC/frame-time capture around battle VFX, then decide whether remaining AttackVfx work should target request entity destruction/drain, VFX pool growth, or combat trace playback. Android baseline is still not captured, and HDR/soft-shadow tier ownership remains unresolved. |
