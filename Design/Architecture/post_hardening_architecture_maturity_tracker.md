...
| Parallel preparatory work | `AM-026` remains dependency-blocked and receives no completion credit; `ui_projection_allocation_inventory.md` inventories 29 surfaces and reconciles every declared route/popup contract; drafts `AM-WP-001` through `AM-WP-016` bind Resource Exchange, Build Drawer, Selection/Passenger, Placement, Loading, Shell Layout, Match Status, Squad Tray/Command Wheel, Settings, Armory, Match resource header, Quick Custom, Main Menu resource header, Commander Profile, shell presentation commands, and Diagnostics ownership, lifecycle, semantic identities, and real-path performance/allocation gates; `AM-WP-007` is additionally blocked on explicit mission/objective owner handoff; `AM-WP-009` preserves separately owned audio/FirstLaunch consumers as read-only dependencies; `AM-WP-010` requires pure Armory gateway reads and one version-gated retained-row projection; `AM-WP-011` keeps Match resources separate from the Main Menu profile/progression contract and removes recurring live-storage aggregation; `AM-WP-012` preserves Quick Custom's event-driven setup/launch behavior while requiring symmetric listener ownership; `AM-WP-013` fails closed until `AM-026` proves the Main Menu resource source and active binding seam; `AM-WP-014` similarly fails closed until `AM-026` proves Commander profile/progression authority and complete route-exit cleanup; `AM-WP-015` preserves one measured managed animation bridge, removes mixed Resource Exchange ownership, and requires a GUID-preserving rename away from the reserved bare `*System` suffix; `AM-WP-016` requires one canonical Diagnostics path, one bounded log subscription/history, zero disabled work, and separately measured enabled cost |
...
| Latest validation | AM-021 Scenario Lab ownership slice: focused lifecycle `8 / 8`, ownership generator `10 / 10`, zero compiler errors, deterministic pinned-baseline regeneration/check, and `git diff --check`; integrated architecture CI passes `138 / 139` with one external FirstLaunch failure, source-growth is externally blocked by four FirstLaunch paths, and five failing APH-506 release-device mock tests are excluded from Core acceptance while release certification is deferred |
| Latest evidence | `am021_scenario_grid_ownership_evidence.json`, ownership matrix, and Markdown projection at rebased implementation commit `981dcc2dc`; AM-021 remains in progress and receives no checklist credit yet |
