...
| Field | Status |
|---|---|
| Checklist complete | 89 / 127 |
| Checklist percent complete | 70.1% |
| Current phase | Phase 8 - Managed Helper Hot-path Migration |
| Quick wins complete | 6 / 6 |
| Current target | Selection, building-placement visual creation, production transport visuals, and `TransportBoardingCommandSystem` have now been measured with editor-only allocation probes. Diagnostic `PerformanceDiagnosticsSystemHelper.LogNoStackTrace` rows are now excluded from player-relevant GC attribution. The latest clean player-relevant table is dominated by `SelectionGameplayStartupSystemHelper.UpdateSelectionRuntimePhases` rows spread across building-placement, menu-bootstrap, ECS hierarchy, selection-camera, road-build, and focused-read-model markers, while the selection runtime allocation probe still reports `0` direct bytes. An editor-only top-level gameplay runtime phase allocation probe has been added to distinguish real RoadBuild/BuildingPlacement/Selection/MainMenu/etc. phase bytes from profiler stack attribution. The next exact action is to rerun the battle GC capture when Unity licensing is available, then use the phase-probe data before changing selection or building-placement behavior. |
| Compiler status | Latest top-level gameplay runtime phase-probe slice passed `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, and `git diff --check` with 0 errors. Unity 6.5.2 battle capture attempt `/private/tmp/warline-arch-followup-gameplay-runtime-phase-probe-windowed-battle.log` was blocked before project execution by the known licensing/headless loop (`Unsupported protocol version '1.18.1'`, missing `com.unity.editor.headless`) and was stopped. |
| Android baseline status | Not started |
| GC baseline status | Latest completed 2026-07-04 21:25:31 UTC battle-state Match GC capture after diagnostic-row filtering: 300 requested frames after 180 warmup frames, 301 scanned profiler frames, 6,460 raw GC.Alloc samples / 397,921 raw bytes, 5,521 player-relevant samples / 332,449 bytes after excluding editor/tooling/diagnostic rows, and 939 editor/tooling/diagnostic samples / 65,472 bytes. Runtime allocation probe assertion passed. `UIShellEcsPresentationSystem.Update`, `MenuBootstrapView.Update`, selection subphases, `BuildingPlacementVisualPresentationSystemHelper.CreateBuildingVisualInstance`, `BuildingProductionRuntimeTick.UpdateActiveProductionTransports`, and `TransportBoardingCommandSystem` all reported `0` bytes. The remaining top player-relevant rows are recurring `SelectionGameplayStartupSystemHelper.UpdateSelectionRuntimePhases` attribution rows. |
...
- [ ] Rerun battle GC capture with the top-level gameplay runtime phase allocation probe once Unity licensing is available.
...
- Next iteration: filter or gate `PerformanceDiagnosticsSystemHelper.LogNoStackTrace` frame-rate/perf/render diagnostic logging during GC captures, rerun the battle capture, and use the cleaner player-relevant table to choose the next gameplay migration.
