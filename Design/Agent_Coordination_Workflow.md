...
On macOS, do not run long Unity validation with raw `Unity -batchmode` from Codex. Use `Tools/CI/invoke_unity_macos.sh` so Unity Hub's generic Licensing Client, stale Unity Licensing Client / Package Manager IPC, and timed-out Unity helper children are cleared before and after the run. If the user's Unity editor or Package Manager is already stuck, run `Tools/CI/reset_unity_macos_ipc.sh --quit-hub` once before retrying Unity or asking the user to restart the Mac.
