...
| Remaining genuine-debt rows | 136 |
| Remaining unique debt items | 60 |
| Projected unclassified rows | 0 |
| Source-growth blockers | 5 |

The row-bound evidence now records `439` non-debt rows and `136` remaining genuine-debt rows, grouped into `60` unique file/rule remediation items. It remains non-accepting because every genuine-debt item must be closed before Phase 2 can pass.
...
| World-owner candidates | 16 lifecycle rows after exact boundary/protected classifications | Reconcile overlaps with the hazard lane and avoid double-counting one production defect as multiple remediation items. |
