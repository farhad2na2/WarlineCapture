...
### Dense-City Candidate Evidence Snapshot: 2026-07-25

The delegated dense-city tracker now has candidate-only evidence for the ECS presentation route: deterministic two-root dense authoring, zero generated-root physics, explicit semantic ownership, simplified surface/blocker proxies, candidate EntityScene bake/readiness, isolated thin-binding and Addressables layout, packed two-cycle transform/bounds and fixed-camera parity, and ECS-only generated-building destruction. The final Windows focused acceptance recorded `5,371` semantic buildings, `33,536` surfaces, `42,730` presentations, `25` urban rocks, and zero prohibited generated-root physics.

This is not production cutover evidence. `opmap.skirmish.desert_base_01` remains `StaticSceneChunks` in production; production Addressables, accepted source scenes/SubScene, Android outputs, and the frozen `Assets/Game/GeneratedStaticMapPresentation/OperationMaps/opmap/skirmish/desert_base_01` rollback package remain unchanged. Android/package/performance acceptance, production static-package retirement, and all unchecked delegated tracker gates remain open. The authoritative details, hashes, owner matrix, and rollback rules are in `dense_city_editor_bake_hybrid_runtime_implementation_tracker.md` and its linked workflow documents.
...
Primary use remains Skirmish, air/armor sandbox, fuel/logistics validation, and base-defense testing. The map supports multiple `ScenarioSetup` definitions. Per the accepted one-map rollout, initial M01, campaign, operation, and Skirmish scenarios reuse this physical map through distinct scenario/objective/spawn/anchor data. A dedicated smaller M01 map is a later optional content milestone, not an initial requirement.
...
| 2A. Local Addressables packaging foundation | Active / selected direction | 18 | 20 | 90% | Source and presentation handles load from real local Addressables. Source/presentation-aware shared shards reduced the closure from 603 MB to 148 MB with zero project-owned duplicate GUIDs; Editor/Android Entities linkage is accepted and final package/device acceptance remains open. |
...
- [x] Add lazy source-scene, optional subscene/heavy metadata, static presentation manifest, map-surface, minimap-raster, and compatibility placement references without introducing a `Game.Configs -> Game.Rendering` dependency. Content version/hash remain small metadata on the definition/content pack. Evidence: `../AgentReports/2026-07-17_operation_map_lazy_content_reference_contract.md`.
...
- [x] Use one integrity ledger per operation map. See `../AgentReports/2026-07-17_static_map_presentation_per_map_integrity_ledger.md`.
...
- [x] Create the small local catalog group without any heavyweight map scene, manifest, surface, texture, mesh, or chunk dependency. See `../AgentReports/2026-07-17_operation_map_local_addressables_group_topology.md`.
...
- [x] Add `OperationMapAddressablesLayoutValidator` for groups, paths, addresses, labels, partitions, catalog inclusion, and one-map ownership. See `../AgentReports/2026-07-17_operation_map_addressables_layout_validator.md`.
- [x] Add `OperationMapAddressablesBuildReport` output for per-map bytes, aggregate bytes, partition counts, required addresses, Entities artifacts, and duplicate dependencies. See `../AgentReports/2026-07-18_operation_map_addressables_build_report.md`; the output is deterministic evidence and explicitly does not accept the measured package/duplication budget failures.
- [x] Add an explicit CI/editor Addressables content-build step that fails when local content, catalog, layout report, or operation-map validation is absent or stale. See `../AgentReports/2026-07-18_operation_map_addressables_content_build_integration.md`.
- [x] Load the selected source scene by stable Addressables address, retain its scene handle for the full map lifetime, and release it through deterministic teardown. See `../AgentReports/2026-07-18_operation_map_source_scene_loading_contract.md`, `../AgentReports/2026-07-18_operation_map_presentation_manifest_loading_contract.md`, and `../AgentReports/2026-07-18_static_map_addressables_production_activation.md`.
...
- [x] Prove that an Addressable source scene resolves its expected Entities subscene stream/content archive in Editor and Android without hand-addressing generated files. See `../AgentReports/2026-07-18_operation_map_entities_linkage_acceptance.md`.
...
- [x] Validate map-authored building, vehicle, aircraft, runway, helipad, blocker, and source-hiding behavior. See `../AgentReports/2026-07-16_current_operation_map_authored_behavior_validation.md`.
...
- [x] Update this tracker with exact paths, hashes, counts, commands, logs, and screenshots. Exact final identities, counts, command evidence, log/result paths, and the reason no screenshot was required are recorded in `../AgentReports/2026-07-16_current_operation_map_authored_behavior_validation.md`.
...
- [x] Keep the original `Match.unity` fully functional until the extracted map passes all parity gates. See `../AgentReports/2026-07-16_current_operation_map_compatibility_checkpoint.md`.
...
**Execution: active for the selected local-Addressables direction.** Readiness and ownership remain delivery-neutral where practical; the concrete one-map loader now uses retained local Addressables handles through the existing composition lifecycle. Normal menu exit now stops gameplay, restores shell runtime ownership, clears map runtime data, and awaits the Addressables source-scene unload only after presentation drain; sequential map switching remains required before the shared exit/switch checklist item closes. See `../AgentReports/2026-07-18_operation_map_awaited_teardown.md`.
...
- [x] On exit/switch, stop map gameplay, drain presentation chunks, and then unload map/subscene content in a deterministic order. The production shell route completes this order twice consecutively through real local Addressables without retaining the extracted source scene. See `../AgentReports/2026-07-18_operation_map_sequential_lifecycle.md`.
...
- [x] Clear map-specific ECS entities, singleton/buffer data, cached anchors, and scene references before a later map loads. Runtime-bootstrap disposal removes the owned root and blob, Match teardown clears cached subscene/view/UI references, and a second production load republishes exactly one clean root in the same World. See `../AgentReports/2026-07-18_operation_map_sequential_lifecycle.md`.
- [x] Reject missing/stale map ids with typed reason codes; do not silently load the current map in production. The active loader now retains `OperationMapLoadResultCode` beside its bounded diagnostic message, distinguishing invalid id, missing definition/content, stale manifest identity, source/manifest load, metadata bind, interruption, and unload failure. See `../AgentReports/2026-07-18_operation_map_typed_failures.md`.
- [x] Add tests for valid load, missing id, stale manifest, interrupted load, teardown, retry, and sequential map switching. Loader `12 / 12`, Match/bootstrap `14 / 14`, and production lifecycle `2 / 2` cover the complete matrix. See the same report and `../AgentReports/2026-07-18_operation_map_sequential_lifecycle.md`.
- [x] Validate no new update-loop `MonoBehaviour`, manager, controller, facade, service locator, or broad shell is introduced. The current architecture/naming gate passes `9 / 9`; this work remains inside the existing scene-system helper and `MatchSceneView` lifecycle boundaries. See `../AgentReports/2026-07-18_operation_map_typed_failures.md`.
...
- [x] Run `git diff --check` and scoped asset/meta integrity checks.
...
- [x] Run original current-map launch before extraction and compatibility launch after each migration phase. See `../AgentReports/2026-07-18_operation_map_launch_parity_acceptance.md`.
...
| 2026-07-16 | Current operation-map scene staging | `../AgentReports/2026-07-16_current_operation_map_scene_staging.md`; focused EditMode `10 / 10`; source-growth/naming `24 / 24`; source/staged SHA-256 `dca7c83b...3a46`; distinct GUIDs; Unity compile; `git diff --check` | Passed | Created canonical folders and a byte-identical distinct-GUID staged scene through AssetDatabase-safe tooling. Original Match scene and all runtime/generated ownership remain unchanged. |
...
| 2026-07-17 | Current operation-map infrastructure anchor records | `../AgentReports/2026-07-17_operation_map_current_infrastructure_anchor_records.md`; compatibility definition `3 / 3`; staged scene/definition `10 / 10`; spatial config `26 / 26`; source-growth `17 / 17`; naming `9 / 9`; byte-identical compatibility/staged regeneration; Unity compile; `git diff --check` | Passed; Phase 6 row remains open | Authored one runway and three helipad records for the accepted faction-1 current-map placements from existing prefab geometry. No faction-0 infrastructure was invented. Building-derived gameplay read models remain authoritative pending consumer integration. |
| 2026-07-17 | Active operation-map runway read-model binding | `../AgentReports/2026-07-17_operation_map_runway_read_model_binding.md`; runway `5 / 5`; affected metadata/ownership/naming/source-growth `119 / 119`; Unity compile; byte-identical ownership evidence; `git diff --check` | Passed; Phase 6 row remains open | Active-map runway anchors now publish through the existing faction runway ECS read model, suppress same-faction building fallbacks, preserve other-faction compatibility, and republish on map generation changes. Helipad consumer integration and runtime aircraft acceptance remain pending. |
...
| 2026-07-17 | Static presentation per-map integrity ledger | `../AgentReports/2026-07-17_static_map_presentation_per_map_integrity_ledger.md`; affected EditMode `89 / 89`; canonical no-op bake; compile; `git diff --check` | Passed; Phase 2 integrity row complete | Derived the ledger path from operation-map identity and made the baker, baseline probe, Android resolver, and integrity validation reject cross-map ledger reuse. The accepted 514-scene bake remained a zero-write/zero-delete no-op. |
...
| 2026-07-17 | Local Addressables layout validator | `../AgentReports/2026-07-17_operation_map_addressables_layout_validator.md`; validator `2 / 2`; compile; `git diff --check` | Passed; Phase 2A `9 / 20` | Fail-closed validation covers exact owned groups, local schemas and packing modes, addresses, labels, partitions, catalog inclusion, manifest ownership, foreign/duplicate addresses, and dependency-owned Entities content. Strict completion still rejects the missing real minimap raster. |
| 2026-07-18 | Local Addressables measured build report | `../AgentReports/2026-07-18_operation_map_addressables_build_report.md`; model/publisher `4 / 4`; real Android-target content build; two byte-identical report hashes; no-op publication; compile; `git diff --check` | Passed for evidence; Phase 2A `12 / 20` | Published 522-address coverage, 105-bundle/603,272,218-byte map closure, 100 partitions, zero named Entities subfiles, and 944 duplicate GUID rows. The report closes the reporting row only; package size, duplicate bytes, and Entities linkage remain explicit later gates. |
| 2026-07-18 | Android editor/CI Addressables content-build integration | `../AgentReports/2026-07-18_operation_map_addressables_content_build_integration.md`; integration/report/layout/naming `19 / 19`; real cached Android-target content build; compile; `git diff --check` | Passed; Phase 2A `13 / 20` | Release and profiler Android builds now perform one fail-closed local content build before scene resolution and player build. Jenkins inherits the same path. Full APK/AAB validation remains a later artifact gate. |
| 2026-07-18 | Static presentation staged-bake checkpoint | `../AgentReports/2026-07-18_static_map_presentation_staged_bake_checkpoint.md`; staged input `15 / 15`; deterministic staged bake/no-op; structural parity `3 / 3`; staged scene parity `10 / 10`; compile; `git diff --check` | Passed, not published; Phase 4 bake remains open | Proved the staged source can transactionally produce all 514 chunks with exact renderer parity and a deterministic no-op rerun. Restored active compatibility outputs because manifest publication must be atomic with the Addressables loader and thin-Match cutover. |
| 2026-07-18 | Loaded operation-map scene-reference contract | `../AgentReports/2026-07-18_operation_map_scene_reference_contract.md`; focused `5 / 5`; warm lookup `0 B`; compile; `git diff --check` | Passed; Phase 5 integration row remains open | Added the planned bounded scene-reference helper with exact single-view and map-id validation. It does not load content or alter runtime behavior; the loader must call it only after source-scene load succeeds. The broad source-growth gate was blocked by an unrelated current-main guardrail schema mismatch. |
| 2026-07-18 | Local Addressables source-scene loading contract | `../AgentReports/2026-07-18_operation_map_source_scene_loading_contract.md`; focused `5 / 5`; compile; `git diff --check` | Passed; Phase 5 load integration remains open | Added one retained-handle, poll-only additive source-scene loader with exact view validation and exactly-once failure/disposal release. It is not yet called by Match composition and changes no runtime behavior or scene asset. |
| 2026-07-18 | Loaded scene-view runtime reference binding | `../AgentReports/2026-07-18_operation_map_scene_view_runtime_references.md`; staged parity `10 / 10`; loader lifecycle `5 / 5`; compile; `git diff --check` | Passed; Match integration remains open | Bound the staged view to exact map-owned decoration, building, vehicle, surface, placement, definition, and subscene references required by bootstrap after additive loading. Match runtime behavior remains unchanged. |
| 2026-07-18 | Compatibility-safe Match source-loader composition | `../AgentReports/2026-07-18_operation_map_match_loader_composition.md`; affected bootstrap/presentation `38 / 38`; compile; `git diff --check` | Passed; thin-shell activation remains open | The existing Match lifecycle now owns source-scene start, polling, validated handoff, and teardown when map-owned compatibility references are absent. Current full Match behavior remains unchanged until atomic scene cutover. |
| 2026-07-18 | Local Addressables presentation-manifest loading contract | `../AgentReports/2026-07-18_operation_map_presentation_manifest_loading_contract.md`; focused `7 / 7`; bootstrap/presentation regression `38 / 38`; compile; `git diff --check` | Passed; Phase 5 static-presentation binding remains open | The source-scene loader now retains and polls the selected manifest operation in parallel, validates exact map/scene identity and required content before readiness, projects it through Match only after success, and releases both handles exactly once on failure or teardown. Current compatibility behavior remains unchanged. |
| 2026-07-18 | Operation-map initial readiness publication | `../AgentReports/2026-07-18_operation_map_initial_readiness_publication.md`; focused `11 / 11`; compile; `git diff --check` | Passed; Phase 5 full readiness gate remains open | Compatibility remains metadata-ready. The additive-map route publishes source-content, metadata, and manifest readiness while explicitly requiring subscene, surface, authored conversion, and presentation preload before complete readiness. |
| 2026-07-18 | Shared operation-map content address contract | `../AgentReports/2026-07-18_operation_map_content_address_contract.md`; address/layout `8 / 8`; real layout no-op; compile; `git diff --check` | Passed; runtime chunk delivery remains open | Editor packaging and the future runtime chunk API now share one validated deterministic presentation-address format. The real one-map layout remained unchanged. |
...
| 2026-07-18 | Addressable operation-map Entities linkage | `../AgentReports/2026-07-18_operation_map_entities_linkage_acceptance.md`; focused EditMode `7 / 7`; Editor PlayMode `1 / 1`; gated Android APK build; zero C# compiler errors | Passed; Phase 2A `18 / 20` | The official Entities build-additions hook registers the map-owned subscene without manually addressing generated files. The APK contains the exact `d50925...` header and section stream, and future Android packages fail closed when the payload is absent. |
| 2026-07-18 | Offline local-catalog hardening | `../AgentReports/2026-07-18_operation_map_offline_catalog_hardening.md`; focused EditMode `17 / 17`; fresh content build; Editor PlayMode `1 / 1`; zero C# compiler errors | Passed foundation; Phase 2A unchanged | Remote catalog generation and startup catalog updates are disabled. Generated runtime settings require the built-in catalog under `Addressables.RuntimePath` and reject HTTP(S). Android device offline acceptance remains open because no device was attached. |
| 2026-07-19 | Hand-edited map rebake and Android hardware launch | `../AgentReports/2026-07-19_operation_map_hand_edit_bake.md`; one-button bake `8 / 8`; focused EditMode `89 / 89`; Android APK; wired-device Skirmish launch | Runtime passed; package/memory and strict offline gates remain open | Regenerated placements, surfaces, static-presentation chunks, integrity metadata, and local Addressables from the hand-edited source. Refreshed both staged and catalog-authoritative surface metadata. The installed build reached the live match without the previous 95% binding failure. Installed base APK is 473,660 KiB and live-match PSS is 3,056,858 KiB, so release optimization remains required. |
| 2026-07-19 | Android operation-map performance characterization | `../AgentReports/2026-07-19_operation_map_android_performance_characterization.md`; wired-device 60-second SurfaceFlinger sample; menu/match `dumpsys meminfo`; Addressables Build Layout | Characterized; Phase 0/10 performance gates remain open | Live match measured 3,050,416 KiB PSS and 38.14 average FPS. Match adds 1,795,682 KiB PSS over menu; graphics and Android Unknown dominate while native heap adds only 9,640 KiB. Object-level Unity Memory Profiler attribution and optimization are required before acceptance. |
| 2026-07-19 | Cross-bundle static-presentation material identity | `Game.Rendering` and `Game.Tests.Editor` compile with zero errors; focused ownership tests add identical-clone and changed-clone coverage; `git diff --check` | Compile passed; Unity/device acceptance pending | Android diagnostics showed legacy fallback because package-owned `Lit.mat` was loaded as content-identical objects from separate Addressables bundles. Ownership validation now accepts identical material CRCs while continuing to reject changed material content. No checklist item closes until focused Unity and Android presentation-path validation pass. |
...
| 2026-07-19 | Operation-map manifest source isolation | `../AgentReports/2026-07-19_operation_map_manifest_source_isolation.md`; focused EditMode `1 / 1`; resolver/build-scene guards `26 / 26`; one-button bake `10 / 10`; loader EditMode `12 / 12`; Editor lifecycle PlayMode `2 / 2`; zero exceptions/invalid handles; `git diff --check` | Passed; no checklist closure | Static-presentation freshness now hashes canonical map-owned presentation sources only. Map changes remain fail-closed while unrelated Match shell or HUD changes no longer force a map rebake; real local Addressables load/unload and sequential reload remain clean in Editor. |
| 2026-07-25 | Dense-city delegated implementation evidence sync | `dense_city_editor_bake_hybrid_runtime_implementation_tracker.md`; Windows dense feature/physics acceptance; candidate bake, layout, runtime-content, parity, and rollback-owner evidence | Candidate-only evidence synchronized; parent checklist unchanged | Recorded the delegated candidate ECS-presentation evidence and its exact production boundary. The dense lane has not changed the parent production route: `opmap.skirmish.desert_base_01` remains `StaticSceneChunks`, production Addressables and frozen static rollback remain protected, and Android/package/performance/static-retirement gates remain open. |
