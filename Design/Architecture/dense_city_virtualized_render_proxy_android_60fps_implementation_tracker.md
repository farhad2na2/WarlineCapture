...
| Phase 4: Jobified runtime assignment | In progress; the ordered camera boundary, exact map-generation native initialization, pure guard-envelope decision, bounded selection, and deterministic assignment kernel are accepted. Burst jobs scan only the precomputed cell table and selected ranges, deduplicate/filter/expand logical rows, then retain reciprocal bindings, release stale rows, assign the lowest free matching-bucket slot, and emit one final command per dirty slot. Assignment validates all fixed capacities, placement prefixes, sorted inputs, and reciprocal slot/logical maps before mutation; it maintains active cell/placement and required/dirty slot bitsets, advances generations only for changed slots, and publishes retained/released/assigned/overflow counts. Runtime scheduling/wiring, parallel apply, metrics publication, and full unload/job-lifetime proof remain open. |
...
Checklist progress: `36 / 80` complete.
