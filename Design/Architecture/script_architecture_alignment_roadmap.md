...
- [x] `BuildingDefinitionSystem`: remove runtime `root.Find("Model")` dependency.
- [x] `BuildingPlacementVisualSystem`: remove runtime `prefab.transform.Find("Model")` dependency.
...
- [x] `BuildingPlacementAdapterSystem`: validated as the placement-context adapter glue named in `gameplay_solid_ecs_contract.md`; keep unless it grows into a broad owner.
...
- [x] Replace `BuildingPlacementVisualSystem` model-child lookup with authoring/configured model root references or cached spawn-time references.
...
- 2026-06-08: Moved `UnitPathfindBatchJob.cs` to `Assets/Game/Scripts/Systems/Pathfinding/PathfindBatchJob.cs`, preserving the job script `.meta` GUID and adding a tracked folder `.meta`. The contained `PathfindBatchJob` type and `UnitPathfindingScheduleSystem` scheduling semantics are unchanged. Validation: old path absence/new path presence passed, stale source-reference scan found only the scheduler and the moved job source, targeted `git diff --check` passed, and the mirrored architecture guardrail check passed. Full working-tree `git diff --check` is still blocked by trailing whitespace in the already-modified `Assets/Game/Prefabs/UI/Shell/Popups/SCN09_BuildDrawerPopup.prefab`; focused Unity EditMode validation was attempted but blocked because the project is already open in Unity.
...
- 2026-06-08: Fixed minimap EditMode/runtime validation fallout found during the validation pass. `MatchHudMinimapInputSystem` no longer calls `DontDestroyOnLoad` outside play mode and cleans up generated textures/camera objects with `DestroyImmediate` during EditMode tests. User-requested minimap zoom now preserves the requested zoomed projection in raster fallback instead of expanding back to the default zoomed-out grid. Validation: `MatchHudMinimapProjectionSystemTests` passed 11/11 in `/private/tmp/warlinecapture-minimap-projection-retry2.xml`, and `git diff --check` passed.
...
- 2026-06-08: Added `MatchHudCommandControlsCurrentPrefabTests` for the current `SCN08_MatchHudContent` prefab instead of retired `Screen_MatchOverlay` paths. The test resolves `MatchOverlayCommandControlsView` from the current serialized content hierarchy, verifies command button references are assigned, binds `MatchOverlayCommandInputSystem`, clicks Select/Move/Attack/Scan/Hold/Stop, and confirms the expected ECS selection command requests are queued. Validation: `MatchHudCommandControlsCurrentPrefabTests` passed 2/2 in `/private/tmp/warlinecapture-matchhud-command-controls-current-tests2.xml`, and `git diff --check` passed.
