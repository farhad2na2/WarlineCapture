...
- Designer M01 routing override: when `Design/AgentTasks/designer_current.md` says `Status: active`, treat Designer as dispatched. The expected next artifact is `Design/AgentReports/2026-05-14_designer_m01-step-by-step-gameplay-spec.md`; if missing, point Designer to that file as the current action. Gameplay and QA/HCI are implementation lanes held until Art/Atlas mockups exist and the user approves them.
