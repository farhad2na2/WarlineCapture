...
namespace Game.Runtime
{
    public partial struct CampaignMissionGuidanceProjectionSystem
    {
        ...
        private bool TryUpdateMarketLifelineGuidance(ref SystemState system,Entity root,in CampaignMissionRuntimeComponent runtime,in AssistantSettingsComponent settings,in CampaignMissionGuidanceProjectionComponent current)
        {
            ...
            {ClearDefenseGuidance(em,root,current);return true;}
            var members=em.GetBuffer<CampaignMissionMarketLifelineMember>(root,true);Entity actor=Entity.Null,target=Entity.Null;int step=market.DeliveredCount<3?1:market.ManifestVerified==0?2:3;
            if(step==3)
            {
                foreach(var m in members)if(m.Kind==2&&MarketLive(em,m.Entity)){target=m.Entity;break;}
                if(target==Entity.Null)step=4;
            }
            if(step==1)
            {
            ...
            float3 destination=step==1?new float3(market.DeliveryCell.x,0,market.DeliveryCell.y):step==2?new float3(market.ManifestCell.x,0,market.ManifestCell.y):em.GetComponentData<LocalTransform>(actor).Position;
            ...
            bool waiting=step==4||moving||step==2&&math.distancesq(em.GetComponentData<LocalTransform>(actor).Position.xz,destination.xz)<=36;
            ...
            var next=new CampaignMissionGuidanceProjectionComponent {GuidanceId=77000+step,Version=Next(current.Version),MissionSourceVersion=runtime.Version,
                Prompt=step switch {1=>CampaignMissionGuidancePromptKind.MarketEscort,2=>CampaignMissionGuidancePromptKind.MarketVerify,3=>CampaignMissionGuidancePromptKind.MarketDefend,_=>CampaignMissionGuidancePromptKind.MarketHold},GuidanceMode=runtime.Guidance,Active=1,Priority=step==3?AssistantMessagePriority.Critical:AssistantMessagePriority.High,
                RecommendationKind=waiting?AssistantRecommendationKind.Explain:step==3?AssistantRecommendationKind.Attack:AssistantRecommendationKind.Move,
