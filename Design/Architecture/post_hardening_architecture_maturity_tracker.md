...
| Current task | `AM-025` blocked from acceptance but active for remediation; tactical attack-camera state is isolated and 13 Editor-only performance switches now have verified reset ownership. The `575` historical rows now contain `424` non-debt and `151` genuine-debt rows grouped into `75` unique file/rule items; five external helper paths remain |
