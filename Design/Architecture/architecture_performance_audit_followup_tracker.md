...
| Field | Status |
|---|---|
| Checklist complete | 59 / 104 |
| Checklist percent complete | 56.7% |
| Current phase | Phase 8 - Fuel/Oil ECS Drift Correction |
| Quick wins complete | 6 / 6 |
| Current target | Fuel/Oil production math extraction slice complete: oil extraction/refinery conversion math now lives in `BuildingResourceProductionSystemHelper`, a pure `Unity.Mathematics` system-helper surface that can be reused by a future Burst-capable ECS owner. The existing managed runtime-building owner still applies the result, preserving behavior and avoiding parallel gameplay. Next step is moving storage state/production ticking into ECS. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, Unity 6.5.2 import/compile log `/private/tmp/warline-arch-followup-fuel-oil-systemhelper-import-compile.log`, Unity focused faction resource validation log `/private/tmp/warline-arch-followup-fuel-oil-systemhelper-faction-validation.log`, and Unity focused hauler validation log `/private/tmp/warline-arch-followup-fuel-oil-systemhelper-hauler-validation.log` passed with 0 compiler errors after the production math extraction. Unity MCP is usable for inspection but currently adds tracing/logging traffic to performance captures, so batchmode/editor-log validation remains the compiler path. |
...
| Fuel/Oil drift status | Drift partially reduced: production/conversion math is no longer embedded in `FactionResourceCompositionSystemHelper` and now uses a pure `BuildingResourceProductionSystemHelper` math surface, but authoritative production ticking, storage mutation, hauler load/unload, and runtime-building state ownership still live in managed helper/runtime-building paths. UI/header paths remain read-only display consumers. |
| Validation status | Phase 0 inventory completed; Phase 1 config edited; Phase 2 release OnGUI strip implemented; Phase 3 release diagnostic system gate implemented; Phase 4 GC quick wins completed; Phase 5 Burst inventory/classification report added and guardrail implemented; Phase 6 instantiate classification report added and first placement-visual pooling slice implemented; Phase 8 Fuel/Oil behavior safety-net tests added and production math extracted into a pure system-helper surface. Latest validations passed: `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, `/private/tmp/warline-arch-followup-fuel-oil-systemhelper-faction-validation.log` with `[FactionResourceFocusedValidation] result=Passed tests=9`, and `/private/tmp/warline-arch-followup-fuel-oil-systemhelper-hauler-validation.log` with `[ResourceHaulerFocusedValidation] result=Passed tests=12`. |
| Still wrong / next iteration | No visual defects are known from code/compile validation, but manual visual smoke checks for placement-preview pooling and command behavior are still pending. Fuel/Oil authoritative storage/tick ownership still lives in managed helper/runtime-building paths; next slice should move production storage state and ticking into ECS while reusing `BuildingResourceProductionSystemHelper`. Android baseline is still not captured, and HDR/soft-shadow tier ownership remains unresolved. |
...
- Known unresolved: visual/playmode smoke validation is pending, HDR/soft-shadow tier ownership is still unresolved, Android baseline is not captured, Fuel/Oil authoritative gameplay still lives in managed helper/runtime-building paths, and Phase 5 still has no truthful blind Burst candidates.
- Next iteration: move Fuel/Oil production storage state and ticking into ECS while reusing `BuildingResourceProductionSystemHelper` and preserving the newly locked behavior tests.
