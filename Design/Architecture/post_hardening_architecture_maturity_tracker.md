...
| Current task | `AM-025` blocked from acceptance but active for remediation; tactical camera state is isolated, Editor-only performance switches have verified reset ownership, and match-start requests now have two-World recovery proof. The `575` historical rows now contain `425` non-debt and `150` genuine-debt rows grouped into `74` unique file/rule items; five external helper paths remain |
