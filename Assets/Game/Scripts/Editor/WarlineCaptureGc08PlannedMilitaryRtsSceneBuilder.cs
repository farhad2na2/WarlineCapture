...
public static class WarlineCaptureGc08PlannedMilitaryRtsSceneBuilder
{
    private const string DemoScenePath = "Assets/Game/Scenes/Demo.unity";
    private const string ScenePath = "Assets/Game/Scenes/Generated/GC08_PlannedMilitaryRts_2048.unity";
    private const string CaptureRoot = "Design/AgentReports/Captures/GeneratedScenes/GC08_PlannedMilitaryRts_2048";
    private const string ReportPath = "Design/AgentReports/2026-05-20_gameplay_gc08-planned-military-rts-scene.md";
    ...
    public static void BuildGc08PlannedMilitaryRtsScene()
    {
        ValidationLog.Clear();
        BuildLog.Clear();
        ...
        GameObject root = new("GC08_PlannedMilitaryRts_2048_Root");
        ...
        Debug.Log($"WARLINECAPTURE_GC08_PLANNED_MILITARY_RTS_BUILT sourceRoots={sourceRootCount} scene={ScenePath} report={ReportPath}");
    ...
    private static void CloneModules(GameObject root, List<ModuleSpec> modules, Dictionary<string, List<GameObject>> sourceByModule)
    {
        ...
        foreach (ModuleSpec module in modules)
        {
            ...
            foreach (GameObject source in sourceByModule[module.Name])
            {
                ...
                clone.name = "GC08_" + module.Name + "_" + source.name;
    ...
    {
        Surface(root, "FlatPlayableSand_2048", Vector3.zero, new Vector2(2048f, 2048f), CreateMaterial("GC08_SandBase", new Color(0.62f, 0.50f, 0.30f, 1f)), -0.03f);
    }

    private static void BuildPlannedRoads(GameObject root)
    {
        GameObject roads = Child(root, "ReadableWalkableRoads_Visual");
        Material mainRoad = CreateMaterial("GC08_MainRoad_CompactedSand", new Color(0.48f, 0.40f, 0.27f, 1f));
        Material lane = CreateMaterial("GC08_ServiceLane_Dust", new Color(0.68f, 0.56f, 0.34f, 1f));
    ...
    {
        BuildCamera(root, "Camera_GC08_TargetStyleOverview", new Vector3(-370f, 690f, -760f), new Vector3(20f, 0f, -20f), 35f);
        BuildCamera(root, "Camera_GC08_PlayableBaseClose", new Vector3(-250f, 455f, -520f), new Vector3(-70f, 0f, -70f), 35f);
        BuildCamera(root, "Camera_GC08_RunwayVehicleClose", new Vector3(-170f, 485f, -370f), new Vector3(350f, 0f, 145f), 35f);
    }
    ...
    private static void Validate(int sourceRootCount)
    {
        ...
        if (ValidationLog.Count == 0)
            ValidationLog.Add("PASS: GC08 rearranged dense Demo military art modules into a new planned 2048 RTS scene with readable roads and proof units.");
    }
    ...
    private static void CaptureScene()
    {
        foreach (Camera camera in Object.FindObjectsByType<Camera>(FindObjectsInactive.Include, FindObjectsSortMode.None))
        {
            ...
            if (camera.name == "Camera_GC08_PlayableBaseClose")
                Render(camera, ProjectPath(CaptureRoot + "/gc08_playable_base_close_1920x1080.png"));
            if (camera.name == "Camera_GC08_RunwayVehicleClose")
                Render(camera, ProjectPath(CaptureRoot + "/gc08_runway_vehicle_close_1920x1080.png"));
        }
    }
    ...
    private static void WriteReport(int sourceRootCount)
    {
        ...
        report.AppendLine("# GC08 Planned Military RTS Playable Scene");
        report.AppendLine();
        report.AppendLine("Lane: Gameplay");
        report.AppendLine("Task: Build a new 2048x2048 planned RTS-playable military base scene using Demo military art modules as source material.");
        report.AppendLine();
        report.AppendLine("Files changed:");
        report.AppendLine("- `Assets/Game/Scripts/Editor/WarlineCaptureGc08PlannedMilitaryRtsSceneBuilder.cs`");
        report.AppendLine("- `Assets/Game/Scenes/Generated/GC08_PlannedMilitaryRts_2048.unity`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC08_PlannedMilitaryRts_2048/gc08_target_style_overview_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC08_PlannedMilitaryRts_2048/gc08_playable_base_close_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC08_PlannedMilitaryRts_2048/gc08_runway_vehicle_close_1920x1080.png`");
        report.AppendLine();
        ...
        report.AppendLine("Validation run: Unity batchmode `WarlineCaptureGc08PlannedMilitaryRtsSceneBuilder.BuildGc08PlannedMilitaryRtsScene`.");
        report.AppendLine("Validation result: " + (ValidationLog.Any(line => line.StartsWith("ERROR:", StringComparison.Ordinal)) ? "failed; see log below." : "passed planned 2048 visual scene generation validation."));
        report.AppendLine("Known gaps: visual/playable basis only; no ECS conversion yet. Walkability is shown by readable roads and proof units, but formal masks still need to be authored after visual acceptance.");
        report.AppendLine("Cross-lane impacts: PM/Design can review whether the planned 2048 scene now matches the requested top-down military RTS direction.");
        report.AppendLine("Next recommended task: iterate visual composition only if review rejects density, camera, or lane clarity.");
        report.AppendLine();
