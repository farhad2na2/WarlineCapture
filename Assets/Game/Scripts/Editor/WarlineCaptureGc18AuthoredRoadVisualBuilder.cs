...
public static class WarlineCaptureGc18AuthoredRoadVisualBuilder
{
    private const string ScenePath = "Assets/Game/Scenes/Generated/GC18_AuthoredRoadVisual_2048.unity";
    private const string CaptureRoot = "Design/AgentReports/Captures/GeneratedScenes/GC18_AuthoredRoadVisual_2048";
    private const string ReportPath = "Design/AgentReports/2026-05-21_gameplay_gc18-authored-road-visual.md";
    private const string DataPath = "Design/AgentReports/Data/GeneratedScenes/GC18_AuthoredRoadVisual_2048/gc18_authored_road_visual_contract.json";
    ...
    [MenuItem("WarlineCapture/Design/Build GC18 Authored Road Visual 2048")]
    public static void BuildGc18AuthoredRoadVisual2048()
    {
        MacroRoads.Clear();
        ...
        GameObject root = new("GC18_AuthoredRoadVisual_2048_Root");
        ...
        Debug.Log($"WARLINECAPTURE_GC18_AUTHORED_ROAD_VISUAL_BUILT scene={ScenePath} report={ReportPath}");
    ...
    private static void BuildEnvironment(GameObject root)
    {
        ...
        Volume volume = Child(root, "GC18_RTS_PresentationVolume").AddComponent<Volume>();
    ...
    {
        Material baseMat = CreateMaterial("GC18_SandBase", new Color(0.61f, 0.50f, 0.31f, 1f));
        Material roadMat = CreateMaterial("GC18_AuthoredCompactedRoad", new Color(0.64f, 0.53f, 0.35f, 1f));
        Material shoulderMat = CreateMaterial("GC18_AuthoredRoadShoulder", new Color(0.53f, 0.43f, 0.27f, 1f));
        Material trackMat = CreateMaterial("GC18_DirtTireTrack", new Color(0.42f, 0.32f, 0.18f, 1f));
        Material runwayMat = CreateMaterial("GC18_RunwayWeatheredAsphalt", new Color(0.36f, 0.35f, 0.31f, 1f));
        Material plazaMat = CreateMaterial("GC18_LocalPackedPlaza", new Color(0.70f, 0.58f, 0.37f, 1f));
        Material vehicleMat = CreateMaterial("GC18_VehicleYardDust", new Color(0.55f, 0.46f, 0.33f, 1f));
    ...
    private static void BuildDebugOverlay(GameObject root)
    {
        ...
        Material green = CreateMaterial("GC18_DebugSoldierWalkable", new Color(0f, 1f, 0.1f, 0.6f));
        Material cyan = CreateMaterial("GC18_DebugVehicleWalkable", new Color(0f, 0.8f, 1f, 0.6f));
        Material red = CreateMaterial("GC18_DebugBlocker", new Color(1f, 0f, 0f, 0.7f));
        foreach (Zone zone in SoldierLocalZones)
    ...
    private static void Validate()
    {
        ...
        if (ValidationLog.Count == 0)
            ValidationLog.Add($"PASS: GC18 preserved the GC17 walkability masks while replacing rectangular macro-road visuals with authored polygon road, shoulder, runway, plaza, and tire-track surfaces. Visual placements: {Placements.Count}; blocker zones: {BlockerZones.Count}; soldier zones: {SoldierLocalZones.Count}; vehicle zones: {VehicleZones.Count}; macro roads: {MacroRoads.Count}.");
    }
    ...
    {
        BuildRtsCamera(root, "Camera_GC18_RtsOverview", new Vector3(-620f, 520f, -820f), BlueprintPoint(388f, 430f), 38f);
        BuildRtsCamera(root, "Camera_GC18_RtsCityReadable", new Vector3(-930f, 330f, -430f), BlueprintPoint(172f, 366f), 34f);
        BuildRtsCamera(root, "Camera_GC18_RtsAirfieldCommand", new Vector3(260f, 430f, -620f), BlueprintPoint(660f, 320f), 36f);

        Camera top = CameraObject(root, "Camera_GC18_TopDownContractVisualProof");
    ...
    private static void CaptureScene()
    {
        foreach (Camera camera in Object.FindObjectsByType<Camera>(FindObjectsInactive.Include, FindObjectsSortMode.None))
        {
            ...
            if (camera.name == "Camera_GC18_RtsCityReadable")
                Render(camera, ProjectPath(CaptureRoot + "/gc18_rts_city_readable_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC18_RtsAirfieldCommand")
                Render(camera, ProjectPath(CaptureRoot + "/gc18_rts_airfield_command_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC18_TopDownContractVisualProof")
                RenderContractProof(camera, ProjectPath(CaptureRoot + "/gc18_topdown_contract_visual_proof_2048x2048.png"));
        }
    }
    ...
    private static void WriteContractJson()
    {
        ...
        {
            generatedBy = nameof(WarlineCaptureGc18AuthoredRoadVisualBuilder),
            scene = ScenePath
        };
    ...
    private static void WriteReport()
    {
        ...
        report.AppendLine("# GC18 Authored Road Visual Scene");
        report.AppendLine();
        report.AppendLine("Lane: Gameplay");
        report.AppendLine("Task: Continue from GC17 by replacing rectangular road/yards presentation with authored polygon road, shoulder, runway, plaza, and tire-track visual surfaces while preserving the GC17 walkability masks.");
        report.AppendLine();
        report.AppendLine("Files changed:");
        report.AppendLine("- `Assets/Game/Scripts/Editor/WarlineCaptureGc18AuthoredRoadVisualBuilder.cs`");
        report.AppendLine("- `Assets/Game/Scenes/Generated/GC18_AuthoredRoadVisual_2048.unity`");
        report.AppendLine("- `Design/AgentReports/Data/GeneratedScenes/GC18_AuthoredRoadVisual_2048/gc18_authored_road_visual_contract.json`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC18_AuthoredRoadVisual_2048/gc18_rts_overview_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC18_AuthoredRoadVisual_2048/gc18_rts_city_readable_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC18_AuthoredRoadVisual_2048/gc18_rts_airfield_command_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC18_AuthoredRoadVisual_2048/gc18_topdown_contract_visual_proof_2048x2048.png`");
        report.AppendLine();
        report.AppendLine("Contracts touched: GC17 walkability visual contract remains the source of truth; GC18 changes visual road/yards presentation only.");
        report.AppendLine("User-visible behavior: no shipped runtime behavior changed; generated scene gives a more authored-looking visual pass for PM review.");
        report.AppendLine("Validation run: Unity batchmode `WarlineCaptureGc18AuthoredRoadVisualBuilder.BuildGc18AuthoredRoadVisual2048`.");
        report.AppendLine("Validation result: " + (ValidationLog.Any(line => line.StartsWith("ERROR:", StringComparison.Ordinal)) ? "failed; see validation log below." : "passed authored-road visual validation."));
        report.AppendLine("Known gaps: This is still a generated visual pass, not final art quality. Building clusters remain sparse and must be densified with legal authored modules without occupying the colored walkable masks.");
        report.AppendLine("Cross-lane impacts: Art/Design can now review curved road/plaza composition separately from walkability proof.");
        report.AppendLine("Next recommended task: GC19 should densify city and military-base visual modules inside blocker footprints, using the GC18 road/plaza surfaces as the visual frame.");
        report.AppendLine();
