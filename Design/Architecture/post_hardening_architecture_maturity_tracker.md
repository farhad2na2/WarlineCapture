...
| Current task | `AM-025` remains blocked from acceptance but active for remediation. Runtime UI diagnostics now count active World owners, keep logging until the last owner exits, and clear retained logs for a new session. The safer GameText reset remains tracked static-registry debt. The `575` historical rows contain `486` non-debt and `89` genuine-debt rows grouped into `35` unique file/rule items. Separately owned FirstLaunch and operation-map helper paths still block the canonical source-growth gate. |
