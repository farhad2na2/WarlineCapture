...
- Tactical-map UI additions for `SCN-09`, `SCN-10`, `POP-01`, `POP-03`, and `POP-05` are tracked in `Design/WarlineCapture_Tactical_UI_Missing_Parts_Work_Order.md`. The first concrete target is `Design/WarlineCapture_M01_FirstContact_Production_Contract.md`. The relevant missing pieces are command target hints, explicit move/attack mode feedback, disabled command reasons, build availability reasons, metadata-backed footprint overlays, threat jump-to-camera behavior, and result binding to the active Mission / ScenarioSetup / Map ids.
- The 2026-05-07 target refresh is tracked in `Design/WarlineCapture_UIUX_Tactical_Strategic_Target_Update_Audit.md`. Before revisiting tactical popups, use the refreshed state targets:
...
- Tactical command/build/threat/result UI additions from `WarlineCapture_Tactical_UI_Missing_Parts_Work_Order.md` have prefab/controller/test owners before M01 production gameplay is marked ready.
- `POP-05 Mission Result` is verified against `saga.ch01.m01.first_contact` ids, stars, stats, and reward rows from `WarlineCapture_M01_FirstContact_Production_Contract.md`.
