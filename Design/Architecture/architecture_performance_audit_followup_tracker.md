...
| Current target | Per-building ECS resource storage mirror added: runtime-created resource buildings now receive `BuildingResourceStorageComponent` from definition metadata with zero stored barrels, matching existing runtime-building initial state. This is a prerequisite mirror only; authoritative production/storage mutation remains on the existing managed runtime-building path until the next ISystem slice. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, and Unity 6.5.2 batchmode compile log `/private/tmp/warline-arch-followup-resource-storage-mirror-compile.log` passed with 0 compiler errors after the per-building ECS resource storage mirror. Unity MCP is usable for inspection but currently adds tracing/logging traffic to performance captures, so batchmode/editor-log validation remains the compiler path. |
...
- Next iteration: add the Burst-capable ECS production tick/apply path while reusing `BuildingResourceProductionSystemHelper`, preserving the newly locked behavior tests, and using populated runtime building combat ids as the bridge to existing runtime-building identity.
