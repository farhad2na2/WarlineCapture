...
| Field | Status |
|---|---|
| Checklist complete | `85 / 107` |
| Checklist percent complete | `79.4%` |
| Checklist in progress | `13 / 107`: `APH-311`, `APH-408`, `APH-501`, `APH-502`, `APH-504`, `APH-507`, `APH-508`, `APH-509`, `APH-601`, `APH-609`, `APH-704`, `APH-803`, `APH-804` |
...
| Last verified commit | `86de3e054` on `codex/aph711-allocation-closeout`; allocation implementation commit `c47c73f62` and its remote-API/diagnostics integration follow-up compile with zero Runtime, Editor, and Editor-test errors against current `origin/main`. |
| Last update | 2026-07-13 - the final allocation-remediation integration rerun passes the unchanged GC gate at `930 / 1,024` bytes and the latest canonical Match baseline at `8.16 ms` p95 with zero allocation. The capture now rejects measurement windows containing building, production-transport, or drop-visual creation and retries after a fresh 180-frame warmup instead of accepting mutation-contaminated evidence. The clean current-`origin/main` integration compiles Runtime, Editor, and Editor-test assemblies with zero errors. `APH-711` is complete in commits `c47c73f62` and `86de3e054`. The two remaining architecture gates are isolated to separately owned Campaign UI/audio source growth and the audio runtime loop. A validated map rebake, clean package-compliant APK, unplugged-device evidence, and visual/texture pilots remain open. |
...
- [~] `APH-704` Design and benchmark a shared read-only unit spatial index for building defense, AI targeting, threat detection, and selection candidates. Adopt it only when it beats the Phase 2 direct-query baseline and preserves results.
  ...
  - Evidence: `Design/AgentReports/2026-07-11_aph-704_shared_unit_spatial_index_plan.md`, `/private/tmp/warline-aph704-shadow-tests-r3.xml`, `/private/tmp/warline-aph704-building-defense-focused-r2.log`, `/private/tmp/warline-aph704-spatial-performance-r2.log`, and `/private/tmp/warline-aph704-spatial-performance-r3.log`. Remaining work requires a materially different production design or another consumer combination that beats the complete baseline; no production consumer adoption is accepted.
