...
5. `WarlineCapture_Economy_Reward_Design.md`
6. `WarlineCapture_Balancing_Automated_Test_Plan.md`
...
Every ability entry in `BalanceConfigs/WarlineCapture_Combat_Balance_Config_v0_1.json` now has an `availability` block and an `implementationSpec` block. Implementation must treat this table as a human-readable index and the JSON as the source of truth.
...
Every upgrade track in `BalanceConfigs/WarlineCapture_Combat_Balance_Config_v0_1.json` now has an `availability` block, an `implementationSpec` block, and a `resolvedItemIds` list. Player upgrades apply before mission launch or during Operation prep. Enemy escalation tracks apply only during encounter setup and stay hidden from player-facing upgrade UI.
