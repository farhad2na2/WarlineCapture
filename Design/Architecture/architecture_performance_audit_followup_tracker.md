...
| Field | Status |
|---|---|
| Checklist complete | 69 / 111 |
| Checklist percent complete | 62.2% |
| Current phase | Phase 8 - Fuel/Oil ECS Drift Correction |
| Quick wins complete | 6 / 6 |
| Current target | Hauler load/revert/unload now prefers the live `BuildingResourceStorageComponent` on each runtime building combat entity, commits mutation to that ECS component, and mirrors the result back to the runtime-building object for compatibility. Runtime-building fallback remains for buildings without an ECS storage component. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, focused Unity 6.5.2 ECS mirror validation `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-ecs-validation.log`, focused Unity 6.5.2 faction resource validation `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-faction-validation.log`, focused Unity 6.5.2 hauler mirror validation `/private/tmp/warline-arch-followup-fuel-oil-hauler-mirror-sync-validation.log`, focused Unity 6.5.2 resource transfer validation `/private/tmp/warline-arch-followup-resource-transfer-helper-validation.log`, focused Unity 6.5.2 hauler ECS transfer validation `/private/tmp/warline-arch-followup-hauler-transfer-ecs-validation.log`, focused Unity 6.5.2 live hauler ECS transfer validation `/private/tmp/warline-arch-followup-hauler-live-ecs-transfer-validation.log`, and focused Unity 6.5.2 live ECS-storage-preferred hauler validation `/private/tmp/warline-arch-followup-hauler-live-storage-prefer-ecs-validation.log` passed with 0 compiler errors after the Fuel/Oil mirror-sync and transfer-surface slices. |
...
| Fuel/Oil drift status | Drift partially reduced: production/conversion math is no longer embedded in `FactionResourceCompositionSystemHelper`, runtime-created building combat entities carry their runtime building ids, resource buildings now have a per-building ECS storage mirror, a disabled Burst-capable ECS tick/apply surface is validated, managed production applies through that ECS surface, production plus hauler updates refresh the live ECS storage mirror for resource-capable buildings, hauler load/unload storage mutation routes through `BuildingResourceHaulerTransferEcsSystem`, and live hauler mutation now prefers the live ECS storage component before mirroring back to runtime-building state. Production/conversion still starts from runtime-building state. UI/header paths remain read-only display consumers. |
| Validation status | Phase 0 inventory completed; Phase 1 config edited; Phase 2 release OnGUI strip implemented; Phase 3 release diagnostic system gate implemented; Phase 4 GC quick wins completed; Phase 5 Burst inventory/classification report added and guardrail implemented; Phase 6 instantiate classification report added and first placement-visual pooling slice implemented; Phase 8 Fuel/Oil behavior safety-net tests added, production math extracted into a pure system-helper surface, runtime-created building combat entities receive their runtime building ids, per-building ECS resource storage mirrors are populated for resource buildings, the disabled ECS resource-production tick/apply surface has focused validation, managed production applies through the ECS surface without double-ticking, production/hauler ticks sync managed results back to `BuildingResourceStorageComponent`, hauler transfer mutation uses the component-based transfer helper, the disabled ECS hauler transfer surface has focused validation, live hauler load/revert/unload enters through that ECS transfer surface, and live hauler mutation now prefers the combat entity storage component. Latest validations passed: `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-ecs-validation.log`, `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-faction-validation.log`, `/private/tmp/warline-arch-followup-fuel-oil-hauler-mirror-sync-validation.log`, `/private/tmp/warline-arch-followup-resource-transfer-helper-validation.log`, `/private/tmp/warline-arch-followup-hauler-transfer-ecs-validation.log`, `/private/tmp/warline-arch-followup-hauler-live-ecs-transfer-validation.log`, and `/private/tmp/warline-arch-followup-hauler-live-storage-prefer-ecs-validation.log`. |
| Still wrong / next iteration | No visual defects are known from code/compile validation, but manual visual smoke checks for placement-preview pooling and command behavior are still pending. Fuel/Oil production/conversion still starts from runtime-building state and mirrors into ECS; next Fuel/Oil slice should make production prefer the live ECS storage component too, or pause Fuel/Oil drift work and return to Android baseline capture. Android baseline is still not captured, and HDR/soft-shadow tier ownership remains unresolved. |
...
- Known unresolved: visual/playmode smoke validation is pending, HDR/soft-shadow tier ownership is still unresolved, Android baseline is not captured, Fuel/Oil production/conversion still starts from runtime-building state and mirrors into ECS, and Phase 5 still has no truthful blind Burst candidates.
- Next iteration: make production/conversion prefer the live ECS storage component too, or pause Fuel/Oil drift work and return to Android baseline capture.
