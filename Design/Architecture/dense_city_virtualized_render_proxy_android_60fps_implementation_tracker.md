...
| Phase 6: Building and attachment state | In progress; VRP-060 now requires every virtualized gameplay-building state-owner index to be the contiguous zero-based position of its collision-checked owner identity in unsigned `(Low, High)` order. The post-baking replacement gate rejects arbitrary, sparse, or reordered indices before removing resident render-root ownership, then copies the accepted index to `OperationMapVirtualizedBuildingPresentationComponent` on the canonical building while retaining `OperationMapBuildingComponent`, `OperationMapBuildingDestroyedComponent`, health, faction, grid, blocker, targeting, and other gameplay data. Focused real-authoring coverage passes with state-owner index `0`; the current render-only Vegetation/Prop pilot still has no state owners and no building family has been virtualized. Destruction adaptation remains VRP-061. |
...
Checklist progress: `47 / 80` complete.
