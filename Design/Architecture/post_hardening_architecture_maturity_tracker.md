...
| Current task | `AM-025` remains blocked from acceptance but active for remediation. Gameplay, camera-input, and initial-focus state now use the EntityManager supplied by the active match instead of finding a global World. The `575` historical rows contain `560` non-debt and `15` genuine-debt rows grouped into `14` unique file/rule items. Operation-map work remains separately owned. |
...
| Blockers | Operation-map source growth remains externally owned while that work is active. Row-bound evidence still exposes `15` genuine-debt rows in `14` unique file/rule items that must be remediated to zero. Release-only certification remains intentionally deferred. |
...
| Latest evidence | Schema-2 `am025_phase2_ownership_delta.json/.md` records one decision for each of `575` historical intake rows: `554` resolved, `6` protected-deferred, and `15` genuine debt grouped into `14` unique file/rule items. Focused Unity validation proves gameplay state remains isolated between explicitly bound match Worlds, and selection startup retains its lifecycle ownership. `am025_phase2_ownership_delta_intake.json` preserves the immutable v1 intake. |
