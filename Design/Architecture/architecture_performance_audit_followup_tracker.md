...
| Field | Status |
|---|---|
| Checklist complete | 94 / 129 |
| Checklist percent complete | 72.9% |
| Current phase | Phase 7 - Android Ground Truth Baseline |
| Quick wins complete | 6 / 6 |
| Current target | Current-branch IL2CPP ARM64 development profiler APK is built. The next exact action is to install `Build/AndroidProfiler/WarlineCapture-Profiler.apk` on the selected Android device and capture the first current-branch on-device baseline before more tuning or helper migration decisions. |
...
| Android baseline status | Device profile selected: USB device `R4M7PZEQZ58T59ZH`, Xiaomi `24090RA29G`, codename `malachite`, MediaTek `MT6878`, Android `16`. Existing `Design/AgentReports/2026-07-04_perf_WarlineCapture_12-15-11_steady_state_summary.md` is useful reference data only because it was captured from `WarlineCapture-Clone`, not this current branch. Current-branch profiler APK built successfully at `Build/AndroidProfiler/WarlineCapture-Profiler.apk` (`405M` on disk, Unity-reported size `3736467585` bytes, `28` warnings). Install/capture is still pending. |
...
- Next iteration: install `Build/AndroidProfiler/WarlineCapture-Profiler.apk` on the selected `24090RA29G` device, then capture the first current-branch on-device baseline.
