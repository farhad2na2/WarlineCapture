...
| Separate mode — prototype integration next | Operations city stabilization | 60-mission target; O001–O003 model/capture prototypes exist; [P4R](Operations/O001_PLAYER_READY_IMPLEMENTATION.md) completes O001's player path before expansion | Every released mission has a real-input ARIA win, manual review, consequence/recovery correctness, EN/FA and device evidence; full-mode acceptance also requires complete city-run ARIA wins |
...
**Environment asset adoption:** [Demo 2 reuse](../Demo2_Asset_Reuse_Plan.md) is now part of future map/content implementation. Use the [integration guide and D2-A01–A04 work items](../Demo2_Asset_Integration_Guide.md) to qualify one IB logistics/utility yard, then reuse accepted modules in Campaign Chapters 2–4 and Operations districts. Attach the work to Skirmish SK-11, Operations P2/P6 and the relevant Campaign production plan. Keep existing mode delivery priorities/counts; source availability and this planning update do not certify assets or missions.
...
**Operations planning is documented in [Operations/PLAN.md](Operations/PLAN.md).** The recommendation is 60 authored missions in six ten-mission district arcs, implemented with 12 shared families. The package includes [all mission briefs](Operations/Missions/README.md), [technical architecture](Operations/ARCHITECTURE.md), [strategic rules](Operations/STRATEGIC_RULES.md), [coding packages](Operations/DELIVERY.md), and [acceptance requiring ARIA to play and win every mission and a complete city run](Operations/ACCEPTANCE.md). P0–P4 implemented isolated models and O001–O003 capture prototypes; [P4R](Operations/O001_PLAYER_READY_IMPLEMENTATION.md) now sequences the missing shared runtime, input, save and player acceptance work. Prove O001's real player journey first, extend it to the three-mission loop, then publish 12, 30 and 60 accepted entries. Operations completion is not a dependency of the 120 Skirmish battles; their implementations remain separate workstreams.
