...
| Current target | Phase 6 first pooling slice implemented for `BuildingPlacementVisualPresentationSystemHelper.CreateBuildingVisualInstance`: canceled building previews and rebuilt wall-preview segment visuals now return to a presentation-edge pool instead of destroying/re-instantiating. Next slice should run a follow-up Match GC capture and then extend pooling only if the measured top rows justify it. |
...
| Validation status | Phase 0 inventory completed; Phase 1 config edited; Phase 2 release OnGUI strip implemented; Phase 3 release diagnostic system gate implemented; Phase 4 GC quick wins completed; Phase 5 Burst inventory/classification report added and guardrail implemented; Phase 6 instantiate classification report added and first placement-visual pooling slice implemented. Latest validations passed: `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, Unity 6.5.2 compile log `/private/tmp/warline-arch-followup-placement-pool-compile.log` with `Exiting batchmode successfully now!`, and earlier focused architecture validation `/private/tmp/warline-arch-followup-toarray-focused.log` with `[EcsBurstHotPathArchitectureValidation] result=Passed tests=10`. |
| Still wrong / next iteration | No visual defects are known from code/compile validation, but this pooling slice still needs a visual smoke check and follow-up Match GC capture before claiming a measured allocation win. Phase 5 still has no truthful blind Burst candidates. Next iteration: run GC capture, inspect whether `BuildingPlacementVisualPresentationSystemHelper.CreateBuildingVisualInstance` drops out of player-relevant top rows, then continue Phase 6 only on measured remaining instantiate families. |
