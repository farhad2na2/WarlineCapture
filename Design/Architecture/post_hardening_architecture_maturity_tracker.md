...
| Blockers | Five exact helper source-growth paths and `13` genuine-debt rows in `12` unique items must be remediated to zero. The full gate also preserves a later Resource Exchange/UI shell responsibility-ratchet failure that becomes reachable after helper-path blockers clear. All other operation-map paths remain protected. Release-only certification remains intentionally deferred. |
| Latest validation | AMFR-006 building production UI read models `12 / 12` and the real-prefab Build Drawer queue snapshot `1 / 1` pass with zero C# errors. The unchanged 17-test source-growth entrypoint now fails closed on five later helper paths and no longer lists the building-production owner. |
| Latest evidence | Retained building production UI helper `349` lines / `13,935` bytes beneath its existing exact `351` / `13,949` authority, plus one stateless operation-map queue projection that creates no cache, coordinator, or scheduling owner. AMFR-003 evidence still classifies all `430` rows once (`412 / 5 / 13`) with zero unclassified. |
