# Runs Operations ARIA evidence Unity validation against the shadow project only.
[CmdletBinding()]
param(
    [string] $ShadowPath = "D:\Projects\WarlineCapture-Operations",
    [string] $SharedPath = "D:\Projects\WarlineCapture",
    [string] $LogFile = "$env:TEMP\operations-aria-evidence-shadow.log",
...
if (Test-Path -LiteralPath $sharedLibrary) {
    Write-Host "[OperationsAriaEvidence] leaving_shared_library_untouched=$sharedLibrary"
}
...
Write-Host "[OperationsAriaEvidence] project=$shadow"
Write-Host "[OperationsAriaEvidence] unity=$unityExe"
Write-Host "[OperationsAriaEvidence] log=$LogFile"
...
& $validateScript `
    -UnityExe $unityExe `
    -ProjectPath $shadow `
    -ExecuteMethod "Game.Tests.Editor.Operations.OperationsAriaEvidenceValidation.RunFocusedValidation" `
    -LogFile $LogFile `
    -RequiredPassMarker "[OperationsAriaEvidenceValidation] result=Passed checks=8" `
    -GuiLicensing:$gui `
    -TimeoutSeconds $TimeoutSeconds

if ($LASTEXITCODE -ne 0) {
    throw "Operations ARIA evidence shadow validation failed."
}

Write-Host "[OperationsAriaEvidence] result=Passed project=$shadow"
