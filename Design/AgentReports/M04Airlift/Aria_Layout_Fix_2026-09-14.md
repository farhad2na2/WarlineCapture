...
The M4 extraction navigation row (Guide plus Team, Landing, or Departure) retained an absolute vertical position while the tutorial body and primary actions resized to their content. The row was absent from the ARIA height calculation. The next-click overlay also placed a second caption above ARIA’s already-labelled action, covering the instructions.
...
- Retain the yellow next-click outline on ARIA actions, suppress its redundant floating caption inside ARIA, and restore captions for controls outside ARIA.
...
- Shared HUD layout: **passed** 192 presentations (M3/M4 × 12 lessons × EN/FA × normal/large text × 16:9/20:9), plus empty/short/long content, independent minimap docking and non-blocking tutorial outline/caption tests. Marker: `[AriaUtilityLayout] result=Passed`.
...
- Live M4 Editor launch: **passed** the normal opening camera tour, visible tutorial/navigation geometry in both languages, all 12 guide topics and 57 classes in both languages, and guide pause/resume. Marker: `[M04EditorProbe] result=Passed`.
- Inspected all four live HUD captures: EN/FA at 1920×1080 and 2400×1080. Full first-lesson instructions remain visible; Continue/Show Me sit above Guide/Team; the yellow outline stays around Continue without a second caption; the panel encloses both rows and clears the minimap.

Logs: `/private/tmp/warline-m04-aria-layout-retry.log`, `/private/tmp/warline-m04-aria-architecture.log`, `/private/tmp/warline-m04-aria-live.log`. Screenshots: `/private/tmp/warline-m04-editor-probe/hud-{en,fa}-{16x9,20x9}.png`. The PNG dimensions were verified directly; Editor `Screen.width/height` diagnostics can report the docked window rather than the capture resolution.
