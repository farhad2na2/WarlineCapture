...
namespace Game.Authoring
{
    ...
    public sealed partial class OperationMapBuildingAuthoring : MonoBehaviour
    {
        ...
        private sealed class OperationMapBuildingBaker : Baker<OperationMapBuildingAuthoring>
        {
            public override void Bake(OperationMapBuildingAuthoring authoring)
            {
                ...
                if (TryGetSelectionLocalBounds(
                        authoring.transform,
                        authoring.intactVisualRoot,
                        out Bounds selectionBounds))
                {
                    AddComponent(entity, new UnitSelectionHitbox
                    {
                        Center = new float3(
                            ...
                            selectionBounds.center.z),
                        Extents = new float3(
                            selectionBounds.extents.x,
                            selectionBounds.extents.y,
                            selectionBounds.extents.z)
                    });
                }
