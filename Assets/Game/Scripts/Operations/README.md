...
| P4 | `Game.Operations.Content` | O001–O003 vertical slice on D01: authored missions (via Loop), EN/FA copy, ARIA planner/input skills, day report, multi-day scaffolding. References contracts, strategic, tactical, and loop. No engine references. |
| Capture | `Game.Operations.Capture` | Ops-owned Play Mode URP tactical world shell + phone-mock HUD/victory presentation for O001–O003 ARIA evidence capture. Runtime assembly (UnityEngine) so AddComponent works in Editor Play Mode. Does not open Match/Watch shared UI. |
