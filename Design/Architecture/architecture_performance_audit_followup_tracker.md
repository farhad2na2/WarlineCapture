...
| Field | Status |
|---|---|
| Checklist complete | 63 / 108 |
| Checklist percent complete | 58.3% |
| Current phase | Phase 8 - Fuel/Oil ECS Drift Correction |
| Quick wins complete | 6 / 6 |
| Current target | Managed Fuel/Oil production now routes its per-building apply step through `BuildingResourceProductionEcsSystem.ApplyTick`, then writes the result back to the existing runtime-building state. This preserves the single live runtime-building owner while using the validated ECS tick surface for production/conversion behavior. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, Unity 6.5.2 import/compile `/private/tmp/warline-arch-followup-debugflags-meta-compile.log`, focused Fuel/Oil validation `/private/tmp/warline-arch-followup-fuel-oil-ecs-handoff-faction-validation.log`, and focused ECS tick validation `/private/tmp/warline-arch-followup-fuel-oil-ecs-handoff-ecs-validation.log` passed with 0 compiler errors after the managed-to-ECS handoff. The `GameplayRuntimeUpdateDebugFlags.cs.meta` importer was repaired because Unity had regenerated `Game.Runtime.csproj` without that source file. |
