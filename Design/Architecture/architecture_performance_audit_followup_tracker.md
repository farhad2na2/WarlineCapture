...
| Field | Status |
|---|---|
| Checklist complete | 66 / 108 |
| Checklist percent complete | 61.1% |
| Current phase | Phase 8 - Fuel/Oil ECS Drift Correction |
| Quick wins complete | 6 / 6 |
| Current target | Hauler load/unload building-storage mutation now routes through `BuildingResourceStorageTransferSystemHelper`, a component-based transfer surface over `BuildingResourceStorageComponent`. Existing runtime-building callers adapt into that component surface and write the result back, preserving behavior while reducing managed-helper ownership drift. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, focused Unity 6.5.2 ECS mirror validation `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-ecs-validation.log`, focused Unity 6.5.2 faction resource validation `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-faction-validation.log`, focused Unity 6.5.2 hauler mirror validation `/private/tmp/warline-arch-followup-fuel-oil-hauler-mirror-sync-validation.log`, and focused Unity 6.5.2 resource transfer validation `/private/tmp/warline-arch-followup-resource-transfer-helper-validation.log` passed with 0 compiler errors after the Fuel/Oil mirror-sync and transfer-surface slices. |
...
- Known unresolved: visual/playmode smoke validation is pending, HDR/soft-shadow tier ownership is still unresolved, Android baseline is not captured, Fuel/Oil live storage is still mirrored rather than fully authoritative on ECS entities, hauler load/unload remains managed-helper owned, and Phase 5 still has no truthful blind Burst candidates.
- Next iteration: sync managed production results into live `BuildingResourceStorageComponent` entities by runtime building id, or move hauler load/unload to ECS storage ownership first if that is the lower-risk single-owner migration step.
