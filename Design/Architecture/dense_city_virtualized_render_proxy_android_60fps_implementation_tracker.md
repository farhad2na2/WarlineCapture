...
| Phase 2: Deterministic database builder | Complete through the VRP-066 candidate expansion; generated schema/ownership, exact source inventory, shared compound recipes, logical placements, spatial index, provisional capacities, transactional config/report, unchanged-input determinism, and persisted source-to-logical field parity are accepted for render-only Vegetation/Prop plus complete House and Shop families. CityHall remains atomically resident under an exact unsupported-policy exclusion. Camera/projection capacity acceptance and broader infrastructure/resident-exception expansion remain explicitly owned by later phases. |
...
| Phase 6: Building and attachment state | In progress through the rejected VRP-067 Android gate. Deterministic contiguous state-owner indices remain owned by 4,933 canonical gameplay buildings: 3,638 complete House owners and 1,295 complete Shop owners. Their 9,866 intact/destroyed state-linked placements coexist with 9,721 render-only Vegetation/Prop placements, producing 19,587 placements, 9,875 prototypes, 17,982 parts, 1,660 cells, and 1,364 fixed slots from 29,268 eligible renderer/material rows. Exact multi-submesh reconciliation distinguishes 29,268 logical rows from 29,081 physical source renderers; direct bake removes every selected renderer and leaves 52,928 of 82,009 physical source renderers resident. CityHall is excluded atomically: all 406 rows remain resident because 44 rows use unsupported render policy; no generated Tent or Military family exists in the current dense candidate. Focused state/destruction contracts remain green and canonical gameplay consumers remain independent of proxy assignment. On the paired Xiaomi, canonical House and Shop destruction state/sequence advanced, but real interior views selected 106/108 cells, exhausted 1,352 enabled slots, reported 875/790 overflow rows, and retained the intact visible presentation after settling. The source capacity report is explicitly provisional and models a 5x5 envelope with its camera/projection sweep pending; VRP-067 therefore remains unchecked until dependency-correct capacity remediation restores zero overflow and the visual matrix passes. The opt-in Android matrix recorder now emits the exact materialized envelope minimum/maximum so the next device sample can drive a deterministic rectangular all-grid capacity sweep rather than a manual constant. Production remains `StaticSceneChunks + ResidentEntities` with `productionCutover=0`; the final 16.667 ms/60 FPS gate remains open. |
...
Checklist progress: `53 / 80` complete.
