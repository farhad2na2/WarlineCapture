...
| Current task | `AM-025` remains blocked from acceptance but active for remediation. AI settings now move from menu-owned state through the ECS match-start boundary, and the obsolete duplicate build UI ECS path is removed while the active Canvas UI remains unchanged. The `575` historical rows contain `538` non-debt and `37` genuine-debt rows grouped into `21` unique file/rule items. Separately owned FirstLaunch and operation-map helper paths still block the canonical source-growth gate. |
...
| Blockers | AM-025 source-growth is externally blocked by four separately owned FirstLaunch helpers and four operation-map-owned helpers. Three operation-map helpers are new since the five-path closure policy was published. Row-bound evidence still exposes `37` genuine-debt rows in `21` unique file/rule items that must be remediated to zero. Release-only certification remains intentionally deferred. |
| Latest validation | Game-text replacement, null initialization, subsystem reset, formatting, and immutable-publication tests pass; Unity compiles with zero errors and the static-registry guard passes. Phase 2 evidence regeneration is byte-stable and `git diff --check` passes. The AM-021 ownership snapshot remains refreshed at implementation commit `2107ff75b` with the same `575 / 553 / 22 / 0` resource, explicit-owner, protected-owner, and gap result. |
| Latest evidence | Schema-2 `am025_phase2_ownership_delta.json/.md` records one decision for each of `575` historical intake rows: `530` resolved, `8` protected-deferred, and `37` genuine debt grouped into `21` unique file/rule items. Focused Unity validation proves AI settings are isolated per menu/match and the duplicate build UI path cannot return. `am025_phase2_ownership_delta_intake.json` preserves the immutable v1 intake, and policy schema 2 lists all five exact source-growth blockers |
