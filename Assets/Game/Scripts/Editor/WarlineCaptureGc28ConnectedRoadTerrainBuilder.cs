...
public static class WarlineCaptureGc28ConnectedRoadTerrainBuilder
{
    private const string ScenePath = "Assets/Game/Scenes/Generated/GC28_ConnectedRoadTerrain_2048.unity";
    private const string CaptureRoot = "Design/AgentReports/Captures/GeneratedScenes/GC28_ConnectedRoadTerrain_2048";
    private const string ReportPath = "Design/AgentReports/2026-05-21_gameplay_gc28-connected-road-terrain.md";
    private const string DataPath = "Design/AgentReports/Data/GeneratedScenes/GC28_ConnectedRoadTerrain_2048/gc28_connected_road_terrain_contract.json";
    ...
    private static readonly List<ArtSurfacePlacement> ArtSurfacePlacements = new();
    ...
    private static readonly Dictionary<string, Material> Demo2FallbackMaterials = new();
    ...
    private static int demo2RoadPlacements;
    private static int demo2GroundPlacements;
    private static int demo2BoundaryPlacements;
    private static int demo2DetailPlacements;
    private static int connectedRoadEdges;
    private static int connectedRoadNodes;
    ...
    private readonly struct ArtSurfacePlacement
    {
        public readonly string Name;
        public readonly string Path;
        public readonly string Category;
        public readonly Rect Footprint;
        public readonly bool MustStayOnWalkable;

        public ArtSurfacePlacement(string name, string path, string category, Rect footprint, bool mustStayOnWalkable)
        {
            Name = name;
            Path = path;
            Category = category;
            Footprint = footprint;
            MustStayOnWalkable = mustStayOnWalkable;
        }
    }
    ...
    private sealed class ContractExport
    {
        ...
        public List<ArtSurfaceExport> artSurfaces = new();
    ...
    [Serializable]
    private sealed class ArtSurfaceExport
    {
        public string name;
        public string prefab;
        ...
        public float width;
        public float depth;
        public bool mustStayOnWalkable;
    }

    [MenuItem("WarlineCapture/Design/Build GC28 Connected Road Terrain 2048")]
    public static void BuildGc28ConnectedRoadTerrain2048()
    {
        MacroRoads.Clear();
        ...
        Modules.Clear();
        Placements.Clear();
        ArtSurfacePlacements.Clear();
        AuthoredClusterPlacements.Clear();
        MissingAssets.Clear();
        ValidationLog.Clear();
        SkippedVisuals.Clear();
        ScaleAuditLog.Clear();
        Demo2FallbackMaterials.Clear();
        mainBlockerPlacements = 0;
        detailBlockerPlacements = 0;
        authoredClusterPlacements = 0;
        demo2RoadPlacements = 0;
        demo2GroundPlacements = 0;
        demo2BoundaryPlacements = 0;
        demo2DetailPlacements = 0;
        connectedRoadEdges = 0;
        connectedRoadNodes = 0;
        ...
        GameObject root = new("GC28_ConnectedRoadTerrain_2048_Root");
        BuildEnvironment(root);
        DefineContract();
        BuildGroundAndRoads(root);
        BuildConnectedRoadTerrainArtLayer(root);
        ...
        Debug.Log($"WARLINECAPTURE_GC28_CONNECTED_ROAD_TERRAIN_BUILT scene={ScenePath} report={ReportPath}");
    ...
    private static void BuildEnvironment(GameObject root)
    {
        ...
        Volume volume = Child(root, "GC28_RTS_PresentationVolume").AddComponent<Volume>();
    ...
    {
        Material baseMat = CreateMaterial("GC28_SandBase", new Color(0.64f, 0.55f, 0.38f, 1f));
        Material sandLightMat = CreateMaterial("GC28_SandWindLight", new Color(0.70f, 0.61f, 0.43f, 1f));
        Material sandDarkMat = CreateMaterial("GC28_SandCompactedDark", new Color(0.56f, 0.47f, 0.32f, 1f));
        Material roadMat = CreateMaterial("GC28_ConnectedCompactedRoadBed", new Color(0.57f, 0.48f, 0.33f, 1f));
        Material roadEdgeMat = CreateMaterial("GC28_SubtleRoadEdge", new Color(0.49f, 0.40f, 0.27f, 1f));
        Material shoulderMat = CreateMaterial("GC28_ConnectedRoadShoulder", new Color(0.61f, 0.51f, 0.35f, 1f));
        Material trackMat = CreateMaterial("GC28_SubtleTireTrack", new Color(0.43f, 0.35f, 0.23f, 1f));
        Material runwayMat = CreateMaterial("GC28_RunwayWeatheredAsphalt", new Color(0.32f, 0.31f, 0.28f, 1f));
        Material runwayMarkMat = CreateMaterial("GC28_RunwayFadedMarking", new Color(0.72f, 0.68f, 0.58f, 1f));
        Material plazaMat = CreateMaterial("GC28_LocalPackedPlaza", new Color(0.63f, 0.52f, 0.35f, 1f));
        Material vehicleMat = CreateMaterial("GC28_VehicleYardDust", new Color(0.50f, 0.42f, 0.31f, 1f));
        Material districtPadMat = CreateMaterial("GC28_DistrictPackedSandPad", new Color(0.61f, 0.51f, 0.34f, 1f));
        ...
        GameObject roads = Child(root, "ConnectedRoadBeds_VisualOnly_PreserveGc17Masks");
    ...
    {
        GameObject variation = Child(root, "GC28_VisualSandVariationTiles");
    ...
    {
        GameObject pads = Child(root, "GC28_AuthoredDistrictPads_VisualOnly");
    ...
    private static void BuildConnectedRoadTerrainArtLayer(GameObject root)
    {
        GameObject parent = Child(root, "ConnectedDemo2RoadTerrainArtLayer_RoadsScaled_BuildingsSourceScale");

        PlaceDemo2GroundPatch(parent, "GrassPad_NorthWestTown_A", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Grass_Square_01.prefab", 112f, 124f, 0f, new Vector2(76f, 76f), 2.0f);
        PlaceDemo2GroundPatch(parent, "GrassPad_NorthWestTown_B", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Grass_Circle_02.prefab", 230f, 188f, 18f, new Vector2(64f, 64f), 1.8f);
        PlaceDemo2GroundPatch(parent, "GrassPad_WestMarket_A", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Grass_Circle_01.prefab", 176f, 366f, 24f, new Vector2(56f, 56f), 1.8f);
        PlaceDemo2GroundPatch(parent, "GrassPad_SouthBase_A", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Grass_Circle_02.prefab", 138f, 768f, -14f, new Vector2(54f, 54f), 1.7f);
        PlaceDemo2GroundPatch(parent, "Concrete_CommandPad_A", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Concrete_Base_01.prefab", 678f, 432f, 0f, new Vector2(74f, 74f), 2.0f);
        PlaceDemo2GroundPatch(parent, "Concrete_AirfieldPad_A", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Port_Concrete_Slab_01.prefab", 706f, 206f, 90f, new Vector2(70f, 70f), 2.1f);

        const float roadScale = 3.2f;
        const float serviceRoadScale = 2.7f;
        const float nodeScale = 3.6f;
        PlaceConnectedRoadEdge(parent, "WestNorthSouth_North", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Straight_01.prefab", new Vector2(288f, 38f), new Vector2(284f, 266f), roadScale);
        PlaceConnectedRoadEdge(parent, "WestNorthSouth_Mid", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Straight_01.prefab", new Vector2(284f, 266f), new Vector2(274f, 454f), roadScale);
        PlaceConnectedRoadEdge(parent, "WestNorthSouth_South", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Straight_01.prefab", new Vector2(274f, 454f), new Vector2(256f, 850f), roadScale);
        PlaceConnectedRoadEdge(parent, "EastNorthSouth_North", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Straight_03.prefab", new Vector2(512f, 36f), new Vector2(512f, 308f), roadScale);
        PlaceConnectedRoadEdge(parent, "EastNorthSouth_South", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Straight_03.prefab", new Vector2(512f, 308f), new Vector2(504f, 848f), roadScale);
        PlaceConnectedRoadEdge(parent, "NorthCityToAirfield", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Straight_02.prefab", new Vector2(74f, 268f), new Vector2(284f, 266f), roadScale);
        PlaceConnectedRoadEdge(parent, "NorthConnectorToEastSpine", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Straight_02.prefab", new Vector2(284f, 266f), new Vector2(512f, 308f), roadScale);
        PlaceConnectedRoadEdge(parent, "CentralCityToCommand", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Straight_Damaged_01.prefab", new Vector2(142f, 454f), new Vector2(274f, 454f), roadScale);
        PlaceConnectedRoadEdge(parent, "CentralConnectorToEastSpine", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Straight_Damaged_01.prefab", new Vector2(274f, 454f), new Vector2(512f, 444f), roadScale);
        PlaceConnectedRoadEdge(parent, "CommandToAirfieldEast", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Straight_Damaged_02.prefab", new Vector2(512f, 308f), new Vector2(886f, 308f), roadScale);
        PlaceConnectedRoadEdge(parent, "SouthPlayerToFuel_A", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Straight_01.prefab", new Vector2(58f, 890f), new Vector2(256f, 850f), roadScale);
        PlaceConnectedRoadEdge(parent, "SouthPlayerToFuel_B", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Straight_01.prefab", new Vector2(256f, 850f), new Vector2(504f, 848f), roadScale);
        PlaceConnectedRoadEdge(parent, "SouthPlayerToFuel_C", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Straight_01.prefab", new Vector2(504f, 848f), new Vector2(812f, 890f), roadScale);

        PlaceConnectedRoadNode(parent, "RoadT_WestNorth", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_T_01.prefab", 284f, 266f, 96f, nodeScale);
        PlaceConnectedRoadNode(parent, "RoadT_WestCentral", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_T_01.prefab", 274f, 454f, 92f, nodeScale);
        PlaceConnectedRoadNode(parent, "RoadCross_EastCommand", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Cross_01.prefab", 512f, 308f, 0f, nodeScale);
        PlaceConnectedRoadNode(parent, "RoadCorner_WestSouth", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Corner_01.prefab", 256f, 850f, 178f, nodeScale);
        PlaceConnectedRoadNode(parent, "RoadCorner_EastSouth", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Corner_02.prefab", 504f, 848f, 182f, nodeScale);
        PlaceConnectedRoadNode(parent, "RoadEnd_NorthAirfield", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Road_Straight_End_01.prefab", 894f, 308f, 90f, nodeScale);

        PlaceConnectedRoadEdge(parent, "DirtServiceLane_CentralBarracks", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_DirtRoad_Straight_01.prefab", new Vector2(362f, 532f), new Vector2(360f, 618f), serviceRoadScale);
        PlaceConnectedRoadEdge(parent, "DirtServiceLane_WestBarracks", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_DirtRoad_Straight_01.prefab", new Vector2(362f, 704f), new Vector2(360f, 768f), serviceRoadScale);
        PlaceConnectedRoadEdge(parent, "DirtServiceLane_ArmorFuel", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_DirtRoad_Straight_01.prefab", new Vector2(616f, 650f), new Vector2(770f, 632f), serviceRoadScale);
        PlaceConnectedRoadEdge(parent, "EntranceLane_NorthWestTown", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_DirtRoad_Straight_01.prefab", new Vector2(178f, 210f), new Vector2(284f, 266f), 3.4f);
        PlaceConnectedRoadEdge(parent, "EntranceLane_WestMarket", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_DirtRoad_Straight_01.prefab", new Vector2(206f, 374f), new Vector2(274f, 454f), 3.4f);
        PlaceConnectedRoadEdge(parent, "EntranceLane_PlayerCamp", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_DirtRoad_Straight_01.prefab", new Vector2(156f, 748f), new Vector2(256f, 850f), 3.4f);
        PlaceConnectedRoadEdge(parent, "EntranceLane_CommandDepot", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_DirtRoad_Straight_01.prefab", new Vector2(640f, 402f), new Vector2(512f, 308f), 3.4f);

        PlaceDemo2BoundaryStrip(parent, "PlayerBaseWall_North", "Assets/Synty/PolygonBattleRoyale/Prefabs/Props/SM_Prop_BaseWall_01.prefab", new Vector2(68f, 674f), new Vector2(276f, 674f), 28f, new Vector2(22f, 6f));
        PlaceDemo2BoundaryStrip(parent, "CommandFence_West", "Assets/Synty/PolygonBattleRoyale/Prefabs/Props/SM_Prop_WireFence_01.prefab", new Vector2(552f, 346f), new Vector2(552f, 524f), 22f, new Vector2(20f, 4f));
        PlaceDemo2BoundaryStrip(parent, "AirfieldFence_South", "Assets/Synty/PolygonBattleRoyale/Prefabs/Props/SM_Prop_WireFence_02.prefab", new Vector2(626f, 340f), new Vector2(860f, 340f), 22f, new Vector2(20f, 4f));
        PlaceDemo2BoundaryStrip(parent, "ArmorDepotBaseWall_East", "Assets/Synty/PolygonBattleRoyale/Prefabs/Props/SM_Prop_BaseWall_01.prefab", new Vector2(846f, 604f), new Vector2(846f, 828f), 28f, new Vector2(22f, 6f));

        PlaceDemo2Detail(parent, "Rubble_CentralRoad_A", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Rubble_Pile_01.prefab", 410f, 438f, 18f, new Vector2(14f, 12f), false);
        PlaceDemo2Detail(parent, "Rubble_NorthRoad_A", "Assets/Synty/PolygonBattleRoyale/Prefabs/Environments/SM_Env_Rubble_Pebbles_02.prefab", 184f, 278f, -24f, new Vector2(14f, 10f), false);
        PlaceDemo2Detail(parent, "Barrier_CommandRoad_A", "Assets/Synty/PolygonBattleRoyale/Prefabs/Props/SM_Prop_Barrier_02.prefab", 552f, 330f, 90f, new Vector2(14f, 8f), false);
        PlaceDemo2Detail(parent, "TankTrap_SouthRoad_A", "Assets/Synty/PolygonBattleRoyale/Prefabs/Props/SM_Prop_TankTrap_01.prefab", 286f, 844f, 12f, new Vector2(14f, 12f), false);
        PlaceDemo2Detail(parent, "Crates_CommandDepot_A", "Assets/Synty/PolygonBattleRoyale/Prefabs/Props/SM_Prop_Crate_Large_01.prefab", 754f, 392f, 18f, new Vector2(12f, 12f), false);
        PlaceDemo2Detail(parent, "Pallets_PlayerBase_A", "Assets/Synty/PolygonBattleRoyale/Prefabs/Props/SM_Prop_Pallet_Loaded_01.prefab", 212f, 740f, -18f, new Vector2(12f, 10f), false);
        PlaceDemo2Detail(parent, "Bush_TownEdge_A", "Assets/Synty/PolygonBattleRoyale/Prefabs/Generic/SM_Env_Bush_03.prefab", 236f, 210f, 0f, new Vector2(10f, 10f), false);
        PlaceDemo2Detail(parent, "Tree_MarketEdge_A", "Assets/Synty/PolygonBattleRoyale/Prefabs/Generic/SM_Generic_Tree_02.prefab", 76f, 476f, 0f, new Vector2(12f, 12f), false);
    }
    ...
    private static void PlaceChildMaskedAuthoredModule(GameObject parent, MaskedModule module, string path)
    {
        ...
        source.name = "GC28_" + module.Name + "_ChildMaskSource";
        ...
        GameObject moduleRoot = Child(parent, "GC28_" + module.Name + "_ChildMaskedPieces");
        int accepted = 0;
        int pieceIndex = 0;
    ...
    private static bool TryPlaceChildMaskedPiece(GameObject parent, MaskedModule module, Zone mask, string sourcePath, GameObject sourceChild, int index)
    {
        ...
        instance.name = "GC28_" + module.Name + "_ChildPiece_" + index + "_" + sourceChild.name;
    ...
    private static void PlaceAuthoredCluster(GameObject parent, MaskedModule module, Zone mask, string path)
    {
        ...
        instance.name = "GC28_" + module.Name + "_AuthoredCluster";
        instance.transform.SetParent(parent.transform, true);
        instance.transform.position = Center(mask.Rect, 0f);
    ...
    private static void PlaceDemo2GroundPatch(GameObject parent, string name, string path, float x, float y, float rotationY, Vector2 footprintBlueprint, float visualScale = 1f)
    {
        Vector3 position = BlueprintPoint(x, y);
        Vector2 scaledFootprint = footprintBlueprint * visualScale;
        Rect footprint = BlueprintFootprint(x, y, scaledFootprint);
        Surface(parent, "GC28_Demo2_" + name + "_FallbackSurface", position, new Vector2(footprint.width, footprint.height), Demo2FallbackMaterial(path, "ground"), 0.072f);
        ArtSurfacePlacements.Add(new ArtSurfacePlacement("GC28_Demo2_" + name + "_FallbackSurface", path, "ground", footprint, false));
        demo2GroundPlacements++;
    }

    private static void PlaceConnectedRoadEdge(GameObject parent, string name, string path, Vector2 startBlueprint, Vector2 endBlueprint, float visualScale)
    {
        const float baseModuleLengthBlueprint = 34f;
        float spacingBlueprint = baseModuleLengthBlueprint * visualScale * 0.78f;
        Vector2 footprintBlueprint = new(34f * visualScale, 22f * visualScale);
        PlaceDemo2RoadStrip(parent, name, path, startBlueprint, endBlueprint, spacingBlueprint, footprintBlueprint, visualScale);
        connectedRoadEdges++;
    }

    private static void PlaceConnectedRoadNode(GameObject parent, string name, string path, float x, float y, float rotationY, float visualScale)
    {
        PlaceDemo2RoadNode(parent, name, path, x, y, rotationY, new Vector2(40f * visualScale, 40f * visualScale), visualScale);
        connectedRoadNodes++;
    }

    private static void PlaceDemo2RoadStrip(GameObject parent, string name, string path, Vector2 startBlueprint, Vector2 endBlueprint, float spacingMeters, Vector2 footprintBlueprint, float visualScale = 1f)
    {
        Vector3 start = BlueprintPoint(startBlueprint.x, startBlueprint.y);
        Vector3 end = BlueprintPoint(endBlueprint.x, endBlueprint.y);
        float lengthBlueprint = Vector2.Distance(startBlueprint, endBlueprint);
        int count = Mathf.Max(2, Mathf.CeilToInt(lengthBlueprint / spacingMeters) + 1);
        float rotationY = RoadRotationY(start, end);
        for (int i = 0; i < count; i++)
        {
            float t = count == 1 ? 0f : i / (float)(count - 1);
            Vector2 blueprint = Vector2.Lerp(startBlueprint, endBlueprint, t);
            Vector3 position = Vector3.Lerp(start, end, t);
            Rect footprint = BlueprintFootprint(blueprint.x, blueprint.y, footprintBlueprint);
            PlaceDemo2ArtPrefab(parent, $"{name}_{i:00}", path, "road", position, rotationY, footprint, true, visualScale);
            demo2RoadPlacements++;
        }
    }

    private static void PlaceDemo2RoadNode(GameObject parent, string name, string path, float x, float y, float rotationY, Vector2 footprintBlueprint, float visualScale = 1f)
    {
        Vector3 position = BlueprintPoint(x, y);
        Rect footprint = BlueprintFootprint(x, y, footprintBlueprint);
        PlaceDemo2ArtPrefab(parent, name, path, "road_node", position, rotationY, footprint, true, visualScale);
        demo2RoadPlacements++;
    }

    private static void PlaceDemo2BoundaryStrip(GameObject parent, string name, string path, Vector2 startBlueprint, Vector2 endBlueprint, float spacingMeters, Vector2 footprintBlueprint)
    {
        Vector3 start = BlueprintPoint(startBlueprint.x, startBlueprint.y);
        Vector3 end = BlueprintPoint(endBlueprint.x, endBlueprint.y);
        float length = Vector3.Distance(start, end);
        int count = Mathf.Max(2, Mathf.CeilToInt(length / spacingMeters));
        float rotationY = RoadRotationY(start, end);
        for (int i = 0; i < count; i++)
        {
            float t = count == 1 ? 0f : i / (float)(count - 1);
            ...
            Vector3 position = Vector3.Lerp(start, end, t);
            Rect footprint = BlueprintFootprint(blueprint.x, blueprint.y, footprintBlueprint);
            PlaceDemo2ArtPrefab(parent, $"{name}_{i:00}", path, "boundary", position, rotationY, footprint, false, 1f);
            demo2BoundaryPlacements++;
        }
    }

    private static void PlaceDemo2Detail(GameObject parent, string name, string path, float x, float y, float rotationY, Vector2 footprintBlueprint, bool mustStayOnWalkable)
    {
        Vector3 position = BlueprintPoint(x, y);
        Rect footprint = BlueprintFootprint(x, y, footprintBlueprint);
        PlaceDemo2ArtPrefab(parent, name, path, "detail", position, rotationY, footprint, mustStayOnWalkable, 1f);
        demo2DetailPlacements++;
    }

    private static void PlaceDemo2ArtPrefab(GameObject parent, string name, string path, string category, Vector3 position, float rotationY, Rect footprint, bool mustStayOnWalkable, float visualScale)
    {
        GameObject prefab = AssetDatabase.LoadAssetAtPath<GameObject>(path);
        if (prefab == null)
        {
        ...
        GameObject instance = (GameObject)PrefabUtility.InstantiatePrefab(prefab);
        instance.name = "GC28_Demo2_" + name;
        instance.transform.SetParent(parent.transform, true);
        instance.transform.position = position;
        instance.transform.rotation = Quaternion.Euler(0f, rotationY, 0f);
        instance.transform.localScale = Vector3.one * visualScale;
        AlignBottomNearGround(instance);
        ApplyDemo2FallbackMaterials(instance, path, category);
        foreach (LODGroup lod in instance.GetComponentsInChildren<LODGroup>(false))
        {
            lod.ForceLOD(0);
            lod.enabled = false;
        ...
        ArtSurfacePlacements.Add(new ArtSurfacePlacement(instance.name, path, category, footprint, mustStayOnWalkable));
    }

    private static void ApplyDemo2FallbackMaterials(GameObject instance, string path, string category)
    {
        if (category != "road" && category != "road_node" && category != "ground")
            return;

        Material material = Demo2FallbackMaterial(path, category);
    ...
    private static Material Demo2FallbackMaterial(string path, string category)
    {
        string key = category + ":" + (path.Contains("Concrete", StringComparison.Ordinal) || path.Contains("Port_Concrete", StringComparison.Ordinal) ? "concrete" : path.Contains("Grass", StringComparison.Ordinal) ? "grass" : "road");
        if (Demo2FallbackMaterials.TryGetValue(key, out Material material))
            return material;

        Color color = key.EndsWith(":grass", StringComparison.Ordinal) ? new Color(0.38f, 0.43f, 0.22f, 1f) :
            key.EndsWith(":concrete", StringComparison.Ordinal) ? new Color(0.58f, 0.53f, 0.43f, 1f) :
            category == "road_node" ? new Color(0.48f, 0.39f, 0.25f, 1f) :
            new Color(0.45f, 0.36f, 0.23f, 1f);

        material = CreateMaterial("GC28_Demo2Fallback_" + key.Replace(':', '_'), color);
        Demo2FallbackMaterials.Add(key, material);
        return material;
    }

    private static Rect BlueprintFootprint(float x, float y, Vector2 footprintBlueprint)
    {
        Vector3 center = BlueprintPoint(x, y);
        return CenterRect(center.x, center.z, footprintBlueprint.x / 890f * MapSize, footprintBlueprint.y / 890f * MapSize);
    }
    ...
    private static void BuildDebugOverlay(GameObject root)
    {
        ...
        Material green = CreateMaterial("GC28_DebugSoldierWalkable", new Color(0f, 1f, 0.1f, 0.6f));
        Material cyan = CreateMaterial("GC28_DebugVehicleWalkable", new Color(0f, 0.8f, 1f, 0.6f));
        Material red = CreateMaterial("GC28_DebugBlocker", new Color(1f, 0f, 0f, 0.7f));
        foreach (Zone zone in SoldierLocalZones)
    ...
    private static void BuildGeneratorAuthoringOverlay(GameObject root)
    {
        ...
        Material city = CreateMaterial("GC28_AuthoringDistrict_City", new Color(0.20f, 0.42f, 0.80f, 0.85f));
        Material baseMat = CreateMaterial("GC28_AuthoringDistrict_Base", new Color(0.30f, 0.62f, 0.28f, 0.85f));
        Material logistics = CreateMaterial("GC28_AuthoringDistrict_Logistics", new Color(0.84f, 0.58f, 0.18f, 0.85f));
        Material airfield = CreateMaterial("GC28_AuthoringDistrict_Airfield", new Color(0.68f, 0.68f, 0.74f, 0.85f));
    ...
    private static void BuildMaskedModuleOverlay(GameObject root)
    {
        ...
        Material moduleMat = CreateMaterial("GC28_ModuleMask_DistrictEnvelope", new Color(0.88f, 0.18f, 0.92f, 0.82f));
        Material blockerMat = CreateMaterial("GC28_ModuleMask_BlockerOwned", new Color(1f, 0.08f, 0.08f, 0.9f));
    ...
    private static void Validate()
    {
        ...
        if (detailBlockerPlacements < 60)
            ValidationLog.Add($"ERROR: GC28 placed only {detailBlockerPlacements} legal detail props; densification target is at least 60.");

        if (Districts.Count < 7)
            ValidationLog.Add($"ERROR: GC28 exported only {Districts.Count} districts; procedural authoring target is at least 7 named districts.");
        ...
        if (Modules.Count < 7)
            ValidationLog.Add($"ERROR: GC28 exported only {Modules.Count} reusable masked modules; target is at least 7.");

        if (authoredClusterPlacements < Modules.Count)
            ValidationLog.Add($"ERROR: GC28 placed only {authoredClusterPlacements} authored Demo clusters for {Modules.Count} reusable modules.");

        foreach (ArtSurfacePlacement art in ArtSurfacePlacements)
        {
            if (art.MustStayOnWalkable && !MacroRoads.Concat(SoldierLocalZones).Concat(VehicleZones).Any(zone => zone.Rect.Overlaps(art.Footprint)))
                ValidationLog.Add($"ERROR: Demo2 art surface {art.Name} ({art.Category}) is not anchored to walkable space.");
        }

        if (connectedRoadEdges < 16)
            ValidationLog.Add($"ERROR: GC28 created only {connectedRoadEdges} connected road edges; target is at least 16 including service roads.");
        if (connectedRoadNodes < 6)
            ValidationLog.Add($"ERROR: GC28 created only {connectedRoadNodes} connected road nodes; target is at least 6.");
        if (demo2RoadPlacements < 65)
            ValidationLog.Add($"ERROR: GC28 placed only {demo2RoadPlacements} Demo2 road modules; target is at least 65 visible scaled modules for connected road coverage.");
        if (demo2GroundPlacements < 6)
            ValidationLog.Add($"ERROR: GC28 placed only {demo2GroundPlacements} Demo2 ground/concrete modules; target is at least 6.");
        if (demo2BoundaryPlacements < 20)
            ValidationLog.Add($"ERROR: GC28 placed only {demo2BoundaryPlacements} Demo2 boundary modules; target is at least 20.");
        if (demo2DetailPlacements < 8)
            ValidationLog.Add($"ERROR: GC28 placed only {demo2DetailPlacements} Demo2 detail modules; target is at least 8.");
        ...
        if (ValidationLog.Count == 0)
            ValidationLog.Add($"PASS: GC28 built a connected Demo2 road/terrain pass with {connectedRoadEdges} connected road edges, {connectedRoadNodes} road nodes, Demo2 road/ground/boundary/detail art ({demo2RoadPlacements}/{demo2GroundPlacements}/{demo2BoundaryPlacements}/{demo2DetailPlacements}), {authoredClusterPlacements} source-scale Demo-authored clusters/modules, {Modules.Count} reusable masked district modules across {Districts.Count} named districts, and preserved the GC17/GC18 walkability masks. Detail props: {detailBlockerPlacements}; primary blocker visuals: {mainBlockerPlacements}; total placements: {Placements.Count}; art placements: {ArtSurfacePlacements.Count}; blocker zones: {BlockerZones.Count}; soldier zones: {SoldierLocalZones.Count}; vehicle zones: {VehicleZones.Count}; macro roads: {MacroRoads.Count}.");
    }
    ...
    {
        BuildRtsCamera(root, "Camera_GC28_RtsOverview", new Vector3(-620f, 520f, -820f), BlueprintPoint(388f, 430f), 38f);
        BuildRtsCamera(root, "Camera_GC28_RtsCityReadable", new Vector3(-930f, 330f, -430f), BlueprintPoint(172f, 366f), 34f);
        BuildRtsCamera(root, "Camera_GC28_RtsAirfieldCommand", new Vector3(260f, 430f, -620f), BlueprintPoint(660f, 320f), 36f);
        BuildRtsCamera(root, "Camera_GC28_RtsDenseCityClose", new Vector3(-700f, 210f, -250f), BlueprintPoint(176f, 392f), 30f);
        BuildRtsCamera(root, "Camera_GC28_ScaleAuditClose", new Vector3(-700f, 92f, -185f), BlueprintPoint(176f, 392f), 22f);

        Camera top = CameraObject(root, "Camera_GC28_TopDownContractVisualProof");
    ...
    private static void CaptureScene()
    {
        ...
        {
            if (camera.name == "Camera_GC28_RtsOverview")
                Render(camera, ProjectPath(CaptureRoot + "/gc28_rts_overview_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC28_RtsCityReadable")
                Render(camera, ProjectPath(CaptureRoot + "/gc28_rts_city_readable_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC28_RtsAirfieldCommand")
                Render(camera, ProjectPath(CaptureRoot + "/gc28_rts_airfield_command_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC28_RtsDenseCityClose")
                Render(camera, ProjectPath(CaptureRoot + "/gc28_rts_dense_city_close_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC28_ScaleAuditClose")
                Render(camera, ProjectPath(CaptureRoot + "/gc28_scale_audit_close_1920x1080.png"), 1920, 1080);
            if (camera.name == "Camera_GC28_TopDownContractVisualProof")
            {
                RenderContractProof(camera, ProjectPath(CaptureRoot + "/gc28_topdown_contract_visual_proof_2048x2048.png"));
                RenderGeneratorBlueprint(camera, ProjectPath(CaptureRoot + "/gc28_topdown_generator_blueprint_2048x2048.png"));
                RenderModuleMaskProof(camera, ProjectPath(CaptureRoot + "/gc28_topdown_module_mask_proof_2048x2048.png"));
            }
        }
    }
    ...
    private static void WriteContractJson()
    {
        ...
        {
            generatedBy = nameof(WarlineCaptureGc28ConnectedRoadTerrainBuilder),
            scene = ScenePath,
            generatorSettings = new GeneratorSettingsExport
            {
                ...
                artStyle = "Hybrid PolygonMilitary compounds with Demo2 PolygonBattleRoyale roads, dirt, concrete, grass, walls, rubble, and dressing",
                authoringRule = "Roads and local yards are walkable; Demo2 road art must stay anchored to walkable lanes; buildings, props, rocks, tents, and static vehicles must live inside blocker zones."
            }
        };
        foreach (District district in Districts)
        ...
        foreach (ArtSurfacePlacement art in ArtSurfacePlacements)
            export.artSurfaces.Add(new ArtSurfaceExport { name = art.Name, prefab = art.Path, category = art.Category, centerX = art.Footprint.center.x, centerZ = art.Footprint.center.y, width = art.Footprint.width, depth = art.Footprint.height, mustStayOnWalkable = art.MustStayOnWalkable });
        foreach (AuthoredClusterPlacement cluster in AuthoredClusterPlacements)
    ...
    private static void WriteReport()
    {
        ...
        report.AppendLine("# GC28 Connected Road Terrain Scene");
        report.AppendLine();
        report.AppendLine("Lane: Gameplay");
        report.AppendLine("Task: Continue from GC27 by fixing disconnected road visuals and unclean terrain, while preserving the source-scale compound layout and the flat walkability/blocker contract.");
        report.AppendLine();
        report.AppendLine("Files changed:");
        report.AppendLine("- `Assets/Game/Scripts/Editor/WarlineCaptureGc28ConnectedRoadTerrainBuilder.cs`");
        report.AppendLine("- `Assets/Game/Scenes/Generated/GC28_ConnectedRoadTerrain_2048.unity`");
        report.AppendLine("- `Design/AgentReports/Data/GeneratedScenes/GC28_ConnectedRoadTerrain_2048/gc28_connected_road_terrain_contract.json`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC28_ConnectedRoadTerrain_2048/gc28_rts_overview_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC28_ConnectedRoadTerrain_2048/gc28_rts_city_readable_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC28_ConnectedRoadTerrain_2048/gc28_rts_airfield_command_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC28_ConnectedRoadTerrain_2048/gc28_rts_dense_city_close_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC28_ConnectedRoadTerrain_2048/gc28_scale_audit_close_1920x1080.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC28_ConnectedRoadTerrain_2048/gc28_topdown_contract_visual_proof_2048x2048.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC28_ConnectedRoadTerrain_2048/gc28_topdown_generator_blueprint_2048x2048.png`");
        report.AppendLine("- `Design/AgentReports/Captures/GeneratedScenes/GC28_ConnectedRoadTerrain_2048/gc28_topdown_module_mask_proof_2048x2048.png`");
        report.AppendLine();
        report.AppendLine("Contracts touched: GC17/GC18 walkability visual contract remains the source of truth; GC28 changes only generated art-road/terrain presentation on top of GC26/GC27 source-scale compound envelopes without changing gameplay masks.");
        report.AppendLine("User-visible behavior: no shipped runtime behavior changed; generated scene now uses a connected scaled Demo2 road graph, cleaner terrain beds, and source-scale building/compound visuals over the existing flat gameplay plane.");
        report.AppendLine("Validation run: Unity batchmode `WarlineCaptureGc28ConnectedRoadTerrainBuilder.BuildGc28ConnectedRoadTerrain2048`.");
        report.AppendLine("Validation result: " + (ValidationLog.Any(line => line.StartsWith("ERROR:", StringComparison.Ordinal)) ? "failed; see validation log below." : "passed connected road/terrain validation."));
        report.AppendLine("Known gaps: This is still a generated prototype scene, not final art. Road modules are scaled for 2048m readability while buildings remain source scale 1; final road art should use authored compound entrances and larger continuous road meshes if Art provides them.");
        report.AppendLine("Cross-lane impacts: Art/design can review whether the connected road graph and cleaner terrain read closer to the target RTS reference before Gameplay turns the contract into runtime movement data.");
        report.AppendLine("Next recommended task: GC29 should increase visual density around compounds and improve authored road-to-compound entrances after PM/art accepts the connected road/terrain direction.");
        report.AppendLine();
        report.AppendLine("Counts:");
        report.AppendLine($"- connected road edges: {connectedRoadEdges}");
        report.AppendLine($"- connected road nodes: {connectedRoadNodes}");
        report.AppendLine($"- Demo2 road modules: {demo2RoadPlacements}");
        report.AppendLine($"- Demo2 ground/concrete/grass modules: {demo2GroundPlacements}");
        report.AppendLine($"- Demo2 boundary modules: {demo2BoundaryPlacements}");
        report.AppendLine($"- Demo2 detail modules: {demo2DetailPlacements}");
        ...
        report.AppendLine($"- Demo2 art placements: {ArtSurfacePlacements.Count}");
        ...
        report.AppendLine();
        report.AppendLine("Demo2 art layer:");
        foreach (ArtSurfacePlacement art in ArtSurfacePlacements.Take(80))
            report.AppendLine($"- {art.Name}: category={art.Category}; prefab={art.Path}; footprint={art.Footprint.width:0.0}m x {art.Footprint.height:0.0}m");
        if (ArtSurfacePlacements.Count > 80)
            report.AppendLine($"- ... {ArtSurfacePlacements.Count - 80} more Demo2 art placements omitted from report body; full list is in the JSON contract.");
        report.AppendLine();
