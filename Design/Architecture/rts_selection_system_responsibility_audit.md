...
- Final deletion Step 7 moved production startup wiring off the retired `RTSSelectionSystem` type: `SelectionGameplayStartupSystem` now constructs the temporary `SelectionRuntimeContextSystem` runtime boundary and production code no longer constructs, stores, or calls `RTSSelectionSystem`.
...
- Final deletion Step 9 removed the architecture allowance that described `RTSSelectionSystem` as temporary compatibility debt and deleted `SelectionRuntimeUpdateSystem.cs`. Runtime selection phases are now composed explicitly by startup instead of hidden behind a monolithic `Update()` shell. The remaining debt is `SelectionRuntimeContextSystem`, which owns temporary context construction only and must not expose `Update()` or become another managed orchestration shell.

## Recommended Next Slices

1. Move the remaining context construction out of `SelectionRuntimeContextSystem`.
2. Delete `SelectionRuntimeContextSystem.cs` once no production or test references remain.
