...
- 2026-06-10: Fixed passenger-first Board preview regression where other soldiers were highlighted because the target predicate used broad `IsBoardCommandAvailable`, which includes soldier passengers. Passenger-first preview now uses `IsBoardTransportWithAvailableSeats` so only valid transports with free seats are marked. Board preview markers now get a cached cyan/green material-property override, while Attack preview clears that override and keeps the normal attack marker style. `git diff --check` passed, no forbidden find/camera/log calls were added, and shadow Unity compile passed with `/private/tmp/warline-board-marker-fix-compile.log`. Focused EditMode command again exited cleanly without writing its XML result file, so focused test-result validation remains open.
- 2026-06-10: Fixed passenger-first Board mode trapping the player when no transport is visible. Passenger-first Board now allows camera pan while it is waiting for a transport target. Mode cancellation remains mobile-first: tap Board again or tap another command. `git diff --check` passed, no forbidden find/camera/log calls were added in the touched code files, and shadow Unity compile passed with `/private/tmp/warline-board-pan-cancel-compile.log`. Focused EditMode command again exited without writing `/private/tmp/warline-board-pan-cancel-tests.xml`, so focused test-result validation remains open.
- 2026-06-10: Refined transport-first Board mode for mobile input. Board remains the visually selected command; it does not enter normal Select mode. A drag starting on a valid passenger candidate becomes the Board passenger rectangle, while a drag starting on empty terrain pans the camera so the player can find soldiers. Added an ECS-backed press-start predicate using the existing clicked-unit resolver plus Board passenger validation. `git diff --check` passed, no forbidden find/camera/log calls were added, and shadow Unity compile passed with `/private/tmp/warline-board-transport-pan-compile.log`.
