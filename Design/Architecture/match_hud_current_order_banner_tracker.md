...
- In progress: `0`.
- Remaining open: `0`.
- Current target: `Runtime shell-binding correction complete for the user-reported Move command case. The missing banner was caused by HeaderContent and FooterContent being installed as separate live sections while the footer runtime feedback view depended on a cross-section serialized banner reference from the full prefab.`
...
- ECS/source status: `Done for the non-manual V1 path. BattleHudRuntimeFeedbackUiSystemHelper publishes sticky command-mode, board-mode, and accepted-result transient banner models through IBattleHudRuntimeFeedbackView. Existing SelectionHudFeedbackBoundary command-mode/result queues flow through the BattleHudRuntimeFeedbackSink service-edge adapter. 2026-07-01 correction: UIShellContentView now binds the live HeaderContent MatchHudCurrentOrderBannerView into the live FooterContent BattleHudRuntimeFeedbackView after separate section install, so the real Match HUD shell no longer depends on a full-prefab cross-section reference.`
...
- Still wrong / next iteration: `No known code-path defect remains for the reported Move command banner after binding the live header banner into the live footer runtime feedback view. Unity validation for this correction is still blocked before tests start by the known licensing loop, despite the documented escalated retry. Next iteration if the user still sees no banner: validate through a Codex session that exposes callable Unity MCP tools or rerun UIShellCurrentContentLoadTests.RunFocusedValidation after the Unity licensing client accepts batchmode.`
