...
- Slice: AI vehicle production fuel gate.
- Files changed: `Assets/Game/Scripts/Systems/AIProductionSystem.cs`, `Assets/Tests/Editor/AIProductionValidationTests.cs`, and this tracker.
- Behavior intent:
  - AI vehicle production now reads existing ECS boundary buffers before requesting vehicle units.
  - Vehicle requests are blocked when the faction has zero usable Fuel, no stored Fuel, and no positive Fuel production rate.
  - The same request is allowed when expected Fuel production exists, so AI can recover instead of permanently starving after logistics come online.
  - Infantry/non-vehicle production is unaffected.
- Validation:
  - Added `AssertVehicleProductionWaitsForUsableFuelOrExpectedProduction`.
  - Updated `AIProductionFocusedValidation` count to 2.
  - `git diff --check` passed.
  - Checklist count command returned `84/99`.
  - `dotnet build Assembly-CSharp-Editor.csproj --no-restore` passed.
  - Unity focused validation remains blocked by the recurring licensing client mismatch/reconnect loop unless the editor/license state is reset. Prior log: `/private/tmp/warline-fuel-authoring-tests.log`.
- Next action:
