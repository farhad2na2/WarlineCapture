    ...
    internal static partial class CampaignMissionCatalogProjection
    {
            ...
            ScenarioSetupConfig scenario)
        {
            if (!Matches(projected.MissionId, mission.MissionId) ||
                ...
                !Matches(projected.DelayedWaveRouteId, scenario.MissionRuntime.DelayedWave.RouteId) ||
                !Matches(projected.DelayedWaveTargetMissionRoleId,
                    scenario.MissionRuntime.DelayedWave.TargetMissionRoleId) ||
            ...
            return MatchesRouteReopened(projected.RouteReopened, scenario.RouteReopened) && MatchesPowerRelay(projected.PowerRelay, scenario.PowerRelay) && MatchesMarketLifeline(projected.MarketLifeline, scenario.MarketLifeline) && MatchesSupplyLine(projected.SupplyLine, scenario.SupplyLine) && MatchesGridlock(projected.Gridlock, scenario.Gridlock) && MatchesBreach(projected.Breach, scenario.Breach) && MatchesExtraction(projected.Extraction, scenario.Extraction) && MatchesDefense(ref projected.Defense, scenario) &&
