...
| Current task | `AM-025` blocked from acceptance but active for remediation; tactical camera state is isolated, Editor-only switches have reset ownership, and match-start, building-AI, diagnostics, logistics telemetry, unit lookup, selected-unit lifecycle and order snapshots, selection startup, building-selection interaction, follow-camera mode, pathfinding pending state, central match state, scene-transition queue, diagnostics state, startup readiness counts, player-input state, visible-unit selection, pointer-target queries, building queries, the building runtime boundary, selection diagnostic routing, plus Scenario Lab action-boundary ownership now have replacement-World proof. The `575` historical rows now contain `446` non-debt and `129` genuine-debt rows grouped into `53` unique file/rule items; five external helper paths remain |
