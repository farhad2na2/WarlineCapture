...
P0 contracts and the P1 city/profile transaction core are **implemented on bookmark branches**. P2 and later stay **Planned**. Host markers are not Unity Editor, device, or ARIA validation. The implementation lead owns integration, scope and evidence; content agents must not independently modify shared contracts.
