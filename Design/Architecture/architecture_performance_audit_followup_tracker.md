...
| Field | Status |
|---|---|
| Checklist complete | 90 / 127 |
| Checklist percent complete | 70.9% |
| Current phase | Phase 8 - Managed Helper Hot-path Migration |
| Quick wins complete | 6 / 6 |
| Current target | The top-level gameplay runtime phase probe rerun passed and reported `0` direct bytes for RoadBuild, BuildingPlacement, Selection, DayNight, CitizenPopulation, MainMenu, LoadingGate, and EndUpdate across the 300-frame capture. The remaining player-relevant table is now dominated by `RtsCameraRequestSystem.TryGetGridConfig`, including thread-pool and main-thread rows attributed through building-placement, menu-bootstrap, transport boarding, selection camera, road-build, and UI shell paths. The next exact action is to inspect and remove the recurring managed allocation in `RtsCameraRequestSystem.TryGetGridConfig` without changing camera behavior. |
| Compiler status | Latest report/tracker update passed `git diff --check`, `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, and `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly` with 0 errors. Unity 6.5.2 battle capture rerun `/private/tmp/warline-arch-followup-gameplay-runtime-phase-probe-retry-windowed-battle.log` passed with no compiler errors in the log and wrote the updated battle GC report. |
| Android baseline status | Not started |
| GC baseline status | Latest completed 2026-07-05 02:15:07 UTC battle-state Match GC capture after diagnostic-row filtering: 300 requested frames after 180 warmup frames, 301 scanned profiler frames, 7,756 raw GC.Alloc samples / 502,467 raw bytes, 6,805 player-relevant samples / 435,981 bytes after excluding editor/tooling/diagnostic rows, and 951 editor/tooling/diagnostic samples / 66,486 bytes. Runtime allocation probe assertion passed. `UIShellEcsPresentationSystem.Update`, `MenuBootstrapView.Update`, top-level gameplay runtime phases, selection subphases, `BuildingPlacementVisualPresentationSystemHelper.CreateBuildingVisualInstance`, `BuildingProductionRuntimeTick.UpdateActiveProductionTransports`, and `TransportBoardingCommandSystem` all reported `0` direct bytes. The remaining top player-relevant rows point at `RtsCameraRequestSystem.TryGetGridConfig`. |
...
- Next iteration: inspect and remove the recurring managed allocation in `RtsCameraRequestSystem.TryGetGridConfig` without changing RTS/tactical camera behavior, then rerun the battle GC capture to confirm the top table moves.
