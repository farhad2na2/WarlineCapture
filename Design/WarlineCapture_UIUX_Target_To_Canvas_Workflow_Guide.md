...
```text
Use `Design/WarlineCapture_UIUX_Target_To_Canvas_Workflow_Guide.md` and the Reusable Prompt - Visual-Lock Canvas Conversion from `Design/WarlineCapture_UIUX_Mockup_To_Canvas_Conversion_Plan.md` to convert [SCREEN_PREFAB_PATH] against [TARGET_MOCKUP_PATH]. Do not stop until the rendered capture and focused crops match the target, or explicitly list remaining differences as not accepted.
```
