...
namespace Game.Runtime
{
    public partial struct CampaignMissionRuntimeSystem
    {
        private bool TryAdvanceMarketLifeline(ref SystemState system,Entity root,in CampaignMissionRuntimeComponent runtime)
        {
            ...
            int delta=(int)math.round(math.max(0,SystemAPI.Time.DeltaTime)*1000);int rifles=0,hostiles=0,deadRifles=0,deadHostiles=0,initialized=0,delivered=0;bool rifleAtManifest=false;
            ...
            for(int i=0;i<members.Length;i++)
            {
                ...
                if(member.Dead!=0){if(member.Kind==0)deadRifles++;else if(member.Kind==1)state.Failure=MarketLifelineFailure.ConvoyLost;else deadHostiles++;members[i]=member;continue;}
                if(member.Initialized!=0)
                {
                    if(member.Kind==0)
                    {
                        rifles++;if(em.HasComponent<UnitGrid>(member.Entity)&&math.distancesq(em.GetComponentData<UnitGrid>(member.Entity).Cell,state.ManifestCell)<=rules.ManifestRadius*rules.ManifestRadius&&!em.HasComponent<UnitPathRequest>(member.Entity)&&!em.HasComponent<UnitPathFollow>(member.Entity))rifleAtManifest=true;
                    }
            ...
            if(active)
            {
                ...
                if(state.ManifestHoldMilliseconds>=rules.ManifestHoldMilliseconds)state.ManifestVerified=1;
