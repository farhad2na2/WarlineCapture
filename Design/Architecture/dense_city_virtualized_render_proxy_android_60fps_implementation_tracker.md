...
| Phase 4: Jobified runtime assignment | Checklist complete; the ordered camera boundary, exact map-generation native initialization, pure guard-envelope decision, bounded selection, deterministic assignment, parallel apply, apply-to-Entities-Graphics dependency boundary, stable apply no-op gate, bounded instrumentation contract, and explicit persistent-owner/database-blob lifetime proof are accepted. The persistent coordinator now schedules selection, assignment, command publication, apply, and initial-view completion for the candidate camera envelope; a camera update wholly inside the materialized guard publishes no jobs, command version, or rebuild. Initialization and dynamic job results publish bounded placement/logical-part/resident/capacity and slot/activity/assignment/overflow/deficit/version/rebuild metrics. Canonical building-state synchronization remains Phase 6 work; the render-only pilot uses a constructed zero-length state array. |
| Phase 5: Render-only pilot | In progress and Android expansion rejected; the exact frozen Vegetation/Prop pilot is persisted and enabled only on the dense candidate as `EntityScene + VirtualizedProxyPool`. Its saved authoring/output validates twice at 9,721 placement identities, 22 repeated prototypes/26 parts, 1,635 cells, 704 disabled fixed slots, 11,299 stripped eligible rows, and 70,710 resident authoring rows; the three excluded Prop rows remain resident. Packed PlayMode validates slow/fast pan, zoom, rotation, teleport, guard-band no-op behavior, exact return bindings, four deterministic rebuilds, 319 active returned slots, and zero overflow. Android VRP-053 measured 10.6 FPS average, 93.94 ms average / 110.68 ms p95 frame, 93.93 ms average / 110.66 ms p95 CPU active, and 15.53 ms average / 16.59 ms p95 GPU; it loaded 704 slots but enabled zero while unchanged presentation scans consumed 69.040 ms/frame combined. The VRP-054 Windows remediation now projects the camera frustum across the presentation height range, terminally excludes operation-map parents before incomplete-unit retry, and transfers unit tint initialization into that one-shot classification. Exact 70,710-child drain plus 300 stationary updates measured the two former CPU owners at 0.0016 ms p95 against an 8.3333 ms 120-Hz Editor CPU budget. This is a CPU-owner proxy, not whole-frame or Android evidence. Expansion, VRP-054 completion, and Phase 6 remain forbidden until the same Android route proves active slots, material CPU-main p95 improvement, visual/state parity, and no replacement dominant owner. Production remains `StaticSceneChunks + ResidentEntities` and `productionCutover=0`. |
...
Checklist progress: `45 / 80` complete.
