...
This document is no longer the active battlefield art direction.

The active direction is full 3D single-map mobile RTS: one large 3D operation map per mission or operation, with runtime soldiers, civilians, vehicles, aircraft, buildings, markers, VFX, and metadata-backed command overlays. Do not use this document to start new 2.5D isometric macro-tile production unless PM explicitly reopens that decision.

## Decision

WarlineCapture's former battlefield art direction was premium 2.5D isometric mobile RTS using large authored terrain macro tiles.
...
- Macro-tile implementation plan: `Design/MacroTile_Terrain_Production_Plan.md`
- 2D isometric art bible: `Design/2D_Isometric_Art_Bible.md`
