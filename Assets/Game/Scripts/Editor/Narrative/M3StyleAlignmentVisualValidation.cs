...
namespace Game.Editor
{
    ...
    public static class M3StyleAlignmentVisualValidation
    {
        private const string Output = "Design/NarrativeVision/M3StyleAlignment/EditorQA";
        ...
        public static void Run()
        {
            ...
            FirstLaunchNarrativeArtImporter.ConfigureApprovedArtImports();
            M02EstablishBaseNarrativeArtImporter.ConfigureFinalArtImports();
            var choices = AssetDatabase.LoadAllAssetsAtPath(FirstLaunchNarrativeDialogueAssetImporter.CommanderPortraitSheetPath)
                .OfType<Sprite>().OrderBy(s => s.name).ToArray();
            if (choices.Length != 7) throw new InvalidOperationException("Expected seven commander choices.");
            foreach (var sprite in choices)
                if (sprite.rect.xMax > sprite.texture.width || sprite.rect.yMax > sprite.texture.height)
                    throw new InvalidOperationException("Commander atlas slice outside texture: " + sprite.name);
            ...
            var sequences = AssetDatabase.FindAssets("t:NarrativeSequenceConfig", new[] { "Assets/Game/Configs/Narrative" })
                .Select(AssetDatabase.GUIDToAssetPath).Distinct()
                .SelectMany(p => AssetDatabase.LoadAllAssetsAtPath(p).OfType<NarrativeSequenceConfig>()).Distinct().ToArray();
            captures = lines = 0;
            try
            {
                ...
                foreach (bool persian in new[] { false, true })
                {
                    ...
                    var resolver = new FirstLaunchNarrativeLocaleTextCompositionSystemHelper(FallbackGameTextResolver.Instance,
                        persian ? Require<NarrativeLocaleConfig>(FirstLaunchNarrativeConfigBuilder.PersianLocalePath) : null);
                    ...
                    foreach (var state in sequence.States.Where(s => s.HasPanelBinding))
                    {
                        ...
                        if (sprite == null && reference != null)
                            sprite = Require<Sprite>(AssetDatabase.GUIDToAssetPath(reference.AssetGUID));
                        if (sprite == null) throw new InvalidOperationException("Missing panel: " + state.StateId);
                        string id = sequence.SequenceId.Replace('.', '_') + "-" + state.StateId;
                    ...
                    // Six selectable cards; the seventh atlas sprite is the faceless fallback.
                    for (int choice = 0; choice < 6; choice++)
                    {
                        int selected = choice;
                        Render("commander-choice-" + choice, width, persian, view =>
                        {
                            view.ApplyLanguage(persian, resolver);
                            view.ApplyPanel(new NarrativePanelPresentationModel { StateId = "identity", Tint = Color.white,
                                PanelSprite = Require<Sprite>("Assets/Game/Art/UI/Generated/CommanderProfile/TargetLockV01/scn03_background_21x9_no_ui.png") });
                            ...
                            view.CommanderIdentityView.SetIdentity("ECHO-7", "Commander", selected);
                            view.CommanderIdentityView.SetControlsInteractable(true);
                            if (view.CommanderIdentityView.PortraitOptionCount != 6 || view.CommanderIdentityView.SelectedPortraitIndex != selected)
                                throw new InvalidOperationException("Commander choice selection mismatch.");
                        });
                    }
                }
                GameLocalization.SetLocale("en", false);
                MainMenuV3PrefabBuilder.CaptureQa();
                MainMenuV3PrefabBuilder.CapturePersianQa();
                foreach (string path in Directory.GetFiles("/private/tmp", "warline-main-menu-v3-*.png"))
                    File.Copy(path, Path.Combine(Output, Path.GetFileName(path)), true);
                Debug.Log($"[M3StyleAlignmentVisualValidation] result=Passed captures={captures} captionChecks={lines} selectableChoices=6 atlasSprites=7 locales=2 aspects=2");
            }
