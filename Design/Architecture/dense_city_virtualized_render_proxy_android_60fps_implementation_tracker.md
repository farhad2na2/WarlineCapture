...
| Phase 3: Candidate pool baking | In progress; candidate root database component with a complete baked logical blob, one sorted shared render-mesh array, 704 fixed disabled leaf proxy slots, opt-in fail-closed eligible source-row stripping, conditional virtualized-building state-index ownership, and mode-specific packed-readiness gates have passed in-memory contracts. Virtualized readiness now requires the exact database identity/schema/hash, nonempty logical tables, exact contiguous slot ownership, zero eligible packed source survivors, and an exact persistent resident-exception row set; resident mode rejects virtualization artifacts. Exact logical/source identity plus mesh/material/submesh parity still gates stripping, and a stateful building additionally requires one canonical gameplay owner, one state-owner index, and complete renderer coverage before resident render-root references are removed. The current Vegetation/Prop pilot has zero state owners, so candidate assets and packed output remain unchanged pending report extension, later admission, and direct-bake/package gates. |
...
Checklist progress: `29 / 80` complete.
