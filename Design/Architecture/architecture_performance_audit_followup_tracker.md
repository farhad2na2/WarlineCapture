...
| Field | Status |
|---|---|
| Checklist complete | 65 / 108 |
| Checklist percent complete | 60.2% |
| Current phase | Phase 8 - Fuel/Oil ECS Drift Correction |
| Quick wins complete | 6 / 6 |
| Current target | Managed Fuel/Oil production and hauler transfer ticks now refresh the live `BuildingResourceStorageComponent` mirror on each resource-capable runtime building combat entity. This preserves the single live runtime-building owner while keeping ECS/UI consumers from reading stale resource storage after production, load, or unload changes. |
| Compiler status | `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, focused Unity 6.5.2 ECS mirror validation `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-ecs-validation.log`, focused Unity 6.5.2 faction resource validation `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-faction-validation.log`, and focused Unity 6.5.2 hauler mirror validation `/private/tmp/warline-arch-followup-fuel-oil-hauler-mirror-sync-validation.log` passed with 0 compiler errors after the Fuel/Oil mirror-sync slices. |
...
| Fuel/Oil drift status | Drift partially reduced: production/conversion math is no longer embedded in `FactionResourceCompositionSystemHelper`, runtime-created building combat entities carry their runtime building ids, resource buildings now have a per-building ECS storage mirror, a disabled Burst-capable ECS tick/apply surface is validated, managed production applies through that ECS surface, and production plus hauler updates refresh the live ECS storage mirror for resource-capable buildings. Authoritative live storage still lives on runtime-building objects, and hauler load/unload logic still runs through managed helper/runtime-building paths. UI/header paths remain read-only display consumers. |
| Validation status | Phase 0 inventory completed; Phase 1 config edited; Phase 2 release OnGUI strip implemented; Phase 3 release diagnostic system gate implemented; Phase 4 GC quick wins completed; Phase 5 Burst inventory/classification report added and guardrail implemented; Phase 6 instantiate classification report added and first placement-visual pooling slice implemented; Phase 8 Fuel/Oil behavior safety-net tests added, production math extracted into a pure system-helper surface, runtime-created building combat entities receive their runtime building ids, per-building ECS resource storage mirrors are populated for resource buildings, the disabled ECS resource-production tick/apply surface has focused validation, managed production applies through the ECS surface without double-ticking, and production/hauler ticks now sync managed results back to `BuildingResourceStorageComponent`. Latest validations passed: `dotnet build Game.Runtime.csproj --no-restore -v:q -clp:ErrorsOnly`, `dotnet build Game.Editor.csproj --no-restore -v:q -clp:ErrorsOnly`, `git diff --check`, `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-ecs-validation.log`, `/private/tmp/warline-arch-followup-fuel-oil-mirror-sync-faction-validation.log`, and `/private/tmp/warline-arch-followup-fuel-oil-hauler-mirror-sync-validation.log`. |
| Still wrong / next iteration | No visual defects are known from code/compile validation, but manual visual smoke checks for placement-preview pooling and command behavior are still pending. Fuel/Oil storage is still mirrored rather than fully authoritative on ECS entities, and hauler load/unload still mutates runtime-building objects through managed helper paths; next Fuel/Oil slice should move hauler transfer decisions/state closer to ECS storage without creating a parallel owner, or pause Fuel/Oil drift work and return to Android baseline capture. Android baseline is still not captured, and HDR/soft-shadow tier ownership remains unresolved. |
