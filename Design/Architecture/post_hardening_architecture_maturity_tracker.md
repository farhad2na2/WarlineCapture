...
| Latest validation | AM-025 preparation: deterministic ownership-delta tests pass `9 / 9`; real regeneration/check classifies `1,215 / 1,215` rows but leaves `575` open; corrected Unity source-growth validator compiles and fails closed on five exact external helper paths. AM-024 final remains accepted and green |
| Latest evidence | `am025_phase2_exit_policy.json` freezes the exit matrix; draft `am025_phase2_ownership_delta.json/.md` records `612` resolved, `28` protected-deferred, `575` open, and `26` new-after-baseline final rows without invented mappings |
