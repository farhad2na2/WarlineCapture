...
| Current task | `AM-025` remains blocked from acceptance but active for remediation. Match startup now publishes its scene view once through ECS instead of making the menu repeatedly scan every Match-scene root object. The `575` historical rows contain `551` non-debt and `24` genuine-debt rows grouped into `20` unique file/rule items. Separately owned FirstLaunch and operation-map helper paths still block the canonical source-growth gate. |
