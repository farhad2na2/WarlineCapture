...
| Current task | `AM-025` remains blocked from acceptance but active for remediation. Child AMFR-011 is accepted at `11 / 22` and AMFR-012 is dependency-ready. The corrected current scan has `430` reviewable rows: `412` resolved, `5` protected, and `13` genuine-debt rows grouped into `12` unique file/rule items. The original `575` rows remain historical intake. Five exact operation-map paths are handed to the AM-025 child while the broader map owner remains active. |
