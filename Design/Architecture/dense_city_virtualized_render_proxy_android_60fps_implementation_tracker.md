...
| Phase 2: Deterministic database builder | In progress; generated bake-config schema, exact output ownership, deterministic source-row inventory, shared compound prototype recipes, exact render-only logical placements, accepted-origin-aligned 32 m spatial index, and provisional per-policy capacity/entity/active-slot budgets accepted; camera/projection acceptance, building state linkage, and config creation remain open |
...
Checklist progress: `17 / 80` complete.
