...
- Citizen population building read paths must use `BuildingRuntimeQuerySystem` for building lists, positions, destroyed state, refugee settings, and approach cells; `BuildingPlacementSystem` may remain only as temporary composition/resource/prefab compatibility debt until those separate boundaries are migrated.
